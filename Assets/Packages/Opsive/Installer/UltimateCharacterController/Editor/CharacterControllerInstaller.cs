    ...
    public class CharacterControllerInstaller : EditorWindow
    {
        ...
        private static GUIStyle LinkStyle
        {
            get
            {
                if (s_LinkStyle == null)
                {
                    ...
                    s_LinkStyle.normal.textColor =
                        EditorGUIUtility.isProSkin ? new Color(0.8f, 0.8f, 1.0f, 1.0f) : Color.blue;
                }
        ...
        private static Texture2D SuccessTexture
        {
            get
            {
                if (s_SuccessTexture == null)
                    s_SuccessTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(
                        AssetDatabase.GUIDToAssetPath(EditorGUIUtility.isProSkin
                            ? c_DarkSuccessTextureGUID
                            : c_LightSuccessTextureGUID));
        ...
        private static Texture2D FailureTexture
        {
            get
            {
                if (s_FailureTexture == null)
                    s_FailureTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(
                        AssetDatabase.GUIDToAssetPath(EditorGUIUtility.isProSkin
                            ? c_DarkFailureTextureGUID
                            : c_LightFailureTextureGUID));
        ...
        private void OnEnable()
        {
            ...
            var packageFolderPath =
                installerPath.Remove(installerPath.Length - 39); // Remove /Editor/CharacterControllerInstaller.cs.
            if (!Directory.Exists(packageFolderPath)) return;
            ...
            for (var i = 0; i < files.Length; ++i)
            {
                ...
                if (extension != ".unitypackage") continue;
                m_InstallPackages[packageCount] = files[i];
                packageCount++;
            }

            if (m_InstallPackages.Length != packageCount) Array.Resize(ref m_InstallPackages, packageCount);
            m_PackageNames = new string[m_InstallPackages.Length];
            for (var i = 0; i < m_InstallPackages.Length; ++i)
        ...
        {
            GUILayout.Label(
                "The character controller can be installed after the following requirements are satisifed:");
            ...
            if (!validInstallPackage)
            {
                EditorGUILayout.HelpBox(
                    "Unable to find the install package. The package can be installed manually within the Opsive/Installer/UltimateCharacterController folder.",
                    MessageType.Warning);
                height += 45;
            }
            ...
            if (!string.IsNullOrEmpty(locomotionLocation) && File.Exists(locomotionLocation))
            {
                var assetInfoType =
                    Type.GetType(
                        "Opsive.UltimateCharacterController.Utility.AssetInfo, Opsive.UltimateCharacterController");
                if (assetInfoType != null)
                {
                    var versionProperty =
                        assetInfoType.GetProperty("Version", BindingFlags.Public | BindingFlags.Static);
            ...
            if (requireCleanInstall)
            {
                ...
                {
                    EditorGUILayout.HelpBox(
                        "Version 2.4 contains major structural changes. Before importing remove the Opsive/UltimateCharacterController and Opsive/Shared folders. " +
                        "Ensure you have first moved your own assets out of that folder and have made a backup.",
                        MessageType.Warning);
                    height += 45;
                }
            }
            ...
            GUILayout.Space(3);
            GUILayout.Label(
                "Updates from versions prior to 2.2 require manual changes in order to upgrade your character.");
            GUILayout.BeginHorizontal();
            GUILayout.Label("See");
            GUILayout.Space(-3);
            if (GUILayout.Button("this page", LinkStyle, GUILayout.Width(55)))
                Application.OpenURL(
                    "https://opsive.com/support/documentation/ultimate-character-controller/getting-started/version-2-2-update-guide/");
            GUILayout.Space(-1);
        ...
        {
            return GetWindow<CharacterControllerInstaller>(false, "Installer");
        }
        ...
        {
            if (string.IsNullOrEmpty(m_InstallPackages[m_SelectedPackage])) return;
