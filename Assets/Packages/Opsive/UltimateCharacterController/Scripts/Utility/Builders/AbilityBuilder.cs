...
namespace Opsive.UltimateCharacterController.Utility.Builders
{
    ...
    {
        private static readonly Dictionary<Type, RequireComponent[]> s_RequiredComponents = new();
        private static readonly Dictionary<Type, DefaultInputName[]> s_DefaultInputName = new();
        private static readonly Dictionary<Type, DefaultStartType> s_DefaultStartType = new();
        private static readonly Dictionary<Type, DefaultStopType> s_DefaultStopType = new();
        private static readonly Dictionary<Type, DefaultAbilityIndex> s_DefaultAbilityIndex = new();
        private static readonly Dictionary<Type, DefaultAbilityIntData> s_DefaultAbilityIntData = new();
        private static readonly Dictionary<Type, DefaultItemStateIndex> s_DefaultItemStateIndex = new();
        private static readonly Dictionary<Type, DefaultState> s_DefaultState = new();
        private static readonly Dictionary<Type, DefaultAllowPositionalInput> s_DefaultAllowPositionalInput = new();
        private static readonly Dictionary<Type, DefaultAllowRotationalInput> s_DefaultAllowRotationalInput = new();
        private static readonly Dictionary<Type, DefaultUseGravity> s_DefaultUseGravity = new();
        private static readonly Dictionary<Type, DefaultUseRootMotionPosition> s_DefaultUseRootMotionPosition = new();
        private static readonly Dictionary<Type, DefaultUseRootMotionRotation> s_DefaultUseRootMotionRotation = new();

        private static readonly Dictionary<Type, DefaultDetectHorizontalCollisions>
            s_DefaultDetectHorizontalCollisions = new();

        private static readonly Dictionary<Type, DefaultDetectVerticalCollisions> s_DefaultDetectVerticalCollisions =
            new();

        private static readonly Dictionary<Type, DefaultObjectDetection> s_DefaultObjectDetection = new();
        private static readonly Dictionary<Type, DefaultUseLookDirection> s_DefaultUseLookDirection = new();
        private static readonly Dictionary<Type, DefaultCastOffset> s_DefaultCastOffset = new();
        private static readonly Dictionary<Type, DefaultEquippedSlots> s_DefaultEquippedSlots = new();
        private static readonly Dictionary<Type, DefaultReequipSlots> s_DefaultReequipSlots = new();
        ...
        public static Ability AddAbility(UltimateCharacterLocomotion characterLocomotion, Type abilityType, int index)
        {
            ...
            for (var i = abilities.Length - 1; i > index; --i) abilities[i] = abilities[i - 1];
            ...
            int index)
        {
            ...
            for (var i = itemAbilities.Length - 1; i > index; --i) itemAbilities[i] = itemAbilities[i - 1];
        ...
        {
            var abilities = characterLocomotion.Abilities == null
                ? new List<Ability>()
                : new List<Ability>(characterLocomotion.Abilities);
            characterLocomotion.AbilityData = Serialization.Serialize<Ability>(abilities);
            characterLocomotion.Abilities = abilities.ToArray();
#if UNITY_EDITOR
            PrefabUtility.RecordPrefabInstancePropertyModifications(characterLocomotion);
#endif
        }
        ...
        {
            var itemAbilities = characterLocomotion.ItemAbilities == null
                ? new List<ItemAbility>()
                : new List<ItemAbility>(characterLocomotion.ItemAbilities);
        ...
        public static void RemoveAbility<T>(UltimateCharacterLocomotion characterLocomotion) where T : Ability
        {
            ...
            if (ability != null) RemoveAbility(characterLocomotion, ability);
        }
        ...
        public static void RemoveAbility(UltimateCharacterLocomotion characterLocomotion, Ability ability)
        {
            ...
            var index = 0;
            for (var i = 0; i < characterLocomotion.Abilities.Length; ++i)
        ...
        public static void RemoveItemAbility(UltimateCharacterLocomotion characterLocomotion, Ability ability)
        {
            ...
            var index = 0;
            for (var i = 0; i < characterLocomotion.ItemAbilities.Length; ++i)
        ...
        private static RequireComponent[] GetRequiredComponents(Type type)
        {
            RequireComponent[] requiredComponents;
            if (s_RequiredComponents.TryGetValue(type, out requiredComponents)) return requiredComponents;

            if (type.GetCustomAttributes(typeof(RequireComponent), true).Length > 0)
                requiredComponents = type.GetCustomAttributes(typeof(RequireComponent), true) as RequireComponent[];
        ...
        private static void SetAbilityDefaultValues(Ability ability)
        {
            ...
            if (defaultStartType != null) ability.StartType = defaultStartType.StartType;
            var defaultStopType = GetDefaultStopType(abilityType);
            if (defaultStopType != null) ability.StopType = defaultStopType.StopType;
            var defaultAbilityIndex = GetDefaultAbilityIndex(abilityType);
            if (defaultAbilityIndex != null) ability.AbilityIndexParameter = defaultAbilityIndex.Value;
            var defaultAbilityIntData = GetDefaultAbilityIntData(abilityType);
            if (defaultAbilityIntData != null) ability.AbilityIntData = defaultAbilityIntData.Value;
            var defaultState = GetDefaultState(abilityType);
            if (defaultState != null) ability.State = defaultState.Value;
            if (typeof(ItemAbility).IsAssignableFrom(abilityType))
            {
            ...
            if (defaultAllowPositionalInput != null) ability.AllowPositionalInput = defaultAllowPositionalInput.Value;
            var defaultAllowRotationalInput = GetDefaultAllowRotationalInput(abilityType);
            if (defaultAllowRotationalInput != null) ability.AllowRotationalInput = defaultAllowRotationalInput.Value;
            var defaultUseGravity = GetDefaultUseGravity(abilityType);
            if (defaultUseGravity != null) ability.UseGravity = defaultUseGravity.Value;
        ...
        private static DefaultInputName[] GetDefaultInputNames(Type type)
        {
            ...
            if (s_DefaultInputName.TryGetValue(type, out defaultInputNames)) return defaultInputNames;
        ...
        private static DefaultStartType GetDefaultStartType(Type type)
        {
            ...
            if (s_DefaultStartType.TryGetValue(type, out defaultStartType)) return defaultStartType;
        ...
        private static DefaultStopType GetDefaultStopType(Type type)
        {
            ...
            if (s_DefaultStopType.TryGetValue(type, out defaultStopType)) return defaultStopType;
        ...
        private static DefaultAbilityIndex GetDefaultAbilityIndex(Type type)
        {
            ...
            if (s_DefaultAbilityIndex.TryGetValue(type, out defaultAbilityIndex)) return defaultAbilityIndex;

            if (type.GetCustomAttributes(typeof(DefaultAbilityIndex), true).Length > 0)
                defaultAbilityIndex =
                    type.GetCustomAttributes(typeof(DefaultAbilityIndex), true)[0] as DefaultAbilityIndex;
        ...
        private static DefaultAbilityIntData GetDefaultAbilityIntData(Type type)
        {
            ...
            if (s_DefaultAbilityIntData.TryGetValue(type, out defaultStateIndex)) return defaultStateIndex;

            if (type.GetCustomAttributes(typeof(DefaultAbilityIntData), true).Length > 0)
                defaultStateIndex =
                    type.GetCustomAttributes(typeof(DefaultAbilityIntData), true)[0] as DefaultAbilityIntData;
        ...
        private static DefaultItemStateIndex GetDefaultItemStateIndex(Type type)
        {
            ...
            if (s_DefaultItemStateIndex.TryGetValue(type, out defaultItemStateIndex)) return defaultItemStateIndex;

            if (type.GetCustomAttributes(typeof(DefaultItemStateIndex), true).Length > 0)
                defaultItemStateIndex =
                    type.GetCustomAttributes(typeof(DefaultItemStateIndex), true)[0] as DefaultItemStateIndex;
        ...
        private static DefaultState GetDefaultState(Type type)
        {
            ...
            if (s_DefaultState.TryGetValue(type, out defaultState)) return defaultState;
        ...
        private static DefaultAllowPositionalInput GetDefaultAllowPositionalInput(Type type)
        {
            ...
            if (type.GetCustomAttributes(typeof(DefaultAllowPositionalInput), true).Length > 0)
                defaultAllowPositionalInput =
                    type.GetCustomAttributes(typeof(DefaultAllowPositionalInput), true)[0] as
                        DefaultAllowPositionalInput;
        ...
        private static DefaultAllowRotationalInput GetDefaultAllowRotationalInput(Type type)
        {
            ...
            if (type.GetCustomAttributes(typeof(DefaultAllowRotationalInput), true).Length > 0)
                defaultAllowRotationalInput =
                    type.GetCustomAttributes(typeof(DefaultAllowRotationalInput), true)[0] as
                        DefaultAllowRotationalInput;
        ...
        private static DefaultUseGravity GetDefaultUseGravity(Type type)
        {
            ...
            if (s_DefaultUseGravity.TryGetValue(type, out defaultUseGravity)) return defaultUseGravity;
        ...
        private static DefaultUseRootMotionPosition GetDefaultUseRootMotionPosition(Type type)
        {
            ...
            if (type.GetCustomAttributes(typeof(DefaultUseRootMotionPosition), true).Length > 0)
                defaultUseRootMotionPosition =
                    type.GetCustomAttributes(typeof(DefaultUseRootMotionPosition), true)[0] as
                        DefaultUseRootMotionPosition;
        ...
        private static DefaultUseRootMotionRotation GetDefaultUseRootMotionRotation(Type type)
        {
            ...
            if (type.GetCustomAttributes(typeof(DefaultUseRootMotionRotation), true).Length > 0)
                defaultUseRootMotionRotation =
                    type.GetCustomAttributes(typeof(DefaultUseRootMotionRotation), true)[0] as
                        DefaultUseRootMotionRotation;
        ...
        private static DefaultDetectHorizontalCollisions GetDefaultDetectHorizontalCollisions(Type type)
        {
            ...
            if (type.GetCustomAttributes(typeof(DefaultDetectHorizontalCollisions), true).Length > 0)
                defaultDetectHorizontalCollisions =
                    type.GetCustomAttributes(typeof(DefaultDetectHorizontalCollisions), true)[0] as
                        DefaultDetectHorizontalCollisions;
        ...
        private static DefaultDetectVerticalCollisions GetDefaultDetectVerticalCollisions(Type type)
        {
            ...
            if (type.GetCustomAttributes(typeof(DefaultDetectVerticalCollisions), true).Length > 0)
                defaultDetectVerticalCollisions =
                    type.GetCustomAttributes(typeof(DefaultDetectVerticalCollisions), true)[0] as
                        DefaultDetectVerticalCollisions;
        ...
        private static DefaultObjectDetection GetDefaultObjectDetection(Type type)
        {
            ...
            if (s_DefaultObjectDetection.TryGetValue(type, out defaultObjectDetection)) return defaultObjectDetection;

            if (type.GetCustomAttributes(typeof(DefaultObjectDetection), true).Length > 0)
                defaultObjectDetection =
                    type.GetCustomAttributes(typeof(DefaultObjectDetection), true)[0] as DefaultObjectDetection;
        ...
        private static DefaultUseLookDirection GetDefaultUseLookDirection(Type type)
        {
            ...
            if (type.GetCustomAttributes(typeof(DefaultUseLookDirection), true).Length > 0)
                defaultUseLookDirection =
                    type.GetCustomAttributes(typeof(DefaultUseLookDirection), true)[0] as DefaultUseLookDirection;
        ...
        private static DefaultCastOffset GetDefaultCastOffset(Type type)
        {
            ...
            if (s_DefaultCastOffset.TryGetValue(type, out defaultCastOffset)) return defaultCastOffset;
        ...
        private static DefaultEquippedSlots GetDefaultEquippedSlots(Type type)
        {
            ...
            if (s_DefaultEquippedSlots.TryGetValue(type, out defaultEquippedSlots)) return defaultEquippedSlots;

            if (type.GetCustomAttributes(typeof(DefaultEquippedSlots), true).Length > 0)
                defaultEquippedSlots =
                    type.GetCustomAttributes(typeof(DefaultEquippedSlots), true)[0] as DefaultEquippedSlots;
        ...
        private static DefaultReequipSlots GetDefaultReequipSlots(Type type)
        {
            ...
            if (s_DefaultReequipSlots.TryGetValue(type, out defaultReequipSlots)) return defaultReequipSlots;

            if (type.GetCustomAttributes(typeof(DefaultReequipSlots), true).Length > 0)
                defaultReequipSlots =
                    type.GetCustomAttributes(typeof(DefaultReequipSlots), true)[0] as DefaultReequipSlots;
