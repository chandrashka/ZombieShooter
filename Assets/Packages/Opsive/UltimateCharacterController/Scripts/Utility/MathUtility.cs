...
namespace Opsive.UltimateCharacterController.Utility
{
    ...
    public class MathUtility
    {
        ...
        private static readonly Dictionary<Collider, Transform> m_ColliderTransformMap = new();
        ...
        public static float ClampAngle(float angle, float min, float max)
        {
            ...
            {
                if (minDiff <= 0) return angle;
                return min;
            }

            if (maxDiff <= 0) return angle;
            return max;
        }
        ...
        public static float ClampAngle(float angle, float deltaAngle, float min, float max)
        {
            ...
            if (Mathf.Abs(minDiff) < Mathf.Abs(maxDiff))
            {
                if (ClampInnerAngle(min - (angle + deltaAngle)) <= 0) return angle + deltaAngle;
                return min;
            }

            if (ClampInnerAngle(angle + deltaAngle - max) <= 0) return angle + deltaAngle;
            return max;
        }
        ...
        /// <summary>
        ///     Returns the matrix of the first Transform with the deltaRotation applied to the root Transform. This is similar to
        ///     calling Transform.rotation = value on
        ...
        public static float CapsuleColliderHeightMultiplier(CapsuleCollider capsuleCollider)
        {
            ...
            if (capsuleCollider.direction == 2) // Z-axis.
        ...
        /// <summary>
        ///     Concatenate three integers into one. The first int will occupy the millions place, the second int will occupy the
        ///     thousands place, and the third
