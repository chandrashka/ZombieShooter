...
namespace Opsive.UltimateCharacterController.Items.Actions
{
    ...
    public class MeleeWeapon : UsableItem, IDamageOriginator
    {
        ...
        {
            [Tooltip("The ID of the Object Identifier component if the collider is null.")] [SerializeField]
            protected int m_ColliderObjectID = -1;

            [Tooltip(
                "The hitbox can detect collisions if the local vertical offset is greater than the specified value relative to the character's position.")]
            [SerializeField]
            protected float m_MinimumYOffset;

            [Tooltip(
                "The hitbox can detect collisions if the local depth offset is greater than the specified value relative to the character's position.")]
            [SerializeField]
            protected float m_MinimumZOffset;

            [Tooltip("Should the hitbox only register a hit once per use?")] [SerializeField]
            protected bool m_SingleHit;

            [Tooltip(
                "The Surface Impact triggered when the hitbox collides with an object. This will override the MeleeWeapon's SurfaceImpact.")]
            [SerializeField]
            ...
            public int ColliderObjectID => m_ColliderObjectID;
            ...
            public bool Initialize(GameObject meleeWeaponObject, Transform characterTransform)
            {
                if (m_Collider == null)
                {
                    ...
                    {
                        var objectIdentifiers = characterTransform.GetComponentsInChildren<ObjectIdentifier>();
                ...
                GameObject = m_Collider.gameObject;
                Transform = m_Collider.transform;
            ...
            /// <summary>
            ///     Resets the position and rotation of the transform. This will be done immediately before the item is started to be
            ///     used.
            /// </summary>
            ...
            public void Reset(bool startUse)
            {
                ...
                if (startUse) m_HitCollider = false;
            }
            ...
            public bool CanUse()
            {
                ...
                if (m_SingleHit && m_HitCollider) return false;
                ...
                var localOffset = m_CharacterTransform.InverseTransformPoint(Transform.position);
                if (localOffset.y < m_MinimumYOffset || localOffset.z < m_MinimumZOffset) return false;

                // The collider must be moving in order for a new collision to occur.
                var moving = false;
                var position = Transform.position;
                var rotation = Transform.rotation;
                if ((m_PreviousPosition - position).sqrMagnitude > 0.01f) moving = true;
                if (!moving && Quaternion.Angle(m_PreviousRotation, rotation) > 0.01f) moving = true;
        ...
        [SerializeField]
        protected bool m_RequireInAirMeleeAbilityInAir = true;

        [Tooltip("Does the weapon require the Melee Counter Attack Item Ability in order to be used?")] [SerializeField]
        protected bool m_RequireCounterAttackAbility;

        [Tooltip("The value to add to the Item Substate Index when the character is aiming.")] [SerializeField]
        protected int m_AimItemSubstateIndexAddition = 100;

        [Tooltip("The maximum number of collision points which the melee weapon can make contact with.")]
        [SerializeField]
        protected int m_MaxCollisionCount = 20;
        ...
        [Range(-1, 1)]
        [SerializeField]
        protected float m_ForwardShieldSensitivity = -0.8f;

        [Tooltip("When the weapon attacks should only one hit be registered per use?")] [SerializeField]
        protected bool m_SingleHit;

        [Tooltip("If multiple hits can be registered, specifies the minimum frame count between each hit.")]
        [SerializeField]
        protected int m_MultiHitFrameCount = 50;

        [Tooltip("The delay after the weapon has been used when a hit can be valid.")] [SerializeField]
        protected float m_CanHitDelay = 0.2f;

        [Tooltip("Can the next use state play between the ItemUsed and ItemUseComplete events?")] [SerializeField]
        protected bool m_AllowAttackCombos = true;

        [Tooltip("A LayerMask of the layers that can be hit by the weapon.")] [SerializeField]
        ...
        [Tooltip("Specifies if the melee weapon can detect triggers.")] [SerializeField]
        protected QueryTriggerInteraction m_TriggerInteraction = QueryTriggerInteraction.Ignore;

        [Tooltip("Processes the damage dealt to a Damage Target.")] [SerializeField]
        protected DamageProcessor m_DamageProcessor;

        [Tooltip("The amount of damage done to the object hit.")] [SerializeField]
        protected float m_DamageAmount = 30;

        [Tooltip("The amount of force to apply to the object hit.")] [SerializeField]
        ...
        [Tooltip("Should recoil be applied when the weapon hits an object?")] [SerializeField]
        protected bool m_ApplyRecoil = true;

        [Tooltip("Specifies the animator and audio state from a recoil.")] [SerializeField]
        protected AnimatorAudioStateSet m_RecoilAnimatorAudioStateSet = new();

        [Tooltip("A reference to the trail prefab that should be spawned.")] [SerializeField]
        protected GameObject m_Trail;

        [Tooltip("Specifies when the melee weapon trail should be shown.")] [SerializeField]
        protected TrailVisibilityType m_TrailVisibility = TrailVisibilityType.Attack;

        [Tooltip("The delay until the trail should be spawned after it is visible.")] [SerializeField]
        protected float m_TrailSpawnDelay;
        ...
        [SerializeField]
        protected AnimationEventTrigger m_AttackStopTrailEvent = new(false, 0.5f);

        [Tooltip("Unity event invoked when the weapon hits another object.")] [SerializeField]
        protected UnityFloatVector3Vector3GameObjectEvent m_OnImpactEvent;

        public GameObject Owner => m_Character;
        public GameObject OriginatingGameObject => m_GameObject;

        public bool RequireInAirMeleeAbilityInAir
        {
            get => m_RequireInAirMeleeAbilityInAir;
            set => m_RequireInAirMeleeAbilityInAir = value;
        }

        public bool RequireCounterAttackAbility
        {
            get => m_RequireCounterAttackAbility;
            set => m_RequireCounterAttackAbility = value;
        }

        public int AimItemSubstateIndexAddition
        {
            get => m_AimItemSubstateIndexAddition;
            set => m_AimItemSubstateIndexAddition = value;
        }

        public int MaxCollisionCount => m_MaxCollisionCount;

        public float ForwardShieldSensitivity
        {
            get => m_ForwardShieldSensitivity;
            set => m_ForwardShieldSensitivity = value;
        }

        public bool SingleHit
        {
            get => m_SingleHit;
            set => m_SingleHit = value;
        }

        public int MultiHitFrameCount
        {
            get => m_MultiHitFrameCount;
            set => m_MultiHitFrameCount = value;
        }

        public float CanHitDelay
        {
            get => m_CanHitDelay;
            set => m_CanHitDelay = value;
        }

        public bool AllowAttackCombos
        {
            get => m_AllowAttackCombos;
            set => m_AllowAttackCombos = value;
        }
        ...
        {
            get => m_TriggerInteraction;
            set => m_TriggerInteraction = value;
        }
        ...
        {
            get => m_ApplyRecoil;
            set => m_ApplyRecoil = value;
        }

        public AnimatorAudioStateSet RecoilAnimatorAudioStateSet
        {
            get => m_RecoilAnimatorAudioStateSet;
            set => m_RecoilAnimatorAudioStateSet = value;
        }
        ...
        public TrailVisibilityType TrailVisibility
        {
            get => m_TrailVisibility;
            set => m_TrailVisibility = value;
        }

        public float TrailSpawnDelay
        {
            get => m_TrailSpawnDelay;
            set => m_TrailSpawnDelay = value;
        }

        public AnimationEventTrigger AttackStopTrailEvent
        {
            get => m_AttackStopTrailEvent;
            set => m_AttackStopTrailEvent = value;
        }

        public UnityFloatVector3Vector3GameObjectEvent OnImpactEvent
        {
            get => m_OnImpactEvent;
            set => m_OnImpactEvent = value;
        }
        ...
        private readonly HashSet<GameObject> m_HitList = new();
        ...
        public UltimateCharacterLocomotion CharacterLocomotion { get; private set; }

        public bool Used { get; private set; }

        public int UsedSubstateIndex { get; private set; } = -1;
        ...
        protected override void Awake()
        {
            ...
            CharacterLocomotion = m_Character.GetCachedComponent<UltimateCharacterLocomotion>();
            m_CharacterTransform = m_Character.transform;
            m_CollidersHit = new Collider[m_MaxCollisionCount];
            m_CollisionsHit = new RaycastHit[m_MaxCollisionCount];

            m_RecoilAnimatorAudioStateSet.DeserializeAnimatorAudioStateSelector(m_Item, CharacterLocomotion);
        ...
        protected override void Start()
        {
            ...
            if (m_MeleeWeaponPerspectiveProperties == null)
            {
                ...
                if (m_MeleeWeaponPerspectiveProperties == null)
                    Debug.LogError(
                        $"Error: The First/Third Person Melee Weapon Properties component cannot be found for the {name}. " +
        ...
        {
            if (m_HasRecoil) return m_RecoilAnimatorAudioStateSet.GetItemSubstateIndex();
            if (m_Attacking)
                return UsedSubstateIndex =
                    base.GetItemSubstateIndex() + (m_Aiming ? m_AimItemSubstateIndexAddition : 0);
            return -1;
        }
        ...
        private void SpawnTrail()
        {
            ...
            if (CharacterLocomotion.FirstPersonPerspective)
                trailLocation = m_FirstPersonPerspectiveProperties != null
                    ? (m_FirstPersonPerspectiveProperties as IMeleeWeaponPerspectiveProperties).TrailLocation
                    : null;
            else
                trailLocation = m_ThirdPersonPerspectiveProperties != null
                    ? (m_ThirdPersonPerspectiveProperties as IMeleeWeaponPerspectiveProperties).TrailLocation
                    : null;
        ...
        {
            if (!base.CanUseItem(itemAbility, abilityState)) return false;
            ...
            if (m_RequireInAirMeleeAbilityInAir && !(itemAbility is InAirMeleeUse) &&
                ((CharacterLocomotion.UsingGravity && !CharacterLocomotion.Grounded) ||
                 CharacterLocomotion.IsAbilityTypeActive<Jump>()))
                return false;

            // The MeleeWeapon may require the MeleeCounterAttack ability in order for it to be used.
            if (m_RequireCounterAttackAbility && !(itemAbility is MeleeCounterAttack)) return false;
                ...
                if (m_Attacking)
                {
                    if (!Used)
                    {
                        ...
                        if (m_AllowAttackCombos && m_UseAnimatorAudioStateSet.States.Length == 1)
                            Debug.LogError(
                                "Error: The MeleeWeapon cannot be used again. Another state should be added to the Use Animator Audio State Set. See the first troubleshooting tip for more info: " +
        ...
        public override void UseItemUpdate()
        {
            ...
            if ((m_SingleHit && m_AttackHit) || m_SolidObjectHit || !m_Attacking) return;

            // The item can't hit anything until after the delay.
            if (m_AttackTime + m_CanHitDelay > Time.time) return;
            ...
            for (var i = 0; i < hitboxes.Length; ++i)
            {
                ...
                if (hitboxCollider is BoxCollider)
                {
                    ...
                    hitCount = Physics.OverlapBoxNonAlloc(hitboxTransform.TransformPoint(boxCollider.center),
                        Vector3.Scale(boxCollider.size, boxCollider.transform.lossyScale) / 2, m_CollidersHit,
                        hitboxTransform.rotation, m_ImpactLayers, m_TriggerInteraction);
                }
                else if (hitboxCollider is SphereCollider)
                {
                    var sphereCollider = hitboxCollider as SphereCollider;
                    hitCount = Physics.OverlapSphereNonAlloc(hitboxTransform.TransformPoint(sphereCollider.center),
                        sphereCollider.radius * MathUtility.ColliderRadiusMultiplier(sphereCollider), m_CollidersHit,
                        m_ImpactLayers, m_TriggerInteraction);
                }
                else if (hitboxCollider is CapsuleCollider)
                {
                    ...
                    MathUtility.CapsuleColliderEndCaps(capsuleCollider,
                        hitboxTransform.TransformPoint(capsuleCollider.center), hitboxTransform.rotation,
                        out startEndCap, out endEndCap);
                    hitCount = Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap,
                        capsuleCollider.radius * MathUtility.CapsuleColliderHeightMultiplier(capsuleCollider),
                        m_CollidersHit, m_ImpactLayers, m_TriggerInteraction);
                }

                hitboxCollider.enabled = true;
                CharacterLocomotion.EnableColliderCollisionLayer(true);
                ...
                if (hitCount > 0)
                {
                    ...
                    if (hitCount == m_MaxCollisionCount)
                        Debug.LogWarning(
                            $"Warning: The maximum number of colliders have been hit by {m_GameObject.name}. Consider increasing the Max Collision Count value.");
#endif
                    for (var j = 0; j < hitCount; ++j)
                    {
                        ...
                        var hitCharacterLocomotion =
                            hitGameObject.GetCachedParentComponent<UltimateCharacterLocomotion>();
                        if (hitCharacterLocomotion != null && hitCharacterLocomotion == CharacterLocomotion) continue;
                        ...
                        if (HitCollider(i, hitCollider, hitCharacterLocomotion))
                        {
                            ...
                            if (m_SingleHit || m_SolidObjectHit) break;
                        }
                    }
                    ...
                    if (m_HasRecoil)
                    {
                        ...
                        if (selector is RecoilAnimatorAudioStateSelector)
                            (selector as RecoilAnimatorAudioStateSelector).NextState(hitCount, m_CollidersHit,
                                m_UseAnimatorAudioStateSet.GetStateIndex());
                        else
                            m_RecoilAnimatorAudioStateSet.NextState();

                        CharacterLocomotion.UpdateItemAbilityAnimatorParameters();
        ...
        private bool HitCollider(int hitboxIndex, Collider other, UltimateCharacterLocomotion hitCharacterLocomotion)
        {
            ...
            if ((other is BoxCollider || other is SphereCollider || other is CapsuleCollider ||
                 (other is MeshCollider && (other as MeshCollider).convex)) &&
                Physics.ComputePenetration(hitbox.Collider, hitbox.Transform.position, hitbox.Transform.rotation, other,
                    other.transform.position, other.transform.rotation, out direction, out distance))
            {
                ...
                var closestPoint = Physics.ClosestPoint(hitbox.Transform.position + offset, other,
                    otherTransform.position, otherTransform.rotation);
                ...
                // hitting the object.
                hitCount = Physics.SphereCastNonAlloc(closestPoint + offset, CharacterLocomotion.ColliderSpacing,
                    -direction, m_CollisionsHit,
                    distance + CharacterLocomotion.ColliderSpacing * 2, 1 << other.gameObject.layer,
            ...
            else
            {
                ...
                if (hitboxCollider is BoxCollider)
                {
                    ...
                    hitCount = Physics.BoxCastNonAlloc(
                        MathUtility.TransformPoint(position, rotation, boxCollider.center), boxCollider.size / 2,
                        m_CharacterTransform.forward,
                ...
                else if (hitboxCollider is SphereCollider)
                {
                    ...
                    hitCount = Physics.SphereCastNonAlloc(
                        MathUtility.TransformPoint(position, rotation, sphereCollider.center),
                        sphereCollider.radius, m_CharacterTransform.forward, m_CollisionsHit, distance,
                        1 << other.gameObject.layer, m_TriggerInteraction);
                }
                else if (hitboxCollider is CapsuleCollider)
                {
                    var capsuleCollider = hitboxCollider as CapsuleCollider;
                    ...
                    MathUtility.CapsuleColliderEndCaps(capsuleCollider, position, rotation, out firstEndCap,
                        out secondEndCap);
                    hitCount = Physics.CapsuleCastNonAlloc(firstEndCap, secondEndCap, radius,
                        m_CharacterTransform.forward, m_CollisionsHit,
            ...
            if (hitCount > 0)
            {
                ...
                {
                    if (m_CollisionsHit[i].collider != other) continue;
                    if (m_MultiHitFrameCount > 0 && Time.frameCount - m_LastHitFrame <= m_MultiHitFrameCount) continue;
                    ...
                    if (hitCharacterLocomotion != null)
                        ...
                        if (Vector3.Dot(hitCharacterLocomotion.transform.forward, m_CharacterTransform.forward) <
                            m_ForwardShieldSensitivity)
                        {
                            ...
                            if (hitInventory != null)
                                for (var k = 0; k < hitInventory.SlotCount; ++k)
                                {
                                    var equippedItem = hitInventory.GetActiveItem(k);
                                    if (equippedItem == null) continue;

                                    // The equipped item must be a shield with a collider.
                                    for (var m = 0; m < equippedItem.ItemActions.Length; ++m)
                                    {
                                        var itemAction = equippedItem.ItemActions[m];
                                        if (!(itemAction is Shield)) continue;

                                        if ((itemAction as Shield).RequireAim &&
                                            !CharacterLocomotion.IsAbilityTypeActive<Aim>()) continue;

                                        var visibleObject = equippedItem.ActivePerspectiveItem.GetVisibleObject();
                                        Collider visibleObjectCollider;
                                        if (visibleObject != null && (visibleObjectCollider =
                                                visibleObject.GetCachedComponent<Collider>()))
                                        {
                    ...
                    if (m_HitList.Contains(hitListGameObject)) continue;
        ...
        public void HitCollider(MeleeHitbox hitbox, RaycastHit raycastHit, GameObject hitGameObject,
            Collider hitCollider, IDamageTarget damageTarget)
        {
            ...
            EventHandler.ExecuteEvent<float, Vector3, Vector3, GameObject, object, Collider>(hitGameObject,
                "OnObjectImpact", damageAmount, raycastHit.point, raycastHit.normal * m_ImpactForce, m_Character, this,
                hitCollider);
            ...
            if (damageAmount > 0)
            {
                if (damageTarget != null)
                {
                    ...
                    pooledDamageData.SetDamage(this, damageAmount, raycastHit.point, -raycastHit.normal, m_ImpactForce,
                        m_ImpactForceFrames, 0, hitCollider);
                    var damageProcessorModule =
                        CharacterLocomotion.gameObject.GetCachedComponent<DamageProcessorModule>();
                ...
                else if (m_ImpactForce > 0 && raycastHit.rigidbody != null && !raycastHit.rigidbody.isKinematic)
                {
                    ...
                    raycastHit.rigidbody.AddForceAtPosition(
                        m_ImpactForce * MathUtility.RigidbodyForceMultiplier * -raycastHit.normal, raycastHit.point);
                }
            }
            ...
            SurfaceManager.SpawnEffect(raycastHit, hitCollider, surfaceImpact, CharacterLocomotion.GravityDirection,
                CharacterLocomotion.TimeScale, hitbox.GameObject);
        }
        ...
        public override void ItemUseComplete()
        {
            ...
            if (!m_AllowAttackCombos) m_NextCanAttackTime = Time.time + Time.fixedDeltaTime;
        }
        ...
        {
            if (m_Attacking && !m_HasRecoil) return false;
        ...
        {
            if (m_TrailVisibility != TrailVisibilityType.Attack || !m_Attacking) return;
        ...
        protected override void OnChangePerspectives(bool firstPersonPerspective)
        {
            ...
            {
                var hitboxes = m_MeleeWeaponPerspectiveProperties.Hitboxes;
                for (var i = 0; i < hitboxes.Length; ++i) hitboxes[i].Reset(false);
            }
