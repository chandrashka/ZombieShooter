...
{
    /// <summary>
    ///     The Shield will absorb damage applied to the character. It has its own strength factor so when too much damage has
    ///     been taken it will no longer be effective.
    /// </summary>
    public class Shield : ItemAction
    {
        [Tooltip("Does the shield only protect the player when the character is aiming?")] [SerializeField]
        protected bool m_RequireAim;
        ...
        [Range(0, 1)]
        [SerializeField]
        protected float m_AbsorptionFactor = 1;

        [Tooltip("Should the shield absorb damage caused by explosions?")] [SerializeField]
        protected bool m_AbsorbExplosions;

        [Tooltip("Should an impact be applied when the weapon is hit by another object?")] [SerializeField]
        protected bool m_ApplyImpact = true;

        [Tooltip("Specifies the animator and audio state for when the shield is impacted by another object.")]
        [SerializeField]
        protected AnimatorAudioStateSet m_ImpactAnimatorAudioStateSet = new();
        ...
        [SerializeField]
        protected AnimationSlotEventTrigger m_ImpactCompleteEvent = new(false, 0.2f);

        [Tooltip(
            "The name of the shield's durability attribute. When the durability reaches 0 the shield will not absorb any damage.")]
        [SerializeField]
        protected string m_DurabilityAttributeName = "Durability";

        [Tooltip("Should the item be dropped from the character when the durability is depleted?")] [SerializeField]
        protected bool m_DropWhenDurabilityDepleted;
        ...
        {
            get => m_RequireAim;
            set => m_RequireAim = value;
        }

        public float AbsorptionFactor
        {
            get => m_AbsorptionFactor;
            set => m_AbsorptionFactor = value;
        }

        public bool AbsorbExplosions
        {
            get => m_AbsorbExplosions;
            set => m_AbsorbExplosions = value;
        }

        public bool ApplyImpact
        {
            get => m_ApplyImpact;
            set => m_ApplyImpact = value;
        }

        public AnimatorAudioStateSet ImpactAnimatorAudioStateSet
        {
            get => m_ImpactAnimatorAudioStateSet;
            set => m_ImpactAnimatorAudioStateSet = value;
        }

        public AnimationSlotEventTrigger ImpactCompleteEvent
        {
            get => m_ImpactCompleteEvent;
            set => m_ImpactCompleteEvent = value;
        }

        public string DurabilityAttributeName
        {
            get => m_DurabilityAttributeName;
            set
            {
        ...
        public bool DropWhenDurabilityDepleted
        {
            get => m_DropWhenDurabilityDepleted;
            set => m_DropWhenDurabilityDepleted = value;
        }

        public float DurabilityValue => m_DurabilityAttribute != null ? m_DurabilityAttribute.Value : 0;
        ...
        protected override void Awake()
        {
            ...
            if (!string.IsNullOrEmpty(m_DurabilityAttributeName))
            {
                ...
                {
                    Debug.LogError("Error: The shield " + m_GameObject.name +
                                   " has a durability attribute specified but no Attribute Manager component.");
                }
                else
                {
                    m_DurabilityAttribute = m_AttributeManager.GetAttribute(m_DurabilityAttributeName);
                    ...
                    if (m_DurabilityAttribute != null)
                        EventHandler.RegisterEvent(m_DurabilityAttribute, "OnAttributeReachedDestinationValue",
                            DurabilityDepleted);
                }
            }

            m_ImpactAnimatorAudioStateSet.DeserializeAnimatorAudioStateSelector(m_Item,
        ...
        protected override void OnDestroy()
        {
            ...
            if (m_DurabilityAttribute != null)
                EventHandler.UnregisterEvent(m_DurabilityAttribute, "OnAttributeReachedDestinationValue",
                    DurabilityDepleted);
        ...
        {
            if (m_HasImpact) return m_ImpactAnimatorAudioStateSet.GetItemSubstateIndex();
            return -1;
        }
        ...
        public float Damage(object source, float amount)
        {
            ...
            if (!m_Aiming && m_RequireAim) return amount;

            // The shield may not be able to absorb damage caused by explosions.
            if (source is Explosion && !m_AbsorbExplosions) return amount;
            ...
            if (m_DurabilityAttribute == null) return 0;

            // If the shield's durability is depleted then the entire damage amount should be applied to the character.
            if (m_DurabilityAttribute.Value == m_DurabilityAttribute.MinValue) return amount;
        ...
        {
            if (!inputStart && !(m_Item.CharacterLocomotion.LookSource is LocalLookSource)) return;
            m_Aiming = aim;
        }
        ...
        {
            if (!m_DropWhenDurabilityDepleted) return;
