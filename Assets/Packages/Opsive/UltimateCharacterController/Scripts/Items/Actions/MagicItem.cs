...
namespace Opsive.UltimateCharacterController.Items.Actions
{
    ...
    /// <summary>
    ///     The Magic Item uses Cast Actions to perform different magical actions when used. Impact Actions will then perform
    ///     an action from the resulting cast impact.
    /// </summary>
    public class MagicItem : UsableItem
    {
        ...
        [Tooltip("Is the character required to be on the ground?")] [SerializeField]
        protected bool m_RequireGrounded = true;

        [Tooltip("The direction of the cast.")] [SerializeField]
        protected CastDirection m_Direction = CastDirection.Forward;

        [Tooltip("Should the look source be used when determining the cast direction?")] [SerializeField]
        protected bool m_UseLookSource = true;

        [Tooltip("The maximum distance of the movement cast direction.")] [SerializeField]
        protected float m_MaxDistance = 100;

        [Tooltip("The radius of the movement cast direction.")] [SerializeField]
        protected float m_Radius = 0.1f;

        [Tooltip("The layers that the movement directions can collide with.")] [SerializeField]
        protected LayerMask m_DetectLayers = ~((1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.UI) |
                                               (1 << LayerManager.SubCharacter) | (1 << LayerManager.Overlay) |
        ...
        [SerializeField]
        protected float m_MaxAngle = 30;

        [Tooltip("The maximum number of colliders that can be detected by the target cast.")] [SerializeField]
        protected int m_MaxCollisionCount = 100;

        [Tooltip("The number of objects that a single cast should cast.")] [SerializeField]
        protected int m_TargetCount = 1;

        [Tooltip("The transform used to indicate the surface. Can be null.")] [SerializeField]
        protected Transform m_SurfaceIndicator;

        [Tooltip("The offset when positioning the surface indicator.")] [SerializeField]
        protected Vector3 m_SurfaceIndicatorOffset = new(0, 0.1f, 0);

        [Tooltip("Specifies how often the cast is used.")] [SerializeField]
        protected CastUseType m_UseType;
        ...
        [SerializeField]
        protected float m_MinContinuousUseDuration = 1;

        [Tooltip("Should the continuous use type cast every update?")] [SerializeField]
        protected bool m_ContinuousCast;

        [Tooltip("The amount of the ItemIdentifier that should be used each cast.")] [SerializeField]
        protected int m_UseAmount;

        [Tooltip("Specifies when the cast should be interrupted.")] [SerializeField]
        protected CastInterruptSource m_InterruptSource;

        [Tooltip("The SurfaceImpact of the cast.")] [SerializeField]
        protected SurfaceImpact m_SurfaceImpact;

        [Tooltip("The value to add to the Item Substate Index when the item can stop.")] [SerializeField]
        protected int m_CanStopSubstateIndexAddition = 10;

        [Tooltip("Unity event invoked when the begin or end actions are started or stopped.")] [SerializeField]
        protected UnityItemBoolBoolEvent m_OnStartStopBeginEndActionsEvent;

        [Tooltip("Unity event invoked when the magic item casts its actions.")] [SerializeField]
        protected UnityItemEvent m_OnCastEvent;

        [Tooltip("Unity event invoked when there is an impact.")] [SerializeField]
        protected UnityItemRaycastHitSurfaceImpactEvent m_OnImpactEvent;

        [Tooltip("The serialization data for the BeginActions.")] [SerializeField] [ForceSerialized]
        protected Serialization[] m_BeginActionData;

        [Tooltip("The serialization data for the CastActions.")] [SerializeField] [ForceSerialized]
        protected Serialization[] m_CastActionData;

        [Tooltip("The serialization data for the MagicImpactActions.")] [SerializeField] [ForceSerialized]
        protected Serialization[] m_ImpactActionData;

        [Tooltip("The serialization data for the EndActions.")] [SerializeField] [ForceSerialized]
        protected Serialization[] m_EndActionData;
        ...
        public bool UseLookSource
        {
            get => m_UseLookSource;
            set => m_UseLookSource = value;
        }
        ...
        {
            get => m_MaxCollisionCount;
            set
            {
                m_MaxCollisionCount = value;
                if (m_TargetColliders != null)
                {
                    Array.Resize(ref m_TargetColliders, m_MaxCollisionCount);
                    Array.Resize(ref m_TargetAngles, m_MaxCollisionCount);
                }
            }
        }
        ...
        public Transform SurfaceIndicator => m_SurfaceIndicator;

        public Vector3 SurfaceIndicatorOffset
        {
            get { return m_SurfaceIndicatorOffset; }
            set
            {
#if ULTIMATE_CHARACTER_CONTROLLER_MULTIPLAYER
        ...
        public CastUseType UseType
        {
            get => m_UseType;
            set => m_UseType = value;
        }

        public float MinContinuousUseDuration
        {
            get => m_MinContinuousUseDuration;
            set => m_MinContinuousUseDuration = value;
        }

        public bool ContinuousCast
        {
            get => m_ContinuousCast;
            set => m_ContinuousCast = value;
        }

        public int UseAmount
        {
            get => m_UseAmount;
            set => m_UseAmount = value;
        }

        public CastInterruptSource InterruptSource
        {
            get => m_InterruptSource;
            set => m_InterruptSource = value;
        }
        ...
        public int CanStopSubstateIndexAddition
        {
            get => m_CanStopSubstateIndexAddition;
            set => m_CanStopSubstateIndexAddition = value;
        }

        public UnityItemBoolBoolEvent OnStartStopBeginEndActionsEvent
        {
            get => m_OnStartStopBeginEndActionsEvent;
            set => m_OnStartStopBeginEndActionsEvent = value;
        }

        public UnityItemEvent OnCastEvent
        {
            get => m_OnCastEvent;
            set => m_OnCastEvent = value;
        }

        public UnityItemRaycastHitSurfaceImpactEvent OnImpactEvent
        {
            get => m_OnImpactEvent;
        ...
        {
            get => m_BeginActionData;
            set
            {
                m_BeginActionData = value;
                if (!Application.isPlaying) DeserializeBeginActions(false);
            }
        }

        public Serialization[] CastActionData
        {
            get => m_CastActionData;
            set
            {
                m_CastActionData = value;
                if (!Application.isPlaying) DeserializeCastActions(false);
            }
        }

        public Serialization[] ImpactActionData
        {
            get => m_ImpactActionData;
            set
            {
                m_ImpactActionData = value;
                if (!Application.isPlaying) DeserializeImpactActions(false);
            }
        }
        ...
        public Serialization[] EndActionData
        {
            ...
            set
            {
                ...
                if (!Application.isPlaying) DeserializeEndActions(false);
            }
        }
        ...
        public IMagicItemPerspectiveProperties MagicItemPerspectiveProperties { get; private set; }
        ...
        protected override void Awake()
        {
            ...
            MagicItemPerspectiveProperties = m_ActivePerspectiveProperties as IMagicItemPerspectiveProperties;
            ...
            if (m_SurfaceIndicator != null) m_SurfaceIndicator.gameObject.SetActive(false);
#if ULTIMATE_CHARACTER_CONTROLLER_MULTIPLAYER
            // The local surface indicator should not show for remote players.
            if (m_NetworkInfo != null && !m_NetworkInfo.IsLocalPlayer()) {
        ...
        public void DeserializeBeginActions(bool forceDeserialization)
        {
            ...
            if (m_BeginActions != null && !forceDeserialization) return;

            if (m_BeginActionData != null)
            {
                m_BeginActions = new BeginEndAction[m_BeginActionData.Length];
                for (var i = 0; i < m_BeginActions.Length; ++i)
                {
                    m_BeginActions[i] =
                        m_BeginActionData[i].DeserializeFields(MemberVisibility.Public) as BeginEndAction;
                    if (Application.isPlaying) m_BeginActions[i].Initialize(m_Character, this, true, i);
                }
            }
        }
        ...
        public void DeserializeImpactActions(bool forceDeserialization)
        {
            ...
            if (m_ImpactActions != null && !forceDeserialization) return;

            if (m_ImpactActionData != null)
            {
                m_ImpactActions = new ImpactAction[m_ImpactActionData.Length];
                for (var i = 0; i < m_ImpactActions.Length; ++i)
                {
                    m_ImpactActions[i] =
                        m_ImpactActionData[i].DeserializeFields(MemberVisibility.Public) as ImpactAction;
                    if (Application.isPlaying) m_ImpactActions[i].Initialize(m_Character, this, i);
                }
            }
        }
        ...
        protected override void Start()
        {
            ...
            if (MagicItemPerspectiveProperties == null)
            {
                MagicItemPerspectiveProperties = m_ActivePerspectiveProperties as IMagicItemPerspectiveProperties;

                if (MagicItemPerspectiveProperties == null)
                    Debug.LogError(
                        $"Error: The First/Third Person Magic Item Properties component cannot be found for the Item {name}." +
        ...
        public override bool CanUseItem(ItemAbility itemAbility, UseAbilityState abilityState)
        {
            ...
            if (abilityState == UseAbilityState.Update)
            {
                ...
                if (m_Used && CanStopItemUse()) return false;
            }
            else if (abilityState == UseAbilityState.Start)
            {
                // Certain items require the character to be grounded.
                if (m_RequireGrounded && !m_CharacterLocomotion.Grounded) return false;
                ...
                if (m_Direction == CastDirection.Target) DetermineTargetColliders();
                if (!DetermineCastValues(0, ref m_CastDirection, ref m_CastPosition, ref m_CastNormal)) return false;
            }
        ...
        {
            if (!m_StartAbility.AllowPositionalInput && ability is StoredInputAbilityBase) return false;
        ...
        public override void StartItemUse(ItemAbility itemAbility)
        {
            ...
            if (m_CastActionUsed != null)
                for (var i = 0; i < m_CastActionUsed.Length; ++i)
        ...
        public void StartStopBeginEndActions(bool beginActions, bool start, bool networkEvent)
        {
            ...
            if (actions != null)
                for (var i = 0; i < actions.Length; ++i)
                    if (start)
                        actions[i].Start(MagicItemPerspectiveProperties.OriginLocation);
                    else
                        actions[i].Stop();
        ...
        public override void UseItem()
        {
            ...
            if (m_CastActions != null)
            {
                ...
                if (!actionsCasted)
                {
                    ...
                    for (var i = 0; i < useCount; ++i)
                    {
                        ...
                        var castEvent = false;
                        for (var j = 0; j < m_CastActions.Length; ++j)
                        {
                            if (!m_ContinuousCast && m_CastActionUsed[j]) continue;

                            // The action may not need to cast if it's not time yet.
                            if (m_StartCastTime + m_CastActions[j].Delay > Time.time) continue;

                            if (m_CastActions[j].CastID == 0) m_CastActions[j].CastID = ++m_CastID;

                            m_CastActions[j].Cast(MagicItemPerspectiveProperties.OriginLocation, m_CastDirection,
                                m_CastPosition);
                            ...
                            if (!castEvent)
                            {
                                ...
                                if (m_OnCastEvent != null) m_OnCastEvent.Invoke(m_Item);
                                castEvent = true;
                            }
                        }
                    }
            ...
            if (!m_Used)
            {
                ...
                if (m_UseType == CastUseType.Single) StartStopBeginEndActions(false, true, true);
        ...
        private void DetermineTargetColliders()
        {
            if (m_TargetColliders == null)
            {
                ...
                for (var i = 0; i < m_TargetAngles.Length; ++i) m_TargetAngles[i] = int.MaxValue;
            }

            var hitCount = Physics.OverlapSphereNonAlloc(m_CharacterTransform.position, m_MaxDistance,
                m_TargetColliders, m_DetectLayers, QueryTriggerInteraction.Ignore);
            ...
            for (var i = 0; i < hitCount; ++i)
            {
                ...
                if (angle < m_MaxAngle * 0.5f)
                {
                    ...
                    if (Physics.Linecast(m_CharacterTransform.position, m_TargetColliders[i].transform.position,
                            out var raycastHit, m_CharacterLayerManager.IgnoreInvisibleCharacterWaterLayers,
                            QueryTriggerInteraction.Ignore))
                    {
                        var raycastTransform = raycastHit.collider.transform;
                        if (raycastTransform.IsChildOf(m_TargetColliders[i].transform) ||
                            raycastTransform.IsChildOf(m_CharacterTransform)) hitTransform = true;
                    }
            ...
            Array.Sort(m_TargetAngles, m_TargetColliders);
        }
        ...
        private bool DetermineCastValues(int index, ref Vector3 direction, ref Vector3 position, ref Vector3 normal)
        {
            if (m_Direction == CastDirection.Forward || m_Direction == CastDirection.Indicate)
            {
                ...
                {
                    direction = m_UseLookSource
                        ? m_LookSource.LookDirection(MagicItemPerspectiveProperties.OriginLocation.position, false,
                            m_CharacterLayerManager.SolidObjectLayers, true, true)
                ...
                if (Physics.SphereCast(castPosition - direction * m_Radius, m_Radius, direction, out var raycastHit,
                        m_MaxDistance, m_DetectLayers, QueryTriggerInteraction.Ignore))
                {
            ...
            {
                if (index >= m_TargetAngles.Length || m_TargetAngles[index] == int.MaxValue) return false;
                ...
                direction = (position - MagicItemPerspectiveProperties.OriginLocation.position).normalized;
        ...
        public override void UseItemUpdate()
        {
            ...
            if (beginEndActions != null)
                for (var i = 0; i < beginEndActions.Length; ++i)
        ...
        public void LateUpdate()
        {
            ...
            if (m_Direction != CastDirection.Indicate ||
                !DetermineCastValues(-1, ref direction, ref position, ref normal))
            {
            ...
            {
                m_SurfaceIndicator.SetPositionAndRotation(
                    position + m_SurfaceIndicator.TransformDirection(m_SurfaceIndicatorOffset),
                    Quaternion.LookRotation(Vector3.ProjectOnPlane(direction, normal)));
                if (!m_SurfaceIndicator.gameObject.activeSelf) m_SurfaceIndicator.gameObject.SetActive(true);
            }
        }
        ...
        {
            if (!IsValidCollisionObject(target)) return;
            ...
            if (m_OnImpactEvent != null) m_OnImpactEvent.Invoke(m_Item, hit, m_SurfaceImpact);
        }
        ...
        {
            if (m_Direction == CastDirection.Target) return MathUtility.InLayerMask(other.layer, m_DetectLayers);
            return true;
        }

        /// <summary>
        ///     The impact actions should be interrupted. Consider the case where the first action deducts health and the second
        ///     action plays a particle effect.
        ...
        public override void TryStopItemUse()
        {
            ...
            if (!m_Stopping && CanStopItemUse())
            {
                ...
                if (m_UseType == CastUseType.Continuous) StartStopBeginEndActions(false, true, true);
            }
        }
        ...
        public override bool CanStopItemUse()
        {
            ...
            return m_UseType == CastUseType.Single ||
                   (m_StopRequested && (m_MinContinuousUseDuration == -1 || (m_MinContinuousUseDuration > 0 &&
                                                                             m_StartUseTime +
                                                                             m_MinContinuousUseDuration < Time.time)));
        }
        ...
        public override void Unequip()
        {
            ...
            EventHandler.UnregisterEvent<float, Vector3, Vector3, GameObject, Collider>(m_Character, "OnHealthDamage",
                OnDamage);
        }
        ...
        {
            if (!active || (!(ability is Jump) && !(ability is Fall))) return;
        ...
        protected override void OnChangePerspectives(bool firstPersonPerspective)
        {
            ...
            MagicItemPerspectiveProperties = targetMagicItemPerspectiveProperties;

            if (m_BeginActions != null)
                for (var i = 0; i < m_BeginActions.Length; ++i)
                    m_BeginActions[i].OnChangePerspectives(MagicItemPerspectiveProperties.OriginLocation);
            if (m_CastActions != null)
                for (var i = 0; i < m_CastActions.Length; ++i)
                    m_CastActions[i].OnChangePerspectives(MagicItemPerspectiveProperties.OriginLocation);
            if (m_EndActions != null)
                for (var i = 0; i < m_EndActions.Length; ++i)
                    m_EndActions[i].OnChangePerspectives(MagicItemPerspectiveProperties.OriginLocation);
        }
