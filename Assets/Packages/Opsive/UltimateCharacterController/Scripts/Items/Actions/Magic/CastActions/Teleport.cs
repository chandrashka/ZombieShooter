...
namespace Opsive.UltimateCharacterController.Items.Actions.Magic.CastActions
{
    ...
    {
        [Tooltip("Should the character's animator be snapped?")] [SerializeField]
        ...
        public override bool IsValidTargetPosition(Vector3 position, Vector3 normal)
        {
            ...
            if (Vector3.Angle(m_CharacterLocomotion.Up, normal) >
                m_CharacterLocomotion.SlopeLimit + m_CharacterLocomotion.SlopeLimitSpacing) return false;

            // There must be enough space to stand.
            if (Physics.SphereCast(
                    new Ray(position - m_CharacterLocomotion.Up * m_CharacterLocomotion.Radius,
                        m_CharacterLocomotion.Up), m_CharacterLocomotion.Radius,
                    m_CharacterLocomotion.Height, m_CharacterLayerManager.IgnoreInvisibleCharacterWaterLayers,
        ...
        {
            direction = Vector3.ProjectOnPlane(targetPosition - m_GameObject.transform.position,
                m_CharacterLocomotion.Up);
            m_CharacterLocomotion.SetPositionAndRotation(targetPosition, Quaternion.LookRotation(direction),
