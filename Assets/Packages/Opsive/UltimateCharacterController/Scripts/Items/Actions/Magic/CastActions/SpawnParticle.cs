...
namespace Opsive.UltimateCharacterController.Items.Actions.Magic.CastActions
{
    ...
    public class SpawnParticle : CastAction, IMagicObjectAction
    {
        ...
        [Tooltip("Should the directional vector be projected onto the character's normal plane?")] [SerializeField]
        protected bool m_ProjectDirectionOnPlane;

        [Tooltip("Should the particle's parent be cleared when the cast stops?")] [SerializeField]
        protected bool m_ClearParentOnStop;

        [Tooltip("Should the particle's Length Scale be set?")] [SerializeField]
        protected bool m_SetRendererLengthScale;

        [Tooltip("Additional value to add to the particle's Length Scale.")] [SerializeField]
        protected float m_AdditionalLength = 0.1f;

        [Tooltip("The layer that the particle should occupy.")] [SerializeField]
        protected int m_ParticleLayer = LayerManager.IgnoreRaycast;

        [Tooltip("The duration of the alpha fade in.")] [SerializeField]
        protected float m_FadeInDuration;

        [Tooltip("The duration of the alpha fade out.")] [SerializeField]
        protected float m_FadeOutDuration;

        [Tooltip("The name of the material's color property.")] [SerializeField]
        protected string m_MaterialColorName = "_TintColor";

        [Tooltip("The delta step of the alpha fade.")] [SerializeField]
        protected float m_FadeStep = 0.05f;
        ...
        public GameObject ParticlePrefab
        {
            ...
            set
            {
                if (m_ParticlePrefab != value)
                {
                    m_ParticlePrefab = value;
                    m_Renderers = null;
                }
            }
        }
        ...
        public bool ProjectDirectionOnPlane
        {
            get => m_ProjectDirectionOnPlane;
            set => m_ProjectDirectionOnPlane = value;
        }

        public bool ClearParentOnStop
        {
            get => m_ClearParentOnStop;
            set => m_ClearParentOnStop = value;
        }

        public bool SetRendererLengthScale
        {
            get => m_SetRendererLengthScale;
            set => m_SetRendererLengthScale = value;
        }

        public float AdditionalLength
        {
            get => m_AdditionalLength;
            set => m_AdditionalLength = value;
        }
        ...
        public int ParticleLayer
        {
            ...
            set
            {
                ...
                if (m_ParticleTransform == null) return;
        ...
        public override void Cast(Transform origin, Vector3 direction, Vector3 targetPosition)
        {
            ...
            if (m_ProjectDirectionOnPlane) direction = Vector3.ProjectOnPlane(direction, m_CharacterLocomotion.Up);
            // The direction can't be 0.
            if (direction.sqrMagnitude == 0) direction = m_CharacterLocomotion.transform.forward;
            var rotation = Quaternion.LookRotation(direction, m_CharacterLocomotion.Up) *
                           Quaternion.Euler(m_RotationOffset);

            // If the cast is currently active then the particle should be reused.
            if (m_Active)
            {
                if (m_SetRendererLengthScale) SetRendererLength(origin.position, targetPosition);
                if (!m_ParentToOrigin) m_ParticleTransform.position = position;
        ...
        {
            if (m_Renderers == null) m_Renderers = m_ParticleSystem.GetComponentsInChildren<ParticleSystemRenderer>();
        ...
        private void StartMaterialFade(GameObject particle)
        {
            ...
            {
                if (m_Renderers == null) m_Renderers = particle.GetComponentsInChildren<ParticleSystemRenderer>();
                SetRendererAlpha(0);
        ...
        private void SetRendererAlpha(float alpha)
        {
            for (var i = 0; i < m_Renderers.Length; ++i)
            {
                if (!m_Renderers[i].material.HasProperty(m_MaterialColorID)) continue;
        ...
        private void FadeMaterials(float interval, float targetAlpha)
        {
            ...
            for (var i = 0; i < m_Renderers.Length; ++i)
            {
                ...
                if (color.a != targetAlpha) arrived = false;
            }
        ...
        {
            if (m_ClearParentOnStop) m_ParticleSystem.transform.parent = null;
        }
        ...
        public override void Stop()
        {
            ...
            if (m_FadeOutDuration > 0)
            {
                ...
                if (m_FadeInDuration == 0) SetRendererAlpha(1);
        ...
        {
            if (!m_Active || m_ParticleSystem.transform.parent == origin) return;
