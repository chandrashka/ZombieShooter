...
namespace Opsive.UltimateCharacterController.Items.Actions.Magic.ImpactActions
{
    ...
    {
        private readonly Dictionary<uint, int> m_ChainCountMap = new();
        ...
        [SerializeField]
        protected int m_MaxChainCount = 1;

        [Tooltip("The maximum number of objects that the ricochet can detect.")] [SerializeField]
        protected int m_MaxCollisionCount = 50;

        [Tooltip("The radius of the ricochet.")] [SerializeField]
        ...
        protected override void ImpactInternal(uint castID, GameObject source, GameObject target, RaycastHit hit)
        {
            ...
            {
                if (m_MaxChainCount != -1 && count >= m_MaxChainCount) return;
            ...
            var hitCount = Physics.OverlapSphereNonAlloc(hit.point, m_Radius, m_HitColliders, m_MagicItem.DetectLayers,
            ...
            for (var i = 0; i < hitCount; ++i)
            {
                ...
                if (HasImpacted(hitTransform)) continue;
                ...
                if (Physics.Linecast(hit.point, position, out var raycastHit,
                ...
                for (var j = 0; j < m_MagicItem.CastActions.Length; ++j)
        ...
        public override void Reset(uint castID)
        {
            ...
            if (m_ChainCountMap.ContainsKey(castID)) m_ChainCountMap[castID] = 0;
        }
    }
}