...
namespace Opsive.UltimateCharacterController.Items.Actions.Magic.CastActions
{
    ...
    public class PhysicsCast : CastAction
    {
        ...
        [Tooltip("Specifies the type of cast to perform.")] [SerializeField]
        protected CastMode m_Mode;

        [Tooltip(
            "Should the look source be used when determining the cast position? If false the origin will be used.")]
        [SerializeField]
        protected bool m_UseLookSourcePosition;

        [Tooltip("The offset to add to the physics cast position.")] [SerializeField]
        protected Vector3 m_PositionOffset;

        [Tooltip("The distance of the cast.")] [SerializeField]
        protected float m_Distance = 5;

        [Tooltip("The radius of the cast.")] [SerializeField]
        protected float m_Radius = 5;

        [Tooltip("The layers that the cast can detect.")] [SerializeField]
        protected LayerMask m_Layers = ~((1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.UI) |
                                         (1 << LayerManager.SubCharacter) | (1 << LayerManager.Overlay) |
                                         (1 << LayerManager.VisualEffect));

        [Tooltip("Specifies if the cast intersect with triggers.")] [SerializeField]
        protected QueryTriggerInteraction m_TriggerInteraction = QueryTriggerInteraction.Ignore;

        [Tooltip("The maximum number of collisions that the cast can detect.")] [SerializeField]
        ...
        public bool UseLookSourcePosition
        {
            get => m_UseLookSourcePosition;
            set => m_UseLookSourcePosition = value;
        }
        ...
        {
            var position = m_UseLookSourcePosition
                ? m_CharacterLocomotion.LookSource.LookPosition(true)
                : origin.position;
            ...
            {
                if (m_HitRaycasts == null) m_HitRaycasts = new RaycastHit[m_MaxCollisionCount];
                if (m_Mode == CastMode.Raycast)
                    hitCount = Physics.RaycastNonAlloc(position, direction, m_HitRaycasts, m_Distance, m_Layers,
                        m_TriggerInteraction);
                else
                    hitCount = Physics.SphereCastNonAlloc(position - direction * m_Radius, m_Radius, direction,
                        m_HitRaycasts, m_Distance, m_Layers, m_TriggerInteraction);
                for (var i = 0; i < hitCount; ++i)
                    m_MagicItem.PerformImpact(m_CastID, m_GameObject, m_HitRaycasts[i].transform.gameObject,
                        m_HitRaycasts[i]);
                return;
            }

            // OverlapSphere.
            if (m_HitColliders == null) m_HitColliders = new Collider[m_MaxCollisionCount];
            hitCount = Physics.OverlapSphereNonAlloc(position, m_Radius, m_HitColliders, m_Layers,
                m_TriggerInteraction);
            for (var i = 0; i < hitCount; ++i)
            {
                // The cast cannot hit the current character.
                if (m_HitColliders[i].transform.IsChildOf(m_CharacterTransform)) continue;
