...
namespace Opsive.UltimateCharacterController.Items.Actions
{
    ...
    public abstract class UsableItem : ItemAction, IUsableItem
    {
        ...
        protected float m_UseRate = 0.1f;

        [Tooltip("Specifies if the inventory can equip an item that doesn't have any consumable items left.")]
        [SerializeField]
        protected bool m_CanEquipEmptyItem = true;

        [Tooltip("Should the character rotate to face the target during use?")] [SerializeField]
        protected bool m_FaceTarget = true;

        [Tooltip("The amount of extra time it takes for the ability to stop after use.")] [SerializeField]
        protected float m_StopUseAbilityDelay = 1f;

        [Tooltip(
            "Specifies if the item should wait for the OnAnimatorItemUse animation event or wait for the specified duration before being used.")]
        [SerializeField]
        protected AnimationSlotEventTrigger m_UseEvent = new(true, 0.2f);

        [Tooltip(
            "Specifies if the item should wait for the OnAnimatorItemUseComplete animation event or wait for the specified duration before completing the use.")]
        [SerializeField]
        protected AnimationSlotEventTrigger m_UseCompleteEvent = new(false, 0.05f);

        [Tooltip("Does the item require root motion position during use?")] [SerializeField]
        protected bool m_ForceRootMotionPosition;

        [Tooltip("Does the item require root motion rotation during use?")] [SerializeField]
        protected bool m_ForceRootMotionRotation;

        [Tooltip("The name of the attribute that should be adjusted when the item is used.")] [SerializeField]
        protected string m_UseAttributeName;

        [Tooltip("The amount to adjust the Use Attribute by when the item is used.")] [SerializeField]
        protected float m_UseAttributeAmount;

        [Tooltip("Should the item be dropped when the use attribute is depleted?")] [SerializeField]
        protected bool m_DropWhenUseDepleted;

        [Tooltip("The name of the character attribute that should be adjusted when the item is used.")] [SerializeField]
        protected string m_CharacterUseAttributeName;

        [Tooltip("The amount to adjust the Character Use Attribute by when the item is used.")] [SerializeField]
        protected float m_CharacterUseAttributeAmount;

        [Tooltip(
            "Should the audio play when the item starts to be used? If false it will be played when the item is used.")]
        [SerializeField]
        protected bool m_PlayAudioOnStartUse;

        [Tooltip("Specifies the animator and audio state that should be triggered when the item is used.")]
        [SerializeField]
        protected AnimatorAudioStateSet m_UseAnimatorAudioStateSet = new(2);
        ...
        public float UseAttributeAmount
        {
            get => m_UseAttributeAmount;
            set => m_UseAttributeAmount = value;
        }

        public bool DropWhenUseDepleted
        {
            get => m_DropWhenUseDepleted;
            set => m_DropWhenUseDepleted = value;
        }

        public string CharacterUseAttributeName
        {
            get => m_CharacterUseAttributeName;
            set
            {
                ...
                if (Application.isPlaying)
                {
                    if (!string.IsNullOrEmpty(m_CharacterUseAttributeName) && m_CharacterUseAttributeManager != null)
                        m_CharacterUseAttribute =
                            m_CharacterUseAttributeManager.GetAttribute(m_CharacterUseAttributeName);
                    else
        ...
        public float CharacterUseAttributeAmount
        {
            get => m_CharacterUseAttributeAmount;
            set => m_CharacterUseAttributeAmount = value;
        }

        public bool PlayAudioOnStartUse
        {
            get => m_PlayAudioOnStartUse;
            set => m_PlayAudioOnStartUse = value;
        }
        ...
        public bool CanEquipEmptyItem
        {
            get => m_CanEquipEmptyItem;
            set => m_CanEquipEmptyItem = value;
        }
        ...
        public float StopUseAbilityDelay
        {
            get => m_StopUseAbilityDelay;
            set => m_StopUseAbilityDelay = value;
        }

        public AnimationSlotEventTrigger UseEvent
        {
            get => m_UseEvent;
            set => m_UseEvent = value;
        }

        public AnimationSlotEventTrigger UseCompleteEvent
        {
            get => m_UseCompleteEvent;
            set => m_UseCompleteEvent = value;
        }

        public bool ForceRootMotionPosition
        {
            get => m_ForceRootMotionPosition;
            set => m_ForceRootMotionPosition = value;
        }

        public bool ForceRootMotionRotation
        {
            get => m_ForceRootMotionRotation;
        ...
        public virtual IItemIdentifier GetConsumableItemIdentifier()
        {
            return null;
        }
        ...
        public virtual int GetConsumableItemIdentifierAmount()
        {
            return 0;
        }
...
#endif
            m_NextAllowedUseTime =
                Time.time + m_UseRate -
                Mathf.Max(Time.time - m_LastUseTime - m_UseRate,
                    0); // Account for being able to use quicker than the framerate.
            m_LastUseTime = Time.time;
            if (m_UseAttribute != null) m_UseAttribute.Value -= m_UseAttributeAmount;
            if (m_CharacterUseAttribute != null) m_CharacterUseAttribute.Value -= m_CharacterUseAttributeAmount;
#if ULTIMATE_CHARACTER_CONTROLLER_MULTIPLAYER
            }
#endif
        ...
        protected bool CanUseItem(ItemAbility itemAbility, UseAbilityState abilityState, bool checkAttributes)
        {
            ...
            if (Time.time < m_NextAllowedUseTime) return false;
            ...
            if (checkAttributes && InvalidUseAttributes())
            {
                ...
                if (abilityState != UseAbilityState.Start) itemAbility.StopAbility();
                return false;
            }
        ...
        protected bool InvalidUseAttributes()
        {
            return (m_UseAttribute != null && !m_UseAttribute.IsValid(-m_UseAttributeAmount)) ||
                   (m_CharacterUseAttribute != null &&
                    !m_CharacterUseAttribute.IsValid(-m_CharacterUseAttributeAmount));
        }
        ...
        {
            if (!m_DropWhenUseDepleted) return;
