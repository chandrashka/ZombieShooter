...
namespace Opsive.UltimateCharacterController.Items.Actions
{
    ...
    /// <summary>
    ///     Any item that can be thrown, such as a grenade or baseball. The GameObject that the ThrowableItem attaches to is
    ///     not the actual object that is thrown - the ThrownObject field
    ///     specifies this instead.
    /// </summary>
    public class ThrowableItem : UsableItem
    {
        [Tooltip("The object that is thrown.")] [SerializeField]
        protected GameObject m_ThrownObject;

        [Tooltip("Should the visible object be disabled?")] [SerializeField]
        protected bool m_DisableVisibleObject;
        ...
        [SerializeField]
        protected AnimationEventTrigger m_ActivateThrowableObjectEvent;

        [Tooltip("Should the object be thrown when the stop use method is called?")] [SerializeField]
        protected bool m_ThrowOnStopUse;

        [Tooltip("The starting velocity of the thrown object.")] [SerializeField]
        protected Vector3 m_Velocity = new(0, 5, 10);

        [Tooltip("The layer that the item should occupy when initially spawned.")] [SerializeField]
        protected int m_StartLayer = LayerManager.IgnoreRaycast;

        [Tooltip("The layer that the thrown object should change to after being thrown.")] [SerializeField]
        protected int m_ThrownLayer = LayerManager.Default;

        [Tooltip("The amount of time after the object has been thrown to change the layer.")] [SerializeField]
        ...
        [Tooltip("The amount of damage applied to the object hit by the thrown object.")] [SerializeField]
        protected float m_DamageAmount = 10;

        [Tooltip("The layers that the thrown object can collide with.")] [SerializeField]
        protected LayerMask m_ImpactLayers = ~((1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.Overlay) |
                                               (1 << LayerManager.UI) | (1 << LayerManager.Overlay));

        [Tooltip("The Surface Impact triggered when the object hits another object.")] [SerializeField]
        ...
        [Tooltip("Should the throwable item be able to be used no matter the inventory count?")] [SerializeField]
        protected bool m_InfiniteUse;
        ...
        [SerializeField]
        protected AnimationEventTrigger m_ReequipEvent = new(false, 0.5f);

        [Tooltip("The value of the Item Substate Animator parameter when the item is being reequipped.")]
        [SerializeField]
        protected int m_ReequipItemSubstateParameterValue = 10;

        [Tooltip("Should the item's trajectory be shown when the character aims?")] [SerializeField]
        protected bool m_ShowTrajectoryOnAim;

        [Tooltip(
            "The offset of the trajectory visualization relative to the trajectory transform set on the Throwable Item Properties.")]
        [SerializeField]
        protected Vector3 m_TrajectoryOffset;

        public GameObject ThrownObject
        {
            get => m_ThrownObject;
            set => m_ThrownObject = value;
        }

        public bool DisableVisibleObject
        {
            get => m_DisableVisibleObject;
            set
            {
                ...
                if (m_Item != null)
                {
                    m_Item.SetVisibleObjectActive(m_Item.VisibleObjectActive,
                        m_Inventory.GetItemIdentifierAmount(m_Item.ItemIdentifier) > 0);
                    EnableObjectMeshRenderers(CanActivateVisibleObject());
                }
            }
        }

        public AnimationEventTrigger ActivateThrowableObjectEvent
        {
            get => m_ActivateThrowableObjectEvent;
            set => m_ActivateThrowableObjectEvent = value;
        }

        public bool ThrowOnStopUse
        {
            get => m_ThrowOnStopUse;
            set => m_ThrowOnStopUse = value;
        }
        ...
        public int StartLayer
        {
            ...
            {
                m_StartLayer = value;
                if (m_InstantiatedThrownObject != null && !m_Thrown) m_InstantiatedThrownObject.layer = m_StartLayer;
            }
        }

        public int ThrownLayer
        {
            get => m_ThrownLayer;
            set => m_ThrownLayer = value;
        }
        ...
        {
            get => m_InfiniteUse;
            set => m_InfiniteUse = value;
        }

        public AnimationEventTrigger ReequipEvent
        {
            get => m_ReequipEvent;
            set => m_ReequipEvent = value;
        }

        public int ReequipItemSubstateParameterValue
        {
            get => m_ReequipItemSubstateParameterValue;
            set => m_ReequipItemSubstateParameterValue = value;
        }

        public bool ShowTrajectoryOnAim
        {
            get => m_ShowTrajectoryOnAim;
            set
            {
        ...
        public Vector3 TrajectoryOffset
        {
            get => m_TrajectoryOffset;
            set => m_TrajectoryOffset = value;
        }
        ...
        protected override void Awake()
        {
            ...
            if (m_ThrownObject != null && m_TrajectoryObject != null)
            {
                ...
                if (trajectoryCollider != null)
                {
                    ...
                    {
                        Debug.LogError(
                            $"Error: The collider of type {trajectoryCollider.GetType()} is not supported on the trajectory object " +
                            m_ThrownObject.name);
                    }
            ...
            if (m_ShowTrajectoryOnAim && m_TrajectoryObject == null)
                Debug.LogError(
                    $"Error: A TrajectoryObject must be added to the {m_GameObject.name} GameObject in order for the trajectory to be shown.");
        ...
        {
            var perspectiveItem = m_CharacterLocomotion.FirstPersonPerspective
                ? m_Item.FirstPersonPerspectiveItem
                : m_Item.ThirdPersonPerspectiveItem;
            ...
            {
                m_ThrowableItemPerpectiveProperties =
                    m_ActivePerspectiveProperties as IThrowableItemPerspectiveProperties;

                if (m_ThrowableItemPerpectiveProperties == null)
                    Debug.LogError(
                        $"Error: The First/Third Person Throwable Item Properties component cannot be found for the Item {name}." +
        ...
        {
            var renderers = m_CharacterLocomotion.FirstPersonPerspective
                ? m_FirstPersonObjectRenderers
                : m_ThirdPersonObjectRenderers;
            if (renderers != null)
        ...
        {
            if (m_Reequipping) return m_ReequipItemSubstateParameterValue;
            // When the item is being thrown the index should be positive.
            if (m_Throwing) return Mathf.Max(1, base.GetItemSubstateIndex());
            return -1;
        }
        ...
        {
            if (m_Aiming && m_ShowTrajectoryOnAim && !m_Throwing && !m_Reequipping && !m_NextItemSet &&
                m_TrajectoryObject != null)
            {
                var trajectoryTransform = m_ThrowableItemPerpectiveProperties.TrajectoryLocation != null
                    ? m_ThrowableItemPerpectiveProperties.TrajectoryLocation
                    : m_CharacterTransform;
                var lookDirection = m_LookSource.LookDirection(trajectoryTransform.TransformPoint(m_TrajectoryOffset),
                    false, m_ImpactLayers, true, true);
                var velocity = MathUtility.TransformDirection(m_Velocity,
                    Quaternion.LookRotation(lookDirection, m_CharacterLocomotion.Up));
                ...
                m_TrajectoryObject.SimulateTrajectory(m_Character,
                    trajectoryTransform.TransformPoint(m_TrajectoryOffset), Quaternion.identity,
        ...
        private void OnAim(bool aim, bool inputStart)
        {
            ...
            if (!m_Aiming && m_ShowTrajectoryOnAim && m_TrajectoryObject != null) m_TrajectoryObject.ClearTrajectory();
        }
        ...
        public override bool CanUseItem(ItemAbility itemAbility, UseAbilityState abilityState)
        {
            ...
            if (abilityState == UseAbilityState.Start && (m_Throwing || m_Reequipping)) return false;

            // The item can't be used if there aren't any items left.
            if (m_Inventory != null && m_Inventory.GetItemIdentifierAmount(m_Item.ItemIdentifier) == 0) return false;

            // The item can't be used if it hasn't been started yet, is reequipping the throwable item, or has been requipped.
            if (abilityState == UseAbilityState.Update && (!m_Throwing || m_Reequipping || m_Reequipped)) return false;

            // Give the item a frame to recover from the reequip.
            if (Time.frameCount == m_ReequipFrame) return false;
        ...
        public override void StartItemUse(ItemAbility itemAbility)
        {
            ...
            if (!m_ThrowOnStopUse) StartThrow();

            // Instantiate the object that will actually be thrown.
            var location = m_ThrowableItemPerpectiveProperties.ThrowLocation;
            m_InstantiatedThrownObject = ObjectPoolBase.Instantiate(m_ThrownObject, location.position,
                location.rotation, m_ObjectTransform.parent);
            ...
            if (m_InstantiatedTrajectoryObject is Destructible)
                (m_InstantiatedTrajectoryObject as Destructible).InitializeDestructibleProperties(m_DamageProcessor,
                    m_DamageAmount, m_ImpactForce, m_ImpactForceFrames,
            ...
            if (m_DisableVisibleObject)
            {
                ...
                if (m_ActivateThrowableObjectEvent.WaitForAnimationEvent)
                    EventHandler.RegisterEvent(m_Character, "OnAnimatorActivateThrowableObject",
                        ActivateThrowableObject);
                else
        ...
        private void ActivateThrowableObject()
        {
            ...
            if (!m_Item.IsActive()) m_Item.SetVisibleObjectActive(true, true);
        ...
        {
            if (m_Thrown || m_InstantiatedThrownObject == null) return;
            m_Thrown = true;
            ...
            if (!m_CharacterLocomotion.ActiveMovementType.UseIndependentLook(false) &&
                Physics.Linecast(m_CharacterLocomotion.LookSource.LookPosition(true),
                    m_InstantiatedTrajectoryObject.transform.position, out m_RaycastHit,
            ...
            // y velocity. Gravity will cause the thrown object to go in the opposite direction.
            if (Vector3.Dot(velocity.normalized, m_CharacterTransform.forward) < 0 &&
                m_CharacterTransform.InverseTransformDirection(velocity.normalized).y > 0)
                velocity = m_CharacterTransform.up * velocity.magnitude;
            m_InstantiatedTrajectoryObject.Initialize(
                m_CharacterLocomotion.Alive
                    ? velocity + m_CharacterTransform.forward * m_CharacterLocomotion.LocalVelocity.z
                    : Vector3.zero, Vector3.zero, m_Character, false);
        ...
        public override void ItemUseComplete()
        {
            ...
            if (!m_DisableVisibleObject && m_Inventory != null &&
                m_Inventory.GetItemIdentifierAmount(m_Item.ItemIdentifier) > 0)
            {
                m_Reequipping = true;
        ...
        {
            if (m_ThrowOnStopUse && !m_Thrown) StartThrow();
        }
        ...
        public override bool CanStopItemUse()
        {
            ...
            if (!m_Thrown || m_Reequipping) return false;
        ...
        public override void StopItemUse()
        {
            ...
            if (!m_Thrown) ThrowItem();
            m_Throwing = m_Thrown = false;
            m_Reequipping = m_Reequipped = false;
            m_ActivateVisibleObject = false;
            m_InstantiatedThrownObject = null;
            if (m_DisableVisibleObject) m_Item.SetVisibleObjectActive(false, true);
        }
        ...
        private void ReequipThrowableItem()
        {
            ...
            if (m_Inventory != null && m_Inventory.GetItemIdentifierAmount(m_Item.ItemIdentifier) == 0) return;
        ...
        public override void Unequip()
        {
            ...
            if (m_ShowTrajectoryOnAim && m_TrajectoryObject != null) m_TrajectoryObject.ClearTrajectory();
#if ULTIMATE_CHARACTER_CONTROLLER_VR
            if (m_VRThrowableItem != null) {
        ...
        protected override void OnChangePerspectives(bool firstPersonPerspective)
        {
            ...
            var perspectiveItem = firstPersonPerspective
                ? m_Item.FirstPersonPerspectiveItem
                : m_Item.ThirdPersonPerspectiveItem;
            m_Object = perspectiveItem.GetVisibleObject();
            m_ObjectTransform = m_Object.transform;
