...
namespace Opsive.UltimateCharacterController.Items.AnimatorAudioStates
{
    ...
    public abstract class AnimatorAudioStateSelector
    {
        ...
        public virtual void Initialize(GameObject gameObject, UltimateCharacterLocomotion characterLocomotion,
            Item item, AnimatorAudioStateSet.AnimatorAudioState[] states)
        {
            m_Item = item;
        ...
        protected void ChangeStates(int fromIndex, int toIndex)
        {
            if (fromIndex != -1)
            {
                ...
                if (!string.IsNullOrEmpty(stateName)) StateManager.SetState(m_Character, stateName, false);
            }

            if (toIndex != -1)
            {
                var stateName = m_States[toIndex].StateName;
                if (!string.IsNullOrEmpty(stateName)) StateManager.SetState(m_Character, stateName, true);
            }
        }
