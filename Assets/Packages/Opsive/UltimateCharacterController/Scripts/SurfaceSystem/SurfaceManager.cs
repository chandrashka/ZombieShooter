...
namespace Opsive.UltimateCharacterController.SurfaceSystem
{
    ...
    public class SurfaceManager : MonoBehaviour
    {
        ...
        [Tooltip("An array of SurfaceTypes which are paired to a UV position within a texture.")] [SerializeField]
        protected ObjectSurface[] m_ObjectSurfaces;

        [Tooltip("The name of the main texture property that should be retrieved.")] [SerializeField]
        protected string m_MainTexturePropertyName = "_MainTex";

        [Tooltip(
            "Should the textures from trees on the terrain be detected? Note that this is a CPU-intensive operation.")]
        [SerializeField]
        protected bool m_DetectTerrainTreeTextures;

        [Tooltip("The fallback SurfaceImpact if no SurfaceImpacts can be found.")] [SerializeField]
        protected SurfaceImpact m_FallbackSurfaceImpact;

        [Tooltip("The fallback SurfaceType if no SurfaceTypes can be found.")] [SerializeField]
        protected SurfaceType m_FallbackSurfaceType;

        [Tooltip("The fallback allow decals if using a fallback SurfaceImpact or SurfaceTYpe.")] [SerializeField]
        protected bool m_FallbackAllowDecals = true;

        private readonly Dictionary<Collider, bool> m_ColliderComplexMaterialsMap = new();
        private readonly Dictionary<Collider, bool> m_ColliderDecalsAllowedMap = new();
        private readonly Dictionary<Collider, Texture> m_ColliderMainTextureMap = new();
        private readonly Dictionary<Collider, Mesh> m_ColliderMeshMap = new();
        private readonly Dictionary<Collider, Renderer> m_ColliderRendererMap = new();

        private readonly Dictionary<Collider, SurfaceIdentifier> m_ColliderSurfaceIdentifiersMap = new();
        private readonly Dictionary<Collider, SurfaceType> m_ColliderSurfacesTypesMap = new();
        private readonly Dictionary<Collider, Terrain> m_ColliderTerrainMap = new();
        private readonly Rect m_DefaultUV = new(0, 0, 1, 1);

        private bool m_HasTerrain;
        private int m_MainTexturePropertyID;
        private readonly int[] m_MaterialHitTriangle = new int[3];

        private readonly Dictionary<SurfaceType, Dictionary<SurfaceImpact, SurfaceEffect>> m_SurfaceImpactEffectMap =
            new();

        private readonly Dictionary<Texture, ObjectSurface> m_TextureObjectSurfaceMap = new();
        private readonly Dictionary<Texture, SurfaceType> m_TextureSurfaceTypeMap = new();
        private readonly Dictionary<Texture, List<UVTexture>> m_TextureUVTextureMap = new();
        private readonly Dictionary<UVTexture, ObjectSurface> m_UVTextureObjectSurfaceMap = new();
        ...
        public ObjectSurface[] ObjectSurfaces => m_ObjectSurfaces;
        public bool DetectTerrainTreeTextures => m_DetectTerrainTreeTextures;
        public SurfaceImpact FallbackSurfaceImpact => m_FallbackSurfaceImpact;
        public SurfaceType FallbackSurfaceType => m_FallbackSurfaceType;
        public bool FallbackAllowDecals => m_FallbackAllowDecals;
        ...
        /// <summary>
        ///     Stores all the textures added as an object surface to the TextureObjectSurfaceMap dictionary (if using the default
        ///     UV) or
        ///     the UVTextureObjectSurfaceMap dictionary (if using any other UV).
        /// </summary>
        protected void InitObjectSurfaces()
        {
            if (m_ObjectSurfaces == null) return;

            for (var i = 0; i < m_ObjectSurfaces.Length; ++i)
            for (var j = 0; j < m_ObjectSurfaces[i].UVTextures.Length; ++j)
            {
                if (ObjectSurfaces[i].UVTextures[j].Texture == null) continue;

                if (m_UVTextureObjectSurfaceMap.ContainsKey(m_ObjectSurfaces[i].UVTextures[j])) continue;
                ...
                if (m_ObjectSurfaces[i].UVTextures[j].UV == m_DefaultUV &&
                    !IsDuplicateObjectTexture(m_ObjectSurfaces[i].UVTextures[j].Texture))
                {
        ...
        private bool IsDuplicateObjectTexture(Texture texture)
        {
            ...
            {
                if (ObjectSurfaces[i].UVTextures[j].Texture == null) continue;
                if (ObjectSurfaces[i].UVTextures[j].Texture == texture) count++;
            }
        ...
        public static bool SpawnEffect(RaycastHit hit, SurfaceImpact surfaceImpact, Vector3 gravityDirection,
            float timeScale, GameObject originator)
        {
            return Instance.SpawnEffectInternal(hit, hit.collider, surfaceImpact, gravityDirection, timeScale,
                originator);
        }
        ...
        /// <param name="originator">The object which spawned the effect.</param>
        /// <returns>True if the effect was spawned.</returns>
        public static bool SpawnEffect(RaycastHit hit, Collider collider, SurfaceImpact surfaceImpact,
            Vector3 gravityDirection, float timeScale, GameObject originator)
        {
        ...
        /// <param name="originator">The object which spawned the effect.</param>
        /// <returns>True if the effect was spawned.</returns>
        private bool SpawnEffectInternal(RaycastHit hit, Collider hitCollider, SurfaceImpact surfaceImpact,
            Vector3 gravityDirection, float timeScale, GameObject originator)
        {
            ...
            if (surfaceEffect == null) return false;
        ...
        /// <returns>True if the effect was spawned.</returns>
        public static bool SpawnEffect(RaycastHit hit, SurfaceImpact surfaceImpact, Vector3 gravityDirection,
            float timeScale, GameObject originator, Vector3 footprintDirection, bool flipFootprint)
        {
            return Instance.SpawnEffectInternal(hit, hit.collider, surfaceImpact, gravityDirection, timeScale,
                originator, footprintDirection, flipFootprint);
        }
        ...
        /// <returns>True if the effect was spawned.</returns>
        public static bool SpawnEffect(RaycastHit hit, Collider collider, SurfaceImpact surfaceImpact,
            Vector3 gravityDirection, float timeScale, GameObject originator, Vector3 footprintDirection,
            bool flipFootprint)
        {
            return Instance.SpawnEffectInternal(hit, collider, surfaceImpact, gravityDirection, timeScale, originator,
                footprintDirection, flipFootprint);
        }
            ...
            bool flipFootprint)
        {
            ...
            var spawnDecals = false;
            var surfaceEffect = GetSurfaceEffect(hit, hitCollider, surfaceImpact, ref surfaceType, ref spawnDecals);
            if (surfaceType == null || surfaceEffect == null) return false;
            ...
            if (surfaceType.AllowFootprints)
                surfaceEffect.SpawnFootprint(hit, gravityDirection, timeScale, originator, spawnDecals,
                    footprintDirection, flipFootprint);
            else
                surfaceEffect.Spawn(hit, gravityDirection, timeScale, originator, spawnDecals);
        ...
        private SurfaceEffect GetSurfaceEffect(RaycastHit hit, Collider hitCollider, SurfaceImpact surfaceImpact,
            ref SurfaceType surfaceType, ref bool spawnDecals)
        {
        ...
        private SurfaceType GetSurfaceType(RaycastHit hit, Collider hitCollider)
        {
            ...
            if (surfaceType != null) return surfaceType;
        ...
        {
            if (hitCollider == null || hitCollider is TerrainCollider) return null;
        ...
        /// <summary>
        ///     Returns true if the object associated with the specified collider has a complex material. A complex material
        ///     includes multiple materials or materials with a secondary map.
        /// </summary>
        ...
        private bool HasComplexMaterial(Collider hitCollider)
        {
            ...
            if (!m_ColliderComplexMaterialsMap.TryGetValue(hitCollider, out var complexMaterials))
            {
                ...
                if (hitRenderer != null) complexMaterials = hitRenderer.sharedMaterials.Length > 1;
                ...
                if (!complexMaterials && hitRenderer != null && hitRenderer.sharedMaterials.Length > 0)
                {
                    ...
                    complexMaterials = material != null && material.HasProperty(s_MaskID) &&
                                       material.HasProperty(s_SecondaryTextureID);
                }
        ...
        private Renderer GetRenderer(Collider hitCollider)
        {
            ...
            if (hitCollider == null || hitCollider.isTrigger) return null;

            // Unity terrains have no renderers.
            if (hitCollider is TerrainCollider) return null;
            ...
            if (!m_ColliderRendererMap.TryGetValue(hitCollider, out hitRenderer))
            {
                ...
                if (hitRenderer == null || !hitRenderer.enabled || hitRenderer is SkinnedMeshRenderer)
                {
                    ...
                    {
                        if (childRenderers[i] == hitRenderer || !childRenderers[i].enabled ||
                            hitRenderer is SkinnedMeshRenderer) continue;
                ...
                if (hitRenderer == null || !hitRenderer.enabled || hitRenderer is SkinnedMeshRenderer)
                {
                    ...
                    for (var i = 0; i < parentRenderers.Length; ++i)
                    {
                        if (parentRenderers[i] == hitRenderer || !parentRenderers[i].enabled ||
                            hitRenderer is SkinnedMeshRenderer) continue;
        ...
        private Texture GetMainTexture(Collider hitCollider)
        {
            ...
            if (!m_ColliderMainTextureMap.TryGetValue(hitCollider, out texture))
            {
                ...
                if (hitRenderer != null && hitRenderer.sharedMaterial != null &&
                    hitRenderer.sharedMaterial.HasProperty(m_MainTexturePropertyID))
        ...
        /// <returns>
        ///     False if the specified collider contains the SurfaceIdentifier component and it does not allow decals,
        ///     otherwise true.
        /// </returns>
        private bool ShouldSpawnDecals(Collider hitCollider)
        {
            ...
            var allowed = true;
            if (!m_ColliderDecalsAllowedMap.TryGetValue(hitCollider, out allowed)) return true;
            return allowed;
        }
        ...
        private Material GetHitMaterial(RaycastHit hit, Collider hitCollider)
        {
            ...
            if (hit.triangleIndex < 0) return null;

            var hitRenderer = GetRenderer(hitCollider);
            if (hitRenderer == null || hitRenderer.sharedMaterials == null) return null;

            // If the renderer only has one material then the uvs do not need to be used.
            if (hitRenderer.sharedMaterials.Length == 1) return hitRenderer.sharedMaterials[0];

            // The mesh not be null.
            var mesh = GetMesh(hitCollider);
            if (mesh == null || hit.triangleIndex * 3 + 2 >= mesh.triangles.Length) return null;
            ...
            for (var i = 0; i < mesh.subMeshCount; i++)
            {
                ...
                for (var j = 0; j < subMeshTriangles.Length; j += 3)
                    if (subMeshTriangles[j] == m_MaterialHitTriangle[0] && subMeshTriangles[j + 1] ==
                                                                        m_MaterialHitTriangle[1]
                                                                        && subMeshTriangles[j + 2] ==
                                                                        m_MaterialHitTriangle[2])
                    {
                        if (hitRenderer.sharedMaterials.Length < i + 1) continue;
                        if (hitRenderer.sharedMaterials[i] == null) continue;
        ...
        private Mesh GetMesh(Collider hitCollider)
        {
            ...
            if (!m_ColliderMeshMap.TryGetValue(hitCollider, out mesh))
            {
                ...
                if (meshFilter == null) meshFilter = hitCollider.GetComponentInChildren<MeshFilter>();

                // If no MeshFilter exists, try to find a MeshFilter in the collider's parent.
                if (meshFilter == null) meshFilter = hitCollider.GetComponentInParent<MeshFilter>();
                ...
                if (meshFilter != null)
                {
                    ...
                    if (!mesh.isReadable || mesh.triangles == null) mesh = null;
                }
        ...
        private SurfaceType GetComplexSurfaceType(Material material, RaycastHit hit, Collider hitCollider)
        {
            ...
            if (material.HasProperty(m_MainTexturePropertyID)) texture = material.GetTexture(m_MainTexturePropertyID);
            ...
            if (material.HasProperty(s_MaskID) && material.HasProperty(s_SecondaryTextureID))
            {
                ...
                if (maskTexture != null)
                {
                    var color = maskTexture.GetPixel((int)(hit.textureCoord.x * maskTexture.width),
                        (int)(hit.textureCoord.y * maskTexture.height));
                    ...
                    if (color.a > 0.5f)
                    {
                        ...
                        if (secondaryTexture != null) texture = secondaryTexture;
                    }
                }
            }
            ...
            {
                for (var v = 0; v < uvTextures.Count; v++)
        ...
        /// <summary>
        ///     Adjusts the texture coordinate for flipping, scale, offset and tiling. The returned value is used for determining
        ///     the exact texture coordinate of a hit point.
        /// </summary>
        ...
        private Vector2 AdjustTextureCoordinate(Vector2 textureCoordinate, Material material)
        {
            ...
            if (textureCoordinate.x < 0) textureCoordinate.x = 1 - Mathf.Abs(textureCoordinate.x);
            if (textureCoordinate.y < 0) textureCoordinate.y = 1 - Mathf.Abs(textureCoordinate.y);
        ...
        {
            if (!m_HasTerrain || hitCollider == null) return null;
        ...
        {
            if (!m_DetectTerrainTreeTextures) return null;

            var terrainData = terrain.terrainData;
            if (terrainData.treeInstanceCount == 0) return null;
            ...
            for (var i = 0; i < terrainData.treeInstanceCount; ++i)
            {
                ...
                if (treeCollider.Raycast(new Ray(hit.point + hit.normal * hit.distance, -hit.normal), out var treeHit,
                        hit.distance + 1)) texture = GetMainTexture(treeCollider);
        ...
        private Texture GetTerrainTexture(Vector3 worldPosition, Terrain terrain)
        {
            ...
            if (terrain.terrainData.terrainLayers == null ||
                terrainTextureID > terrain.terrainData.terrainLayers.Length - 1) return null;
        ...
        private int GetDominantTerrainTexture(Vector3 worldPosition, Terrain terrain)
        {
            ...
            var terrainData = terrain.terrainData;
            if (terrainData.alphamapTextures == null || terrainData.alphamapTextures.Length == 0) return 0;
            ...
            for (var n = 0; n < mix.Length; ++n) mix[n] = splatmapData[0, 0, n];
        ...
        private SurfaceEffect GetSurfaceEffect(SurfaceImpact surfaceImpact, ref SurfaceType surfaceType,
            ref bool spawnDecals)
        {
            ...
            if (surfaceImpact == null) return null;
            ...
            if (surfaceType == null || surfaceType.ImpactEffects == null ||
                surfaceType.ImpactEffects.Length == 0) return null;
            ...
            if (surfaceEffect == null)
            {
                ...
                if (surfaceEffect == null) return null;
            }

            // A SurfaceEffect was found! Determine if the decal can be spawned.
            if (spawnDecals && surfaceEffect.Decals.Length > 0)
            {
                if (usingFallbackSurface || usingFallbackImpact) spawnDecals = m_FallbackAllowDecals;
            }
            else
            {
                spawnDecals = false;
        ...
        private SurfaceEffect GetPrimarySurfaceEffect(SurfaceImpact surfaceImpact, SurfaceType surfaceType)
        {
            ...
            if (surfaceImpactMap == null || surfaceImpactMap.Count == 0) return null;
        ...
        private Dictionary<SurfaceImpact, SurfaceEffect> GetSurfaceImpactMap(SurfaceType surfaceType)
        {
            ...
            if (!m_SurfaceImpactEffectMap.TryGetValue(surfaceType, out surfaceImpactSurfaceEffect))
            {
                ...
                for (var i = 0; i < surfaceType.ImpactEffects.Length; i++)
                {
                    if (surfaceType.ImpactEffects[i].SurfaceImpact == null) continue;

                    if (surfaceImpactSurfaceEffect.ContainsKey(surfaceType.ImpactEffects[i].SurfaceImpact))
                    {
                        Debug.LogWarning("Warning: Surface Type '" + surfaceType + "' has more than one '" +
                                         surfaceType.ImpactEffects[i].SurfaceImpact +
                                         "' added. Only the first one will be used.");
                        continue;
                    }

                    surfaceImpactSurfaceEffect.Add(surfaceType.ImpactEffects[i].SurfaceImpact,
                        surfaceType.ImpactEffects[i].SurfaceEffect);
                }
