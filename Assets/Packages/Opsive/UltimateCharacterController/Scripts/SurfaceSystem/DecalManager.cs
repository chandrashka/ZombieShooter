...
{
    /// <summary>
    ///     The DecalManager is responsible for managing the spawned decals. The decals can be capped at a limit to prevent too
    ///     many from being
    ///     spawned. These decals can then slowly be faded (weathered) for a smooth transition rather than the decal just
    ...
    public class DecalManager : MonoBehaviour
    {
        ...
        [Tooltip("The maximum number of decals.")] [SerializeField]
        protected int m_DecalLimit = 100;

        [Tooltip("The number of decals which should slowly fade after the decal limit has been reached.")]
        [SerializeField]
        protected int m_WeatheredDecalLimit = 20;

        [Tooltip("The speed that the decals should fadeout after they have been removed from the weathered array.")]
        [SerializeField]
        protected int m_RemoveFadeoutSpeed = 10;

        private readonly Dictionary<GameObject, Mesh> m_DecalMeshMap = new();
        private readonly Dictionary<GameObject, Renderer> m_DecalRendererMap = new();

        private readonly List<GameObject> m_Decals = new();
        private readonly List<Renderer> m_DecalsToFade = new();
        private readonly List<Renderer> m_WeatheredDecals = new();
        ...
        {
            get => m_DecalLimit;
            set => m_DecalLimit = value;
        }

        public int WeatheredDecalLimit
        {
            get => m_WeatheredDecalLimit;
            set => m_WeatheredDecalLimit = value;
        }

        public int RemoveFadeoutSpeed
        {
            get => m_RemoveFadeoutSpeed;
            set => m_RemoveFadeoutSpeed = value;
        }
        ...
        private void Update()
        {
            for (var i = m_DecalsToFade.Count - 1; i >= 0; --i)
            {
            ...
            if (m_DecalsToFade.Count == 0) enabled = false;
        }
        ...
        {
            SpawnDecal(original, hit,
                Quaternion.LookRotation(hit.normal) * Quaternion.AngleAxis(Random.Range(0, 360), Vector3.forward),
                scale, allowedEdgeOverlap);
        }
        ...
        public static void SpawnFootprint(GameObject original, RaycastHit hit, float scale, float allowedEdgeOverlap,
            Vector3 footprintDirection, bool flipFootprint)
        {
            Instance.SpawnFootprintInternal(original, hit, scale, allowedEdgeOverlap, footprintDirection,
                flipFootprint);
        }
        ...
        /// <param name="footprintDirection">The direction that the footprint decal should face.</param>
        /// <param name="flipFootprint">Should the footprint decal be flipped?</param>
        private void SpawnFootprintInternal(GameObject original, RaycastHit hit, float scale, float allowedEdgeOverlap,
            Vector3 footprintDirection, bool flipFootprint)
        {
            var decal = SpawnDecal(original, hit, Quaternion.LookRotation(hit.normal, footprintDirection), scale,
                allowedEdgeOverlap);
        ...
        private GameObject SpawnDecal(GameObject original, RaycastHit hit, Quaternion rotation, float scale,
            float allowedEdgeOverlap)
        {
            ...
            if (MathUtility.IsUniform(hit.transform.localScale)) decal.transform.parent = hit.transform;
            if (scale != 1)
            {
        ...
        private bool CacheMeshAndRenderer(GameObject decal)
        {
            ...
            if (!m_DecalRendererMap.TryGetValue(decal, out decalRenderer))
            {
                ...
                if (decalRenderer == null) return false;
                if (decalRenderer.material == null) return false;
        ...
        {
            Mesh mesh;
            if (!m_DecalMeshMap.TryGetValue(decal, out mesh)) return false;

            RaycastHit hit;
            for (var i = 0; i < 4; i++)
                // The decal isn't hitting anything if the raycast returns false.
                if (!Physics.Raycast(
                        decal.transform.TransformPoint(mesh.vertices[i] * (1 - allowedEdgeOverlap * 2)) +
                        decal.transform.forward * 0.1f,
                        -decal.transform.forward, out hit, 0.2f, ~((1 << LayerManager.TransparentFX) |
        ...
        private void WeatherDecals()
        {
            ...
            for (var i = 0; i < m_WeatheredDecals.Count; ++i)
            {
