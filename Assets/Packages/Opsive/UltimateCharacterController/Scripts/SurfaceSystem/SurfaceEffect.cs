...
{
    /// <summary>
    ///     Specifies a recipe for effects that can be spawned in response to a certain type of collision. This collision might
    ///     occur when
    ...
    {
        [Tooltip("Objects that should be spawned during a collision.")] [SerializeField]
        protected ObjectSpawnInfo[] m_SpawnedObjects;

        [Tooltip(
            "An array of decals that can be spawned. A single decal will be randomlly chosen when the SurfaceEffect should spawn.")]
        [SerializeField]
        protected GameObject[] m_Decals;

        [Tooltip("The minimum scale of the decal.")] [SerializeField]
        protected float m_MinDecalScale = 1;

        [Tooltip("The maximum scale of the decal.")] [SerializeField]
        protected float m_MaxDecalScale = 1;

        [Tooltip(
            "How close to the edge the decal is allowed to spawn. A value of 0 requires the full quad is required to sit on the background surface.")]
        [Range(0, 0.5f)]
        [SerializeField]
        protected float m_AllowedDecalEdgeOverlap = 0.25f;

        [Tooltip("The AudioConfig that can be triggered from a collision.")] [SerializeField]
        protected AudioConfig m_AudioConfig;

        [Tooltip("The AudioClips that can be triggered from a collision.")] [SerializeField]
        protected AudioClip[] m_AudioClips;

        [Tooltip("The minimum volume of the AudioClip.")] [SerializeField]
        protected float m_MinAudioVolume = 1;

        [Tooltip("The maximum volume of the AudioClip.")] [SerializeField]
        protected float m_MaxAudioVolume = 1;

        [Tooltip("The minimum pitch of the AudioClip.")] [SerializeField]
        protected float m_MinAudioPitch = 1;

        [Tooltip("The maximum pitch of the AudioClip.")] [SerializeField]
        protected float m_MaxAudioPitch = 1;

        [Tooltip(
            "Should a maximum of one clip be played per frame? This prevents too many sounds from playing at once.")]
        [SerializeField]
        protected bool m_OneClipPerFrame;

        [Tooltip("Should the AudioClip be randomly selected? If false the clips will be played sequentially.")]
        [SerializeField]
        protected bool m_RandomClipSelection = true;

        [Tooltip("The name of the state that should be activated upon impact.")] [SerializeField]
        protected string m_StateName;

        [Tooltip(
            "The number of seconds until the specified state is disabled. A value of -1 will require the state to be disabled manually.")]
        [SerializeField]
        protected float m_StateDisableTimer = 10;
        ...
        public ObjectSpawnInfo[] SpawnedObjects
        {
            get => m_SpawnedObjects;
        ...
        public float MinDecalScale
        {
            get => m_MinDecalScale;
            set => m_MinDecalScale = value;
        }

        public float MaxDecalScale
        {
            get => m_MaxDecalScale;
            set => m_MaxDecalScale = value;
        }

        public float AllowedDecalEdgeOverlap
        {
            get => m_AllowedDecalEdgeOverlap;
            set => m_AllowedDecalEdgeOverlap = value;
        }
        ...
        {
            get => m_MinAudioVolume;
            set => m_MinAudioVolume = value;
        }

        public float MaxAudioVolume
        {
            get => m_MaxAudioVolume;
            set => m_MaxAudioVolume = value;
        }

        public float MinAudioPitch
        {
            get => m_MinAudioPitch;
            set => m_MinAudioPitch = value;
        }

        public float MaxAudioPitch
        {
            get => m_MaxAudioPitch;
            set => m_MaxAudioPitch = value;
        }

        public bool OneClipPerFrame
        {
            get => m_OneClipPerFrame;
            set => m_OneClipPerFrame = value;
        }

        public bool RandomClipSelection
        {
            get => m_RandomClipSelection;
            set => m_RandomClipSelection = value;
        }
        ...
        public float StateDisableTimer
        {
            get => m_StateDisableTimer;
            set => m_StateDisableTimer = value;
        }
        ...
        public virtual void Spawn(RaycastHit hit, Vector3 gravityDirection, float timeScale, GameObject originator,
            bool spawnDecals)
        {
        ...
        {
            if (m_Decals.Length == 0) return null;
        ...
        public virtual void SpawnFootprint(RaycastHit hit, Vector3 gravityDirection, float timeScale,
            GameObject originator, bool spawnDecals, Vector3 footprintDirection, bool flipFootprint)
        {
            SpawnObjects(hit, gravityDirection);
            ...
            DecalManager.SpawnFootprint(GetDecal(), hit, Random.Range(m_MinDecalScale, m_MaxDecalScale),
                m_AllowedDecalEdgeOverlap, footprintDirection, flipFootprint);
        }
        ...
        private void PlayAudioClip(RaycastHit hit, float timeScale, GameObject originator)
        {
            ...
            if (m_OneClipPerFrame && m_LastPlayedAudioClipFrame == Time.frameCount) return;
            ...
            else
            {
                ...
                if (m_AudioClips == null || m_AudioClips.Length == 0) return;
        ...
        {
            if (string.IsNullOrEmpty(m_StateName)) return;
