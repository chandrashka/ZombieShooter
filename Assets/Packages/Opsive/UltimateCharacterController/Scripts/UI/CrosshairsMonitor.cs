...
namespace Opsive.UltimateCharacterController.UI
{
    ...
    public class CrosshairsMonitor : CharacterMonitor
    {
        ...
        [SerializeField]
        protected bool m_AutoAssignAimAssistTarget = true;
        ...
        [Tooltip("The maximum number of colliders that the crosshairs can detect.")] [SerializeField]
        protected int m_MaxCollisionCount = 40;

        [Tooltip("Specifies if the crosshairscan detect triggers.")] [SerializeField]
        protected QueryTriggerInteraction m_TriggerInteraction = QueryTriggerInteraction.Ignore;

        [Tooltip("Should the crosshairs move with the crosshairs?")] [SerializeField]
        protected bool m_MoveWithCursor;

        [Tooltip("The amount of spread to add based on the character's movement speed.")]
        [Range(0, 360)]
        [SerializeField]
        protected float m_MovementSpread;

        [Tooltip(
            "The maximum squared magnitude of the character. The spread will be at the max value when the character is moving at this magnitude.")]
        [SerializeField]
        protected float m_MaxSpreadVelocitySqrMagnitude = 50;

        [Tooltip("The speed at which the spread value should change values.")] [SerializeField]
        protected float m_MovementSpreadSpeed = 6;

        [Tooltip("The crosshairs used when the item doesn't specify a crosshairs.")] [SerializeField]
        protected Sprite m_DefaultSprite;

        [Tooltip("The default color of the crosshairs.")] [SerializeField]
        protected Color m_DefaultColor = Color.white;

        [Tooltip("The color of the crosshairs when a target is in sight.")] [SerializeField]
        protected Color m_TargetColor = Color.red;

        [Tooltip("A reference to the image used for the center crosshairs.")] [SerializeField]
        protected Image m_CenterCrosshairsImage;

        [Tooltip("A reference to the image used for the left crosshairs.")] [SerializeField]
        protected Image m_LeftCrosshairsImage;

        [Tooltip("A reference to the image used for the top crosshairs.")] [SerializeField]
        protected Image m_TopCrosshairsImage;

        [Tooltip("A reference to the image used for the right crosshairs.")] [SerializeField]
        protected Image m_RightCrosshairsImage;

        [Tooltip("A reference to the image used for the bottom crosshairs.")] [SerializeField]
        protected Image m_BottomCrosshairsImage;

        [Tooltip("Should the crosshairs be disabled when the character dies?")] [SerializeField]
        protected bool m_DisableOnDeath = true;
        ...
        public bool AutoAssignAimAssistTarget
        {
            get => m_AutoAssignAimAssistTarget;
            set => m_AutoAssignAimAssistTarget = value;
        }
        ...
        public bool MoveWithCursor
        {
            get => m_MoveWithCursor;
            set
            {
        ...
        {
            get => m_MovementSpread;
            set => m_MovementSpread = value;
        }

        public float MaxSpreadVelocitySqrMagnitude
        {
            get => m_MaxSpreadVelocitySqrMagnitude;
            set => m_MaxSpreadVelocitySqrMagnitude = value;
        }

        public float MovementSpreadSpeed
        {
            get => m_MovementSpreadSpeed;
            set => m_MovementSpreadSpeed = value;
        }
        ...
        {
            get => m_DisableOnDeath;
            set => m_DisableOnDeath = value;
        }
        ...
        protected override void Awake()
        {
            ...
            if (m_CenterCrosshairsImage == null) m_CenterCrosshairsImage = GetComponent<Image>();
        ...
        private void Update()
        {
            ...
            if (m_DebugDrawLookRay)
                Debug.DrawRay(crosshairsRay.origin, crosshairsRay.direction * m_CameraController.LookDirectionDistance,
                    Color.white);
#endif
            var hitCount = Physics.SphereCastNonAlloc(crosshairsRay, m_CollisionRadius, m_RaycastHits,
                m_CameraController.LookDirectionDistance, m_CharacterLayerManager.IgnoreInvisibleLayers,
                m_TriggerInteraction);
#if UNITY_EDITOR
            if (hitCount == m_MaxCollisionCount)
                Debug.LogWarning(
                    "Warning: The crosshairs detected the maximum number of objects. Consider increasing the Max Collision Count on the Crosshairs Monitor.");
#endif
            if (hitCount > 0)
                for (var i = 0; i < hitCount; ++i)
                {
                    ...
                    if (closestRaycastHitTransform.IsChildOf(m_CharacterTransform)) continue;

                    if (MathUtility.InLayerMask(closestRaycastHitTransform.gameObject.layer,
            ...
            if (m_AutoAssignAimAssistTarget && m_AimAssist != null) m_AimAssist.SetTarget(target);

            // The crosshairs can move with the cursor position.
            if (m_MoveWithCursor && Cursor.visible)
                m_RectTransform.position = new Vector3(m_PlayerInput.GetMousePosition().x,
                    m_PlayerInput.GetMousePosition().y, 0);

            // The spread of the crosshairs can change based on how quickly the character is moving.
            if (m_MovementSpread > 0)
            {
                var velocity = Mathf.Clamp01(m_CharacterLocomotion.LocalLocomotionVelocity.sqrMagnitude /
                                             m_MaxSpreadVelocitySqrMagnitude);
                m_SmoothedMovementVelocity = Mathf.SmoothStep(m_SmoothedMovementVelocity, velocity,
                    m_MovementSpreadSpeed * Time.deltaTime);
            }

            if (m_EquippedItem != null)
            {
                var movementSpread = (m_MovementSpread / 360 + m_SmoothedMovementVelocity) / 2 *
                                     m_EquippedItem.MaxQuadrantSpread;
        ...
        protected override void OnAttachCharacter(GameObject character)
        {
            ...
            if (inventory != null)
                for (var i = 0; i < inventory.SlotCount; ++i)
                {
                    ...
                    if (item != null) OnEquipItem(item, i);
                }
        }
        ...
        {
            var modifier = Mathf.Clamp01(m_CharacterLocomotion.LocalLocomotionVelocity.sqrMagnitude /
                                         m_MaxSpreadVelocitySqrMagnitude);
        ...
        {
            if (!item.DominantItem) return;

            m_CurrentCrosshairsSpread = m_TargetCrosshairsSpread = 0;
            m_EquippedItem = item;
            m_CenterCrosshairsImage.sprite = m_EquippedItem.CenterCrosshairs != null
                ? m_EquippedItem.CenterCrosshairs
                : m_DefaultSprite;
        ...
        private void OnUpdateDominantItem(Item item, bool dominantItem)
        {
            ...
            {
                m_EquippedItemCount--;
                if (m_EquippedItemCount == 0) ResetMonitor();
            }
        }
        ...
        private void EnableCrosshairsImage(bool enable)
        {
            ...
            if (m_LeftCrosshairsImage != null)
                m_LeftCrosshairsImage.enabled = enable && m_LeftCrosshairsImage.sprite != null;
            if (m_TopCrosshairsImage != null)
                m_TopCrosshairsImage.enabled = enable && m_TopCrosshairsImage.sprite != null;
            if (m_RightCrosshairsImage != null)
                m_RightCrosshairsImage.enabled = enable && m_RightCrosshairsImage.sprite != null;
            if (m_BottomCrosshairsImage != null)
                m_BottomCrosshairsImage.enabled = enable && m_BottomCrosshairsImage.sprite != null;
        ...
        {
            if (m_DisableOnDeath) m_GameObject.SetActive(false);
        }
        ...
        {
            if (m_DisableOnDeath && base.CanShowUI()) m_GameObject.SetActive(true);
        ...
        protected override bool CanShowUI()
        {
            return base.CanShowUI() &&
                   (!m_DisableOnDeath || (m_CharacterLocomotion != null && m_CharacterLocomotion.Alive));
        }
    }
}