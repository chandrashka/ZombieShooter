...
namespace Opsive.UltimateCharacterController.UI
{
    ...
    public class SlotItemMonitor : ItemMonitor
    {
        ...
        [Tooltip("A reference to the text used for the usable item loaded count.")] [SerializeField]
        protected Text m_LoadedCount;

        [Tooltip("A reference to the text used for the usable item unloaded count.")] [SerializeField]
        protected Text m_UnloadedCount;

        [Tooltip("The ID that UI represents.")] [SerializeField]
        protected int m_ID;

        [Tooltip("A reference to the image used for the item's icon.")] [SerializeField]
        protected Image m_ItemIcon;

        [Tooltip("The action ID that the UI represents.")] [SerializeField]
        protected int m_ItemActionID;
        ...
        protected override void Awake()
        {
            ...
            if (m_ItemIcon == null) m_ItemIcon = GetComponent<Image>();
        ...
        protected override void OnPickupItemIdentifier(IItemIdentifier itemIdentifier, int amount, bool immediatePickup,
            bool forceEquip)
        {
            if (itemIdentifier != m_ConsumableItemIdentifier) return;
        ...
        {
            if (!item.DominantItem || item.UIMonitorID != m_ID) return;
            ...
            if (m_ItemActionID < item.ItemActions.Length) itemAction = item.ItemActions[m_ItemActionID];
            ...
            if (itemAction is IUsableItem)
            {
                ...
                if ((m_ConsumableItemIdentifier = usableItem.GetConsumableItemIdentifier()) != null)
                {
                    ...
                    if (consumableItemIdentifierAmount != -1)
                    {
                        ...
                        m_UnloadedCount.text = m_CharacterInventory
                            .GetItemIdentifierAmount(usableItem.GetConsumableItemIdentifier()).ToString();
                        m_LoadedCount.enabled = m_UnloadedCount.enabled = true;
                        m_PrimaryCount.enabled = false;
                    }
                    else
                    {
                        m_PrimaryCount.text = m_CharacterInventory
                            .GetItemIdentifierAmount(usableItem.GetConsumableItemIdentifier()).ToString();
                        m_PrimaryCount.enabled = true;
                        m_LoadedCount.enabled = m_UnloadedCount.enabled = false;
                    }

                    if (m_CountParent != null) m_CountParent.SetActive(true);
                }
                else
                {
                    DisableCountText();
        ...
        {
            if (m_CountParent != null) m_CountParent.SetActive(false);
            if (m_PrimaryCount != null) m_PrimaryCount.enabled = false;
            if (m_LoadedCount != null) m_LoadedCount.enabled = false;
            if (m_UnloadedCount != null) m_UnloadedCount.enabled = false;
        }
        ...
        {
            if ((m_EquippedItem != null && item != m_EquippedItem) ||
                m_CharacterInventory.GetItemIdentifierAmount(item.ItemIdentifier) == 0 ||
                m_CharacterInventory.GetActiveItem(item.SlotID) != item) return;
        ...
        {
            if (item.UIMonitorID != m_ID || itemIdentifier != m_ConsumableItemIdentifier) return;
        ...
        private void OnUnequipItem(Item item, int slotID)
        {
            ...
            if (!item.DominantItem || item.UIMonitorID != m_ID ||
                ((equippedItem = m_CharacterInventory.GetActiveItem(slotID)) != null && equippedItem.DominantItem &&
                 equippedItem != item)) return;
