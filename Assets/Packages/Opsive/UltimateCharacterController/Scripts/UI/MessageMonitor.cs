...
{
    /// <summary>
    ///     The MessageMonitor will update the UI for any external object messages such as an ability being able to start or
    ///     the character picking up an item.
    /// </summary>
    public class MessageMonitor : CharacterMonitor
    {
        [Tooltip("A reference to the object that will show the message icon.")] [SerializeField]
        protected Image m_Icon;

        [Tooltip("A reference to the object that will show the message text.")] [SerializeField]
        protected Text m_Text;

        [Tooltip("The length of time that the message should be visible for after picking up an object.")]
        [SerializeField]
        protected float m_ObjectVisiblityDuration = 1.5f;

        [Tooltip("The amount to fade the message after it should no longer be displayed.")] [SerializeField]
        protected float m_ObjectFadeSpeed = 0.05f;
        ...
        private void OnAbilityCanStart(Ability ability, bool canStart)
        {
            ...
            if (m_Ability != null && m_Ability != ability && m_Ability.Index < ability.Index) return;
        ...
        private void OnObjectPickedUp(ObjectPickup objectPickup)
        {
            if (!string.IsNullOrEmpty(objectPickup.PickupMessageText) || objectPickup.PickupMessageIcon != null)
            {
                ...
                if (m_ShouldFade) SchedulerBase.Cancel(m_ScheduledFade);
            }
            else
            {
                // Keep showing the previous object message if the new message doesn't have text or icons.
                if (m_ObjectPickup != null && m_ObjectPickup != objectPickup) return;
        ...
        private void UpdateMessage()
        {
            ...
            if (m_Text != null)
            {
                m_Text.text = m_Ability != null ? m_Ability.AbilityMessageText :
                    m_ObjectPickup != null ? m_ObjectPickup.PickupMessageText : string.Empty;
                m_Text.enabled = !string.IsNullOrEmpty(m_Text.text);
            }

            if (m_Icon != null)
            {
                m_Icon.sprite = m_Ability != null ? m_Ability.AbilityMessageIcon :
                    m_ObjectPickup != null ? m_ObjectPickup.PickupMessageIcon : null;
            ...
            if (messageVisible)
            {
                ...
                if (m_ShouldFade) m_ScheduledFade = SchedulerBase.Schedule(m_ObjectVisiblityDuration, FadeMessage);
            }
