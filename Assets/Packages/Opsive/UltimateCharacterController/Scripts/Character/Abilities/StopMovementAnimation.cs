...
{
    /// <summary>
    ///     Prevents the movement animation from playing when the character would run into a solid object. The move direction
    ///     is predicted if the character is using
    ...
    {
        [Tooltip("The distance that is checked to determine if there is a collision.")] [SerializeField]
        protected float m_CollisionCheckDistance = 0.1f;

        private RaycastHit m_RaycastHit;
        private RestrictPosition m_RestrictPosition;

        [Tooltip(
            "The maximum Character Locomotion Wall Glide Curve value that the ability should start with. The higher the value to more the ability will ignore the Wall Glide Curve value.")]
        [SerializeField]
        protected float m_WallGlideCurveThreshold = 0.1f;

        public float CollisionCheckDistance
        {
            get => m_CollisionCheckDistance;
            set => m_CollisionCheckDistance = value;
        }

        public float WallGlideCurveThreshold
        {
            get => m_WallGlideCurveThreshold;
            set => m_WallGlideCurveThreshold = value;
        }
        ...
        /// <summary>
        ///     Predicts if there will be a collision based on the input vector. If using root motion this isn't completely
        ///     accurate because the animation is
        ...
        private bool PredicatedCollision()
        {
            ...
            for (var i = 0; i < m_CharacterLocomotion.ActiveAbilityCount; ++i)
                if ((!m_CharacterLocomotion.ActiveAbilities[i].IsConcurrent ||
                     m_CharacterLocomotion.ActiveAbilities[i] is MoveTowards) &&
                    m_CharacterLocomotion.ActiveAbilities[i].Index < Index)
                    return false;

            // If restrict position is stopping the move then the animation should also stop.
            var direction =
                m_Transform.TransformDirection(new Vector3(m_CharacterLocomotion.InputVector.x, 0,
                    m_CharacterLocomotion.InputVector.y)).normalized * m_CollisionCheckDistance;
            var targetPosition = m_Transform.position + direction;
            if (m_RestrictPosition != null && m_RestrictPosition.IsActive &&
                m_RestrictPosition.RestrictedPosition(ref targetPosition)) return true;

            // Stop the animation if the character would collide with a solid object.
            if (m_CharacterLocomotion.SingleCast(direction, Vector3.zero, m_CharacterLayerManager.SolidObjectLayers,
                    ref m_RaycastHit))
            {
                if (m_RaycastHit.rigidbody != null && !m_RaycastHit.rigidbody.isKinematic) return false;

                var slope = Vector3.Angle(m_CharacterLocomotion.Up, m_RaycastHit.normal);
                if (slope <= m_CharacterLocomotion.SlopeLimit + m_CharacterLocomotion.SlopeLimitSpacing) return false;
