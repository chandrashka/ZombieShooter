...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    {
        [Tooltip(
            "Specifies if the ability should wait for the OnAnimatorFallComplete animation event or wait for the specified duration before ending the fall.")]
        [SerializeField]
        protected AnimationEventTrigger m_LandEvent = new(true, 0f);

        [Tooltip("A reference to the Surface Impact triggered when the character hits the ground.")] [SerializeField]
        protected SurfaceImpact m_LandSurfaceImpact;

        [Tooltip(
            "The minimum height between the ground and character that the ability can start. Set to 0 to start at any height.")]
        [SerializeField]
        protected float m_MinFallHeight = 0.2f;

        [Tooltip("The minimum velocity required for the Surface Impact to play.")] [SerializeField]
        protected float m_MinSurfaceImpactVelocity = -4f;
        ...
        {
            get => m_MinFallHeight;
            set => m_MinFallHeight = value;
        }

        public SurfaceImpact LandSurfaceImpact
        {
            get => m_LandSurfaceImpact;
            set => m_LandSurfaceImpact = value;
        }

        public float MinSurfaceImpactVelocity
        {
            get => m_MinSurfaceImpactVelocity;
            set => m_MinSurfaceImpactVelocity = value;
        }

        public AnimationEventTrigger LandEvent
        {
            get => m_LandEvent;
        ...
        public override float AbilityFloatData => m_CharacterLocomotion.LocalLocomotionVelocity.y;

        [Snapshot] protected bool Landed { get; set; }
        ...
        public override bool CanStartAbility()
        {
            ...
            RaycastHit hit;
            if (m_MinFallHeight != 0 && Physics.Raycast(m_Transform.position, -m_CharacterLocomotion.Up, out hit,
                    m_MinFallHeight, m_CharacterLayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore))
                return false;
        ...
        private void OnGrounded(bool grounded)
        {
            if (grounded)
            {
                ...
                if (m_LandSurfaceImpact != null && m_CharacterLocomotion.LocalVelocity.y < m_MinSurfaceImpactVelocity)
                    SurfaceManager.SpawnEffect(m_CharacterLocomotion.GroundRaycastHit, m_LandSurfaceImpact,
                        m_CharacterLocomotion.GravityDirection, m_CharacterLocomotion.TimeScale, m_GameObject,
                        m_Transform.forward, false);

                // Move to the fall end state when the character lands.
                m_StateIndex = 1;
                if (!m_LandEvent.WaitForAnimationEvent) SchedulerBase.ScheduleFixed(m_LandEvent.Duration, Land);
            }
            else
            {
                m_StateIndex = 0;
        ...
        private void Land()
        {
            ...
            if (m_CharacterLocomotion.Grounded) StopAbility();
        }
        ...
        {
            if (!m_CharacterLocomotion.Grounded && !snapAnimator) return;
