...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    [AllowDuplicateTypes]
    public class Ride : DetectObjectAbilityBase, IItemToggledReceiver
    {
        [Tooltip(
        ...
        [SerializeField]
        protected AnimationEventTrigger m_DismountEvent;
        ...
        [SerializeField]
        protected AnimationEventTrigger m_MountEvent;

        [Tooltip("The speed at which the character moves towards the ride location.")] [SerializeField]
        ...
        [SerializeField]
        protected bool m_ReequipItemAfterMount = true;

        private Rideable m_Rideable;
        private RideState m_RideState = RideState.DismountComplete;

        [Tooltip("The speed at which the character rotates towards ride location.")] [SerializeField]
        protected float m_RotationSpeed = 2f;

        private KinematicObjectManager.UpdateLocation m_StartUpdateLocation;

        public AnimationEventTrigger MountEvent
        {
            get => m_MountEvent;
            set => m_MountEvent = value;
        }
        ...
        public AnimationEventTrigger DismountEvent
        {
            get => m_DismountEvent;
            set => m_DismountEvent = value;
        }

        public bool ReequipItemAfterMount
        {
            get => m_ReequipItemAfterMount;
            set => m_ReequipItemAfterMount = value;
        }
        ...
        public override int AbilityIntData
        {
            get
            {
                ...
                if (m_RideState == RideState.Ride)
                    return 3;
                if (m_RideState == RideState.Dismount) return m_LeftMount ? 4 : 5;
        ...
        {
            if (m_RideState != RideState.WaitForItemUnequip) return;
        ...
        public override bool ShouldBlockAbilityStart(Ability startingAbility)
        {
            ...
            if (startingAbility is ItemPullback) return true;
#endif
#if ULTIMATE_CHARACTER_CONTROLLER_MELEE
            if (startingAbility is InAirMeleeUse) return true;
#endif
            // The character cannot interact with any items while mounting/dismounting.
            if (m_RideState != RideState.Ride && startingAbility is ItemAbility) return true;
        ...
        {
#if THIRD_PERSON_CONTROLLER
            if (activeAbility is ItemPullback) return true;
#endif
        ...
        public override void UpdateRotation()
        {
            ...
            if (m_RideState != RideState.Ride && m_RideState != RideState.WaitForItemUnequip)
            {
                var upNormal = m_RideState == RideState.Mount
                    ? m_Rideable.Transform.up
                ...
                if (proj.sqrMagnitude > 0.0001f)
                {
                    var speed = m_RotationSpeed * m_CharacterLocomotion.TimeScale * Time.timeScale * Time.deltaTime *
                                (m_RideState == RideState.DismountComplete ? 100 : 1);
            ...
            else if (m_Rideable.RideLocation != null)
            {
                ...
                deltaRotation =
                    MathUtility.InverseTransformQuaternion(m_Transform.rotation, m_Rideable.RideLocation.rotation);
            }
        ...
        {
            if (m_RideState != RideState.Ride && m_RideState != RideState.WaitForItemUnequip) return;

            var transformPosition = m_Transform.position;
            m_CharacterLocomotion.MotorThrottle = Vector3.zero;
            var deltaPosition = Vector3.MoveTowards(transformPosition, m_Rideable.RideLocation.position, m_MoveSpeed) -
                                transformPosition;
        ...
        {
            if (m_RideState != RideState.Ride) return;

            // The character may not have space to dismount.
            if (!m_Rideable.CanDismount(ref m_LeftMount)) return;

            if (m_CharacterLocomotion.ItemEquipVerifierAbility != null)
            {
                // Don't allow a dismount if the character is equipping an item just after mounting.
                if (m_CharacterLocomotion.ItemEquipVerifierAbility.IsActive) return;
        ...
        private void StartDismount()
        {
            ...
            var aimAbility = m_CharacterLocomotion.GetAbility<Aim>();
            if (aimAbility != null) aimAbility.StopAbility();
        ...
        public override bool CanStopAbility()
        {
            ...
            return m_RideState == RideState.DismountComplete &&
                   Vector3.Dot(m_Transform.rotation * Vector3.up, -m_CharacterLocomotion.GravityDirection) >= m_Epsilon;
        }
