...
{
    /// <summary>
    ///     The ComboTimeout is an AbilityStarter that will start the ability when a combo has been performed. If the next
    ///     combo input isn't performed within the timeout value
    ///     then the combo will need to be reset from the beginning.
    /// </summary>
    public class ComboTimeout : AbilityStarter
    {
        [Tooltip("Specifies the combo that should be performed before the ability starts.")] [SerializeField]
        protected ComboInputElement[] m_ComboInputElements;

        private int m_CurrentComboElementIndex;
        private float m_LastComboElementTime = -1;

        public ComboInputElement[] ComboInputElements
        {
            get => m_ComboInputElements;
            set => m_ComboInputElements = value;
        }
        ...
        public override bool CanInputStartAbility(PlayerInput playerInput)
        {
            ...
            if (m_LastComboElementTime == -1 || m_LastComboElementTime + currentComboElement.Timeout >= Time.time)
            {
                ...
                if ((!currentComboElement.AxisInput && playerInput.GetButtonDown(currentComboElement.InputName)) ||
                    (currentComboElement.AxisInput &&
                     Mathf.Abs(playerInput.GetAxisRaw(currentComboElement.InputName)) > 0.00001f))
                {
                    // The end of the combos has been reached - start the ability.
                    if (m_CurrentComboElementIndex == m_ComboInputElements.Length - 1) return true;
        ...
        public struct ComboInputElement
        {
            ...
            [Tooltip("Should the axis be checked? If false then a button down will be checked.")] [SerializeField]
            private bool m_AxisInput;

            [Tooltip(
                "The amount of time the current input can be performed in. The first element does not use the timeout value.")]
            [SerializeField]
