...
{
    /// <summary>
    ///     The Jump ability allows the character to jump into the air. Jump is only active when the character has a positive y
    ///     velocity.
    /// </summary>
    ...
    public class Jump : Ability
    {
        ...
        [Tooltip("Contains an array of AudioClips toat can be played when a repeated jump occurs.")] [SerializeField]
        protected AudioClipSet m_AirborneJumpAudioClipSet = new();

        private int m_AirborneJumpCount;

        [Tooltip("The amount of force that applied when the character performs an airborne jump.")] [SerializeField]
        protected float m_AirborneJumpForce = 0.6f;

        [Tooltip("The number of frames that the repeated jump force is applied in.")] [SerializeField]
        protected int m_AirborneJumpFrames = 10;

        private ActiveInputEvent m_AirborneJumpInput;
        private bool m_AirborneJumpRegistered;
        private bool m_ApplyHoldForce;

        [Tooltip("A multiplier applied to the force while moving backwards.")] [SerializeField]
        protected float m_BackwardsForceMultiplier = 0.7f;

        [Tooltip("The amount of force that should be applied when the character jumps.")] [SerializeField]
        protected float m_Force = 0.22f;

        [Tooltip("Determines how quickly the jump force wears off.")] [SerializeField]
        protected float m_ForceDamping = 0.08f;

        [Tooltip("Determines how quickly the jump hold force wears off.")] [SerializeField]
        protected float m_ForceDampingHold = 0.5f;
        ...
        [SerializeField]
        protected float m_ForceHold = 0.003f;

        private bool m_ForceImmediateJump;

        [Tooltip("The number of frames that the force is applied in.")] [SerializeField]
        protected int m_Frames = 1;

        [Tooltip(
            "The amount of time after the character is airborne that the character can still jump. Also known as coyote time. " +
            "Set to -1 to allow for the character to jump at any time after being airborne.")]
        [SerializeField]
        protected float m_GroundedGracePeriod = 0.05f;
        ...
        [Tooltip(
            "Specifies if the ability should wait for the OnAnimatorJump animation event or wait for the specified duration before applying the jump force.")]
        [SerializeField]
        protected AnimationEventTrigger m_JumpEvent = new(true, 0f);

        [Tooltip("The Surface Impact triggered when the character jumps.")] [SerializeField]
        protected SurfaceImpact m_JumpSurfaceImpact;

        private float m_JumpTime = -1;
        private float m_LandTime = -1;

        [Tooltip(
            "Specifies the number of times the character can perform a airborne jump (double jump, triple jump, etc). Set to -1 to allow an infinite number of airborne jumps.")]
        [SerializeField]
        protected int m_MaxAirborneJumpCount;
        ...
        [SerializeField]
        protected float m_MinCeilingJumpHeight = 0.05f;
        ...
        [SerializeField]
        protected bool m_PreventSlopeLimitJump = true;
        ...
        [Tooltip("A multiplier applied to the force while moving sideways.")] [SerializeField]
        protected float m_SidewaysForceMultiplier = 0.8f;

        [Tooltip("A vertical velocity value below the specified amount will stop the ability.")] [SerializeField]
        protected float m_VerticalVelocityStopThreshold = -0.001f;

        public float MinCeilingJumpHeight
        {
            get => m_MinCeilingJumpHeight;
            set => m_MinCeilingJumpHeight = value;
        }

        public float GroundedGracePeriod
        {
            get => m_GroundedGracePeriod;
            set => m_GroundedGracePeriod = value;
        }

        public bool PrevntSlopeLimitJump
        {
            get => m_PreventSlopeLimitJump;
            set => m_PreventSlopeLimitJump = value;
        }
        ...
        {
            get => m_SidewaysForceMultiplier;
            set => m_SidewaysForceMultiplier = value;
        }

        public float BackwardsForceMultiplier
        {
            get => m_BackwardsForceMultiplier;
            set => m_BackwardsForceMultiplier = value;
        }
        ...
        {
            get => m_ForceDamping;
            set => m_ForceDamping = value;
        }

        public AnimationEventTrigger JumpEvent
        {
            get => m_JumpEvent;
            set => m_JumpEvent = value;
        }

        public SurfaceImpact JumpSurfaceImpact
        {
            get => m_JumpSurfaceImpact;
            set => m_JumpSurfaceImpact = value;
        }

        public float ForceHold
        {
            get => m_ForceHold;
            set => m_ForceHold = value;
        }

        public float ForceDampingHold
        {
            get => m_ForceDampingHold;
            set => m_ForceDampingHold = value;
        }

        public int MaxAirborneJumpCount
        {
            get => m_MaxAirborneJumpCount;
            set => m_MaxAirborneJumpCount = value;
        }

        public float AirborneJumpForce
        {
            get => m_AirborneJumpForce;
            set => m_AirborneJumpForce = value;
        }

        public int AirborneJumpFrames
        {
            get => m_AirborneJumpFrames;
            set => m_AirborneJumpFrames = value;
        }

        public AudioClipSet AirborneJumpAudioClipSet
        {
            get => m_AirborneJumpAudioClipSet;
            set => m_AirborneJumpAudioClipSet = value;
        }

        public float VerticalVelocityStopThreshold
        {
            get => m_VerticalVelocityStopThreshold;
            set => m_VerticalVelocityStopThreshold = value;
        }

        public float RecurrenceDelay
        {
            get => m_RecurrenceDelay;
            set => m_RecurrenceDelay = value;
        }

        [Snapshot] protected float HoldForce { get; set; }

        [Snapshot] protected bool JumpApplied { get; set; }

        [Snapshot] protected bool Jumping { get; set; }

        public bool ForceImmediateJump
        {
            set => m_ForceImmediateJump = value;
        }
        ...
        public override float AbilityFloatData
        {
            get
            {
                if (Jumping) return m_CharacterLocomotion.LocalLocomotionVelocity.y;
                return -1;
            }
        }

        public override int AbilityIntData => m_AirborneJumpCount > 0 ? 2 : JumpApplied ? 0 : 1;
        ...
        public override bool CanStartAbility()
        {
            ...
            if (m_MinCeilingJumpHeight != -1)
                ...
                if (m_CharacterLocomotion.SingleCast(
                        m_CharacterLocomotion.Up * (m_MinCeilingJumpHeight + m_CharacterLocomotion.SkinWidth +
                                                    m_CharacterLocomotion.ColliderSpacing),
            ...
            if (m_CharacterLocomotion.Grounded)
            {
                ...
                if (m_LandTime + m_RecurrenceDelay >= Time.time) return false;
                ...
                if (m_PreventSlopeLimitJump)
                {
                    ...
                    if (slope > m_CharacterLocomotion.SlopeLimit) return false;
                }
            }
            else
            {
                ...
                if ((!JumpApplied && m_GroundedGracePeriod != -1 && m_InAirTime + m_GroundedGracePeriod <= Time.time) ||
                    (m_AirborneJumpCount != -1 && m_AirborneJumpCount > m_MaxAirborneJumpCount) ||
        ...
        {
            if (activeAbility is Fall) return true;
        ...
        protected override void AbilityStarted()
        {
            ...
            if (m_ForceHold > 0)
            {
                if (m_Handler != null && InputIndex != -1)
                {
                    ...
                    m_HoldInput.Initialize(ActiveInputEvent.Type.ButtonUp, InputNames[InputIndex],
                        "OnJumpAbilityReleaseHold");
            ...
            if (!m_AirborneJumpRegistered)
            {
                if (m_Handler != null && InputIndex != -1)
                {
                    ...
                    m_AirborneJumpInput.Initialize(ActiveInputEvent.Type.ButtonDown, InputNames[InputIndex],
                        "OnJumpAbilityAirborneJump");
        ...
        public override void UpdateAnimator()
        {
            ...
            if (Jumping) SetAbilityFloatDataParameter(m_CharacterLocomotion.LocalLocomotionVelocity.y);
        }
        ...
        {
            if (IsActive && !JumpApplied)
            {
                // A surface effect can optionally play when the character leaves the ground.
                if (m_JumpSurfaceImpact != null)
                    SurfaceManager.SpawnEffect(m_CharacterLocomotion.GroundRaycastHit, m_JumpSurfaceImpact,
                        m_CharacterLocomotion.GravityDirection, m_CharacterLocomotion.TimeScale, m_GameObject);
                ...
                if (m_CharacterLocomotion.InputVector.y < 0)
                    ...
                    force *= Mathf.Lerp(1, m_SidewaysForceMultiplier,
                        Mathf.Abs(m_CharacterLocomotion.InputVector.x) -
                        Mathf.Abs(m_CharacterLocomotion.InputVector.y));
        ...
        /// <summary>
        ///     After jumping the character should be in the air. If the character is not in the air then another object prevented
        ///     the character from jumping and the
        ...
        {
            if (m_MaxAirborneJumpCount != -1 && m_AirborneJumpCount >= m_MaxAirborneJumpCount) return;
        ...
        public override void UpdatePosition()
        {
            if (!Jumping)
            {
                if (JumpApplied) Jumping = true;
                return;
            }

            if (m_AirborneJumpApplied) m_AirborneJumpApplied = false;
            ...
            {
                HoldForce += m_ForceHold * deltaTime;
                HoldForce /= 1 + m_ForceDampingHold * deltaTime;
                force += HoldForce;
            }

            // When the jump force is added it is added to the character's external force. Dampen this external force.
            if (force != 0) AddForce(m_CharacterLocomotion.Up * force, 1, false, false);
        }
        ...
        public override bool CanStopAbility()
        {
            ...
            if (Jumping && m_CharacterLocomotion.LocalLocomotionVelocity.y <= m_VerticalVelocityStopThreshold &&
                !m_AirborneJumpApplied) return true;
            return false;
        }
