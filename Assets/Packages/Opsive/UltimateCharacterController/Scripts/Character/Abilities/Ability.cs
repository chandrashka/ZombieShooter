...
{
    /// <summary>
    ///     Abilities extend the character controller to allow new functionality without having to directly change the core
    ///     character controller code. Abilities can change
    ///     any aspect of the character controller, from disabling gravity to setting the Animator Controller parameters.
    ///     Abilities are synchronized over the network.
    /// </summary>
    [Serializable]
    [Preserve]
    public abstract class Ability : StateObject
    {
        ...
        /// <summary>
        ///     Specifies how the ability can be stopped. Button toggle means that the same button has to be pressed again after
        ///     the ability has started to stop the ability.
        /// </summary>
        ...
        [Tooltip("Specifies how the ability can be started.")] [HideInInspector] [SerializeField]
        protected AbilityStartType m_StartType = AbilityStartType.Automatic;

        [Tooltip("Specifies how the ability can be stopped.")] [HideInInspector] [SerializeField]
        protected AbilityStopType m_StopType = AbilityStopType.Manual;

        [Tooltip("The button name(s) that can start or stop the ability.")] [HideInInspector] [SerializeField]
        ...
        [HideInInspector]
        [SerializeField]
        protected int m_AbilityIndexParameter = -1;

        [Tooltip("A set of AudioClips that can be played when the ability is started.")]
        [HideInInspector]
        [SerializeField]
        protected AudioClipSet m_StartAudioClipSet = new();

        [Tooltip("A set of AudioClips that can be played when the ability is started.")]
        [HideInInspector]
        [SerializeField]
        protected AudioClipSet m_StopAudioClipSet = new();

        [Tooltip("Does the ability allow positional input?")] [HideInInspector] [SerializeField]
        protected bool m_AllowPositionalInput = true;

        [Tooltip("Does the ability allow rotational input?")] [HideInInspector] [SerializeField]
        protected bool m_AllowRotationalInput = true;

        [Tooltip("Should the character use gravity while the ability is active?")] [HideInInspector] [SerializeField]
        protected AbilityBoolOverride m_UseGravity = AbilityBoolOverride.NoOverride;

        [Tooltip("Can the character use root motion for positioning?")] [HideInInspector] [SerializeField]
        protected AbilityBoolOverride m_UseRootMotionPosition = AbilityBoolOverride.NoOverride;

        [Tooltip("Can the character use root motion for rotation?")] [HideInInspector] [SerializeField]
        protected AbilityBoolOverride m_UseRootMotionRotation = AbilityBoolOverride.NoOverride;

        [Tooltip("Should the character detect horizontal collisions while the ability is active?")]
        [HideInInspector]
        [SerializeField]
        protected AbilityBoolOverride m_DetectHorizontalCollisions = AbilityBoolOverride.NoOverride;

        [Tooltip("Should the character detect vertical collisions while the ability is active?")]
        [HideInInspector]
        [SerializeField]
        protected AbilityBoolOverride m_DetectVerticalCollisions = AbilityBoolOverride.NoOverride;

        [Tooltip("A reference to the AnimatorMotion that the ability uses.")] [HideInInspector] [SerializeField]
        protected AnimatorMotion m_AnimatorMotion;

        [Tooltip("The effect that should be started when the ability starts.")] [HideInInspector] [SerializeField]
        protected string m_StartEffectName;

        [Tooltip("The index of the effect that should be started when the ability starts.")]
        [HideInInspector]
        [SerializeField]
        protected int m_StartEffectIndex = -1;

        [Tooltip("A mask specifying which slots can have the item equipped when the ability is active.")]
        [HideInInspector]
        [SerializeField]
        protected int m_AllowEquippedSlotsMask = -1;

        [Tooltip(
            "An array of ItemDefinitions that are allowed to be equipped when the ability starts. Any item can be equipped if no ItemDefinitions are specified.")]
        [HideInInspector]
        [SerializeField]
        protected ItemDefinitionBase[] m_AllowItemDefinitions;

        [Tooltip("Should the items be unequipped immediately?")] [HideInInspector] [SerializeField]
        protected bool m_ImmediateUnequip;

        [Tooltip("Should the ability equip the slots that were unequipped when the ability started?")]
        [HideInInspector]
        [SerializeField]
        protected bool m_ReequipSlots = true;

        [Tooltip("The text that should be shown by the message monitor when the ability can start.")]
        [HideInInspector]
        [SerializeField]
        protected string m_AbilityMessageText;

        [Tooltip("The sprite that should be drawn by the message monitor when the ability can start.")]
        [HideInInspector]
        [SerializeField]
        protected Sprite m_AbilityMessageIcon;
#if UNITY_EDITOR
        [Tooltip("An editor only description of the ability.")] [HideInInspector] [SerializeField]
        ...
        public bool Enabled
        {
            ...
            {
                m_Enabled = value;
                if (!m_Enabled && IsActive && CanForceStopAbility()) StopAbility(true, false);
                SetEnabled(value);
            }
        }
        ...
        public AbilityStartType StartType
        {
            ...
            set
            {
                if (m_CharacterLocomotion != null && m_StartType == AbilityStartType.Automatic &&
                    value != AbilityStartType.Automatic && value != AbilityStartType.Manual)
                {
                    var stopAbility = true;
                    if (m_ActiveInput != null)
                        for (var i = 0; i < m_ActiveInput.Length; ++i)
                            if (m_ActiveInput[i])
                            {
                                stopAbility = false;
                                break;
        ...
        public string[] InputNames
        {
            ...
            set
            {
                ...
                for (var i = 0; i < m_InputNames.Length; ++i) m_ButtonUp[i] = true;
        ...
        {
            get => m_LongPressDuration;
            set => m_LongPressDuration = value;
        }

        public bool WaitForLongPressRelease
        {
            get => m_WaitForLongPressRelease;
            set => m_WaitForLongPressRelease = value;
        }
        ...
        public Serialization StarterData
        {
            ...
            {
                m_StarterData = value;
                if (!Application.isPlaying) DeserializeAbilityStarter();
            }
        }
        ...
        public virtual int AbilityIndexParameter
        {
            get => m_AbilityIndexParameter;
            set => m_AbilityIndexParameter = value;
        }

        public AudioClipSet StartAudioClipSet
        {
            get => m_StartAudioClipSet;
            set => m_StartAudioClipSet = value;
        }

        public AudioClipSet StopAudioClipSet
        {
            get => m_StopAudioClipSet;
            set => m_StopAudioClipSet = value;
        }

        public bool AllowPositionalInput
        {
            get => m_AllowPositionalInput;
            set => m_AllowPositionalInput = value;
        }

        public bool AllowRotationalInput
        {
            get => m_AllowRotationalInput;
            set => m_AllowRotationalInput = value;
        }

        public AbilityBoolOverride UseGravity
        {
            get => m_UseGravity;
            set => m_UseGravity = value;
        }

        public AbilityBoolOverride UseRootMotionPosition
        {
            get => m_UseRootMotionPosition;
            set => m_UseRootMotionPosition = value;
        }

        public AbilityBoolOverride UseRootMotionRotation
        {
            get => m_UseRootMotionRotation;
            set => m_UseRootMotionRotation = value;
        }

        public AbilityBoolOverride DetectHorizontalCollisions
        {
            get => m_DetectHorizontalCollisions;
            set => m_DetectHorizontalCollisions = value;
        }

        public AbilityBoolOverride DetectVerticalCollisions
        {
            get => m_DetectVerticalCollisions;
            set => m_DetectVerticalCollisions = value;
        }

        public AnimatorMotion AnimatorMotion
        {
            get => m_AnimatorMotion;
            set => m_AnimatorMotion = value;
        }

        public string StartEffectName
        {
            get => m_StartEffectName;
            set => m_StartEffectName = value;
        }

        public int StartEffectIndex
        {
            get => m_StartEffectIndex;
            set => m_StartEffectIndex = value;
        }

        public int AllowEquippedSlotsMask
        {
            get => m_AllowEquippedSlotsMask;
            set => m_AllowEquippedSlotsMask = value;
        }

        public ItemDefinitionBase[] AllowItemDefinitions
        {
            get => m_AllowItemDefinitions;
            set => m_AllowItemDefinitions = value;
        }

        public bool ImmediateUnequip
        {
            get => m_ImmediateUnequip;
            set => m_ImmediateUnequip = value;
        }

        public bool ReequipSlots
        {
            get => m_ReequipSlots;
            set => m_ReequipSlots = value;
        }

        public virtual string AbilityMessageText
        {
            get => m_AbilityMessageText;
            set
            {
                m_AbilityMessageText = value;
                CheckForAbilityMessage = !string.IsNullOrEmpty(m_AbilityMessageText) || m_AbilityMessageIcon != null;
            }
        }

        public Sprite AbilityMessageIcon
        {
            get => m_AbilityMessageIcon;
            set
            {
        ...
        {
            get => m_InputAxisValue;
            set => m_InputAxisValue = value;
        }

        public float StartTime { get; private set; } = -1;

        public bool CheckForAbilityMessage { get; private set; }
        ...
        public virtual bool IgnorePriority => false;
        public virtual bool CanReceiveMultipleStarts => false;
        public virtual bool CanStayActivatedOnDeath => false;
        public virtual bool ImmediateStartItemVerifier => false;

        [Shared.Utility.NonSerialized]
        public virtual int AbilityIntData
        {
            get => -1;
            set
            {
        ...
        public virtual float AbilityFloatData => -1;
#if UNITY_EDITOR
        // Allows the ability to show a brief description within the inspector's ReorderableList.
        public virtual string AbilityDescription => string.Empty;
#endif
        protected bool AllowRootMotionPosition
        {
            set => m_CharacterLocomotion.AllowRootMotionPosition = value;
        }

        protected bool AllowRootMotionRotation
        {
            set => m_CharacterLocomotion.AllowRootMotionRotation = value;
        }
        ...
        public void Initialize(UltimateCharacterLocomotion characterLocomotion, int index)
        {
            ...
            if (m_AttributeModifier != null) m_AttributeModifier.Initialize(m_GameObject);
            if (!string.IsNullOrEmpty(m_StartEffectName))
                m_StartEffect =
                    m_CharacterLocomotion.GetEffect(TypeUtility.GetType(m_StartEffectName), m_StartEffectIndex);
        ...
        private void DeserializeAbilityStarter()
        {
            if (m_StarterData != null)
            {
                ...
                if (m_Starter != null && Application.isPlaying) m_Starter.Initialize(this);
            }
        }
        ...
        public virtual bool CanInputStartAbility(PlayerInput playerInput)
        {
            ...
            if (m_InputNames != null && m_InputNames.Length > 0)
                for (var i = 0; i < m_InputNames.Length; ++i)
                {
                    if (m_ActiveInput[i] && !CanReceiveMultipleStarts) continue;
                    ...
                    if (!m_ButtonUp[i] && (m_StartType == AbilityStartType.ButtonDown ||
                                           m_StartType == AbilityStartType.DoublePress ||
                                           m_StartType == AbilityStartType.LongPress ||
                                           m_StartType == AbilityStartType.Tap))
                    ...
                    if ((m_ButtonUp[i] &&
                        ...
                        (m_StartType == AbilityStartType.ButtonDownContinuous &&
                         playerInput.GetButton(m_InputNames[i])) ||
                        (m_StartType == AbilityStartType.DoublePress && playerInput.GetDoublePress(m_InputNames[i])) ||
                        (m_StartType == AbilityStartType.LongPress && playerInput.GetLongPress(m_InputNames[i],
                            m_LongPressDuration, m_WaitForLongPressRelease)) ||
                    ...
                    float axisValue;
                    if (m_StartType == AbilityStartType.Axis &&
                        Mathf.Abs(axisValue = playerInput.GetAxisRaw(m_InputNames[i])) > 0.00001f)
                    {
                        m_InputIndex = i;
                    ...
                    if (m_StopType == AbilityStopType.ButtonToggle)
                    {
                        ...
                        if (!m_ButtonUp[i] && !inputButton)
                        {
                            ...
                            m_ButtonUp[i] = true;
                             ...
                             (m_StopType == AbilityStopType.LongPress && playerInput.GetLongPress(m_InputNames[i],
                                 m_LongPressDuration, m_WaitForLongPressRelease)))
                    {
                        m_InputIndex = i;
                        return true;
                    }
                    else if (m_StopType == AbilityStopType.Axis &&
                             Mathf.Abs(playerInput.GetAxisRaw(m_InputNames[i])) <= 0.00001f)
                    {
                        m_InputIndex = i;
                        return true;
                    }
        ...
        /// <summary>
        ///     Method called by MonoBehaviour.Start. This method is called on all abilities when the MonoBehaviour.Start method is
        ///     called.
        /// </summary>
        ...
        public virtual MoveTowardsLocation[] GetMoveTowardsLocations()
        {
            return null;
        }
        ...
        public virtual bool ShouldBlockAbilityStart(Ability startingAbility)
        {
            return false;
        }
        ...
        public virtual bool ShouldStopActiveAbility(Ability activeAbility)
        {
            return false;
        }
        ...
        public virtual bool AbilityWillStart()
        {
            return true;
        }
        ...
        protected void AbilityStarted(bool enableAttributeModifier)
        {
            ...
            if (m_InputIndex != -1)
            {
                ...
                if (m_StartType == AbilityStartType.ButtonDown ||
                    m_StartType == AbilityStartType.ButtonDownContinuous ||
                    m_StartType == AbilityStartType.DoublePress ||
            ...
            m_InputIndex = -1;
            if (m_StartEffect != null) m_CharacterLocomotion.TryStartEffect(m_StartEffect);
            if (m_Starter != null) m_Starter.AbilityStarted();
            ...
            else if (m_UseGravity == AbilityBoolOverride.False) m_CharacterLocomotion.AllowUseGravity = false;
            ...
            SetState(true);
            if (enableAttributeModifier && m_AttributeModifier != null) m_AttributeModifier.EnableModifier(true);
        ...
        public virtual void Update()
        {
            ...
            // can no longer run.
            if (m_AttributeModifier != null && !m_AttributeModifier.IsValid()) StopAbility();
        }
        ...
        /// <summary>
        ///     Callback when the ability tries to be stopped. This method allows for any actions required before the ability
        ///     actually stops.
        /// </summary>
        ...
        public virtual bool CanForceStopAbility()
        {
            return true;
        }
        ...
        public bool StopAbility(bool force, bool fromController)
        {
            ...
            if (!fromController) return m_CharacterLocomotion.TryStopAbility(this, force);
        ...
        protected virtual void AbilityStopped(bool force)
        {
            ...
            ResetInput(force);
            if (m_Starter != null) m_Starter.AbilityStopped();

            if (m_UseGravity == AbilityBoolOverride.True)
                m_CharacterLocomotion.ForceUseGravity = false;
            else if (m_UseGravity == AbilityBoolOverride.False) m_CharacterLocomotion.AllowUseGravity = true;
            if (m_UseRootMotionPosition == AbilityBoolOverride.True)
            ...
            SetState(false);
            if (m_AttributeModifier != null) m_AttributeModifier.EnableModifier(false);
        ...
        protected void ResetInput(bool force)
        {
            if (m_ActiveInput != null)
                for (var i = 0; i < m_ActiveInput.Length; ++i)
                {
                    ...
                    if (force && (m_StartType == AbilityStartType.ButtonDown ||
                                  m_StartType == AbilityStartType.ButtonDownContinuous ||
                                  m_StartType == AbilityStartType.DoublePress ||
                                  m_StartType == AbilityStartType.LongPress || m_StartType == AbilityStartType.Tap))
                        m_ButtonUp[i] = true;
        ...
        {
            if (m_ButtonUp == null) return;

            // Reset the button input if the input shouldn't be checked.
            var forceUp = m_StartType == AbilityStartType.Automatic || m_StartType == AbilityStartType.Axis ||
                          m_StartType == AbilityStartType.Manual || !ShouldCheckInput();
            for (var i = 0; i < m_ButtonUp.Length; ++i)
        ...
        /// </summary>
        protected virtual bool ShouldCheckInput()
        {
            return true;
        }
        ...
        /// <summary>
        ///     The Animator is requesting the animation's delta position and rotation. The ability can use an AnimationMotion
        ///     ScriptableObject to generate move data without using
        ///     root motion.
        /// </summary>
        ...
        {
            if (m_AnimatorMotion == null) return;

            // Evaluate the curve at the current time.
            if (updatePosition)
            {
                var deltaPosition = m_CharacterLocomotion.AnimatorDeltaPosition;
                m_AnimatorMotion.EvaluatePosition(Time.time - StartTime, ref deltaPosition);
            ...
            else
            {
                ...
                m_AnimatorMotion.EvaluateRotation(Time.time - StartTime, ref deltaRotation);
        ...
        {
            if (m_Starter != null) m_Starter.OnDestroy();
        }
        ...
        /// <summary>
        ///     Helper method which adds a force to the character in the specified number of frames.  The force will either be an
        ///     external or soft force.
        /// </summary>
        /// <param name="force">The force to add.</param>
        ...
        /// <summary>
        ///     Helper method which adds a force relative to the character in the specified number of frames. The force will either
        ///     be an external or soft force.
        /// </summary>
        /// <param name="force">The force to add.</param>
        /// <param name="frames">The number of frames to add the force to.</param>
        ...
        protected void SetSpeedParameter(float value)
        {
            ...
            m_AnimatorMonitor.SetSpeedParameter(
                m_CharacterLocomotion.SpeedParameterOverride ? value : m_CharacterLocomotion.SpeedParameterValue,
