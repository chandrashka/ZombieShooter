...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public abstract class AlignToGravity : Ability
    {
        ...
        private readonly float m_Epsilon = 1f - Mathf.Epsilon;

        [Tooltip("Specifies the speed that the character can rotate to align to the ground.")] [SerializeField]
        protected float m_RotationSpeed = 10;

        [Tooltip("The direction of gravit that should be set when the ability stops. Set to Vector3.zero to disable.")]
        [SerializeField]
        protected Vector3 m_StopGravityDirection = Vector3.zero;
        ...
        public Vector3 StopGravityDirection
        {
            get => m_StopGravityDirection;
            set => m_StopGravityDirection = value;
        }
        ...
        protected override void AbilityStarted()
        {
            ...
            if (m_AlignToGravityReset != null) SchedulerBase.Cancel(m_AlignToGravityReset);
        ...
        protected void Rotate(Vector3 targetNormal)
        {
            ...
            var proj = rotation * Vector3.forward -
                       Vector3.Dot(rotation * Vector3.forward, targetNormal) * targetNormal;
            if (proj.sqrMagnitude > 0.0001f)
            {
                Quaternion targetRotation;
                if (m_CharacterLocomotion.Platform == null && !m_Stopping)
                {
                    var alignToGroundSpeed = m_RotationSpeed * m_CharacterLocomotion.TimeScale * Time.timeScale *
                                             Time.deltaTime;
                    targetRotation = Quaternion.Slerp(rotation, Quaternion.LookRotation(proj, targetNormal),
                        alignToGroundSpeed);
                }
                else
                {
        ...
        public override bool CanStopAbility()
        {
            ...
            if (m_StoppingFromUpdate && (m_StopGravityDirection.sqrMagnitude == 0 ||
                                         Vector3.Dot(m_Transform.rotation * Vector3.up, -m_StopGravityDirection) >=
                                         m_Epsilon)) return true;
