...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public class Rideable : Ability
    {
        ...
        [SerializeField]
        protected Collider m_LeftDismountCollider;
        ...
        [SerializeField]
        protected Transform m_RideLocation;

        [Tooltip(
            "The collider area that should be checked to determine if the character can dismount on the right side.")]
        [SerializeField]
        protected Collider m_RightDismountCollider;

        public Transform RideLocation => m_RideLocation;

        [Shared.Utility.NonSerialized]
        public Collider LeftDismountCollider
        {
            get => m_LeftDismountCollider;
            set => m_LeftDismountCollider = value;
        }

        [Shared.Utility.NonSerialized]
        public Collider RightDismountCollider
        {
            get => m_RightDismountCollider;
            set => m_RightDismountCollider = value;
        }

        public override int AbilityIntData =>
            // The rideable character should stay synchronized with the ride character.
            m_Ride.AbilityIntData;
        ...
        public override void Awake()
        {
            ...
            {
                Debug.LogWarning(
                    "Warning: The RidePLocation is null. This should be set to the Transform that the Ride character is parented to.");
            ...
            if (m_LeftDismountCollider != null) m_LeftDismountCollider.enabled = false;
            if (m_RightDismountCollider != null) m_RightDismountCollider.enabled = false;
        }
        ...
        protected override void AbilityStarted()
        {
            ...
            if (m_OriginalColliders == null)
                ...
                Array.Resize(ref m_OriginalColliders, m_CharacterLocomotion.ColliderCount);
        ...
        /// <summary>
        ///     Can the character dismount? The MoveTowardsLocation dismount colliders must be clear for the charcter to be able to
        ///     dismount.
        /// </summary>
        ...
        public bool CanDismount(ref bool leftDismount)
        {
            ...
            if (!DismountColliderOverlap(dismountCollider)) return true;
        ...
        {
            if (dismountCollider == null) return true;
            ...
            if (dismountCollider is CapsuleCollider)
            {
                ...
                MathUtility.CapsuleColliderEndCaps(capsuleCollider,
                    dismountTransform.TransformPoint(capsuleCollider.center), dismountTransform.rotation,
            ...
            else if (dismountCollider is BoxCollider)
            {
                ...
                hitCount = Physics.OverlapBoxNonAlloc(dismountTransform.TransformPoint(boxCollider.center),
                    Vector3.Scale(boxCollider.size, dismountTransform.lossyScale) / 2,
                    m_OverlapColliders, dismountTransform.rotation, m_CharacterLayerManager.SolidObjectLayers,
            ...
            else
            {
                ...
                hitCount = Physics.OverlapSphereNonAlloc(dismountTransform.TransformPoint(sphereCollider.center),
