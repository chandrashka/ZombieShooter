...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public class AlignToGround : AlignToGravity
    {
        ...
        [Tooltip("The depth offset when checking the ground normal.")] [SerializeField]
        protected float m_DepthOffset = 0.05f;

        [Tooltip("The distance from the ground that the character should align itself to.")] [SerializeField]
        ...
        [SerializeField]
        protected bool m_NormalizeDirection;
        ...
        {
            get => m_NormalizeDirection;
            set => m_NormalizeDirection = value;
        }
        ...
        public override void UpdateRotation()
        {
            ...
            var targetNormal = m_Stopping
                ? m_StopGravityDirection.sqrMagnitude > 0
                    ? -m_StopGravityDirection
            ...
            if (!m_Stopping)
            {
                ...
                if (m_DepthOffset != 0)
                {
                    ...
                    frontHit = Physics.Raycast(m_Transform.TransformPoint(0, m_CharacterLocomotion.Radius,
                            m_DepthOffset * Mathf.Sign(m_CharacterLocomotion.InputVector.y)), -m_CharacterLocomotion.Up,
                        out raycastHit, m_Distance + m_CharacterLocomotion.Radius,
                    ...
                    if (Physics.Raycast(
                            m_Transform.TransformPoint(0, m_CharacterLocomotion.Radius,
                                m_DepthOffset * -Mathf.Sign(m_CharacterLocomotion.InputVector.y)),
                            -m_CharacterLocomotion.Up, out raycastHit, m_Distance + m_CharacterLocomotion.Radius,
                            m_CharacterLayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore))
                    {
                        if (frontHit)
                        {
                            if (m_NormalizeDirection)
                            {
                                ...
                                targetNormal = Vector3
                                    .Cross(direction, Vector3.Cross(m_CharacterLocomotion.Up, direction)).normalized;
                            }
                            else
                            {
                ...
                else
                {
                    var hitCount = m_CharacterLocomotion.Cast(-m_CharacterLocomotion.Up * m_Distance,
                        m_CharacterLocomotion.PlatformMovement +
                        m_CharacterLocomotion.Up * m_CharacterLocomotion.ColliderSpacing, ref m_CombinedRaycastHits,
                        ref m_ColliderIndexMap);
                    ...
                    for (var i = 0; i < hitCount; ++i)
                    {
                        ...
                        var activeCollider = m_CharacterLocomotion.ColliderCount > 1
                            ? m_CharacterLocomotion.Colliders[m_ColliderIndexMap[closestRaycastHit]]
                            : m_CharacterLocomotion.Colliders[0];
            ...
            if (updateNormalRotation) Rotate(targetNormal);
        }
