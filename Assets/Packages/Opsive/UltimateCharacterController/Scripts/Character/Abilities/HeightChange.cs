...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    {
        [Tooltip("Can the SpeedChange ability run while the HeightChange ability is active?")] [SerializeField]
        protected bool m_AllowSpeedChange;
        ...
        [SerializeField]
        protected float m_CapsuleColliderHeightAdjustment = -0.4f;

        private Vector3[] m_CapsuleColliderPosition;
        private Quaternion[] m_CapsuleColliderRotation;

        [Tooltip("Is the ability a concurrent ability?")] [SerializeField]
        protected bool m_ConcurrentAbility = true;

        [Tooltip("Specifies the value to set the Height Animator parameter value to.")] [SerializeField]
        protected int m_Height = 1;

        private readonly Collider[] m_OverlapColliders = new Collider[1];

        private Vector3[] m_StartColliderCenter;

        public bool ConcurrentAbility
        {
            get => m_ConcurrentAbility;
            set => m_ConcurrentAbility = value;
        }

        public float CapsuleColliderHeightAdjustment
        {
            get => m_CapsuleColliderHeightAdjustment;
            set => m_CapsuleColliderHeightAdjustment = value;
        }

        public bool AllowSpeedChange
        {
            get => m_AllowSpeedChange;
            set => m_AllowSpeedChange = value;
        }

        public override bool IsConcurrent => m_ConcurrentAbility;
        ...
        public override void Awake()
        {
            ...
            for (var i = 0; i < m_CharacterLocomotion.Colliders.Length; ++i)
            ...
            {
                Array.Resize(ref m_CapsuleColliderHeight, capsuleColliderCount);
                Array.Resize(ref m_CapsuleColliderPosition, capsuleColliderCount);
                Array.Resize(ref m_CapsuleColliderRotation, capsuleColliderCount);
            }
        }
        ...
        protected override void AbilityStarted()
        {
            ...
            {
                Array.Resize(ref m_CapsuleColliderHeight, m_CharacterLocomotion.ColliderCount);
                Array.Resize(ref m_CapsuleColliderPosition, m_CharacterLocomotion.ColliderCount);
                Array.Resize(ref m_CapsuleColliderRotation, m_CharacterLocomotion.ColliderCount);
                Array.Resize(ref m_StartColliderCenter, m_CharacterLocomotion.ColliderCount);
            }
            ...
            for (var i = 0; i < m_CharacterLocomotion.ColliderCount; ++i)
                if (m_CharacterLocomotion.Colliders[i] is CapsuleCollider)
                {
                    ...
                    m_CapsuleColliderPosition[capsuleColliderCount] =
                        m_Transform.InverseTransformPoint(capsuleCollider.transform.position);
                    m_CapsuleColliderRotation[capsuleColliderCount] =
                        MathUtility.InverseTransformQuaternion(m_Transform.rotation,
            ...
            if (m_Height != -1) SetHeightParameter(m_Height);
        ...
        public override bool CanStopAbility()
        {
            if (!m_CharacterLocomotion.UsingHorizontalCollisionDetection ||
                !m_CharacterLocomotion.DetectVerticalCollisions) return true;
            ...
            for (var i = 0; i < m_CharacterLocomotion.ColliderCount; ++i)
                ...
                if (m_CharacterLocomotion.Colliders[i] is CapsuleCollider)
                {
                    ...
                    MathUtility.CapsuleColliderEndCaps(
                        m_CapsuleColliderHeight[capsuleColliderCount] *
                        MathUtility.CapsuleColliderHeightMultiplier(capsuleCollider),
                        (capsuleCollider.radius - m_CharacterLocomotion.ColliderSpacing) * radiusMultiplier,
                        Vector3.Scale(m_StartColliderCenter[i], capsuleCollider.transform.lossyScale),
                        MathUtility.CapsuleColliderDirection(capsuleCollider),
                        m_Transform.TransformPoint(m_CapsuleColliderPosition[capsuleColliderCount]),
                        MathUtility.TransformQuaternion(m_Transform.rotation,
                            m_CapsuleColliderRotation[capsuleColliderCount]), out startEndCap, out endEndCap);
                    // If there is overlap then the ability can't stop.
                    if (Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap,
                            (capsuleCollider.radius - m_CharacterLocomotion.ColliderSpacing) * radiusMultiplier,
                ...
                else
                {
                    ...
                    // If there is overlap then the ability can't stop.
                    if (Physics.OverlapSphereNonAlloc(sphereCollider.transform.TransformPoint(sphereCollider.center),
                            (sphereCollider.radius - m_CharacterLocomotion.ColliderSpacing) *
        ...
        protected void AbilityStopped(bool force, bool resetCapsuleColliderHeight)
        {
            ...
            if (m_Height != -1) SetHeightParameter(0);
            if (resetCapsuleColliderHeight)
                EventHandler.ExecuteEvent(m_GameObject, "OnHeightChangeAdjustHeight",
                    -m_CapsuleColliderHeightAdjustment);
        }
    }
}