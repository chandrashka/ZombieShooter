    ...
    public class AlignToGravityZone : AlignToGravity
    {
        ...
        public void RegisterGravityZone(GravityZone gravityZone)
        {
            if (m_GravityZones == null)
                ...
                Array.Resize(ref m_GravityZones, m_GravityZoneCount + 1);
        ...
        {
            for (var i = 0; i < m_GravityZoneCount; ++i)
            {
                if (m_GravityZones[i] != gravityZone) continue;

                // Shift all of the array elements down one.
                for (var j = i; j < m_GravityZoneCount - 1; ++j) m_GravityZones[j] = m_GravityZones[j + 1];
                m_GravityZoneCount--;
                m_GravityZones[m_GravityZoneCount] = null;
                break;
            }

            if (m_GravityZoneCount == 0) StopAbility();
        }
            ...
            if (!m_Stopping)
            {
                ...
                for (var i = 0; i < m_GravityZoneCount; ++i)
                    ...
                    if (m_CharacterLocomotion.Grounded)
                    {
                        ...
                        if (normal.sqrMagnitude > targetNormal.sqrMagnitude) targetNormal = normal;
                    }
                    else
                    {
                ...
                if (targetNormal.sqrMagnitude == 0) return;
        ...
        public override void WillTryStopAbility()
        {
            ...
            if (m_GravityZoneCount > 0) return;
        ...
        {
            if (!base.CanForceStopAbility()) return false;
            return m_GravityZoneCount == 0;
        }
        ...
        protected override void AbilityStopped(bool force)
        {
            ...
            if (!m_CharacterLocomotion.IsAbilityTypeActive<AlignToGround>()) ResetAlignToGravity();
        }
    }
}