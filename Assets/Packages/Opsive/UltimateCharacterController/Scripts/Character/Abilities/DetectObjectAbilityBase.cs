...
{
    /// <summary>
    ///     An abstract class for any ability that needs another object to start (such as picking an object up, vaulting,
    ///     climbing, interacting, etc).
    /// </summary>
    public abstract class DetectObjectAbilityBase : Ability
    {
        ...
        public enum ObjectDetectionMode
        {
            ...
            Charactercast =
                2, // Use the character colliders to do a cast in order to detect if the character is near an object.
        ...
        [HideInInspector]
        [SerializeField]
        protected float m_AngleThreshold = 360;

        [Tooltip(
            "The distance of the cast. Used if the Object Detection Mode uses anything other then a trigger detection mode.")]
        [HideInInspector]
        [SerializeField]
        protected float m_CastDistance = 1;
        ...
        [HideInInspector]
        [SerializeField]
        protected int m_CastFrameInterval = 0;

        [Tooltip("The offset to applied to the cast.")] [HideInInspector] [SerializeField]
        protected Vector3 m_CastOffset = new(0, 1, 0);
        ...
        [Tooltip("The LayerMask of the object or trigger that should be detected.")] [HideInInspector] [SerializeField]
        ...
        [HideInInspector]
        [SerializeField]
        protected int m_MaxTriggerObjectCount = 1;

        [Tooltip("Should the character move with the detected object?")] [HideInInspector] [SerializeField]
        protected bool m_MoveWithObject;

        [Tooltip("Mask which specifies how the ability should detect other objects.")]
        [HideInInspector]
        [SerializeField]
        protected ObjectDetectionMode m_ObjectDetection = ObjectDetectionMode.Charactercast;
        ...
        [Tooltip("The radius of the spherecast.")] [HideInInspector] [SerializeField]
        protected float m_SpherecastRadius = 0.5f;

        [Tooltip("Specifies if the cast should interact with triggers.")] [HideInInspector] [SerializeField]
        ...
        [HideInInspector]
        [SerializeField]
        protected bool m_UseLookDirection = true;

        [Tooltip(
            "Should the detection method use the look source position? If false the character position will be used.")]
        [HideInInspector]
        [SerializeField]
        protected bool m_UseLookPosition;

        public ObjectDetectionMode ObjectDetection
        {
            get => m_ObjectDetection;
            set
            {
        ...
        public float DetectAngleThreshold
        {
        ...
        public bool UseLookPosition
        {
            get => m_UseLookPosition;
            set => m_UseLookPosition = value;
        }

        public bool UseLookDirection
        {
            get => m_UseLookDirection;
            set => m_UseLookDirection = value;
        }
        ...
        public GameObject DetectedObject
        {
            ...
            protected set
            {
                if (m_DetectedObject == value) return;
        ...
        public override void Awake()
        {
            ...
            if (m_LookSource != null) m_LookSourceTransform = m_LookSource.GameObject.transform;
        ...
        public override bool CanStartAbility()
        {
            ...
            if (m_ObjectDetection == 0) return false;
            ...
            if ((m_ObjectDetection & ObjectDetectionMode.Trigger) != 0 && m_DetectedTriggerObjectsCount > 0)
                for (var i = 0; i < m_DetectedTriggerObjectsCount; ++i)
                    if (ValidateObject(m_DetectedTriggerObjects[i], null))
                    {
                        DetectedObject = m_DetectedTriggerObjects[i];
                        return true;
                    }
                    else if (m_DetectedTriggerObjects[i] == null || !m_DetectedTriggerObjects[i].activeInHierarchy)
                    {
                        // The OnTriggerExit callback doesn't occur when the object is deactivated. 
                        if (TriggerExit(
                                m_DetectedTriggerObjects[i]))
            ...
            if (m_ObjectDetection == ObjectDetectionMode.Trigger ||
                ((m_UseLookPosition || m_UseLookDirection) && m_LookSource == null)) return false;

            // Don't perform the cast if the number of casts are being culled.
            if (m_LastCastFrame + m_CastFrameInterval > Time.frameCount) return m_DetectedObject != null;
            m_LastCastFrame = Time.frameCount;

            // The ability may have its own custom cast that should be performed.
            if (m_ObjectDetection == ObjectDetectionMode.Customcast) return true;
            ...
            if ((m_ObjectDetection & ObjectDetectionMode.Charactercast) != 0)
                if (m_CharacterLocomotion.SingleCast(castDirection * m_CastDistance,
                        castTransform.TransformDirection(m_CastOffset), m_DetectLayers, ref m_RaycastResult))
                {
            ...
            if ((m_ObjectDetection & ObjectDetectionMode.Raycast) != 0)
                if (Physics.Raycast(castTransform.TransformPoint(m_CastOffset), castDirection, out m_RaycastResult,
            ...
            if ((m_ObjectDetection & ObjectDetectionMode.Spherecast) != 0)
                if (Physics.SphereCast(
                        castTransform.TransformPoint(m_CastOffset) - castTransform.forward * m_SpherecastRadius,
                        m_SpherecastRadius, castDirection, out m_RaycastResult,
                        m_CastDistance, m_DetectLayers, m_TriggerInteraction))
                {
                    var hitObject = m_RaycastResult.collider.gameObject;
                    if (ValidateObject(hitObject, m_RaycastResult))
        ...
        public override void OnTriggerEnter(Collider other)
        {
            ...
            if ((m_ObjectDetection & ObjectDetectionMode.Trigger) == 0) return;

            // The object has to use the correct mask.
            if (!MathUtility.InLayerMask(other.gameObject.layer, m_DetectLayers)) return;
            ...
            if (ValidateObject(other.gameObject, null))
            {
                ...
                {
                    Debug.LogError(
                        $"Error: The maximum number of trigger objects should be increased on the {GetType().Name} ability.");
                    return;
                }
        ...
        protected virtual bool TriggerExit(GameObject other)
        {
            for (var i = 0; i < m_DetectedTriggerObjectsCount; ++i)
                if (other == m_DetectedTriggerObjects[i] || m_DetectedTriggerObjects[i] == null)
                {
                    ...
                    for (var j = i; j < m_DetectedTriggerObjectsCount - 1; ++j)
        ...
        protected virtual bool ValidateObject(GameObject obj, RaycastHit? raycastHit)
        {
            ...
            if (m_ObjectID != -1)
            {
                var objectIdentifiers = obj.GetCachedParentComponents<ObjectIdentifier>();
                if (objectIdentifiers == null) return false;
                var hasID = false;
            ...
            if (raycastHit.HasValue)
            {
                var castDirection = m_UseLookDirection ? m_LookSource.LookDirection(true) : m_Transform.forward;
                float angle;
                var objectFaces = obj.GetCachedParentComponent<ObjectForwardFaces>();
                if (objectFaces != null)
                {
                    ...
                    angle = Quaternion.Angle(Quaternion.LookRotation(castDirection, m_CharacterLocomotion.Up),
                        Quaternion.LookRotation(-obj.transform.forward, m_CharacterLocomotion.Up));
                    angle = Mathf.Abs(
                ...
                else
                {
                    ...
                    angle = Quaternion.Angle(Quaternion.LookRotation(castDirection, m_CharacterLocomotion.Up),
                        Quaternion.LookRotation(-raycastHit.Value.normal, m_CharacterLocomotion.Up));
                }

                if (angle <= m_AngleThreshold) return true;
                return false;
            }
        ...
        protected void AbilityStopped(bool force, bool triggerExit)
        {
            ...
            if (m_MoveWithObject && m_DetectedObject != null &&
                m_CharacterLocomotion.Platform == m_DetectedObject.transform) m_CharacterLocomotion.SetPlatform(null);
        ...
        private void OnImmediateTransformChange(bool snapAnimator)
        {
            ...
            for (var i = 0; i < m_DetectedTriggerObjectsCount - 1; ++i) m_DetectedTriggerObjects[i] = null;
            m_DetectedTriggerObjectsCount = 0;
            if (m_ObjectDetection == ObjectDetectionMode.Trigger || m_ObjectDetection == 0) DetectedObject = null;
            m_LastCastFrame = 0;
        }
