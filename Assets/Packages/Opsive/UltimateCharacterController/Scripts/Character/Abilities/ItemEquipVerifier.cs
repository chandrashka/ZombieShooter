...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    {
        private readonly HashSet<ItemAbility> m_ActiveEquipUnequipAbilities = new();
        ...
        /// <summary>
        ///     Tries to toggle an equip or unequip based on the AllowedEquippedSlotsMask. True will be returned if the ability
        ///     starts.
        /// </summary>
        ...
        {
            if (!Enabled || !m_CanToggleItem || ability is Pickup || ability == null) return false;
            ...
            if (m_StartingAbility != null && ((!activate && m_StartingAbility != ability) ||
                                              (activate && !m_Equip && m_StartingAbility == ability)))
            {
            ...
            if (activate && (ability.AllowEquippedSlotsMask != -1 ||
                             (ability.AllowItemDefinitions != null && ability.AllowItemDefinitions.Length > 0)))
            {
                startPossible = true;
                ...
                if (ability.AllowEquippedSlotsMask != -1)
                {
                    ...
                    for (var i = 0; i < m_Inventory.SlotCount; ++i)
                        if ((item = m_Inventory.GetActiveItem(i)) != null)
                        {
                            ...
                            if (IsDefaultItemIdentifier(item.ItemIdentifier)) continue;
                            currentEquippedSlots |= 1 << i;
                        }

                    if (currentEquippedSlots != 0 &&
                        !MathUtility.InLayerMask(currentEquippedSlots, 1 << ability.AllowEquippedSlotsMask))
                    {
                        start = true;
                ...
                if (!unequippedItems && ability.AllowItemDefinitions != null && ability.AllowItemDefinitions.Length > 0)
                {
                    ...
                    for (var i = 0; i < m_Inventory.SlotCount; ++i)
                        if ((item = m_Inventory.GetActiveItem(i)) != null)
                        {
                            ...
                            {
                                if (ability.AllowItemDefinitions[j] == null) continue;
        ...
        public override bool ShouldBlockAbilityStart(Ability startingAbility)
        {
            ...
            if (m_StartingAbility == null || startingAbility is EquipUnequip) return false;
            return m_StartingAbility.ShouldBlockAbilityStart(startingAbility) ||
                   m_StartingAbility.Index < startingAbility.Index;
        }
        ...
        {
            // StartingAbility will be null on remote clients.
            if (m_StartingAbility == null) return false;
            return m_StartingAbility.ShouldStopActiveAbility(activeAbility) ||
                   m_StartingAbility.Index < activeAbility.Index;
        }
        ...
        protected override void AbilityStarted()
        {
            ...
            if (m_ActiveEquipUnequipAbilities.Count == 0) ItemToggled();
        }
        ...
        private void StartEquipUnequip(bool immediateEquipUnequip)
        {
            for (var i = 0; i < m_EquipUnequipAbilities.Length; ++i)
                if (m_Equip)
                {
                    ...
                    if (m_ItemSetManager.ActiveItemSetIndex[m_EquipUnequipAbilities[i].ItemSetCategoryIndex] !=
                        m_TargetItemSetIndex[i]) continue;
                    m_EquipUnequipAbilities[i]
                        .StartEquipUnequip(m_StartingItemSetIndex[i], true, immediateEquipUnequip);
                }
                else
                {
                    var targetItemSetIndex =
                        m_ItemSetManager.GetTargetItemSetIndex(m_EquipUnequipAbilities[i].ItemSetCategoryIndex,
                            m_AllowEquippedSlotsMask);
                    var activeItemSetIndex =
                        m_ItemSetManager.ActiveItemSetIndex[m_EquipUnequipAbilities[i].ItemSetCategoryIndex];
                    ...
                    if (targetItemSetIndex != activeItemSetIndex)
                    {
                        ...
                        m_EquipUnequipAbilities[i].StartEquipUnequip(targetItemSetIndex, true,
                            m_StartingAbility.ImmediateUnequip || immediateEquipUnequip);
                    }
                    else
                    {
        ...
        private void OnItemAbilityActive(ItemAbility itemAbility, bool active)
        {
            if (itemAbility is EquipUnequip)
            {
                ...
                if (m_ActiveEquipUnequipAbilities.Count == 0 && m_CanStopAbility) ItemToggled();
            }
        }
        ...
        {
            if (m_StartingAbility == null) return;
            ...
            if (!m_Equip && (m_CharacterLocomotion.MoveTowardsAbility == null ||
                             m_CharacterLocomotion.MoveTowardsAbility.OnArriveAbility == null))
            {
