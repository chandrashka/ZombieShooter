...
{
    /// <summary>
    ///     Plays a counter attack in response to an opponent's attack. In order for the counter attack ability to start the
    ///     character
    ...
    {
        [Tooltip("The maximum distance away from the opponent that the counter attack can start.")] [SerializeField]
        protected float m_AttackDistance = 0.6f;

        [Tooltip(
            "The counter attack can start if the character blocked an attack within the specified amount of time.")]
        [SerializeField]
        protected float m_CounterAttackTimeFrame = 1f;
        ...
        public override bool CanStartAbility()
        {
            ...
            if (m_ImpactTime == -1 || m_ImpactTime + m_CounterAttackTimeFrame < Time.time) return false;
            ...
            if (!usableMeleeWeapon) return false;

            // The opponent must be in front of the character.
            if (!m_CharacterLocomotion.SingleCast(m_Transform.forward * m_AttackDistance, Vector3.zero,
                    m_CharacterLayerManager.EnemyLayers, ref m_RaycastHit) ||
                m_RaycastHit.collider.gameObject.GetCachedParentComponent<UltimateCharacterLocomotion>() !=
                m_OpponentLocomotion)
                return false;
        ...
        protected override void AbilityStarted()
        {
            ...
            if (opponentResponseAbility == null) return;

            var substateIndex = -1;
            for (var i = 0; i < m_Inventory.SlotCount; ++i)
            {
                substateIndex = GetItemSubstateIndex(i);
                if (substateIndex != -1) break;
            }

            if (substateIndex != -1) opponentResponseAbility.StartResponse(substateIndex);
        }
        ...
        public override int GetItemSubstateIndex(int slotID)
        {
            ...
            return MathUtility.Concatenate(m_OpponentMeleeWeapon.Item.AnimatorItemID,
                m_OpponentMeleeWeapon.UsedSubstateIndex, substateIndex);
        }
        ...
        private void OnItemAbilityActive(ItemAbility itemAbility, bool active)
        {
            ...
            m_OpponentMeleeWeapon = null;
            for (var i = 0; i < blockAbility.ImpactSources.Length; ++i)
            {
                if (blockAbility.ImpactSources[i] == null || !(blockAbility.ImpactSources[i] is MeleeWeapon)) continue;

                m_OpponentMeleeWeapon = blockAbility.ImpactSources[i] as MeleeWeapon;
                break;
            }

            if (m_OpponentMeleeWeapon == null) return;
            ...
            for (var i = 0; i < useAbilities.Length; ++i)
            {
                if (!useAbilities[i].IsActive) continue;

                // The ability is active. Ensure it is using a melee weapon.
                for (var j = 0; j < useAbilities[i].UsableItems.Length; ++j)
                {
                    var meleeWeapon = useAbilities[i].UsableItems[j] as MeleeWeapon;
                    if (meleeWeapon != m_OpponentMeleeWeapon) continue;

                    m_OpponentUseAbility = useAbilities[i];
                    break;
                }

                if (m_OpponentUseAbility != null) break;
            }
