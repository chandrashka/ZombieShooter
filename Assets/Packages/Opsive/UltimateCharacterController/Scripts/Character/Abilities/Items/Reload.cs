...
namespace Opsive.UltimateCharacterController.Character.Abilities.Items
{
    ...
    ///     - The second part can wait for a small amount of time after the first part to ensure the reload animation is
    ///     complete before ending the ability.
    /// </summary>
    ...
    public class Reload : ItemAbility
    {
        ...
        [Tooltip("The ID of the ItemAction component that can be reloaded.")] [SerializeField]
        protected int m_ActionID;

        private IReloadableItem[] m_CanReloadItems;
        private readonly HashSet<Item> m_EquippedItems = new();

        private bool[] m_Reloaded;
        private ScheduledEventBase[] m_ReloadEvents;

        [Tooltip("The slot that should be reloaded. -1 will use all of the slots.")] [SerializeField]
        ...
        public IReloadableItem[] ReloadableItems { get; private set; }
        ...
        public override void Awake()
        {
            ...
            ReloadableItems = new IReloadableItem[m_SlotID == -1 ? m_Inventory.SlotCount : 1];
            m_ReloadEvents = new ScheduledEventBase[ReloadableItems.Length];
            m_Reloaded = new bool[ReloadableItems.Length];
        ...
        private void RegisterSlotEvents(int slotID)
        {
            if (slotID == 0)
            {
                ...
                EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemReloadCompleteFirstSlot",
                    OnItemReloadCompleteFirstSlot);
            }
            else if (slotID == 1)
            {
                EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemReloadSecondSlot", OnItemReloadSecondSlot);
                EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemReloadCompleteSecondSlot",
                    OnItemReloadCompleteSecondSlot);
            }
            else if (slotID == 2)
            {
                EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemReloadThirdSlot", OnItemReloadThirdSlot);
                EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemReloadCompleteThirdSlot",
                    OnItemReloadCompleteThirdSlot);
            }
            else if (slotID != -1)
            {
        ...
        private void UnregisterSlotEvents(int slotID)
        {
            if (slotID == 0)
            {
                ...
                EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemReloadCompleteFirstSlot",
                    OnItemReloadCompleteFirstSlot);
            }
            else if (slotID == 1)
            {
                EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemReloadSecondSlot", OnItemReloadSecondSlot);
                EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemReloadCompleteSecondSlot",
                    OnItemReloadCompleteSecondSlot);
            }
            else if (slotID == 2)
            {
                EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemReloadThirdSlot", OnItemReloadThirdSlot);
                EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemReloadCompleteThirdSlot",
                    OnItemReloadCompleteThirdSlot);
            }
        }
        ...
        public override bool CanStartAbility()
        {
            ...
            if (m_SlotID == -1)
            {
                for (var i = 0; i < ReloadableItems.Length; ++i)
                {
                    ...
                    if (itemAction == null)
                    {
                        Debug.LogWarning("Warning: The item " + item.name +
                                         " must have an ItemAction component attached to it in order to be reloaded.");
                        continue;
                    }

                    ReloadableItems[i] = itemAction as IReloadableItem;
                    // The item can't be reloaded if it isn't a reloadable item.
                    if (ReloadableItems[i] != null && ReloadableItems[i].CanReloadItem(true))
                        canReload = true;
                    else
                        // The ability should not attempt to reload the item if IReloadableItem says that it cannot reload.
                        ReloadableItems[i] = null;
                }
            }
            else
            {
                ...
                if (item != null)
                {
                    ...
                    {
                        Debug.LogWarning("Warning: The item " + item.name +
                                         " must have an ItemAction component attached to it in order to be used.");
                    }
                    else
                    {
                        ReloadableItems[0] = itemAction as IReloadableItem;
                        canReload = ReloadableItems[0] != null && ReloadableItems[0].CanReloadItem(true);
                    }
                }
            }
        ...
        protected override void AbilityStarted()
        {
            ...
            for (var i = 0; i < ReloadableItems.Length; ++i)
                if (ReloadableItems[i] != null)
                {
                    m_Reloaded[i] = false;
                    ReloadableItems[i].StartItemReload();
                    if (!ReloadableItems[i].ReloadEvent.WaitForAnimationEvent)
                        m_ReloadEvents[i] =
                            SchedulerBase.ScheduleFixed(ReloadableItems[i].ReloadEvent.Duration, ReloadItem, i);
                }
        }
        ...
        {
            if (ReloadableItems[slotID] == null) return;

            ReloadableItems[slotID].ItemReloadComplete(false, false);
            ReloadableItems[slotID] = null;
        ...
        public override bool ShouldBlockAbilityStart(Ability startingAbility)
        {
            ...
            if (startingAbility is Use)
            {
                ...
                for (var i = 0; i < ReloadableItems.Length; ++i)
                {
                    if (ReloadableItems[i] == null) continue;

                    if (!ReloadableItems[i].Item.DominantItem)
                    {
                        dominantItem = false;
                        break;
                    }
                ...
                for (var i = 0; i < useAbility.UsableItems.Length; ++i)
                {
                    if (useAbility.UsableItems[i] == null) continue;

                    if (dominantItem != useAbility.UsableItems[i].Item.DominantItem) return true;
                }
            }
        ...
        public override int GetItemStateIndex(int slotID)
        {
            ...
            if (m_SlotID == -1)
            {
                if (ReloadableItems[slotID] != null) return m_ItemStateIndex;
            }
            else if (m_SlotID == slotID && ReloadableItems[0] != null)
            {
        ...
        public override int GetItemSubstateIndex(int slotID)
        {
            if (m_SlotID == -1)
            {
                if (ReloadableItems[slotID] != null)
                {
                    if (m_Reloaded[slotID]) return 0;
                    return ReloadableItems[slotID].ReloadAnimatorAudioStateSet.GetItemSubstateIndex();
                }
            }
            else if (m_SlotID == slotID && ReloadableItems[0] != null)
            {
                if (m_Reloaded[0]) return 0;
                return ReloadableItems[0].ReloadAnimatorAudioStateSet.GetItemSubstateIndex();
            }
        ...
        {
            for (var i = 0; i < ReloadableItems.Length; ++i)
                if (ReloadableItems[i] != null && !m_Reloaded[i] && !ReloadableItems[i].ReloadEvent.WaitForSlotEvent)
                    ReloadItem(i);
        }
        ...
        {
            if (ReloadableItems[0] == null || !ReloadableItems[0].ReloadEvent.WaitForSlotEvent ||
                ReloadableItems[0].Item.SlotID != 0) return;
            ReloadItem(0);
        }
        ...
        private void OnItemReloadSecondSlot()
        {
            ...
            if (ReloadableItems[index] == null || !ReloadableItems[index].ReloadEvent.WaitForSlotEvent ||
                ReloadableItems[index].Item.SlotID != 1) return;
            ReloadItem(index);
        }
        ...
        private void OnItemReloadThirdSlot()
        {
            ...
            if (ReloadableItems[index] == null || !ReloadableItems[index].ReloadEvent.WaitForSlotEvent ||
                ReloadableItems[index].Item.SlotID != 2) return;
            ReloadItem(index);
        }
        ...
        {
            var reloadableItem = ReloadableItems[slotID];
            if (reloadableItem == null) return;

            reloadableItem.ReloadItem(false);
            m_ReloadEvents[slotID] = null;

            // Each reload should update the attribute.
            if (m_AttributeModifier != null) m_AttributeModifier.EnableModifier(true);

            var canReload = true;
            // An attribute may prevent the reload from being able to continue.
            if (m_AttributeModifier != null && !m_AttributeModifier.IsValid()) canReload = false;
            ...
            if (canReload && reloadableItem.CanReloadItem(true))
            {
                ...
                if (!reloadableItem.ReloadEvent.WaitForAnimationEvent)
                    m_ReloadEvents[slotID] =
                        SchedulerBase.ScheduleFixed(reloadableItem.ReloadEvent.Duration, ReloadItem, slotID);
            }
            else
            {
                ...
                if (!reloadableItem.ReloadCompleteEvent.WaitForAnimationEvent)
                    m_ReloadEvents[slotID] = SchedulerBase.ScheduleFixed(reloadableItem.ReloadCompleteEvent.Duration,
                        ReloadItemComplete, slotID);
            }
        }
        ...
        {
            for (var i = 0; i < ReloadableItems.Length; ++i)
                if (ReloadableItems[i] != null && !ReloadableItems[i].ReloadCompleteEvent.WaitForSlotEvent)
        ...
        {
            if (ReloadableItems[0] == null || !ReloadableItems[0].ReloadCompleteEvent.WaitForSlotEvent ||
                ReloadableItems[0].Item.SlotID != 0) return;
        ...
        {
            var index = m_SlotID == -1 ? 1 : 0;
            if (ReloadableItems[index] == null || !ReloadableItems[index].ReloadCompleteEvent.WaitForSlotEvent ||
                ReloadableItems[index].Item.SlotID != 1) return;
        ...
        private void ReloadItemComplete(int slotID)
        {
            ...
            ReloadableItems[slotID].ItemReloadComplete(true, false);
            ReloadableItems[slotID] = null;
        ...
        {
            if (m_SlotID != -1 && slotID != -1 && m_SlotID != slotID) return;
            ...
            {
                var item = allItems[i];
                if (slotID != -1 && item.SlotID != slotID) continue;

                IReloadableItem reloadableItem;
                if ((reloadableItem = ShouldReload(item, itemIdentifier, slotID == -1)) != null)
                {
                    // -1 indicates that the item is being picked up.
                    if (m_CanReloadItems == null || m_CanReloadItems.Length == canReloadCount)
                        Array.Resize(ref m_CanReloadItems, canReloadCount + 1);
            ...
            if (canReloadCount > 0)
            {
                ...
                for (var i = 0; i < canReloadCount; ++i)
                {
                    ...
                    else
                    {
                        startAbility = true;
                        if (m_SlotID == -1)
                            ReloadableItems[reloadableItem.Item.SlotID] = reloadableItem;
                        else
                            ReloadableItems[0] = reloadableItem;
                    }
                }
        ...
        private IReloadableItem ShouldReload(Item item, IItemIdentifier itemIdentifier, bool fromPickup)
        {
            ...
            if (reloadableItem == null) return null;

            // Don't reload if the ItemIdentifier doesn't match.
            if (reloadableItem.GetReloadableItemIdentifier() != itemIdentifier) return null;

            var autoReload = false;
            if ((reloadableItem.AutoReload & AutoReloadType.Empty) != 0 && reloadableItem is IUsableItem &&
                (reloadableItem as IUsableItem).GetConsumableItemIdentifierAmount() == 0)
                // The item is empty.
                autoReload = true;
            ...
            if (!reloadableItem.CanReloadItem(true)) return null;
        ...
        {
            for (var i = 0; i < ReloadableItems.Length; ++i)
                if (ReloadableItems[i] != null && !ReloadableItems[i].CanCameraZoom)
                    return false;
            return true;
        }
        ...
        protected override void AbilityStopped(bool force)
        {
            ...
            for (var i = 0; i < ReloadableItems.Length; ++i)
                if (ReloadableItems[i] != null)
                {
                    ReloadableItems[i].ItemReloadComplete(!force, force);
                    ReloadableItems[i] = null;
        ...
        public override void OnDestroy()
        {
            ...
            EventHandler.UnregisterEvent<IItemIdentifier, int, bool, bool>(m_GameObject,
                "OnInventoryPickupItemIdentifier", OnPickupItemIdentifier);
            EventHandler.UnregisterEvent<int, IItemIdentifier, bool, bool>(m_GameObject, "OnItemTryReload",
                OnTryReload);
