...
namespace Opsive.UltimateCharacterController.Character.Abilities.Items
{
    ...
    public class EquipUnequip : ItemSetAbilityBase
    {
        ...
        [Tooltip("Mask which specifies when to auto equip a new item.")] [HideInInspector] [SerializeField]
        protected AutoEquipType m_AutoEquip = AutoEquipType.Unequipped | AutoEquipType.OutOfUsableItem |
                                              AutoEquipType.NotPreset | AutoEquipType.FirstTime;

        private bool m_CanEquip;
        private Item[] m_EquipItems;

        [Tooltip("The Item State Index while equipping.")] [SerializeField]
        protected int m_EquipItemStateIndex = 4;
        ...
        private readonly Dictionary<IItemIdentifier, int> m_InventoryAmount = new();
        ...
        [Tooltip("The Item State Index while unequipping.")] [SerializeField]
        protected int m_UnequipItemStateIndex = 5;

        private bool[] m_UnequippingItems;

        public AutoEquipType AutoEquip
        {
            get => m_AutoEquip;
            set => m_AutoEquip = value;
        }

        public override bool CanReceiveMultipleStarts => true;
        public int ActiveItemSetIndex { get; private set; } = -1;
        ...
        public override void Awake()
        {
            ...
            EventHandler.RegisterEvent<IItemIdentifier, int, bool, bool>(m_GameObject,
                "OnInventoryPickupItemIdentifier", OnPickupItemIdentifier);
            ...
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemUnequipCompleteFirstSlot",
                OnItemUnequipCompleteFirstSlot);
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemUnequipSecondSlot", OnItemUnequipSecondSlot);
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemUnequipCompleteSecondSlot",
                OnItemUnequipCompleteSecondSlot);
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemUnequipThirdSlot", OnItemUnequipThirdSlot);
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemUnequipCompleteThirdSlot",
                OnItemUnequipCompleteThirdSlot);
            ...
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemEquipCompleteFirstSlot",
                OnItemEquipCompleteFirstSlot);
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemEquipSecondSlot", OnItemEquipSecondSlot);
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemEquipCompleteSecondSlot",
                OnItemEquipCompleteSecondSlot);
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemEquipThirdSlot", OnItemEquipThirdSlot);
            EventHandler.RegisterEvent(m_GameObject, "OnAnimatorItemEquipCompleteThirdSlot",
                OnItemEquipCompleteThirdSlot);
        ...
        public void WillStartPickup()
        {
            ...
            for (var i = 0; i < allItems.Count; ++i)
            {
                ...
                if (m_InventoryAmount.ContainsKey(allItems[i].ItemIdentifier)) continue;

                m_InventoryAmount.Add(allItems[i].ItemIdentifier,
                    m_Inventory.GetItemIdentifierAmount(allItems[i].ItemIdentifier));
            }
        ...
        private void OnPickupItem(Item item, int amount, bool immediatePickup, bool forceEquip)
        {
            ...
            if (!m_ItemSetManager.IsCategoryMember(item.ItemDefinition, m_ItemSetCategoryIndex) || !Enabled) return;

            // If another EquipUnequip ability exists with an exact category match then that ability should be used instead.
            if (m_ItemSetManager.CategoryItemSets[m_ItemSetCategoryIndex].ItemCategory !=
                item.ItemDefinition.GetItemCategory())
                for (var i = 0; i < m_EquipUnequipAbilities.Length; ++i)
                {
                    if (m_EquipUnequipAbilities[i] == this) continue;

                    if (m_ItemSetManager.CategoryItemSets[m_EquipUnequipAbilities[i].ItemSetCategoryIndex]
                            .ItemCategory == item.ItemDefinition.GetItemCategory()) return;
                }
            ...
            if (ShouldEquip(item.ItemIdentifier, item.SlotID, amount))
            {
                ...
                if (itemSetIndex != -1)
                {
                    ...
                    if (immediatePickup &&
                        (forceEquip || ActiveItemSetIndex == -1 || itemSetIndex < ActiveItemSetIndex))
                    {
                        // The ItemSet should be updated immediately.
                        m_StartEquipUnequipIndex = ActiveItemSetIndex = itemSetIndex;
                        EventHandler.ExecuteEvent(this, "OnEquipUnequipItemSetIndexChange", ActiveItemSetIndex);
                        m_ItemSetManager.UpdateNextItemSet(m_ItemSetCategoryIndex, ActiveItemSetIndex);
                        m_ItemSetManager.UpdateActiveItemSet(m_ItemSetCategoryIndex, ActiveItemSetIndex);
                        ...
                        for (var i = 0; i < m_Inventory.SlotCount; ++i)
                        {
                            ...
                            var itemIdentifier =
                                m_ItemSetManager.GetEquipItemIdentifier(m_ItemSetCategoryIndex, itemSetIndex, i);
                            if (itemIdentifier == null)
                            {
                                ...
                                if (unequipItem != null &&
                                    m_ItemSetManager.IsCategoryMember(unequipItem.ItemIdentifier.GetItemDefinition(),
                                        m_ItemSetCategoryIndex))
                                {
                                    m_Inventory.UnequipItem(i);
                                    EventHandler.ExecuteEvent(m_GameObject, "OnAbilityUnequipItemComplete", unequipItem,
                                        i);
                                }
                            ...
                            if (m_ItemSetManager.IsCategoryMember(itemIdentifier.GetItemDefinition(),
                                    m_ItemSetCategoryIndex))
                            {
                                ...
                                if (equippedItem != null)
                                {
                                    ...
                                    if (itemIdentifier == equippedItem.ItemIdentifier) continue;
                                    m_Inventory.UnequipItem(i);
                                    EventHandler.ExecuteEvent(m_GameObject, "OnAbilityUnequipItemComplete",
                                        equippedItem, i);
                                }
        ...
        public bool ShouldEquip(IItemIdentifier itemIdentifier, int slotID, int amount)
        {
            ...
            if (!m_InventoryAmount.TryGetValue(itemIdentifier, out var itemAmount)) itemAmount = -1;
            if ((m_AutoEquip & AutoEquipType.Always) != 0)
                shouldEquip = true;
            else if ((m_AutoEquip & AutoEquipType.Unequipped) != 0 && currentItem == null &&
                ...
                shouldEquip = true;
            else if ((m_AutoEquip & AutoEquipType.OutOfUsableItem) != 0 && currentItem != null &&
                     currentItem.ItemActions != null)
                for (var i = 0; i < currentItem.ItemActions.Length; ++i)
                {
            ...
            if (shouldEquip && !IsEquipAllowed(-1, itemIdentifier.GetItemDefinition(), slotID)) shouldEquip = false;
        ...
        {
            if (itemDefinition == null || (itemSetIndex != -1 && m_ItemSetManager != null &&
                                           m_ItemSetManager.CategoryItemSets[m_ItemSetCategoryIndex]
                                               .DefaultItemSetIndex == itemSetIndex)) return true;
            for (var i = 0; i < m_CharacterLocomotion.ActiveAbilityCount; ++i)
            {
                var ability = m_CharacterLocomotion.ActiveAbilities[i];
                if (!MathUtility.InLayerMask(slotID, ability.AllowEquippedSlotsMask)) return false;
                ...
                {
                    var allowed = false;
                    for (var j = 0; j < ability.AllowItemDefinitions.Length; ++j)
        ...
        /// <param name="immediatePickup">Was the item be picked up immediately?</param>
        /// <param name="forceEquip">Should the item be force equipped?</param>
        private void OnPickupItemIdentifier(IItemIdentifier itemIdentifier, int amount, bool immediatePickup,
            bool forceEquip)
        {
            // The ability doesn't need to respond if the category doesn't match.
            var itemDefinition = itemIdentifier.GetItemDefinition();
            if (!m_ItemSetManager.IsCategoryMember(itemDefinition, m_ItemSetCategoryIndex) || !Enabled) return;
            ...
            for (var i = 0; i < allItems.Count; ++i)
            {
                if (allItems[i].ItemIdentifier == itemIdentifier) continue;
                ...
                for (var j = 0; j < itemActions.Length; ++j)
                {
                    ...
                    if (usableItem != null && usableItem.GetConsumableItemIdentifier() == itemIdentifier)
                        OnPickupItem(allItems[i], m_Inventory.GetItemIdentifierAmount(allItems[i].ItemIdentifier),
                            immediatePickup, forceEquip);
                }
            }
        }
        ...
        /// </summary>
        /// <param name="itemSetIndex">The ItemSet to equip/unequip the items to.</param>
        /// <param name="forceEquipUnequip">
        ///     Should the ability be force started? This will stop all abilities that would prevent
        ///     EquipUnequip from starting.
        /// </param>
        /// <returns>True if the ability was started.</returns>
        ...
        public bool StartEquipUnequip(int itemSetIndex, bool forceEquipUnequip, bool immediateEquipUnequip)
        {
            ...
            if ((!IsActive && itemSetIndex == m_ItemSetManager.ActiveItemSetIndex[m_ItemSetCategoryIndex]) ||
                (IsActive && !immediateEquipUnequip &&
                 itemSetIndex == m_ItemSetManager.NextItemSetIndex[m_ItemSetCategoryIndex]))
                return false;
            ...
            if (forceEquipUnequip)
            {
                ...
                for (var i = m_CharacterLocomotion.ActiveItemAbilityCount - 1; i > -1; --i)
        ...
        public override bool CanStartAbility()
        {
            ...
            var itemSetIndex = ActiveItemSetIndex;
            ...
            if (InputIndex != -1 && (!m_ItemSetManager.IsItemSetValid(m_ItemSetCategoryIndex, itemSetIndex, true) ||
                                     (!IsActive && itemSetIndex ==
                                         m_ItemSetManager.ActiveItemSetIndex[m_ItemSetCategoryIndex]) ||
                                     (IsActive && itemSetIndex ==
                                         m_ItemSetManager.NextItemSetIndex[m_ItemSetCategoryIndex])))
            {
                InputIndex = -1;
                return false;
            }
        ...
        protected override void AbilityStarted()
        {
            ...
            m_StartEquipUnequipIndex = ActiveItemSetIndex;
            if (m_ImmediateEquipUnequip) Update();
        }
        ...
        public override bool ShouldBlockAbilityStart(Ability startingAbility)
        {
            ...
            if (startingAbility is EquipScroll) return true;
            ...
            if (startingAbility is Use
                ...
                for (var i = 0; i < m_UnequipItems.Length; ++i)
        ...
        {
            if (m_UnequipItems[slotID] != null) return m_UnequipItemStateIndex;
            if (m_CanEquip && m_EquipItems[slotID] != null) return m_EquipItemStateIndex;
            return -1;
        }
        ...
        public override int GetItemSubstateIndex(int slotID)
        {
            if (m_UnequipItems[slotID] != null)
                return m_UnequipItems[slotID].UnequipAnimatorAudioStateSet.GetItemSubstateIndex() +
                       (m_Aiming ? m_AimItemSubstateIndexAddition : 0);
            if (m_CanEquip && m_EquipItems[slotID] != null)
                return m_EquipItems[slotID].EquipAnimatorAudioStateSet.GetItemSubstateIndex() +
                       (m_Aiming ? m_AimItemSubstateIndexAddition : 0);
            return -1;
        }
        ...
        public override void Update()
        {
            ...
            if (m_StartEquipUnequip)
            {
                ...
                ActiveItemSetIndex = m_StartEquipUnequipIndex;
                ...
                if (ActiveItemSetIndex != -1 &&
                    !m_ItemSetManager.IsItemSetValid(m_ItemSetCategoryIndex, ActiveItemSetIndex, false))
                {
                    StopAbility();
                    return;
                }
                ...
                for (var i = 0; i < m_Inventory.SlotCount; ++i)
                {
                    ...
                    if (currentItemIdentifier != null) currentItem = m_Inventory.GetItem(currentItemIdentifier, i);
                    var targetItemIdentifier =
                        m_ItemSetManager.GetEquipItemIdentifier(m_ItemSetCategoryIndex, ActiveItemSetIndex, i);
                    var skipEquip = false;
                    ...
                    for (var j = 0; j < m_ItemSetCategoryIndex; ++j)
                    {
                    ...
                    if (currentItem == targetItem &&
                        (targetItem == null || (targetItem != null && targetItem.IsActive()))) continue;
                    ...
                    if (m_UnequipItems[i] != null)
                    {
                        unequip = true;
                             ...
                             m_ItemSetManager.IsCategoryMember(currentItem.ItemIdentifier.GetItemDefinition(),
                                 m_ItemSetCategoryIndex))
                    {
                    ...
                    if (targetItemIdentifier != null &&
                        m_ItemSetManager.IsCategoryMember(targetItemIdentifier.GetItemDefinition(),
                            m_ItemSetCategoryIndex))
                    {
                ...
                if (unequip || equip)
                {
                    ...
                    var
                        canEqup =
                            m_CanEquip; // The can equp status may change if an item is unequipped immediately. Remember the initial value so the item isn't equipped twice.
                    ...
                    for (var i = 0; i < m_Inventory.SlotCount; ++i)
                    {
                        if (m_UnequipItems[i] == null && m_EquipItems[i] == null) continue;
            ...
            for (var i = 0; i < m_EquipUnequipActions.Length; ++i)
            {
                if (m_EquipUnequipActions[i] == EquipUnequipAction.Inactive) continue;
            ...
            if (m_EquipItems[slotID] != null || m_UnequipItems[slotID] != null)
            {
                ...
                if (startAbility)
                {
                    if (m_EquipItems[slotID] != null && m_EquippingItems[slotID])
                    {
                        ...
                        EventHandler.ExecuteEvent(m_GameObject, "OnAbilityUnequipItemComplete", m_EquipItems[slotID],
                            slotID);
                    }
                    else if (m_EquipItems[slotID] != null && m_CanEquip)
                    {
                        // Don't unequip the item currently being equipped if the ItemSet uses it.
                        var equipItemIdentifier =
                            m_ItemSetManager.GetEquipItemIdentifier(m_ItemSetCategoryIndex, ActiveItemSetIndex, slotID);
                ...
                if (!startUnequip && m_UnequipItems[slotID] != null)
                {
                    ...
                    else
                    {
                        ...
                        m_Inventory.UnequipItem(m_UnequipItems[slotID].ItemIdentifier, slotID);
                        EventHandler.ExecuteEvent(m_GameObject, "OnAbilityUnequipItemComplete", m_UnequipItems[slotID],
                            slotID);
                        m_UnequipItems[slotID] = null;
                        m_UnequippingItems[slotID] = false;
                    }
        ...
        {
            if (m_UnequipItems[0] == null || !m_UnequipItems[0].UnequipEvent.WaitForSlotEvent) return;
            ItemUnequip(0, false);
        }
        ...
        {
            if (m_UnequipItems[1] == null || !m_UnequipItems[1].UnequipEvent.WaitForSlotEvent) return;
            ItemUnequip(1, false);
        }
        ...
        {
            if (m_UnequipItems[2] == null || !m_UnequipItems[2].UnequipEvent.WaitForSlotEvent) return;
            ItemUnequip(2, false);
        }
        ...
        ///     Can the item be updated? If false the status enum will be set and the item will be updated
        ///     within the Update loop.
        /// </param>
        private void ItemUnequip(int slotID, bool canUpdate)
        {
            var unequipItem = m_UnequipItems[slotID];
            if (unequipItem == null || !m_UnequippingItems[slotID]) return;
            ...
            if (!unequipItem.UnequipCompleteEvent.WaitForAnimationEvent)
            {
                ...
                m_ItemEvents[slotID] =
                    SchedulerBase.ScheduleFixed(duration, ItemUnequipComplete, slotID, duration == 0);
            }
        }
        ...
        {
            if (m_UnequipItems[0] == null || !m_UnequipItems[0].UnequipCompleteEvent.WaitForSlotEvent) return;
        ...
        {
            if (m_UnequipItems[1] == null || !m_UnequipItems[1].UnequipCompleteEvent.WaitForSlotEvent) return;
        ...
        {
            if (m_UnequipItems[2] == null || !m_UnequipItems[2].UnequipCompleteEvent.WaitForSlotEvent) return;
        ...
        private void ItemUnequipComplete(int slotID, bool canUpdate)
        {
            ...
            for (var i = 0; i < m_EquipItems.Length; ++i)
                if (m_EquipItems[i] != null && !m_EquippingItems[i])
                {
                    ...
                    if (m_EquipItems[i] != null &&
                        (m_ImmediateEquipUnequip || !m_EquipItems[i].EquipEvent.WaitForAnimationEvent))
                    {
                        var duration = m_ImmediateEquipUnequip ? 0 : m_EquipItems[i].EquipEvent.Duration;
                        m_ItemEvents[i] = SchedulerBase.ScheduleFixed(duration, ItemEquip, i, duration == 0);
                    }
            ...
            if (!m_StartEquipUnequip && stopAbility) TryStopEquipUnequipAbility();
        }
        ...
        {
            if (m_UnequipItems[slotID] == null || m_ItemSetCategoryIndex == categoryIndex) return;
        ...
        {
            if (m_EquipItems[0] == null || !m_EquipItems[0].EquipCompleteEvent.WaitForSlotEvent) return;
            ItemEquip(0, false);
        }
        ...
        {
            if (m_EquipItems[1] == null || !m_EquipItems[1].EquipCompleteEvent.WaitForSlotEvent) return;
            ItemEquip(1, false);
        }
        ...
        {
            if (m_EquipItems[2] == null || !m_EquipItems[2].EquipCompleteEvent.WaitForSlotEvent) return;
            ItemEquip(2, false);
        }
        ...
        private void ItemEquip(int slotID, bool canUpdate)
        {
            ...
            if (equipItem == null || !m_EquippingItems[slotID]) return;
            ...
            if (!equip)
            {
                ...
                EventHandler.ExecuteEvent(m_GameObject, "OnEquipUnequipVerifyUnequipItem", m_ItemSetCategoryIndex,
                    slotID);
            }
        ...
        {
            if (m_ItemSetCategoryIndex != categoryIndex) return;
            ActiveItemSetIndex = itemSetIndex;
        }
        ...
        private void OnRemoveItem(Item item, int slotID)
        {
            if (m_ItemSetManager.IsCategoryMember(item.ItemDefinition, m_ItemSetCategoryIndex))
                ...
                if (ActiveItemSetIndex != -1 && ActiveItemSetIndex ==
                    m_ItemSetManager.GetItemSetIndex(item, m_ItemSetCategoryIndex, false))
                {
        ...
        {
            m_PrevActiveItemSetIndex = ActiveItemSetIndex;
            ...
            if (m_Inventory.RemoveAllOnDeath) m_InventoryAmount.Clear();
        }
        ...
        public override void OnDestroy()
        {
            ...
            EventHandler.UnregisterEvent<IItemIdentifier, int, bool, bool>(m_GameObject,
                "OnInventoryPickupItemIdentifier", OnPickupItemIdentifier);
            ...
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemUnequipCompleteFirstSlot",
                OnItemUnequipCompleteFirstSlot);
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemUnequipSecondSlot", OnItemUnequipSecondSlot);
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemUnequipCompleteSecondSlot",
                OnItemUnequipCompleteSecondSlot);
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemUnequipThirdSlot", OnItemUnequipThirdSlot);
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemUnequipCompleteThirdSlot",
                OnItemUnequipCompleteThirdSlot);
            ...
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemEquipCompleteFirstSlot",
                OnItemEquipCompleteFirstSlot);
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemEquipSecondSlot", OnItemEquipSecondSlot);
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemEquipCompleteSecondSlot",
                OnItemEquipCompleteSecondSlot);
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemEquipThirdSlot", OnItemEquipThirdSlot);
            EventHandler.UnregisterEvent(m_GameObject, "OnAnimatorItemEquipCompleteThirdSlot",
                OnItemEquipCompleteThirdSlot);
