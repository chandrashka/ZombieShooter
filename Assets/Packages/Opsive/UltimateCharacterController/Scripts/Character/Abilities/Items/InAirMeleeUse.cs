...
namespace Opsive.UltimateCharacterController.Character.Abilities.Items
{
    ...
    {
        [Tooltip("Can the ability stop while the character is still in the air?")] [SerializeField]
        protected bool m_AllowInAirStop;

        private bool m_AppliedDownwardForce;

        [Tooltip("The amount of force that should be applied when the character starts to fall.")] [SerializeField]
        protected Vector3 m_DownwardForce = new(0, -15, 0);

        [Tooltip("The number of frames that the downward force is applied in.")] [SerializeField]
        protected int m_DownwardForceFrames = 4;

        [Tooltip("The impact effect that should be played when the character is grounded.")] [SerializeField]
        protected SurfaceImpact m_GroundImpact;

        [Tooltip("The amount of positional force to add to the camera when the character is grounded.")]
        [SerializeField]
        protected MinMaxVector3 m_GroundPositionCameraRecoil;

        [Tooltip("The amount of rotational recoil to add to the camera when the character is grounded.")]
        [SerializeField]
        protected MinMaxVector3 m_GroundRotationCameraRecoil =
            new(new Vector3(-40f, -40f, 0), new Vector3(40f, 40f, 0), new Vector3(20, 20, 0));
        ...
        [SerializeField]
        protected int m_OnGroundedSubstateIndex = 11;

        [Tooltip("The amount of force that should be applied at the start of the ability.")] [SerializeField]
        protected Vector3 m_UpwardForce = new(0, 1, 0);

        [Tooltip("The number of frames that the upward force is applied in.")] [SerializeField]
        protected int m_UpwardForceFrames = 4;
        ...
        {
            get => m_UpwardForce;
            set => m_UpwardForce = value;
        }

        public int UpwardForceFrames
        {
            get => m_UpwardForceFrames;
            set => m_UpwardForceFrames = value;
        }

        public Vector3 DownwardForce
        {
            get => m_DownwardForce;
            set => m_DownwardForce = value;
        }

        public int DownwardForceFrames
        {
            get => m_DownwardForceFrames;
            set => m_DownwardForceFrames = value;
        }

        public bool AllowInAirStop
        {
            get => m_AllowInAirStop;
            set => m_AllowInAirStop = value;
        }

        public MinMaxVector3 GroundPositionCameraRecoil
        {
            get => m_GroundPositionCameraRecoil;
            set => m_GroundPositionCameraRecoil = value;
        }

        public MinMaxVector3 GroundRotationCameraRecoil
        {
            get => m_GroundRotationCameraRecoil;
            set => m_GroundRotationCameraRecoil = value;
        }

        public override bool CanReceiveMultipleStarts => false;
        ...
        {
            if (base.ShouldStopActiveAbility(activeAbility)) return true;
        ...
        protected override void AbilityStarted()
        {
            ...
            for (var i = 0; i < m_UsableItems.Length; ++i) ScheduleCompleteEvent(i, false);
        }
        ...
        public override bool CanStopAbility()
        {
            ...
            if ((m_AllowInAirStop || !m_CharacterLocomotion.Grounded) &&
                (m_JumpAbility == null || !m_JumpAbility.IsActive)) return false;
        ...
        private void OnGrounded(bool grounded)
        {
            ...
            if (grounded)
            {
                SurfaceManager.SpawnEffect(m_CharacterLocomotion.GroundRaycastHit, m_GroundImpact,
                    m_CharacterLocomotion.GravityDirection, m_CharacterLocomotion.TimeScale, m_GameObject);
                EventHandler.ExecuteEvent(m_GameObject, "OnAddSecondaryCameraForce",
                    m_GroundPositionCameraRecoil.RandomValue, m_GroundRotationCameraRecoil.RandomValue, 0f);
