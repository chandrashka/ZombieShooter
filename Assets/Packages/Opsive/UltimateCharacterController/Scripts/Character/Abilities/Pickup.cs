...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public class Pickup : DetectObjectAbilityBase
    {
        ...
        [Tooltip("Specifies the type of objects that can be picked up.")] [SerializeField]
        protected AllowedPickups m_AllowedPickups = AllowedPickups.Item;
        ...
        [Tooltip(
            "Specifies if the ability should wait for the OnAnimatorPickupComplete animation event or wait for the specified duration before stopping the ability.")]
        [SerializeField]
        protected AnimationEventTrigger m_PickupCompleteEvent = new(false, 0.4f);

        [Tooltip(
            "Specifies if the ability should wait for the OnAnimatorPickup animation event or wait for the specified duration before picking up the item.")]
        [SerializeField]
        protected AnimationEventTrigger m_PickupEvent = new(true, 0.2f);

        [Tooltip(
            "Specifies a list of ItemIdentifiers that should be picked up. If the list is empty any ItemIdentifier will trigger the animation.")]
        [FormerlySerializedAs("m_PickupItemTypes")]
        [SerializeField]
        protected ItemDefinitionBase[] m_PickupItemDefinitions;

        [Tooltip("The slot ID to pick up. A value of -1 indicates any slot.")] [SerializeField]
        protected int m_SlotID = -1;

        public AllowedPickups AllowedPickup
        {
            get => m_AllowedPickups;
            set => m_AllowedPickups = value;
        }
        ...
        public ItemDefinitionBase[] PickupItemDefinitions
        {
            get => m_PickupItemDefinitions;
            set => m_PickupItemDefinitions = value;
        }

        public AnimationEventTrigger PickupEvent
        {
            get => m_PickupEvent;
            set => m_PickupEvent = value;
        }

        public AnimationEventTrigger PickupCompleteEvent
        {
            get => m_PickupCompleteEvent;
            set => m_PickupCompleteEvent = value;
        }
        ...
        public bool CanPickupItem()
        {
            ...
            if (m_StartType != AbilityStartType.Automatic) return false;
            // Item pickups may not be allowed.
            if ((m_AllowedPickups & AllowedPickups.Item) == 0) return false;
            // The ItemPickup component should always pickup the item if the Ride ability is active.
            if (m_CharacterLocomotion.IsAbilityTypeActive<Ride>()) return true;
            ...
            {
                if (m_CharacterLocomotion.ActiveAbilities[i].Index > Index) break;
                if (!m_CharacterLocomotion.ActiveAbilities[i].IsConcurrent) return true;
            }
        ...
        protected override bool ValidateObject(GameObject obj, RaycastHit? raycastHit)
        {
            ...
            {
                if (m_AvailableItemPickupCount > 0)
                    for (var i = 0; i < m_AvailableItemPickupCount; ++i)
                        if (obj == m_AvailableItemPickups[i].gameObject)
                            return true;

                ItemPickupBase itemPickup;
                if ((itemPickup = obj.GetCachedComponent<ItemPickupBase>()) != null &&
                    !itemPickup.PickupOnTriggerEnter && !itemPickup.IsDepleted)
                {
                    if (m_AvailableItemPickups.Length == m_AvailableItemPickupCount)
                    {
                        Debug.LogWarning("Warning: Unable to pickup " + itemPickup.name +
                                         ". Ensure the MaxTriggerObject count is at least " +
        ...
        public override bool CanStartAbility()
        {
            if ((m_AllowedPickups & AllowedPickups.Item) != 0)
            {
                if (m_EquipUnequipAbilities == null) return false;
                ...
                if (!base.CanStartAbility())
                {
                    if (m_AvailableItemPickupCount > 0)
                    {
                        for (var i = 0; i < m_AvailableItemPickupCount; ++i) m_AvailableItemPickups[i] = null;
                ...
                if (m_AvailableItemPickupCount == 0) return false;
            }
        ...
        protected override void AbilityStarted()
        {
            ...
            if ((m_AllowedPickups & AllowedPickups.Item) != 0 && m_AvailableItemPickupCount > 0 &&
                !PrepareItemPickup()) return;
        ...
        private bool PrepareItemPickup()
        {
            ...
            if (m_PickupItemDefinitions != null && m_PickupItemDefinitions.Length > 0)
            {
                ...
                for (var i = 0; i < m_PickupItemDefinitions.Length; ++i)
                {
                    for (var j = 0; j < itemDefinitionAmounts.Length; ++j)
            ...
            for (var i = 0; i < m_EquipUnequipAbilities.Length; ++i) m_EquipUnequipAbilities[i].WillStartPickup();

            var allowedEquippedSlotsMask = 0;
            for (var i = 0; i < itemDefinitionAmounts.Length; ++i)
            {
                var itemDefinition = itemDefinitionAmounts[i].ItemDefinition;
                var itemIdentifier = itemDefinitionAmounts[i].ItemIdentifier;

                for (var j = 0; j < m_Inventory.SlotCount; ++j)
                {
                    ...
                    for (var k = 0; k < m_EquipUnequipAbilities.Length; ++k)
        ...
        protected override bool TriggerExit(GameObject other)
        {
            if (base.TriggerExit(other) && !IsActive)
            {
                ...
                {
                    m_AvailableItemPickupCount--;
                    for (var i = index; i < m_AvailableItemPickupCount; ++i)
                        m_AvailableItemPickups[i] = m_AvailableItemPickups[i + 1];
                }
