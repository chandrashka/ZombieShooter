...
{
    /// <summary>
    ///     The CharacterLocomotion class serves as the base character controller class. It handles the base locomotion with
    ...
    public abstract class CharacterLocomotion : StateBehavior, IForceObject
    {
        ...
        private const float c_ColliderSpacingCubed =
            c_ColliderSpacing * c_ColliderSpacing * c_ColliderSpacing * c_ColliderSpacing;

        // Padding value used to add a bit of spacing for the slope limit.
        private const float c_SlopeLimitSpacing = 0.3f;

        [Tooltip("Should root motion be used to move the character?")] [SerializeField]
        ...
        [SerializeField]
        protected float m_RootMotionAirForceMultiplier = 1;

        [Tooltip("Should root motion be used to rotate the character?")] [SerializeField]
        protected bool m_UseRootMotionRotation;

        [Tooltip("If using root motion, applies a multiplier to the root motion delta rotation.")] [SerializeField]
        protected float m_RootMotionRotationMultiplier = 1;

        [Tooltip("The rate at which the character can rotate. Only used by non-root motion characters.")]
        [SerializeField]
        protected float m_MotorRotationSpeed = 10;

        [Tooltip(
            "Should forces be applied independent of the framerate? If false it will be applied independent of time. This is only necessary for the Update loop.")]
        [SerializeField]
        protected bool m_FramerateIndependentForce;

        [Tooltip("The mass of the character.")] [SerializeField]
        protected float m_Mass = 100;

        [Tooltip("Specifies the width of the characters skin, use for ground detection.")] [SerializeField]
        protected float m_SkinWidth = 0.08f;

        [Tooltip("The maximum object slope angle that the character can traverse (in degrees).")] [SerializeField]
        protected float m_SlopeLimit = 40f;

        [Tooltip("The maximum height that the character can step on top of.")] [SerializeField]
        protected float m_MaxStepHeight = 0.35f;

        [Tooltip(
            "The rate at which the character's motor force accelerates while on the ground. Only used by non-root motion characters.")]
        [SerializeField]
        protected Vector3 m_MotorAcceleration = new(0.18f, 0, 0.18f);

        [Tooltip(
            "The rate at which the character's motor force decelerates while on the ground. Only used by non-root motion characters.")]
        [SerializeField]
        protected float m_MotorDamping = 0.27f;

        [Tooltip(
            "The rate at which the character's motor force accelerates while in the air. Only used by non-root motion characters.")]
        [SerializeField]
        protected Vector3 m_MotorAirborneAcceleration = new(0.12f, 0, 0.12f);

        [Tooltip(
            "The rate at which the character's motor force decelerates while in the air. Only used by non-root motion characters.")]
        [SerializeField]
        protected float m_MotorAirborneDamping = 0.3f;

        [Tooltip("A multiplier which is applied to the motor while moving backwards.")] [SerializeField]
        protected float m_MotorBackwardsMultiplier = 0.7f;

        [Tooltip(
            "A (0-1) value specifying the amount of influence the previous acceleration direction has on the current velocity.")]
        [SerializeField]
        protected float m_PreviousAccelerationInfluence = 1;

        [Tooltip("Should the motor force be adjusted while on a slope?")] [SerializeField]
        protected bool m_AdjustMotorForceOnSlope = true;

        [Tooltip("If adjusting the motor force on a slope, the force multiplier when on an upward slope.")]
        [SerializeField]
        protected float m_MotorSlopeForceUp = 1f;

        [Tooltip("If adjusting the motor force on a slope, the force multiplier when on a downward slope.")]
        [SerializeField]
        protected float m_MotorSlopeForceDown = 1.25f;

        [Tooltip("The rate at which the character's external force decelerates.")] [SerializeField]
        protected float m_ExternalForceDamping = 0.1f;

        [Tooltip("The rate at which the character's external force decelerates while in the air.")] [SerializeField]
        protected float m_ExternalForceAirDamping = 0.05f;

        [Tooltip("Should the character stick to the ground?")] [SerializeField]
        ...
        [SerializeField]
        protected float m_Stickiness = 0.2f;

        [Tooltip("The local time scale of the character.")] [SerializeField]
        protected float m_TimeScale = 1;

        [Tooltip("Should gravity be applied?")] [SerializeField]
        protected bool m_UseGravity = true;

        [Tooltip("The normalized direction of the gravity force.")] [SerializeField]
        protected Vector3 m_GravityDirection = new(0, -1, 0);

        [Tooltip("A amount of gravity force to apply.")] [SerializeField]
        protected float m_GravityMagnitude = 5.22f;

        [Tooltip("Can the character detect horizontal collisions?")] [SerializeField]
        protected bool m_DetectHorizontalCollisions = true;

        [Tooltip("Can the character detect vertical collisions?")] [SerializeField]
        protected bool m_DetectVerticalCollisions = true;

        [Tooltip("The layers that can act as colliders for the character.")] [SerializeField]
        protected LayerMask m_ColliderLayerMask =
            ~((1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.SubCharacter) |
              (1 << LayerManager.Overlay) | (1 << LayerManager.VisualEffect));

        [Tooltip("The maximum number of colliders that the character can detect.")] [SerializeField]
        protected int m_MaxCollisionCount = 100;

        [Tooltip("The maximum number of frames that the soft force can be distributed by.")] [SerializeField]
        protected int m_MaxSoftForceFrames = 100;

        [Tooltip("The maximum number of collision checks that should be performed when rotating.")] [SerializeField]
        protected int m_RotationCollisionCheckCount = 10;

        [Tooltip("The maximum number of iterations to detect collision overlaps.")] [SerializeField]
        protected int m_MaxOverlapIterations = 6;

        [Tooltip(
            "A curve specifying the amount to move when gliding along a wall. The x variable represents the dot product between the character look direction and wall normal. " +
        ...
        [SerializeField]
        protected AnimationCurve m_WallGlideCurve =
            new(new Keyframe(0, 0), new Keyframe(0.1f, 1.5f, 0, 0), new Keyframe(1, 1.5f));

        [Tooltip("A multiplier to apply when hitting a wall. Allows for the character to bounce off of a wall.")]
        [SerializeField]
        protected float m_WallBounceModifier = 1;
        ...
        [SerializeField]
        protected bool m_StickToMovingPlatform = true;
        ...
        [SerializeField]
        protected float m_MovingPlatformSeperationVelocity = 5;
        ...
        [SerializeField]
        protected float m_MinHorizontalMovingPlatformStickSpeed = 2;

        [Tooltip(
            "The rate at which the character's moving platform force decelerates when the character is no longer on the platform.")]
        [SerializeField]
        protected float m_MovingPlatformForceDamping;

        [Tooltip("An array of bones that should be smoothed by the Kinematic Object Manager.")] [SerializeField]
        protected Transform[] m_SmoothedBones;
        ...
        private readonly RaycastHit m_BlankRaycastHit = new();
        ...
        protected UnityEngineUtility.RaycastHitComparer m_RaycastHitComparer = new();
        ...
        public float ColliderSpacingCubed => c_ColliderSpacingCubed;
        public float SlopeLimitSpacing => c_SlopeLimitSpacing;

        public bool UseRootMotionPosition
        {
            get => m_UseRootMotionPosition;
            set => m_UseRootMotionPosition = value;
        }

        public float RootMotionSpeedMultiplier
        {
            get => m_RootMotionSpeedMultiplier;
            set => m_RootMotionSpeedMultiplier = value;
        }

        public float RootMotionAirForceMultiplier
        {
            get => m_RootMotionAirForceMultiplier;
            set => m_RootMotionAirForceMultiplier = value;
        }

        public bool UseRootMotionRotation
        {
            get => m_UseRootMotionRotation;
            set => m_UseRootMotionRotation = value;
        }

        public float RootMotionRotationMultiplier
        {
            get => m_RootMotionRotationMultiplier;
            set => m_RootMotionRotationMultiplier = value;
        }

        public float MotorRotationSpeed
        {
            get => m_MotorRotationSpeed;
            set => m_MotorRotationSpeed = value;
        }

        public bool FramerateIndependentForce
        {
            get => m_FramerateIndependentForce;
            set => m_FramerateIndependentForce = value;
        }
        ...
        public Vector3 MotorAcceleration
        {
            get => m_MotorAcceleration;
        ...
        {
            get => m_MotorDamping;
            set => m_MotorDamping = value;
        }

        public Vector3 MotorAirborneAcceleration
        {
            get => m_MotorAirborneAcceleration;
            set => m_MotorAirborneAcceleration = value;
        }

        public float MotorAirborneDamping
        {
            get => m_MotorAirborneDamping;
            set => m_MotorAirborneDamping = value;
        }

        public float MotorBackwardsMultiplier
        {
            get => m_MotorBackwardsMultiplier;
            set => m_MotorBackwardsMultiplier = value;
        }

        public float PreviousAccelerationInfluence
        {
            get => m_PreviousAccelerationInfluence;
            set => m_PreviousAccelerationInfluence = value;
        }

        public bool AdjustMotorForceOnSlope
        {
            get => m_AdjustMotorForceOnSlope;
            set => m_AdjustMotorForceOnSlope = value;
        }

        public float MotorSlopeForceUp
        {
            get => m_MotorSlopeForceUp;
            set => m_MotorSlopeForceUp = value;
        }

        public float MotorSlopeForceDown
        {
            get => m_MotorSlopeForceDown;
            set => m_MotorSlopeForceDown = value;
        }

        public float ExternalForceDamping
        {
            get => m_ExternalForceDamping;
            set => m_ExternalForceDamping = value;
        }

        public float ExternalForceAirDamping
        {
            get => m_ExternalForceAirDamping;
            set => m_ExternalForceAirDamping = value;
        }
        ...
        public bool DetectHorizontalCollisions
        {
            get => m_DetectHorizontalCollisions;
            set => m_DetectHorizontalCollisions = value;
        }

        public bool DetectVerticalCollisions
        {
            get => m_DetectVerticalCollisions;
            set => m_DetectVerticalCollisions = value;
        }

        public LayerMask ColliderLayerMask
        {
            get => m_ColliderLayerMask;
            set => m_ColliderLayerMask = value;
        }

        public int MaxCollisionCount => m_MaxCollisionCount;
        public int MaxSoftForceFrames => m_MaxSoftForceFrames;

        public int RotationCollisionCheckCount
        {
            get => m_RotationCollisionCheckCount;
            set => m_RotationCollisionCheckCount = value;
        }

        public int MaxOverlapIterations
        {
            get => m_MaxOverlapIterations;
            set => m_MaxOverlapIterations = value;
        }

        public AnimationCurve WallGlideCurve
        {
            get => m_WallGlideCurve;
            set => m_WallGlideCurve = value;
        }

        public float WallBounceModifier
        {
            get => m_WallBounceModifier;
            set => m_WallBounceModifier = value;
        }

        public bool StickToMovingPlatform
        {
            get => m_StickToMovingPlatform;
            set => m_StickToMovingPlatform = value;
        }

        public float MovingPlatformSeperationVelocity
        {
            get => m_MovingPlatformSeperationVelocity;
            set => m_MovingPlatformSeperationVelocity = value;
        }

        public float MinHorizontalMovingPlatformStickSpeed
        {
            get => m_MinHorizontalMovingPlatformStickSpeed;
            set => m_MinHorizontalMovingPlatformStickSpeed = value;
        }

        public float MovingPlatformForceDamping
        {
            get => m_MovingPlatformForceDamping;
            set => m_MovingPlatformForceDamping = value;
        }

        public Transform[] SmoothedBones => m_SmoothedBones;
        ...
        public bool AllowUseGravity
        {
            set => m_AllowUseGravity = value;
        }

        public bool ForceUseGravity
        {
            set => m_ForceUseGravity = value;
        }

        public bool UsingGravity => (m_UseGravity || m_ForceUseGravity) && m_AllowUseGravity;

        public bool AllowRootMotionPosition
        {
            set => m_AllowRootMotionPosition = value;
        }

        public bool ForceRootMotionPosition
        {
            set => m_ForceRootMotionPosition = value;
        }

        public bool UsingRootMotionPosition =>
            (m_UseRootMotionPosition || m_ForceRootMotionPosition) && m_AllowRootMotionPosition;

        public bool AllowRootMotionRotation
        {
            set => m_AllowRootMotionRotation = value;
        }

        public bool ForceRootMotionRotation
        {
            set => m_ForceRootMotionRotation = value;
        }

        public bool UsingRootMotionRotation =>
            (m_UseRootMotionRotation || m_ForceRootMotionRotation) && m_AllowRootMotionRotation;

        public Vector3 AnimatorDeltaPosition
        {
            get => m_AnimatorDeltaPosition;
            set => m_AnimatorDeltaPosition = value;
        }

        public Quaternion AnimatorDeltaRotation
        {
            get => m_AnimatorDeltaRotation;
            set => m_AnimatorDeltaRotation = value;
        }

        public bool AllowHorizontalCollisionDetection
        {
            set => m_AllowHorizontalCollisionDetection = value;
        }

        public bool ForceHorizontalCollisionDetection
        {
            set => m_ForceHorizontalCollisionDetection = value;
        }

        public bool UsingHorizontalCollisionDetection =>
            (m_DetectHorizontalCollisions || m_ForceHorizontalCollisionDetection) &&
            m_AllowHorizontalCollisionDetection;

        public bool AllowVerticalCollisionDetection
        {
            set => m_AllowVerticalCollisionDetection = value;
        }

        public bool ForceVerticalCollisionDetection
        {
            set => m_ForceVerticalCollisionDetection = value;
        }

        public bool UsingVerticalCollisionDetection =>
            (m_DetectVerticalCollisions || m_ForceVerticalCollisionDetection) && m_AllowVerticalCollisionDetection;

        [Shared.Utility.NonSerialized] public Vector3 MoveDirection { get; set; }
        ...
        public Vector3 LocomotionVelocity
        {
            get => Velocity - m_PlatformVelocity;
            protected set => Velocity = value;
        }

        public Vector3 LocalLocomotionVelocity => m_Transform.InverseTransformDirection(Velocity - m_PlatformVelocity);
        [Snapshot] public Vector3 Up { get; protected set; }

        public float Height { get; private set; }

        public float Radius { get; private set; } = float.MaxValue;

        public Vector3 Center => m_Transform.InverseTransformPoint(m_Transform.position + Up * Height / 2);
        ...
        public Vector3 LocalExternalForce => m_Transform.InverseTransformDirection(m_ExternalForce);
        public RaycastHit GroundRaycastHit => m_GroundRaycastHit;
        public RaycastHit HorizontalRaycastHit { get; private set; }

        public float TimeScaleSquared => m_TimeScale * m_TimeScale;
        public Transform Platform => m_Platform;
        public Vector3 PlatformMovement { get; private set; }

        public Quaternion PlatformTorque => m_PlatformTorque;
        public bool CollisionLayerEnabled { get; private set; } = true;

        [Shared.Utility.NonSerialized] public bool AlignToGravity { get; set; }

        [Shared.Utility.NonSerialized] public bool SmoothGravityYawDelta { get; set; } = true;

        [Shared.Utility.NonSerialized] public bool ManualMove { get; set; }

        [Snapshot]
        public Vector3 PlatformVelocity
        {
            get => m_PlatformVelocity;
            set => m_PlatformVelocity = value;
        }

        [Snapshot] protected float SlopeFactor { get; set; } = 1;
        ...
        [Shared.Utility.NonSerialized] public Vector3 Velocity { get; set; }

        public Vector3 LocalVelocity => m_Transform.InverseTransformDirection(Velocity);

        [Snapshot] public Vector3 MotorThrottle { get; set; }

        [Snapshot] public Quaternion MotorRotation { get; set; }

        [Snapshot] public Quaternion PrevMotorRotation { get; set; }

        [Snapshot]
        public Vector3 ExternalForce
        {
            get => m_ExternalForce;
            protected set => m_ExternalForce = value;
        }

        [Snapshot] public float GravityAmount { get; set; }

        [Snapshot] protected Vector3 PrevPosition { get; set; }

        [Snapshot] protected Vector3 PlatformRelativePosition { get; set; }
        ...
        protected override void Awake()
        {
            ...
            MotorRotation = PrevMotorRotation = m_Transform.rotation;
            ...
            for (var i = 0; i < colliders.Length; ++i)
            {
                ...
                if (!MathUtility.InLayerMask(colliders[i].gameObject.layer, m_ColliderLayerMask) ||
                    !(colliders[i] is CapsuleCollider || colliders[i] is SphereCollider))
                {
                ...
                if (!m_CheckRotationCollision) m_CheckRotationCollision = CanCauseRotationCollision(colliders[i]);
            }

            // Resize the array depending on the number of valid colliders.
            if (m_Colliders.Length != m_ColliderCount) Array.Resize(ref m_Colliders, m_ColliderCount);
            if (m_Colliders.Length == 0)
                Debug.LogWarning(
                    $"Warning: The character {name} doesn't contain any colliders. Only capsule and sphere colliders are supported.",
                    this);
            ...
            for (var i = 0; i < m_ColliderGameObjects.Length; ++i) m_ColliderGameObjects[i] = m_Colliders[i].gameObject;
            m_IgnoredColliderGameObjects = new GameObject[m_IgnoredColliders.Length];
            for (var i = 0; i < m_IgnoredColliderGameObjects.Length; ++i)
            ...
            {
                m_ColliderIndexMap =
                    new Dictionary<RaycastHit, int>(new UnityEngineUtility.RaycastHitEqualityComparer());
        ...
        protected virtual void OnAnimatorMove()
        {
            ...
            if (float.IsNaN(m_AnimatorDeltaPosition.x) || m_TimeScale == 0 || Time.timeScale == 0 ||
                Time.deltaTime == 0) return;

            if (m_AnimatorDeltaPosition.sqrMagnitude > 0)
            {
                m_LocalRootMotionForce += m_Transform.InverseTransformDirection(m_AnimatorDeltaPosition) *
                                          m_RootMotionSpeedMultiplier;
        ...
        /// <summary>
        ///     Adds a force to the character in the specified number of frames. The force will either be an external or soft
        ///     force.
        /// </summary>
        /// <param name="force">The force to add.</param>
        ...
        /// <summary>
        ///     Can the specified collider cause a collision when the character is rotating? The collider can cause a rotation
        ///     collision when it would rotate on
        ...
        private bool CanCauseRotationCollision(Collider rotationCollider)
        {
            ...
            if (rotationCollider is CapsuleCollider)
            {
                ...
                direction = m_Transform.InverseTransformDirection(
                    colliderTransform.TransformPoint(capsuleCollider.center) - m_Transform.position);
                if (Mathf.Abs(direction.x) > 0.0001f || Mathf.Abs(direction.z) > 0.0001f) return true;
                MathUtility.CapsuleColliderEndCaps(capsuleCollider, colliderTransform.position,
                    colliderTransform.rotation, out startEndCap, out endEndCap);
            ...
            else
            {
                // SphereCollider.
                direction = m_Transform.InverseTransformDirection(
                    rotationCollider.transform.TransformPoint((rotationCollider as SphereCollider).center) -
        ...
        /// <summary>
        ///     Moves the character according to the input. This method exists to allow AI to easily move the character instead of
        ...
        {
            if (m_TimeScale == 0 || Time.deltaTime == 0) return;
        ...
        /// <summary>
        ///     Provides a virtual method for the UltimateCharacterLocomotion to perform its updates (such as updating the
        ///     abilities).
        /// </summary>
        ...
        /// <summary>
        ///     Updates the position and rotation. This should be done after the animator has updated so the root motion is
        ...
        private void UpdatePositionAndRotation(bool fromAnimatorMove)
        {
            if (m_AnimatorMonitor != null && m_AnimatorMonitor.AnimatorEnabled)
                ...
                if (fromAnimatorMove == ManualMove)
                    return;
        ...
        private void UpdatePlatformMovement()
        {
            ...
            {
                var moveDirection = m_Transform.position - m_Platform.TransformPoint(PlatformRelativePosition);
                ...
                {
                    var closestRaycastHit =
                        QuickSelect.SmallestK(m_CombinedRaycastHits, hitCount, i, m_RaycastHitComparer);
            ...
            PlatformMovement = m_Platform.TransformPoint(PlatformRelativePosition) - m_Transform.position;
            MoveDirection += PlatformMovement;
            ...
            var platformVelocity = PlatformMovement / (m_TimeScale * Time.deltaTime);
            if (!m_StickToMovingPlatform)
                if (platformVelocity.sqrMagnitude < m_PlatformVelocity.sqrMagnitude &&
                    (platformVelocity - m_PlatformVelocity).magnitude > m_MovingPlatformSeperationVelocity)
                    AddForce(PlatformMovement, 1, false, true);
            ...
            m_PlatformTorque = MathUtility.InverseTransformQuaternion(platformRotation, m_PlatformRotationOffset) *
                               Quaternion.Inverse(MathUtility.InverseTransformQuaternion(platformRotation,
                                   m_Transform.rotation * Quaternion.Inverse(platformRotation)));
            if (!AlignToGravity)
            {
                // Only the local y rotation should affect the character's rotation.
                var transformRotation = m_Transform.rotation;
                var localPlatformTorque = MathUtility.InverseTransformQuaternion(transformRotation, m_PlatformTorque)
                    .eulerAngles;
                localPlatformTorque.x = localPlatformTorque.z = 0;
                m_PlatformTorque =
                    MathUtility.TransformQuaternion(transformRotation, Quaternion.Euler(localPlatformTorque));
            }
        ...
        protected virtual bool UpdatePlaformDisconnectMovement()
        {
            ...
            MoveDirection += m_PlatformDisconnectVelocity * Time.deltaTime;

            m_PlatformDisconnectTorque =
                Quaternion.RotateTowards(m_PlatformDisconnectTorque, Quaternion.identity, damping);
            ...
            return m_PlatformDisconnectVelocity.magnitude > m_PlatformDisconnectVelocityMaxMagnitude ||
                   Quaternion.Angle(m_PlatformDisconnectTorque, Quaternion.identity) >
                   m_PlatformDisconnectTorqueMaxMagnitude;
        }
        ...
        protected virtual void UpdateRotation()
        {
            ...
            if (UsingRootMotionRotation)
            {
                ...
                if (AlignToGravity) targetRotation *= Quaternion.Euler(m_DeltaRotation);
            }
            else
            {
                if (AlignToGravity)
                {
                    ...
                    if (SmoothGravityYawDelta)
                        m_DeltaRotation.y = Mathf.LerpAngle(0, MathUtility.ClampInnerAngle(m_DeltaRotation.y),
                            m_MotorRotationSpeed * m_TimeScale * TimeUtility.DeltaTimeScaled);
                    targetRotation = rotation * Quaternion.Euler(m_DeltaRotation);
                }
                else
                {
                    targetRotation = Quaternion.Slerp(rotation, rotation * Quaternion.Euler(m_DeltaRotation),
                        m_MotorRotationSpeed * m_TimeScale * TimeUtility.DeltaTimeScaled);
                }
            }
        ...
        ///     Should the rotation be force checked? This is used when the character is aligning to the
        ///     ground.
        /// </param>
        ...
        public Quaternion CheckRotation(Quaternion rotationDelta, bool forceCheck)
        {
            ...
            if (!UsingHorizontalCollisionDetection) return rotationDelta;
            ...
            if (m_CheckRotationCollision && rotationDelta != Quaternion.identity)
            {
                ...
                for (var i = 0; i < m_ColliderCount; ++i)
                {
                    ...
                    if (!forceCheck && !CanCauseRotationCollision(m_Colliders[i])) continue;

                    // Prevent the character from intersecting with another object while rotating.
                    var targetRotationDelta = rotationDelta;
                    var hitCount = 0;
                    for (var k = 0; k < m_RotationCollisionCheckCount; ++k)
                    {
                        // Slerp towards Quaternion.identity which will not add any rotation at all.
                        rotationDelta = Quaternion.Slerp(targetRotationDelta, Quaternion.identity,
                            k / (float)(m_RotationCollisionCheckCount - 1));
                        ...
                        var matrix = MathUtility.ApplyRotationToChildMatrices(m_Colliders[i].transform, m_Transform,
                            rotationDelta);
                        ...
                        if (m_Colliders[i] is CapsuleCollider)
                        {
                            ...
                            MathUtility.CapsuleColliderEndCaps(capsuleCollider, targetPosition, targetRotation,
                                out startEndCap, out endEndCap);
                            hitCount = Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap,
                                capsuleCollider.radius * MathUtility.ColliderRadiusMultiplier(capsuleCollider),
                                m_OverlapColliderHit, m_CharacterLayerManager.SolidObjectLayers,
                        ...
                        else
                        {
                            ...
                            hitCount = Physics.OverlapSphereNonAlloc(targetPosition,
                        ...
                        if (hitCount > 0)
                        {
                            ...
                            for (var j = 0; j < hitCount; ++j)
                                if (Physics.ComputePenetration(m_Colliders[i], targetPosition, targetRotation,
                                        m_OverlapColliderHit[j], m_OverlapColliderHit[j].transform.position,
                                        m_OverlapColliderHit[j].transform.rotation, out direction, out distance))
                                {
                                    ...
                                    var slope = Vector3.Angle(m_Transform.rotation * rotationDelta * Vector3.up,
                                        direction);
                                    if (slope <= m_SlopeLimit + c_SlopeLimitSpacing) continue;
        ...
        protected virtual void ApplyRotation()
        {
            ...
            Up = m_Transform.up;
            MotorRotation = m_Transform.rotation;
            m_Torque = Quaternion.identity;

            if (m_Platform != null) m_PlatformRotationOffset = MotorRotation * Quaternion.Inverse(m_Platform.rotation);
        }
        ...
        protected virtual void UpdatePosition()
        {
            ...
            var deltaTime = m_TimeScale * Time.timeScale *
                            (m_FramerateIndependentForce ? 1 : TimeUtility.FramerateDeltaTime);
            MoveDirection += m_ExternalForce * deltaTime +
                             (UsingRootMotionPosition ? 1 : deltaTime) * m_TimeScale * MotorThrottle -
                             GravityAmount * deltaTime * m_GravityDirection;
        ...
        protected virtual void UpdateMotorThrottle()
        {
            ...
            {
                MotorThrottle = (m_Grounded ? 1 : m_RootMotionAirForceMultiplier) * SlopeFactor *
                                MathUtility.TransformDirection(m_LocalRootMotionForce, MotorRotation);
            }
            else
            {
                ...
                var prevLocalMotorThrottle = MathUtility.InverseTransformDirection(MotorThrottle, PrevMotorRotation) *
                                             m_PreviousAccelerationInfluence;
                var rotation = Quaternion.Slerp(PrevMotorRotation, MotorRotation, m_PreviousAccelerationInfluence);
                var acceleration = SlopeFactor * backwardsMultiplier * 0.1f *
                                   (m_Grounded ? m_MotorAcceleration : m_MotorAirborneAcceleration);
                ...
                MotorThrottle = MathUtility.TransformDirection(new Vector3(
                    ...
                    rotation);
                // Dampen motor forces.
                MotorThrottle /= 1 + (m_Grounded ? m_MotorDamping : m_MotorAirborneDamping) * m_TimeScale *
                    Time.timeScale;
            }

            PrevMotorRotation = MotorRotation;
        ...
        {
            HorizontalRaycastHit = m_BlankRaycastHit;
            // No horizontal collision checks are necessary if no collisions are being detected.
            if (!UsingHorizontalCollisionDetection) return;
            ...
            var platformIndependentMoveDirection = MoveDirection - PlatformMovement;
            var horizontalDirection = Vector3.ProjectOnPlane(platformIndependentMoveDirection, Up);
            // No casts are necessary if there is no movement.
            if (horizontalDirection.sqrMagnitude < c_ColliderSpacingCubed &&
                Vector3.ProjectOnPlane(PlatformMovement, Up).sqrMagnitude < c_ColliderSpacingCubed)
            {
                ...
                MoveDirection = m_Transform.TransformDirection(localDirection) + PlatformMovement;
                return;
            }

            var hitCount = NonAllocCast(horizontalDirection, PlatformMovement);
            if (hitCount == 0) return;

            var localPlatformIndependentMoveDirection =
                m_Transform.InverseTransformDirection(platformIndependentMoveDirection);
            var moveDistance = 0f;
            var hitStrength = 0f;
            ...
            for (var i = 0; i < hitCount; ++i)
            {
                ...
                var activeCollider = m_ColliderCount > 1
                    ? m_Colliders[m_ColliderIndexMap[closestRaycastHit]]
                    : m_Colliders[0];
                ...
                if (closestRaycastHit.distance == 0)
                {
                    ...
                    ComputePenetration(activeCollider, closestRaycastHit.collider, horizontalDirection, false,
                        out offset);
                    if (offset.sqrMagnitude >= c_ColliderSpacingCubed)
                    {
                        moveDistance = Mathf.Max(0,
                            horizontalDirection.magnitude - offset.magnitude - c_ColliderSpacing);
                        platformIndependentMoveDirection =
                            Vector3.ProjectOnPlane(horizontalDirection.normalized * moveDistance, Up) +
                            Up * localPlatformIndependentMoveDirection.y;
                    }
                    else
                    {
                ...
                if (hitRigidbody != null)
                {
                    var radius = activeCollider is CapsuleCollider
                        ? (activeCollider as CapsuleCollider).radius *
                          MathUtility.ColliderRadiusMultiplier(activeCollider as CapsuleCollider)
                        : (activeCollider as SphereCollider).radius *
                          MathUtility.ColliderRadiusMultiplier(activeCollider);
                ...
                if (m_Grounded && canStep)
                {
                    ...
                    if (groundPoint.y <= m_MaxStepHeight + c_ColliderSpacing)
                    {
                        ...
                        if (!Physics.Raycast(m_SlopeRay, out m_RaycastHit, c_ColliderSpacing + 0.11f,
                                1 << hitGameObject.layer, QueryTriggerInteraction.Ignore))
                            m_RaycastHit = closestRaycastHit;
                        var slope = Vector3.Angle(Up, m_RaycastHit.normal);
                        if (slope <= m_SlopeLimit + c_SlopeLimitSpacing) continue;
                        ...
                        if (SingleCast(activeCollider, horizontalDirection,
                                PlatformMovement + Up * (m_MaxStepHeight - c_ColliderSpacing)))
                        {
                            if (m_RaycastHit.distance - c_ColliderSpacing < horizontalDirection.magnitude)
                            {
                                ...
                                m_SlopeRay.direction = horizontalDirection.normalized;
                                m_SlopeRay.origin = closestRaycastHit.point -
                                                    m_SlopeRay.direction * (c_ColliderSpacing + 0.1f);
                                var normal = m_RaycastHit.normal;
                                if (Physics.Raycast(m_SlopeRay, out m_RaycastHit, c_ColliderSpacing + 0.11f,
                                        1 << hitGameObject.layer, QueryTriggerInteraction.Ignore))
                                    normal = m_RaycastHit.normal;
                                slope = Vector3.Angle(Up, normal);
                                if (slope <= m_SlopeLimit + c_SlopeLimitSpacing) continue;
                            }
                        }
                        else
                        {
                            ...
                            if (OverlapCount(activeCollider,
                                    direction.normalized * (direction.magnitude + Radius * 0.5f) + PlatformMovement +
                                    Up * (m_MaxStepHeight - c_ColliderSpacing)) == 0)
                ...
                var hitNormal = Vector3.ProjectOnPlane(closestRaycastHit.normal, Up).normalized;
                var targetDirection = Vector3.Cross(hitNormal, Up).normalized;
                var closestPoint = MathUtility.ClosestPointOnCollider(m_Transform, activeCollider,
                    closestRaycastHit.point, MoveDirection, true, false);
                if (Vector3.Dot(
                        Vector3.Cross(Vector3.ProjectOnPlane(closestRaycastHit.point - closestPoint, Up).normalized,
                            horizontalDirection).normalized, Up) > 0) targetDirection = -targetDirection;
                ...
                if (moveDistance < 0.001f ||
                    Vector3.Angle(Up, m_GroundRaycastHit.normal) > m_SlopeLimit + c_SlopeLimitSpacing) moveDistance = 0;
                ...
                var dynamicFrictionValue = Mathf.Clamp01(1 - MathUtility.FrictionValue(activeCollider.material,
                    closestRaycastHit.collider.material, true));
                hitStrength = 1 - Vector3.Dot(horizontalDirection.normalized, -hitNormal);
                hitMoveDirection = (horizontalDirection.magnitude - moveDistance) *
                                   m_WallGlideCurve.Evaluate(hitStrength) * dynamicFrictionValue * targetDirection;
                if (hitMoveDirection.magnitude <= c_ColliderSpacing)
                {
                    hitMoveDirection = Vector3.zero;
                    hitStrength = 0;
                    HorizontalRaycastHit = closestRaycastHit;
                }

                platformIndependentMoveDirection = horizontalDirection.normalized * moveDistance + hitMoveDirection +
                                                   Up * localPlatformIndependentMoveDirection.y;
                ...
                var bouncinessValue =
                    MathUtility.BouncinessValue(activeCollider.material, closestRaycastHit.collider.material);
            ...
            {
                hitCount = NonAllocCast(hitMoveDirection, PlatformMovement);
                for (var i = 0; i < hitCount; ++i)
                {
                    var closestRaycastHit =
                        QuickSelect.SmallestK(m_CombinedRaycastHits, hitCount, i, m_RaycastHitComparer);
                    ...
                    if (m_Grounded)
                    {
                        ...
                        if (groundPoint.y > c_ColliderSpacing && groundPoint.y <= m_MaxStepHeight + c_ColliderSpacing)
                        {
                            ...
                            // Cast a ray directly in front of the character. If it doesn't hit an object then the object is shorter than the step height and should be stepped on.
                            // Continue out of the loop to prevent the character from stopping in front of the object.
                            if (SingleCast(activeCollider, hitMoveDirection,
                                    PlatformMovement + Up * (m_MaxStepHeight - c_ColliderSpacing)))
                            {
                            ...
                            else
                            {
                                ...
                                var direction = groundPoint - m_Transform.position;
                                if (OverlapCount(activeCollider,
                                        direction.normalized * (direction.magnitude + Radius * 0.5f) +
                                        PlatformMovement + Up * (m_MaxStepHeight - c_ColliderSpacing)) == 0)
                                    // Step over the object if there are no objects in the way.
                                    continue;
                            }
                        }
                    ...
                    var moveDistanceContribution = moveDistance *
                                                   Mathf.Cos(Vector3.Angle(horizontalDirection.normalized,
                                                       hitMoveDirection.normalized) * Mathf.Deg2Rad);
                    var hitMoveDistance =
                        Mathf.Min(
                            closestRaycastHit.distance - moveDistanceContribution - hitMoveDirection.magnitude -
                            c_ColliderSpacing, hitMoveDirection.magnitude);
                    if (hitMoveDistance < 0.001f || Vector3.Angle(Up, m_GroundRaycastHit.normal) >
                        m_SlopeLimit + c_SlopeLimitSpacing)
                    {
                        hitMoveDistance = 0;
                        HorizontalRaycastHit = closestRaycastHit;
                    }

                    platformIndependentMoveDirection = horizontalDirection.normalized * moveDistance +
                                                       hitMoveDirection.normalized * hitMoveDistance +
                                                       Up * localPlatformIndependentMoveDirection.y;
                    break;
                }
            ...
            MoveDirection = platformIndependentMoveDirection + PlatformMovement;
        }
        ...
        private void DeflectVerticalCollisions()
        {
            ...
            var localMoveDirection = m_Transform.InverseTransformDirection(MoveDirection);
            if (UsingVerticalCollisionDetection && localMoveDirection.y > 0)
            {
                var horizontalDirection = Vector3.ProjectOnPlane(MoveDirection - PlatformMovement, Up);
                var hitCount = NonAllocCast(Up * (localMoveDirection.y + c_ColliderSpacing),
                    PlatformMovement + horizontalDirection);
                if (hitCount > 0)
                {
                    var closestRaycastHit =
                        QuickSelect.SmallestK(m_CombinedRaycastHits, hitCount, 0, m_RaycastHitComparer);
                    if (closestRaycastHit.distance == 0)
                    {
                        ...
                        var activeCollider = m_ColliderCount > 1
                            ? m_Colliders[m_ColliderIndexMap[closestRaycastHit]]
                            : m_Colliders[0];
                        var overlap = ComputePenetration(activeCollider, closestRaycastHit.collider,
                            horizontalDirection, true, out var offset);
                        if (overlap)
                            overlap = ComputePenetration(activeCollider, closestRaycastHit.collider,
                                horizontalDirection, false, out offset);
                        if (!overlap)
                            // The offset is valid.
                    ...
                    MoveDirection = m_Transform.TransformDirection(localMoveDirection);
                }
            }

            // Is the character on the ground?
            var grounded = CheckGround();
            localMoveDirection = m_Transform.InverseTransformDirection(MoveDirection);
            ...
            {
                verticalOffset =
                    m_Transform.InverseTransformDirection(m_GroundRaycastHit.point - m_GroundRaycastOrigin).y +
                    c_ColliderSpacing;
                if (Mathf.Abs(verticalOffset) < 0.0001f) verticalOffset = 0;
                ...
                var localPlatformVerticalDirection =
                    m_Platform != null ? m_Transform.InverseTransformDirection(PlatformMovement).y : 0;
                if ((m_Grounded || grounded) && localMoveDirection.y - localPlatformVerticalDirection < m_SkinWidth &&
                    verticalOffset > -c_ColliderSpacing -
                    (m_Grounded && m_StickToGround ? m_Stickiness : 0))
                {
                    ...
                    if (localMoveDirection.y < -m_GroundRaycastHit.distance + localPlatformVerticalDirection)
                        localMoveDirection.y = -m_GroundRaycastHit.distance + localPlatformVerticalDirection +
                                               c_ColliderSpacing;
            ...
            if (accumulateGravity)
            {
                ...
                // Accumulate gravity.
                GravityAmount += m_GravityMagnitude * -0.001f / Time.timeScale;
            }
            else if (grounded)
            {
                ...
                if (m_GroundRaycastHit.rigidbody != null)
                    m_GroundRaycastHit.rigidbody.AddForceAtPosition(
                        -Up * (m_Mass / m_GroundRaycastHit.rigidbody.mass + GravityAmount) / Time.deltaTime,
                        m_Transform.position + MoveDirection, ForceMode.Force);
        ...
        private bool CheckGround()
        {
            ...
            var verticalMoveDirection = m_Transform.InverseTransformDirection(MoveDirection).y;
            var horizontalDirection = Vector3.ProjectOnPlane(MoveDirection, -m_GravityDirection);
            var localPlatformVerticalDirection =
                m_Platform != null ? m_Transform.InverseTransformDirection(PlatformMovement).y : 0;
            var horizontalCastOffset = horizontalDirection + -m_GravityDirection * localPlatformVerticalDirection;
            var hitCount = NonAllocCast(
                m_GravityDirection * (Mathf.Abs(verticalMoveDirection) + m_MaxStepHeight +
                                      (m_Grounded && m_StickToGround ? m_Stickiness : 0) + c_ColliderSpacing),
            ...
            for (var i = 0; i < hitCount; ++i)
            {
                ...
                if (closestRaycastHit.distance == 0)
                {
                    ...
                    {
                        var horizontalPlatformMovement = Vector3.ProjectOnPlane(PlatformMovement, Up);
                        if (SingleCast(activeCollider, horizontalDirection, horizontalPlatformMovement))
                        {
                            ...
                            if (groundPoint.y <= m_MaxStepHeight + c_ColliderSpacing)
                                ...
                                if (OverlapCount(activeCollider,
                                        horizontalDirection + horizontalPlatformMovement +
                                        Up * (m_MaxStepHeight - c_ColliderSpacing)) == 0)
                                    horizontalStep = true;
                        }
                    }
                    ...
                    var offset = Vector3.zero;
                    var overlap = !horizontalStep && UsingHorizontalCollisionDetection &&
                                  ComputePenetration(activeCollider, closestRaycastHit.collider, horizontalCastOffset,
                                      true, out offset);
                    if (overlap)
                        overlap = ComputePenetration(activeCollider, closestRaycastHit.collider, horizontalCastOffset,
                            false, out offset);
                    ...
                    if (!overlap)
                    {
                        ...
                        if (!UsingHorizontalCollisionDetection && Mathf.Abs(localOffset.y) <= 0.001f)
                            if (!SingleCast(activeCollider, m_GravityDirection * (m_MaxStepHeight + c_ColliderSpacing),
                                    Vector3.ProjectOnPlane(MoveDirection, -m_GravityDirection) +
                                    m_Transform.up * (m_MaxStepHeight + c_ColliderSpacing)))
                                continue;
                        ...
                        MoveDirection += m_Transform.TransformDirection(localOffset);

                        // One more cast should be performed to determine the ground object now that the character is in a non-overlapping position.
                        if (SingleCast(activeCollider,
                                m_GravityDirection * (m_MaxStepHeight + verticalOffset + c_ColliderSpacing),
                                Vector3.ProjectOnPlane(MoveDirection, -m_GravityDirection) + m_Transform.up *
                                (m_MaxStepHeight + verticalOffset + c_ColliderSpacing)))
                ...
                m_GroundRaycastOrigin = MathUtility.ClosestPointOnCollider(m_Transform, activeCollider,
                    m_GroundRaycastHit.point, MoveDirection, false, true);
                ...
                var localDirection =
                    m_Transform.InverseTransformDirection(m_GroundRaycastHit.point - m_GroundRaycastOrigin);
            ...
            if (grounded != m_Grounded || m_GroundRaycastHit.transform != m_GroundHitTransform)
            {
                ...
                if (target != m_GroundHitTransform)
                {
                    ...
                    if (!m_PlatformOverride) UpdateMovingPlatformTransform(m_GroundHitTransform, true);
                }
            }
        ...
        private bool ComputePenetration(Collider activeCollider, Collider hitCollider, Vector3 horizontalDirection,
            bool constantVelocity, out Vector3 offset)
        {
            ...
            while (iterations > 0)
            {
                if (Physics.ComputePenetration(activeCollider,
                        activeCollider.transform.position + horizontalDirection + offset,
                        activeCollider.transform.rotation, m_OverlapColliderHit[0],
                        m_OverlapColliderHit[0].transform.position, m_OverlapColliderHit[0].transform.rotation,
                ...
                if (constantVelocity)
                    ...
                    offset = (horizontalDirection + offset).normalized * horizontalDirection.magnitude -
                             horizontalDirection;
        ...
        private bool UpdateMovingPlatformTransform(Transform hitTransform, bool groundUpdate)
        {
            ...
            if (m_Platform != null && m_GroundedMovingPlatform != groundUpdate) return false;
            ...
            if (hitTransform != null)
            {
                ...
                {
                    if (hitTransform != m_Platform) SetPlatform(hitTransform, false);
            ...
            else if (m_Platform != null && hitTransform == null)
            {
                // The character is no longer on a moving platform.
        ...
        {
            if (m_Platform == platform) return false;
            m_Platform = platform;
            ...
            if (m_Platform != null)
            {
                var localDirection = m_Grounded
                    ? m_Transform.InverseTransformDirection(m_GroundRaycastHit.point - m_GroundRaycastOrigin +
                                                            Up * c_ColliderSpacing)
                    : Vector3.zero;
                PlatformRelativePosition = m_Platform.InverseTransformPoint(m_Transform.position) + localDirection;
        ...
        /// <summary>
        ///     Updates the slope factor. This gives the option of slowing the character down while moving up a slope or increasing
        ///     the speed while moving down.
        /// </summary>
        protected virtual void UpdateSlopeFactor()
        {
            ...
            SlopeFactor = 1 + (1 - Vector3.Angle(m_GroundRaycastHit.normal, MotorThrottle) / 90);

            if (Mathf.Abs(1 - SlopeFactor) <
                0.01f) // Standing still, moving on flat ground, or moving perpendicular to a slope.
                SlopeFactor = 1;
            else if (SlopeFactor > 1) // Downhill.
                SlopeFactor = m_MotorSlopeForceDown / SlopeFactor;
            else // Uphill.
                SlopeFactor *= m_MotorSlopeForceUp;
        }
        ...
        protected virtual bool UpdateGroundState(bool grounded, bool sendEvents)
        {
            ...
            if (m_Grounded != grounded)
            {
                ...
                if (m_Grounded)
                {
                    ...
                    PlatformMovement = m_PlatformDisconnectVelocity = Vector3.zero;
        ...
        {
            // Apply the position.
            var position = m_Transform.position + MoveDirection;

            Velocity = (position - PrevPosition) / (m_TimeScale * Time.deltaTime);
            m_Transform.position = PrevPosition = position;
            MoveDirection = Vector3.zero;
            // Update the platform variables.
            if (m_Platform != null) PlatformRelativePosition = m_Platform.InverseTransformPoint(PrevPosition);
        }
        ...
        /// <summary>
        ///     Casts a ray in the specified direction. If the character has multiple colliders added then a ray will be cast from
        ///     each collider.
        ///     A CapsuleCast or SphereCast is used depending on the type of collider that has been added. The result is stored in
        ///     the m_CombinedRaycastHits array.
        /// </summary>
        ...
        private int NonAllocCast(Vector3 direction, Vector3 offset)
        {
            ...
            for (var i = 0; i < m_ColliderCount; ++i)
            {
                ...
                if (m_Colliders[i] is CapsuleCollider)
                {
                    ...
                    MathUtility.CapsuleColliderEndCaps(capsuleCollider, colliderTrasnform.position + offset,
                        colliderTrasnform.rotation, out startEndCap, out endEndCap);
                    var radius = capsuleCollider.radius * MathUtility.ColliderRadiusMultiplier(capsuleCollider) -
                                 c_ColliderSpacing;
                    localHitCount = Physics.CapsuleCastNonAlloc(startEndCap, endEndCap, radius, direction.normalized,
                        m_RaycastHits, direction.magnitude + c_ColliderSpacing,
                ...
                else
                {
                    ...
                    var radius = sphereCollider.radius * MathUtility.ColliderRadiusMultiplier(sphereCollider) -
                                 c_ColliderSpacing;
                    localHitCount = Physics.SphereCastNonAlloc(
                        sphereCollider.transform.TransformPoint(sphereCollider.center) + offset, radius,
                ...
                if (localHitCount > 0)
                {
                    ...
                    {
                        var validHitCount = 0;
                        for (var j = 0; j < localHitCount; ++j)
                        {
                            if (m_ColliderIndexMap.ContainsKey(m_RaycastHits[j])) continue;
                            // Ensure the array is large enough.
                            if (hitCount + validHitCount >= m_CombinedRaycastHits.Length)
                            {
                                Debug.LogWarning(
                                    "Warning: The maximum number of collisions has been reached. Consider increasing the CharacterLocomotion MaxCollisionCount value.");
                                continue;
                            }
        ...
        /// <summary>
        ///     Casts a ray using in the specified direction.  A CapsuleCast or SphereCast is used depending on the type of
        ///     collider that has been added.
        ...
        private bool SingleCast(Collider castCollider, Vector3 direction, Vector3 offset)
        {
            ...
            if (castCollider is CapsuleCollider)
            {
                ...
                MathUtility.CapsuleColliderEndCaps(capsuleCollider, colliderTransform.position + offset,
                    colliderTransform.rotation, out startEndCap, out endEndCap);
                var radius = capsuleCollider.radius * MathUtility.ColliderRadiusMultiplier(capsuleCollider) -
                             c_ColliderSpacing;
                return Physics.CapsuleCast(startEndCap, endEndCap, radius, direction.normalized, out m_RaycastHit,
                    direction.magnitude + c_ColliderSpacing, m_CharacterLayerManager.SolidObjectLayers,
                    QueryTriggerInteraction.Ignore);
            }
            else
            {
                ...
                var radius = sphereCollider.radius * MathUtility.ColliderRadiusMultiplier(sphereCollider) -
                             c_ColliderSpacing;
                return Physics.SphereCast(sphereCollider.transform.TransformPoint(sphereCollider.center) + offset,
                    radius, direction.normalized,
                    out m_RaycastHit, direction.magnitude + c_ColliderSpacing,
                    m_CharacterLayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
            }
        }
        ...
        protected int OverlapCount(Collider overlapCollider, Vector3 offset)
        {
            if (overlapCollider is CapsuleCollider)
            {
                ...
                MathUtility.CapsuleColliderEndCaps(capsuleCollider, colliderTransform.position + offset,
                    colliderTransform.rotation, out startEndCap, out endEndCap);
                return Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap,
                    capsuleCollider.radius * MathUtility.ColliderRadiusMultiplier(capsuleCollider) - c_ColliderSpacing,
        ...
        /// <summary>
        ///     Resets the m_CombinedRaycastHits array to blank RaycastHit objects. This will prevent old hit objects from being
        ///     used during the current frame.
        /// </summary>
        private void ResetCombinedRaycastHits()
        {
            if (m_CombinedRaycastHits == null) return;

            for (var i = 0; i < m_CombinedRaycastHits.Length; ++i)
            {
                if (m_CombinedRaycastHits[i].collider == null) break;
        ...
        /// <summary>
        ///     If the collision layer is disabled then all of the character's colliders will be set to an IgnoreRaycast layer.
        ///     This
        ...
        public void EnableColliderCollisionLayer(bool enable)
        {
            ...
            if (CollisionLayerEnabled == enable) return;
            CollisionLayerEnabled = enable;

            if (enable)
            {
                for (var i = 0; i < m_ColliderCount; ++i) m_ColliderGameObjects[i].layer = m_ColliderLayers[i];
                for (var i = 0; i < m_IgnoredColliderCount; ++i)
        ...
        {
            for (var i = 0; i < m_ColliderCount; ++i) m_ColliderLayers[i] = m_ColliderGameObjects[i].layer = layer;
        }
        ...
        {
            m_ColliderCount = AddCollider(addCollider, ref m_Colliders, ref m_ColliderLayers, ref m_ColliderGameObjects,
                m_ColliderCount);
        ...
        private int AddCollider(Collider addCollider, ref Collider[] existingColliders,
            ref int[] existingColliderLayers, ref GameObject[] existingColliderGameObjects, int existingColliderCount)
        {
            // Don't add an already added collider.
            for (var i = 0; i < existingColliderCount; ++i)
            ...
            if (!CollisionLayerEnabled)
            {
                existingColliderLayers[existingColliderCount] =
                    existingColliderGameObjects[existingColliderCount].layer;
        ...
        {
            m_ColliderCount = RemoveCollider(removeCollider, ref m_Colliders, ref m_ColliderLayers,
                ref m_ColliderGameObjects, m_ColliderCount);
        }
        ...
        public int RemoveCollider(Collider removeCollider, ref Collider[] existingColliders,
            ref int[] existingColliderLayers, ref GameObject[] existingColliderGameObjects, int existingColliderCount)
        {
            for (var i = existingColliders.Length - 1; i > -1; --i)
            {
                if (existingColliders[i] != removeCollider) continue;
                // The collider may be removed when collisions are disabled. The layer should be reverted back to its original.
                if (!CollisionLayerEnabled) existingColliderGameObjects[i].layer = existingColliderLayers[i];

                // Do not resize the array for performance reasons. Move all of the next colliders back a slot instead.
                for (var j = i; j < existingColliderCount - 1; ++j)
                {
        ...
        {
            m_ColliderCount = AddColliders(colliders, ref m_Colliders, ref m_ColliderLayers, ref m_ColliderGameObjects,
                m_ColliderCount);
        ...
        /// <summary>
        ///     Adds the colliders array to the existing colliders array. The existing colliders array length will be resized if
        ///     the new
        ...
        /// <param name="existingColliderCount">The count of the existing colliders array.</param>
        /// <returns>The new count of the existing colliders array.</returns>
        private int AddColliders(Collider[] colliders, ref Collider[] existingColliders,
            ref int[] existingColliderLayers, ref GameObject[] existingColliderGameObjects, int existingColliderCount)
        {
        ...
        {
            m_ColliderCount = RemoveColliders(colliders, ref m_Colliders, ref m_ColliderLayers,
                ref m_ColliderGameObjects, m_ColliderCount);
        }
        ...
        /// <param name="existingColliderCount">The count of the existing colliders array.</param>
        /// <returns>The number of colliders within the existing colliders array.</returns>
        private int RemoveColliders(Collider[] colliders, ref Collider[] existingColliders,
            ref int[] existingColliderLayers, ref GameObject[] existingColliderGameObjects, int existingColliderCount)
        {
            for (var i = existingColliderCount - 1; i > -1; --i)
            for (var j = colliders.Length - 1; j > -1; --j)
            {
                if (existingColliders[i] != colliders[j]) continue;
                // The collider may be removed when collisions are disabled. The layer should be reverted back to its original.
                if (!CollisionLayerEnabled) existingColliderGameObjects[i].layer = existingColliderLayers[i];

                // Do not resize the array for performance reasons. Move all of the next colliders back a slot instead.
                for (var k = i; k < existingColliderCount - 1; ++k)
                {
        ...
        {
            m_IgnoredColliderCount = AddCollider(addCollider, ref m_IgnoredColliders, ref m_IgnoredColliderLayers,
                ref m_IgnoredColliderGameObjects, m_IgnoredColliderCount);
        }
        ...
        {
            m_IgnoredColliderCount = RemoveCollider(removeCollider, ref m_IgnoredColliders, ref m_IgnoredColliderLayers,
                ref m_IgnoredColliderGameObjects, m_IgnoredColliderCount);
        }
        ...
        {
            m_IgnoredColliderCount = AddColliders(colliders, ref m_IgnoredColliders, ref m_IgnoredColliderLayers,
                ref m_IgnoredColliderGameObjects, m_IgnoredColliderCount);
        }
        ...
        {
            m_IgnoredColliderCount = RemoveColliders(colliders, ref m_IgnoredColliders, ref m_IgnoredColliderLayers,
                ref m_IgnoredColliderGameObjects, m_IgnoredColliderCount);
        }
        ...
        private void AddExternalForce(Vector3 force, bool scaleByTime)
        {
            ...
            if (scaleByTime)
            {
                ...
                if (timeScale < 1) force /= timeScale;
            }
        ...
        private void AddSoftForce(Vector3 force, float frames, bool scaleByTime)
        {
            ...
            for (var v = 0; v < Mathf.RoundToInt(frames) - 1; v++) m_SoftForceFrames[v] += force / (frames * timeScale);
        }
        ...
        /// <summary>
        ///     Adds a force relative to the character in the specified number of frames. The force will either be an external or
        ///     soft force.
        /// </summary>
        /// <param name="force">The force to add.</param>
        /// <param name="frames">The number of frames to add the force to.</param>
        ...
        protected virtual bool PushRigidbody(Rigidbody targetRigidbody, Vector3 moveDirection, Vector3 point,
            float radius)
        {
            if (targetRigidbody.isKinematic || !MathUtility.InLayerMask(targetRigidbody.gameObject.layer,
                    m_CharacterLayerManager.SolidObjectLayers)) return false;

            targetRigidbody.AddForceAtPosition(m_Mass / targetRigidbody.mass * 0.01f * (moveDirection / Time.deltaTime),
        ...
        {
            m_Transform.rotation = MotorRotation = PrevMotorRotation = rotation;
            m_LocalRootMotionRotation = m_Torque = Quaternion.identity;
            Up = m_Transform.up;
            if (AlignToGravity) m_GravityDirection = -Up;
        }
        ...
        {
            m_Transform.position = PrevPosition = position;
            MotorThrottle = m_LocalRootMotionForce = MoveDirection = Velocity = m_ExternalForce = Vector3.zero;
            GravityAmount = 0;
            ...
            if (m_Platform != null)
            {
                if (m_PlatformOverride)
                    PlatformRelativePosition = m_Platform.InverseTransformPoint(m_Transform.position);
                else
        ...
        public void AddDefaultSmoothedBones()
        {
            ...
            if (bones.Count > 0) m_SmoothedBones = bones.ToArray();
        }
        ...
        private void AddBone(List<Transform> bones, Animator animator, HumanBodyBones bone)
        {
            ...
            if (boneTransform != null) bones.Add(boneTransform);
        }
    }
}