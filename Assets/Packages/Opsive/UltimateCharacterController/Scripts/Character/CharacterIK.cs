...
{
    /// <summary>
    ///     Allows the character to stand on uneven surfaces and rotates and positions the character's limbs to face in the
    ///     look direction.
    /// </summary>
    public class CharacterIK : CharacterIKBase
    {
#if UNITY_EDITOR
        [Tooltip("Draw a debug line to see the direction that the character is facing (editor only).")] [SerializeField]
        protected bool m_DebugDrawLookRay;
#endif
        [Tooltip("The index of the base layer within the Animator Controller.")] [SerializeField]
        protected int m_BaseLayerIndex;

        [Tooltip("The index of the upper body layer within the Animator Controller.")] [SerializeField]
        protected int m_UpperBodyLayerIndex = 4;

        [Tooltip("The layers that the component should use when determining the objects to test against.")]
        [SerializeField]
        protected LayerMask m_LayerMask = ~((1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.TransparentFX) |
        ...
        [Tooltip("An offset to apply to the look at direction for the body and arms.")] [SerializeField]
        protected Vector3 m_LookAtOffset;
        ...
        [Range(0, 1)]
        [SerializeField]
        protected float m_LookAtEyesWeight = 1;
        ...
        [Tooltip("The adjustment speed to apply to the Look At position.")] [SerializeField]
        protected float m_LookAtAdjustmentSpeed = 0.5f;

        [Tooltip("The name of the state when the character is actively looking at a target.")] [SerializeField]
        protected string m_ActiveLookAtStateName = "LookAt";

        [Tooltip("The speed at which the look at weight should adjust.")] [SerializeField]
        protected float m_LookAtWeightAdjustmentSpeed = 0.2f;

        [Tooltip("The speed at which the hips position should adjust to using IK and not using IK.")] [SerializeField]
        protected float m_HipsPositionAdjustmentSpeed = 4;

        [InspectorFoldout("Feet")]
        [Tooltip("The offset of the foot between the foot bone and the base of the foot.")]
        [SerializeField]
        protected float m_FootOffsetAdjustment = 0.005f;

        [Tooltip("The speed at which the foot weight should adjust to when foot IK is active.")] [SerializeField]
        protected float m_FootWeightActiveAdjustmentSpeed = 10;

        [Tooltip("The speed at which the foot weight should adjust to when foot IK is inactive.")] [SerializeField]
        protected float m_FootWeightInactiveAdjustmentSpeed = 2;

        [Tooltip("The overridden weight for foot IK. Set to -1 to disable.")] [SerializeField]
        protected float m_OverrideFootIKWeight = -1f;

        [InspectorFoldout("Upper Arm")]
        [Tooltip("Determines how much weight is applied to the upper arms when looking at the target (0-1).")]
        [Range(0, 1)]
        [SerializeField]
        protected float m_UpperArmWeight = 1;
        ...
        [Range(0, 1)]
        [SerializeField]
        protected float m_HandWeight = 1;

        [Tooltip("The speed at which the hand position/rotation should adjust to using IK and not using IK.")]
        [SerializeField]
        protected float m_HandAdjustmentSpeed = 10;

        [Tooltip("Specifies a local offset to add to the position of the hands.")] [SerializeField]
        protected Vector3 m_HandPositionOffset;

        [Tooltip("The left hand positional spring used for IK movement.")] [SerializeField]
        protected Spring m_LeftHandPositionSpring = new();

        [Tooltip("The left hand rotational spring used for IK movement.")] [SerializeField]
        protected Spring m_LeftHandRotationSpring = new(0.2f, 0.05f);

        [Tooltip("The right hand positional spring used for IK movement.")] [SerializeField]
        protected Spring m_RightHandPositionSpring = new();

        [Tooltip("The right hand rotational spring used for IK movement.")] [SerializeField]
        protected Spring m_RightHandRotationSpring = new(0.2f, 0.05f);
        ...
        private readonly float[] m_FootIKWeight = new float[2];
        private readonly float[] m_FootOffset = new float[2];

        private GameObject m_GameObject;
        private readonly float[] m_GroundDistance = new float[2];
        private readonly Vector3[] m_GroundNormal = new Vector3[2];
        private readonly Vector3[] m_GroundPoint = new Vector3[2];
        private Vector3 m_HandOffset;
        private readonly float[] m_HandPositionIKWeight = new float[2];

        private readonly float[] m_HandRotationIKWeight = new float[2];
        private readonly int[] m_HandSlotID = new int[2];
        ...
        private readonly float[] m_MaxLegLength = new float[2];
        ...
        private readonly float[] m_RaycastDistance = new float[2];
        ...
        {
            get => m_LookAtBodyWeight;
            set => m_LookAtBodyWeight = value;
        }

        public float LookAtHeadWeight
        {
            get => m_LookAtHeadWeight;
            set => m_LookAtHeadWeight = value;
        }

        public float LookAtEyesWeight
        {
            get => m_LookAtEyesWeight;
            set => m_LookAtEyesWeight = value;
        }

        public float LookAtClampWeight
        {
            get => m_LookAtClampWeight;
            set => m_LookAtClampWeight = value;
        }

        public float LookAtAdjustmentSpeed
        {
            get => m_LookAtAdjustmentSpeed;
            set => m_LookAtAdjustmentSpeed = value;
        }

        public string ActiveLookAtStateName
        {
            get => m_ActiveLookAtStateName;
            set => m_ActiveLookAtStateName = value;
        }

        public float LookAtWeightAdjustmentSpeed
        {
            get => m_LookAtWeightAdjustmentSpeed;
            set => m_LookAtWeightAdjustmentSpeed = value;
        }

        public float HipsPositionAdjustmentSpeed
        {
            get => m_HipsPositionAdjustmentSpeed;
            set
            {
        ...
        public float FootOffsetAdjustment
        {
            get => m_FootOffsetAdjustment;
            set => m_FootOffsetAdjustment = value;
        }

        public float FootWeightActiveAdjustmentSpeed
        {
            get => m_FootWeightActiveAdjustmentSpeed;
            set => m_FootWeightActiveAdjustmentSpeed = value;
        }

        public float FootWeightInactiveAdjustmentSpeed
        {
            get => m_FootWeightInactiveAdjustmentSpeed;
            set => m_FootWeightInactiveAdjustmentSpeed = value;
        }

        public float OverrideFootIKWeight
        {
            get => m_OverrideFootIKWeight;
            set => m_OverrideFootIKWeight = value;
        }

        public float UpperArmWeight
        {
            get => m_UpperArmWeight;
            set => m_UpperArmWeight = value;
        }

        public float UpperArmAdjustmentSpeed
        {
            get => m_UpperArmAdjustmentSpeed;
            set => m_UpperArmAdjustmentSpeed = value;
        }

        public float HandWeight
        {
            get => m_HandWeight;
            set => m_HandWeight = value;
        }

        public float HandAdjustmentSpeed
        {
            get => m_HandAdjustmentSpeed;
            set => m_HandAdjustmentSpeed = value;
        }

        public Vector3 HandPositionOffset
        {
            get => m_HandPositionOffset;
            set => m_HandPositionOffset = value;
        }

        public Spring LeftHandPositionSpring
        {
            get => m_LeftHandPositionSpring;
            set
            {
        ...
        protected override void Awake()
        {
            ...
            for (var i = 0; i < m_StartInterpolation.Length; ++i) m_StartInterpolation[i] = -1;
        ...
        private bool InitializeBones(bool fromAwake)
        {
            ...
            {
                if (!fromAwake) Debug.LogError("Error: The CharacterIK component only works with humanoid models.");
                return false;
            }
            ...
            if (m_LeftFoot == null || m_RightFoot == null)
            {
                if (!fromAwake)
                    Debug.LogError(
                        $"Error: The Left or Right foot bone is not assigned to the character {m_GameObject.name}.");
                return false;
            }
            ...
            if (m_LeftHand == null || m_RightHand == null)
            {
                if (!fromAwake)
                    Debug.LogError(
                        $"Error: The Left or Right hand bone is not assigned to the character {m_GameObject.name}.");
                return false;
            }
            ...
            for (var i = 0; i < 2; ++i)
            {
                ...
                m_MaxLegLength[i] =
                    m_Transform.InverseTransformPoint(i == 0 ? m_LeftLowerLeg.position : m_RightLowerLeg.position).y -
            ...
            if (itemSlot != null) m_HandSlotID[0] = itemSlot.ID;
            itemSlot = m_RightHand.GetComponentInChildren<ItemSlot>();
            if (itemSlot != null) m_HandSlotID[1] = itemSlot.ID;
        ...
        private void OnAttachLookSource(ILookSource lookSource)
        {
            ...
            if (enabled && !prevEnabled && m_Animator.isActiveAndEnabled) m_Animator.Update(0);
        }
        ...
        {
            if (active) m_LookAtTargetPosition = position;
            if (m_ActiveLookAtTarget != active)
            {
                if (!string.IsNullOrEmpty(m_ActiveLookAtStateName))
                    StateManager.SetState(m_GameObject, m_ActiveLookAtStateName, active);
        ...
        public override void SetItemIKTargets(Transform itemTransform, Transform itemHand,
            Transform nonDominantHandTarget, Transform nonDominantHandElbowTarget)
        {
        ...
        private void UpdateIKTargets()
        {
            ...
            m_IKTarget[(int)IKGoal.LeftHand] = m_StartInterpolation[(int)IKGoal.LeftHand] != -1
                ?
                m_InterpolationTarget[(int)IKGoal.LeftHand]
                :
                m_AbilityIKTarget[(int)IKGoal.LeftHand] != null
                    ? m_AbilityIKTarget[(int)IKGoal.LeftHand]
                    : m_LeftHandItemIKTarget;
            m_IKTarget[(int)IKGoal.LeftElbow] = m_StartInterpolation[(int)IKGoal.LeftElbow] != -1
                ?
                m_InterpolationTarget[(int)IKGoal.LeftElbow]
                :
                m_AbilityIKTarget[(int)IKGoal.LeftElbow] != null
                    ? m_AbilityIKTarget[(int)IKGoal.LeftElbow]
                    : m_LeftHandItemIKHintTarget;
            m_IKTarget[(int)IKGoal.RightHand] = m_StartInterpolation[(int)IKGoal.RightHand] != -1
                ?
                m_InterpolationTarget[(int)IKGoal.RightHand]
                :
                m_AbilityIKTarget[(int)IKGoal.RightHand] != null
                    ? m_AbilityIKTarget[(int)IKGoal.RightHand]
                    : m_RightHandItemIKTarget;
            m_IKTarget[(int)IKGoal.RightElbow] = m_StartInterpolation[(int)IKGoal.RightElbow] != -1
                ?
                m_InterpolationTarget[(int)IKGoal.RightElbow]
                :
                m_AbilityIKTarget[(int)IKGoal.RightElbow] != null
                    ? m_AbilityIKTarget[(int)IKGoal.RightElbow]
                    : m_RightHandItemIKHintTarget;
            for (var i = 0; i < (int)IKGoal.LeftFoot; ++i)
            {
                m_RequireSecondHandPositioning = m_IKTarget[i] != null;
                if (m_RequireSecondHandPositioning) break;
            }

            // The feet targets are not affected by items so they can easily be iterated.
            for (var i = (int)IKGoal.LeftFoot; i < (int)IKGoal.Last; ++i)
        ...
        {
            if (m_Inventory == null) m_Inventory = m_GameObject.GetCachedComponent<InventoryBase>();
            ...
            {
                m_Unequipping = false;
                if (dominantItem.SlotID == m_HandSlotID[0])
                {
                    // Left Hand.
                ...
                else if (dominantItem.SlotID == m_HandSlotID[1])
                {
                    // Right Hand.
        ...
        {
            if (useAbility.SlotID == -1 || useAbility.SlotID == m_DominantSlotID) m_ItemInUse = start;
        }
        ...
        public override void Move(bool fixedUpdate)
        {
            ...
            // during runtime.
            if (m_ImmediatePosition) m_ImmediatePosition = false;
        }
        ...
        private void PositionLowerBody()
        {
            ...
            if (m_OverrideFootIKWeight > 0 ||
                (m_CharacterLocomotion.Grounded && m_CharacterLocomotion.UsingVerticalCollisionDetection))
                ...
                for (var i = 0; i < 2; ++i)
                {
                    ...
                    if (Physics.Raycast(GetFootRaycastPosition(target, lowerLeg, out distance),
                            -m_CharacterLocomotion.Up, out m_RaycastHit,
                            distance + m_FootOffset[i] + m_MaxLegLength[i], m_LayerMask,
                    ...
                    if (target != null && Physics.Raycast(GetFootRaycastPosition(target, lowerLeg, out distance),
                            -m_CharacterLocomotion.Up, out m_RaycastHit,
                            distance + m_FootOffset[i] + m_MaxLegLength[i], m_LayerMask,
                            QueryTriggerInteraction.Ignore) &&
                        m_Transform.InverseTransformPoint(m_RaycastHit.point).y < m_CharacterLocomotion.MaxStepHeight)
                        ...
                        if (m_RaycastHit.distance + m_CharacterLocomotion.ColliderSpacing < m_GroundDistance[i] &&
                            m_RaycastHit.normal == m_CharacterLocomotion.Up)
                        {
                            m_RaycastDistance[i] = distance * m_Transform.lossyScale.y;
                            m_GroundDistance[i] = m_RaycastHit.distance;
                            m_GroundPoint[i] = m_RaycastHit.point;
                    ...
                    if (m_GroundDistance[i] != float.MaxValue)
                    {
                        ...
                        float offset;
                        var foot = i == 0 ? m_LeftFoot : m_RightFoot;
                        if ((offset = m_GroundDistance[i] - m_RaycastDistance[i] -
                                      m_Transform.InverseTransformPoint(foot.position).y) >
                            hipsOffset) hipsOffset = offset;
                    }
                }

            // Smoothly position the hips.
            m_HipsOffset = m_ImmediatePosition
                ? hipsOffset
                : Mathf.Lerp(m_HipsOffset, hipsOffset, m_HipsPositionAdjustmentSpeed * Time.deltaTime);
            m_HipsPosition = m_Transform.InverseTransformPoint(m_Hips.position);
            ...
            for (var i = 0; i < 2; ++i)
            {
                ...
                else
                {
                    ...
                    if (m_CharacterLocomotion.Grounded)
                        ...
                        if (m_GroundDistance[i] != float.MaxValue && m_GroundDistance[i] > 0 &&
                            m_Transform.InverseTransformDirection(position - m_GroundPoint[i]).y - m_FootOffset[i] -
                            m_HipsOffset < 0)
                        {
                            ...
                            position = m_Transform.TransformPoint(localFootPosition) +
                                       m_CharacterLocomotion.Up * (m_FootOffset[i] + m_HipsOffset);
                            rotation = Quaternion.LookRotation(
                                Vector3.Cross(m_GroundNormal[i], rotation * -Vector3.right), m_CharacterLocomotion.Up);
                            targetWeight = 1f;
                            adjustmentSpeed = m_FootWeightActiveAdjustmentSpeed;
                        }
                }
                ...
                if (adjustmentSpeed == 0)
                    ...
                    m_FootIKWeight[i] = Mathf.Clamp01(m_ImmediatePosition
                        ? targetWeight
                        : Mathf.MoveTowards(m_FootIKWeight[i], targetWeight, adjustmentSpeed * Time.deltaTime));
        ...
        private void LookAtTarget()
        {
            ...
            m_LookAtPosition = m_ImmediatePosition
                ? lookAtPosition
                : Vector3.Lerp(m_LookAtPosition, lookAtPosition, m_LookAtAdjustmentSpeed);
        ...
        private void RotateHands()
        {
            ...
            m_NonDominantHandOffset = MathUtility.InverseTransformPoint(m_Animator.GetIKPosition(dominantHandGoal),
                m_Animator.GetIKRotation(dominantHandGoal), m_Animator.GetIKPosition(nonDominantHandGoal));
            ...
            for (var i = 0; i < 2; ++i)
            {
                ...
                // is being used.
                var targetWeight = 0f;
                if (m_HandWeight > 0 && (ikTarget != null || m_Aiming || m_ItemInUse ||
                                         m_CharacterLocomotion.FirstPersonPerspective ||
                                         m_OnUpdateIKRotation != null)) targetWeight = m_HandWeight;

                m_HandRotationIKWeight[i] = Mathf.Clamp01(m_ImmediatePosition
                    ? targetWeight
                    : Mathf.MoveTowards(m_HandRotationIKWeight[i], targetWeight,
                        m_HandAdjustmentSpeed * Time.deltaTime));
                ...
                {
                    targetWeight = 1;
                    targetRotation = m_OnUpdateIKRotation(i == 0 ? IKGoal.LeftHand : IKGoal.RightHand, targetRotation,
                        m_Animator.GetIKPosition(ikGoal));
                }
        ...
        {
            if (m_HandRotationIKWeight[leftHand ? 0 : 1] == 0) return m_Animator.GetIKRotation(ikGoal);

            // Use the distant hand so the hands are always pointing in the same direction.
            if (distantHand == null)
            {
                if (m_Transform.InverseTransformPoint(m_RightHand.position).z <
                    m_Transform.InverseTransformPoint(m_LeftHand.position).z)
                    distantHand = m_LeftHand;
                else
                    distantHand = m_RightHand;
            }

            var lookDirection = (m_LookSource.LookDirection(distantHand.position, false, 0, true, false) +
                                 m_Transform.TransformDirection(m_LookAtOffset)).normalized;
            return Quaternion.LookRotation(lookDirection, m_CharacterLocomotion.Up) *
                   Quaternion.Inverse(m_Transform.rotation) *
                   Quaternion.Euler(m_Transform.TransformDirection(leftHand
                       ? m_LeftHandRotationSpring.Value
                       : m_RightHandRotationSpring.Value)) *
        ...
        {
            var targetWeight = 0f;
            if (m_DominantUpperArm != null && m_UpperArmWeight > 0 && !m_Unequipping) targetWeight = m_UpperArmWeight;

            var prevUpperArmWeight = m_DominantUpperArmWeight;
            m_DominantUpperArmWeight = Mathf.Clamp01(m_ImmediatePosition
                ? targetWeight
                : Mathf.MoveTowards(m_DominantUpperArmWeight, targetWeight,
                    m_UpperArmAdjustmentSpeed * Time.deltaTime));
            if (prevUpperArmWeight > 0 && m_DominantUpperArmWeight == 0) DetermineDominantHand();
            if (m_DominantUpperArm != null)
            {
                if (m_DominantUpperArmWeight > 0)
                {
                    // The dominant upper arm should rotate to face the target.
                    var localLookDirection =
                        m_Transform.InverseTransformDirection(m_LookSource.LookDirection(m_DominantUpperArm.position,
                            false, 0, true, false));
                    ...
                    if (localLookDirection.y < 0)
                        lookDirection = Vector3.Lerp(m_Transform.forward, lookDirection,
                            1 - Mathf.Abs(localLookDirection.y));

                    var dominantUpperArmRotation = m_DominantUpperArm.rotation;
                    var targetRotation = Quaternion.FromToRotation(m_Transform.forward, lookDirection) *
                                         dominantUpperArmRotation;
                    targetRotation =
                        Quaternion.Slerp(dominantUpperArmRotation, targetRotation, m_DominantUpperArmWeight);

                    // When the hand IK positions are set they should use the updated rotation.
                    var offset = Vector3.Scale(m_DominantUpperArm.InverseTransformPoint(m_DominantHand.position),
                        m_DominantHand.lossyScale);
                    m_DominantHandPosition =
                        MathUtility.TransformPoint(m_DominantUpperArm.position, targetRotation, offset);
                    ...
                    {
                        m_NonDominantHandPosition = MathUtility.TransformPoint(m_DominantHandPosition,
                            m_Animator.GetIKRotation(m_DominantHand == m_RightHand
                                ? AvatarIKGoal.RightHand
                                : AvatarIKGoal.LeftHand), m_NonDominantHandOffset);
                    }
                    else
                    {
                        offset = Vector3.Scale(m_DominantUpperArm.InverseTransformPoint(m_NonDominantHand.position),
                            m_NonDominantHand.lossyScale);
                        m_NonDominantHandPosition =
                            MathUtility.TransformPoint(m_DominantUpperArm.position, targetRotation, offset);
                    }
                }
                else if (m_DominantHand != null)
                {
                    ...
                    m_HandOffset = Vector3.Scale(m_DominantHand.InverseTransformPoint(m_NonDominantHand.position),
                        m_NonDominantHand.lossyScale);
                }
            }
        }
        ...
        {
            if (!m_InterpolateIKTargets) return;

            m_InterpolateIKTargets = false;
            var updateIKTargets = false;
            Transform distantHand = null;
            for (var i = 0; i < (int)IKGoal.Last; ++i)
                if (m_StartInterpolation[i] != -1)
                {
                    ...
                    var time = m_InterpolationDuration[i] > 0
                        ? Mathf.Clamp01((Time.time - m_StartInterpolation[i]) / m_InterpolationDuration[i])
                        : 1;
                    if (m_AbilityIKTarget[i] == null)
                    {
                        m_InterpolationTarget[i].SetPositionAndRotation(
                            Vector3.Lerp(m_InterpolationTarget[i].position, ikPosition, time),
                            Quaternion.Slerp(m_InterpolationTarget[i].rotation, ikRotation, time));
                        if (time == 1)
                        {
                    ...
                    {
                        m_InterpolationTarget[i].SetPositionAndRotation(
                            Vector3.Lerp(ikPosition, m_AbilityIKTarget[i].position, time),
                            Quaternion.Slerp(ikRotation, m_AbilityIKTarget[i].rotation, time));
                    }

                    m_InterpolateIKTargets = true;
                }

            if (updateIKTargets) UpdateIKTargets();
        }
        ...
        private void PositionHands()
        {
            for (var i = 0; i < 2; ++i)
            {
                ...
                // is being used.
                if (ikTarget != null || m_Aiming || m_ItemInUse || m_CharacterLocomotion.FirstPersonPerspective ||
                    m_UpperArmWeight > 0 || m_OnUpdateIKPosition != null) targetWeight = m_HandWeight;

                m_HandPositionIKWeight[i] = Mathf.Clamp01(m_ImmediatePosition
                    ? targetWeight
                    : Mathf.MoveTowards(m_HandPositionIKWeight[i], targetWeight,
                        m_HandAdjustmentSpeed * Time.deltaTime));
                ...
                if (m_HandPositionIKWeight[i] > 0)
                {
                    if (ikTarget != null)
                    {
                        ...
                        if (hintTarget != null) m_Animator.SetIKHintPosition(hintGoal, hintTarget.position);
                    }
                    else
                    {
                ...
                {
                    targetWeight = 1;
                    targetPosition = m_OnUpdateIKPosition(i == 0 ? IKGoal.LeftHand : IKGoal.RightHand, targetPosition,
                        m_Animator.GetIKRotation(ikGoal));
                }
        ...
        private Vector3 GetTargetHandPosition(Transform hand, bool leftHand)
        {
            ...
            if (m_DominantUpperArmWeight > 0)
                ...
                handPosition = hand == m_DominantHand || m_DominantHand == null
                    ? hand.position
                    : m_DominantHand.TransformPoint(m_HandOffset);
            return handPosition +
                   m_Transform.TransformDirection(leftHand
                       ? m_LeftHandPositionSpring.Value
                       : m_RightHandPositionSpring.Value) +
