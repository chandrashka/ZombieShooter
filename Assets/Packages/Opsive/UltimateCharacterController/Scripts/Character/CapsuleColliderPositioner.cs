...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    {
        [Tooltip("Should the positioner rotate the collider to match the targets?")] [SerializeField]
        protected bool m_RotateCollider;

        [Tooltip(
            "If Rotate Collider is enabled, should the collider be rotated based on the end caps? If false the collider will be rotated based on the specified bone.")]
        [SerializeField]
        protected bool m_EndCapRotation = true;

        [Tooltip("Specifies the bone that the collider should rotate with if not using End Cap Rotation.")]
        [SerializeField]
        protected Transform m_RotationBone;

        [Tooltip("An offset to apply to the rotation bone.")] [SerializeField]
        protected Vector3 m_RotationBoneOffset;

        [Tooltip("Should the positioner adjust the height of the collider?")] [SerializeField]
        protected bool m_AdjustHeight = true;

        [Tooltip("A reference to the target that is near the first end cap.")] [SerializeField]
        protected Transform m_FirstEndCapTarget;

        [Tooltip("A reference to the target that is near the second end cap.")] [SerializeField]
        protected Transform m_SecondEndCapTarget;

        [Tooltip("The padding on top of the second end cap target.")] [SerializeField]
        protected float m_SecondEndCapPadding;

        [Tooltip("Specifies the bone that the collider should positioned to if not using Adjust Height.")]
        [SerializeField]
        protected Transform m_PositionBone;

        [Tooltip("The overridden height if the height is not being adjusted. Set to -1 to disable.")] [SerializeField]
        protected float m_HeightOverride = -1;

        [Tooltip("The offset to apply to the collider's center position.")] [SerializeField]
        ...
        public bool RotateCollider
        {
            get => m_RotateCollider;
            set
            {
        ...
        public bool EndCapRotation
        {
            get => m_EndCapRotation;
            set => m_EndCapRotation = value;
        }
        ...
        {
            get => m_RotationBone;
            set => m_RotationBone = value;
        }

        public Vector3 RotationBoneOffset
        {
            get => m_RotationBoneOffset;
            set => m_RotationBoneOffset = value;
        }
        ...
        [NonSerialized]
        public Transform FirstEndCapTarget
        {
            get => m_FirstEndCapTarget;
            set => m_FirstEndCapTarget = value;
        }

        [NonSerialized]
        public Transform SecondEndCapTarget
        {
            get => m_SecondEndCapTarget;
            set
            {
                m_SecondEndCapTarget = value;
                Initialize();
            }
        }

        [NonSerialized]
        public float SecondEndCapPadding
        {
            get => m_SecondEndCapPadding;
            set => m_SecondEndCapPadding = value;
        }
        ...
        {
            get => m_PositionBone;
            set => m_PositionBone = value;
        }

        public float HeightOverride
        {
            get => m_HeightOverride;
            set => m_HeightOverride = value;
        }
        ...
        protected override void Awake()
        {
            ...
            {
                Debug.LogError(
                    "Error: The CapsuleColliderPositioner only works with capsule colliders that are in the Y-axis direction.");
                enabled = false;
                return;
            }
            ...
            EventHandler.RegisterEvent<bool>(m_CharacterGameObject, "OnCharacterImmediateTransformChange",
        ...
        private void OnDestroy()
        {
            ...
            EventHandler.UnregisterEvent<float>(m_CharacterGameObject, "OnHeightChangeAdjustHeight",
                AdjustCapsuleColliderHeight);
            EventHandler.UnregisterEvent<bool>(m_CharacterGameObject, "OnCharacterImmediateTransformChange",
                OnImmediateTransformChange);
        ...
        private void Initialize()
        {
            ...
            if (!Application.isPlaying || (m_FirstEndCapTarget == null && m_SecondEndCapTarget == null)) return;
            ...
            if (m_SecondEndCapTarget == null)
            {
                ...
                EventHandler.RegisterEvent<float>(m_CharacterGameObject, "OnHeightChangeAdjustHeight",
                    AdjustCapsuleColliderHeight);
            }

            Vector3 firstEndCap, secondEndCap;
            MathUtility.CapsuleColliderEndCaps(m_CapsuleCollider, m_Transform.position, m_Transform.rotation,
                out firstEndCap, out secondEndCap);

            if ((m_FirstEndCapTarget.position - firstEndCap).sqrMagnitude >
                (m_SecondEndCapTarget.position - firstEndCap).sqrMagnitude)
            {
            ...
            m_SecondEndCapOffset = m_Transform.InverseTransformDirection(m_SecondEndCapTarget.position - secondEndCap) -
                                   m_CharacterTransform.up * m_SecondEndCapPadding;
        }

        /// <summary>
        ///     Does the actual rotation and height changes. This method is separate from FixedUpdate so the rotation/height can be
        ///     determined
        ...
        public void UpdateRotationHeight()
        {
            ...
            if (m_RotateCollider)
            {
                if (m_EndCapRotation)
                {
                    ...
                    m_Transform.rotation = Quaternion.LookRotation(
                        Vector3.Cross(m_CharacterTransform.forward, direction.normalized), direction.normalized);
                }
                else
                {
            ...
            if (m_AdjustHeight)
            {
                ...
                MathUtility.CapsuleColliderEndCaps(m_CapsuleCollider, m_Transform.position, m_Transform.rotation,
                    out firstEndCap, out secondEndCap);
                var firstEndCapOffset =
                    m_Transform.InverseTransformDirection(m_FirstEndCapTarget.position - firstEndCap);
                var secondEndCapOffset =
                    m_Transform.InverseTransformDirection(m_SecondEndCapTarget.position - secondEndCap);
                ...
                if (targetHeight >= m_CapsuleCollider.radius * 2 &&
                    (localDirection.y < 0 || !m_CharacterLocomotion.UsingVerticalCollisionDetection ||
                     Physics.OverlapCapsuleNonAlloc(firstEndCap,
                         secondEndCap + m_CharacterLocomotion.Up * localDirection.y,
                         m_CapsuleCollider.radius * MathUtility.ColliderRadiusMultiplier(m_CapsuleCollider),
                         m_OverlapColliders, m_CharacterLayerManager.SolidObjectLayers,
            ...
            else
            {
                if (m_PositionBone != null)
                    m_Transform.position = MathUtility.TransformPoint(m_PositionBone.position, m_Transform.rotation,
        ...
        private void AdjustCenterOffset(Vector3 targetOffset)
        {
            ...
            MathUtility.CapsuleColliderEndCaps(m_CapsuleCollider, m_Transform.position, m_Transform.rotation,
                out firstEndCap, out secondEndCap);
            if (Physics.OverlapCapsuleNonAlloc(firstEndCap, secondEndCap,
                    m_CapsuleCollider.radius * MathUtility.ColliderRadiusMultiplier(m_CapsuleCollider),
