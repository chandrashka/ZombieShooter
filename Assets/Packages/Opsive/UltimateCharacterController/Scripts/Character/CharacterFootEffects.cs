...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class CharacterFootEffects : StateBehavior
    {
        ...
        {
            [Tooltip(
                "A reference to the foot Transform. This reference should be pointing in the character's forward direction so the foot is placed correctly.")]
            [SerializeField]
            private Transform m_Object;
#pragma warning disable 0649
            [Tooltip(
                "The grouping of the foot. If for example the character is a dog, the two left feet will be in the first group while the right feet are in the second group.")]
            [SerializeField]
            private int m_Group;
#pragma warning restore 0649
            [Tooltip("Should the footprint be flipped for this foot?")] [SerializeField]
            private bool m_FlippedFootprint;
            ...
            public bool FlippedFootprint
            {
                get => m_FlippedFootprint;
                set => m_FlippedFootprint = value;
            }
        }
        ...
        [Tooltip("The Surface Impact triggered when there is a footstep.")] [SerializeField]
        protected SurfaceImpact m_SurfaceImpact;

        [Tooltip("Specifies how the footsteps are placed.")] [SerializeField]
        protected FootstepPlacementMode m_FootstepMode;

        [Tooltip("The character's feet. Only used with the BodyStep and Trigger placement modes.")] [SerializeField]
        protected Foot[] m_Feet;
        ...
        [SerializeField]
        protected int m_MoveDirectionFrameCount = 7;

        [Tooltip(
            "Specifies an offset for when a raycast is cast to determine if the character's foot is considered down.")]
        [SerializeField]
        protected float m_FootOffset = 0.07f;

        [Tooltip(
            "If using the Trigger mode, specifies the amount of time that must elapse before another effect can be spawned.")]
        [SerializeField]
        protected float m_MinTriggerInterval = 0.1f;

        [Tooltip(
            "If using the Trigger mode, specifies if the character needs to be moving in order for the footstep to be placed.")]
        [SerializeField]
        protected bool m_RequireMovement = true;

        [Tooltip(
            "If using the FixedInterval mode, specifies how often the footsteps occur when the character is moving.")]
        [FormerlySerializedAs("m_Interval")] // 2.3.4.
        [SerializeField]
        protected float m_FixedInterval = 0.3f;

        [Tooltip(
            "If using the CameraBob mode, specifies the minimum time that must elapse before another footstep occurs.")]
        [SerializeField]
        protected float m_MinBobInterval = 0.2f;

        public FootstepPlacementMode FootstepMode
        {
            get => m_FootstepMode;
            set
            {
                if (m_FootstepMode != value)
                {
                    ...
                    if (Application.isPlaying) PrepareVerticalOffsetLists();
                }
            }
        }
        ...
        public int MoveDirectionFrameCount
        {
            get => m_MoveDirectionFrameCount;
            set => m_MoveDirectionFrameCount = value;
        }

        public float FootOffset
        {
            get => m_FootOffset;
            set => m_FootOffset = value;
        }

        public float MinTriggerInterval
        {
            get => m_MinTriggerInterval;
            set => m_MinTriggerInterval = value;
        }

        public bool RequireMovement
        {
            get => m_RequireMovement;
            set => m_RequireMovement = value;
        }

        public float FixedInterval
        {
            get => m_FixedInterval;
            set => m_FixedInterval = value;
        }

        public float MinBobInterval
        {
            get => m_MinBobInterval;
            set => m_MinBobInterval = value;
        }
        ...
        private readonly List<List<Transform>> m_FeetGrouping = new();
        private readonly HashSet<Transform> m_FlippedFootprints = new();
        ...
        protected override void Awake()
        {
            ...
            if (m_Feet == null) InitializeHumanoidFeet(false);

            if (m_Feet != null && m_Feet.Length != 0)
            {
                for (var i = 0; i < m_Feet.Length; ++i)
                {
                    if (m_Feet[i].Object == null) continue;

                    // The FeetGrouping list should be at least the size of the current group index.
                    while (m_Feet[i].Group >= m_FeetGrouping.Count) m_FeetGrouping.Add(new List<Transform>());
                    ...
                    if (m_Feet[i].FlippedFootprint) m_FlippedFootprints.Add(m_Feet[i].Object);
                }
            }
            else
            {
            ...
            if (m_FootstepMode == FootstepPlacementMode.Trigger || m_FootstepMode == FootstepPlacementMode.CameraBob ||
                m_FootstepMode == FootstepPlacementMode.None)
                ...
                enabled = false;
            else if (m_Feet != null) PrepareVerticalOffsetLists();
        ...
        public void InitializeHumanoidFeet(bool addAudioSource)
        {
            ...
            if (animator != null)
            {
                ...
                if (head != null)
                {
                    ...
                    if (addAudioSource && m_Feet[0].Object != null &&
                        m_Feet[0].Object.gameObject.GetComponent<AudioSource>() == null)
                    {
                    ...
                    if (addAudioSource && m_Feet[1].Object != null &&
                        m_Feet[1].Object.gameObject.GetComponent<AudioSource>() == null)
                    {
        ...
        {
            if (m_FootstepMode == FootstepPlacementMode.Trigger ||
                m_FootstepMode == FootstepPlacementMode.FixedInterval ||
                m_FootstepMode == FootstepPlacementMode.None) return;
            ...
            {
                Array.Resize(ref m_VerticalOffset, count);
                Array.Resize(ref m_LastVerticalOffset, count);
            }
            else
            {
            ...
            if (m_FootstepMode == FootstepPlacementMode.CameraBob)
            {
                if (m_LookSource != null)
                    m_LastVerticalOffset[0] = m_VerticalOffset[0] =
                        m_Transform.InverseTransformPoint(m_LookSource.LookPosition(true)).y;
            }
            else
            {
                // Body Step.
                ...
                {
                    Array.Resize(ref m_UpCount, count);
                    Array.Resize(ref m_DownCount, count);
                }
        ...
        {
            if (m_FootstepMode == FootstepPlacementMode.None) return;
            ...
            if (!m_CharacterLocomotion.Grounded || !m_CharacterLocomotion.Moving) return;
#if ULTIMATE_CHARACTER_CONTROLLER_MULTIPLAYER
            }
#endif
        ...
        private void DetectBodyStep()
        {
            for (var i = 0; i < m_Feet.Length; ++i)
            {
                ...
                if (verticalOffset < m_LastVerticalOffset[i])
                {
                    ...
                    if ((m_UpCount[i] >
                         m_MoveDirectionFrameCount / (m_CharacterLocomotion.TimeScale * Time.timeScale) ||
                         m_DownCount[i] > m_MoveDirectionFrameCount /
                         (m_CharacterLocomotion.TimeScale * Time.timeScale)) &&
                        m_Feet[i].Object != m_LastFootDown && FootStep(m_Feet[i].Object, m_Feet[i].FlippedFootprint))
                    {
                        ...
                        for (var j = 0; j < m_UpCount.Length; ++j)
                        {
                            m_UpCount[j] = 0;
                            m_DownCount[j] = 0;
                        }
        ...
        private void UpdateFixedInterval()
        {
            ...
            if (m_LastFootstepTime + m_FixedInterval * m_CharacterLocomotion.TimeScale > Time.time) return;
        ...
        private void DetectCameraBob()
        {
            ...
            if (m_LastVerticalOffset[0] >= m_VerticalOffset[0] && verticalOffset > m_VerticalOffset[0] &&
                m_LastFootstepTime + m_MinBobInterval * m_CharacterLocomotion.TimeScale < Time.time)
            {
                GroupFootStep();
                m_LastFootstepTime = Time.time;
            }
        ...
        private void GroupFootStep()
        {
            ...
            for (var i = 0; i < grouping.Count; ++i) FootStep(grouping[i], m_FlippedFootprints.Contains(grouping[i]));
        ...
        {
            if (!m_CharacterLocomotion.Moving && m_RequireMovement) return false;
            ...
            if ((!m_CharacterLocomotion.Moving && m_RequireMovement) ||
                m_LastFootstepTime + m_MinTriggerInterval > Time.time) return false;
        ...
        public virtual bool FootStep(Transform foot, bool flipFootprint)
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(foot.position + m_Transform.up * 0.2f, -m_Transform.up, out hit, 0.21f + m_FootOffset,
                    m_CharacterLayerManager.IgnoreInvisibleCharacterWaterLayers, QueryTriggerInteraction.Ignore))
            {
                SurfaceManager.SpawnEffect(hit, m_SurfaceImpact, m_CharacterLocomotion.GravityDirection,
                    m_CharacterLocomotion.TimeScale, foot.gameObject, m_Transform.forward, flipFootprint);
                return true;
            }
        ...
        private void OnMoving(bool moving)
        {
            ...
            if (moving) m_LastFootstepTime = Time.time;
        }
        ...
        {
            if (GetComponent<Animator>() == null) FootstepMode = FootstepPlacementMode.FixedInterval;
        }
