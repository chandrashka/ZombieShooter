...
namespace Opsive.UltimateCharacterController.Camera
{
    ...
    {
        [Tooltip("The name of the zoom input mapping.")] [SerializeField]
        protected string m_ZoomInputName = "Fire2";

        [Tooltip("Does the zoom button need to be held down in order for the camera to zoom?")] [SerializeField]
        protected bool m_ContinuousZoom = true;
        ...
        {
            get => m_ContinuousZoom;
            set => m_ContinuousZoom = value;
        }
#if FIRST_PERSON_CONTROLLER && THIRD_PERSON_CONTROLLER
        public string TogglePerspectiveInputName { get { return m_TogglePerspectiveInputName; } set { m_TogglePerspectiveInputName
 = value; } }
#endif
        ...
        private void Update()
        {
            ...
            if (m_ContinuousZoom)
                ...
                zoom = m_PlayerInput.GetButtonDown(m_ZoomInputName)
                    ? !m_CameraController.ZoomInput
                    : m_CameraController.ZoomInput;

            if (zoom != m_CameraController.ZoomInput) m_CameraController.TryZoom(zoom);
        ...
        {
            if (m_CameraController.KinematicObjectIndex == -1) return;
