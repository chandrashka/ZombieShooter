...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Items
{
    ...
    /// </summary>
    public class ThirdPersonGrenadeItemProperties : ThirdPersonThrowableItemProperties,
        IGrenadeItemPerspectiveProperties
    {
        [Tooltip("A reference to the pin attachment transform.")] [SerializeField]
        protected Transform m_PinAttachmentLocation;

        [Tooltip(
            "The ID of the pin attachment transform. This field will be used if the value is not -1 and the location is null.")]
        [SerializeField]
        protected int m_PinAttachmentLocationID = -1;
        ...
        protected override void Awake()
        {
            ...
            if (m_PinAttachmentLocation == null && m_PinAttachmentLocationID != -1)
            {
                ...
                var objectIdentifiers = character.gameObject.GetComponentsInChildren<ObjectIdentifier>();
                if (objectIdentifiers.Length > 0)
                {
                    ...
                    {
                        Debug.LogWarning(
                            "Warning: Unable to find the third person pin attachment ObjectIdentifier with the ID " +
                            m_PinAttachmentLocationID + " for item " + name + ".");
        ...
        [NonSerialized]
        public Transform PinAttachmentLocation
        {
            get => m_PinAttachmentLocation;
            set => m_PinAttachmentLocation = value;
        }

        [NonSerialized]
        public int PinAttachmentLocationID
        {
            get => m_PinAttachmentLocationID;
            set => m_PinAttachmentLocationID = value;
        }
    }
}