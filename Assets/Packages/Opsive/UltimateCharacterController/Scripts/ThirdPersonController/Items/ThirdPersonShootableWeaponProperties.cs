...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Items
{
    ...
    /// </summary>
    public class ThirdPersonShootableWeaponProperties : ThirdPersonWeaponProperties,
        IShootableWeaponPerspectiveProperties
    {
        [Tooltip(
            "The sensitivity amount for how much the weapon must be looking in the look source direction (-1 is least sensitive and 1 is most).")]
        [Range(-1, 1)]
        [SerializeField]
        protected float m_LookSensitivity = 0.97f;

        [Tooltip("The location that the weapon is fired at.")] [SerializeField]
        protected Transform m_FirePointLocation;

        [Tooltip("The transform that the fire point should be attached to.")] [SerializeField]
        protected Transform m_FirePointAttachmentLocation;

        [Tooltip(
            "The ID of the transform that the fire point should be attached to. This field will be used if the value is not -1 and the attachment is null.")]
        [SerializeField]
        protected int m_FirePointAttachmentLocationID = -1;

        [Tooltip("The location that the muzzle flash is spawned at.")] [SerializeField]
        protected Transform m_MuzzleFlashLocation;

        [Tooltip("The location that the shell is ejected at.")] [SerializeField]
        protected Transform m_ShellLocation;

        [Tooltip("The location that the smoke is spawned at.")] [SerializeField]
        protected Transform m_SmokeLocation;

        [Tooltip("The location that the tracer is spawned at.")] [SerializeField]
        protected Transform m_TracerLocation;

        [Tooltip("A reference to the weapon's clip that can be reloaded.")] [SerializeField]
        protected Transform m_ReloadableClip;

        [Tooltip("A reference to the reloadable clip attachment transform.")] [SerializeField]
        protected Transform m_ReloadableClipAttachment;

        [Tooltip(
            "The ID of the reloadable clip attachment transform. This field will be used if the value is not -1 and the attachment is null.")]
        [SerializeField]
        protected int m_ReloadableClipAttachmentID = -1;

        [Tooltip("A reference to the attachment transform that the projectile should be parented to when reloading.")]
        [SerializeField]
        protected Transform m_ReloadProjectileAttachment;

        [Tooltip(
            "The ID of the attachment transform that hte projectile should be parented to when reloading. This field will be used if the value is not -1 and the attachment is null.")]
        [SerializeField]
        protected int m_ReloadProjectileAttachmentID = -1;

        [Tooltip("Optionally specify if the weapon has a camera for the scope.")] [SerializeField]
        protected GameObject m_ScopeCamera;
        ...
        [NonSerialized]
        public int ReloadableClipAttachmentID
        {
            get => m_ReloadableClipAttachmentID;
            set => m_ReloadableClipAttachmentID = value;
        }

        [NonSerialized]
        public int ReloadProjectileAttachmentID
        {
            get => m_ReloadProjectileAttachmentID;
            set => m_ReloadProjectileAttachmentID = value;
        }
        ...
        protected override void Awake()
        {
            ...
            if (m_FirePointAttachmentLocationID != -1 ||
                (m_ReloadableClipAttachment == null && m_ReloadableClip != null) ||
                m_ReloadProjectileAttachmentID != -1)
            {
                ...
                if (objectIdentifiers.Length > 0)
                {
                    for (var i = 0; i < objectIdentifiers.Length; ++i)
                    {
                        ...
                        if ((m_ReloadableClip == null || m_ReloadableClipAttachmentID == -1 ||
                             m_ReloadableClipAttachment != null) &&
                    ...
                    if (m_FirePointAttachmentLocation == null && m_FirePointAttachmentLocationID != -1)
                        Debug.LogWarning(
                            "Warning: Unable to find the third person fire point attachment ObjectIdentifier with the ID " +
                            m_FirePointAttachmentLocationID + " for item " + name + ".");
                    if (m_ReloadableClipAttachment == null && m_ReloadableClip != null &&
                        m_ReloadableClipAttachmentID != -1)
                        Debug.LogWarning(
                            "Warning: Unable to find the third person reload attachment ObjectIdentifier with the ID " +
                            m_ReloadableClipAttachmentID + " for item " + name + ".");
                    if (m_ReloadProjectileAttachment == null && m_ReloadProjectileAttachmentID != -1)
                        Debug.LogWarning(
                            "Warning: Unable to find the third person reload projectile ObjectIdentifier with the ID " +
                            m_ReloadProjectileAttachmentID + " for item " + name + ".");
                }
            }
        ...
        {
            get => m_MuzzleFlashLocation;
            set => m_MuzzleFlashLocation = value;
        }
        ...
        {
            get => m_ShellLocation;
            set => m_ShellLocation = value;
        }

        [NonSerialized]
        public Transform SmokeLocation
        {
            get => m_SmokeLocation;
            set => m_SmokeLocation = value;
        }
        ...
        [NonSerialized]
        public Transform ReloadableClip
        {
            get => m_ReloadableClip;
            set => m_ReloadableClip = value;
        }

        [NonSerialized]
        public Transform ReloadableClipAttachment
        {
            get => m_ReloadableClipAttachment;
            set => m_ReloadableClipAttachment = value;
        }

        [NonSerialized]
        public Transform ReloadProjectileAttachment
        {
            get => m_ReloadProjectileAttachment;
            set => m_ReloadProjectileAttachment = value;
        }
        ...
        public bool CanFire(bool abilityActive, bool fireInLookSourceDirection)
        {
            ...
            {
                var lookSensitivity = Vector3.Dot(m_ObjectTransform.forward,
                    m_LookSource.LookDirection(m_ObjectTransform.position, false, 0, true, false));
#if UNITY_EDITOR
