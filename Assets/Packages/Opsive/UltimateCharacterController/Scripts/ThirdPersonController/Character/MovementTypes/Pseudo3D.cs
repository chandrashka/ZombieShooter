...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Character.MovementTypes
{
    ...
    {
        [Tooltip("Can the character move along the depth axis (the z axis relative to the camera)?")] [SerializeField]
        protected bool m_AllowDepthMovement;

        private UnityEngine.Camera m_Camera;
        private Plane m_HitPlane;
        private bool m_InitialOrientation = true;

        [Tooltip("Should the character look in the direction of the movement?")] [SerializeField]
        protected bool m_LookInMoveDirection;
        ...
        [SerializeField]
        protected float m_LookRotateBuffer = 0.2f;
        ...
        {
            get => m_AllowDepthMovement;
            set => m_AllowDepthMovement = value;
        }

        public bool LookInMoveDirection
        {
            get => m_LookInMoveDirection;
            set => m_LookInMoveDirection = value;
        }

        public float LookRotateBuffer
        {
            get => m_LookRotateBuffer;
            set => m_LookRotateBuffer = value;
        }
            ...
            float cameraHorizontalMovement, float cameraVerticalMovement)
        {
            ...
            if (m_LookInMoveDirection)
            {
                if (characterHorizontalMovement != 0 || (m_AllowDepthMovement ? characterForwardMovement : 0) != 0)
                {
                    ...
                    inputVector.Set(characterHorizontalMovement, 0,
                        m_AllowDepthMovement ? characterForwardMovement : 0);
                    Quaternion lookRotation;
                    if (m_Path != null)
                        lookRotation = Quaternion.LookRotation(
                            Quaternion.LookRotation(Vector3.Cross(
                                m_Path.GetTangent(m_Transform.position, ref m_PathIndex), m_CharacterLocomotion.Up)) *
                            inputVector.normalized);
                    else
                        lookRotation =
                            Quaternion.LookRotation(m_LookSource.Transform.rotation * inputVector.normalized);
            ...
            else
            {
                ...
                {
                    var characterCenter = m_Transform.position +
                                          m_CharacterLocomotion.Up * m_CharacterLocomotion.Height / 2;
                    Vector3 forward;
                    if (m_Path != null)
                        forward = Vector3.Cross(m_Path.GetTangent(m_Transform.position, ref m_PathIndex),
                    ...
                    m_HitPlane.SetNormalAndPosition(m_Transform.TransformDirection(localLookDirection),
                        characterCenter);
                    ...
                    if (m_HitPlane.Raycast(ray, out distance))
                    {
                        ...
                        localHitPoint.y = 0;
                        if (localHitPoint.magnitude > m_LookRotateBuffer *
                            Mathf.Max(1, m_CharacterLocomotion.LocomotionVelocity.magnitude) || m_InitialOrientation)
                        {
                            ...
                            var rotation = Quaternion.LookRotation(
                                (m_Transform.TransformPoint(localHitPoint) - characterCenter).normalized,
                                m_CharacterLocomotion.Up);
                            return MathUtility.ClampInnerAngle(MathUtility
                                .InverseTransformQuaternion(m_Transform.rotation, rotation).eulerAngles.y);
                        }
                    }
                }
                else
        ...
        public override Vector2 GetInputVector(Vector2 inputVector)
        {
            ...
            if (m_LookSource != null)
            {
                var localEuler = MathUtility.InverseTransformQuaternion(m_LookSource.Transform.rotation,
                    Quaternion.LookRotation(Vector3.forward, m_CharacterLocomotion.Up)).eulerAngles;
                localEuler.x = localEuler.z = 0;
                rotation *= MathUtility.TransformQuaternion(Quaternion.Euler(localEuler),
        ...
        {
            if (base.UseIndependentLook(characterLookDirection)) return true;
