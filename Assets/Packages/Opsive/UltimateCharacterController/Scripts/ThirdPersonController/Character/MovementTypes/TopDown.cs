...
{
    /// <summary>
    ///     The TopDown MovementType can move the character relative to a top down camera Controlled by TopDownAnyAngle
    ///     ViewType.
    /// </summary>
    public class TopDown : MovementType
    {
        ...
        [Tooltip(
            "A reference to the character's head. This value will be retrieved automatically if using a humanoid.")]
        [SerializeField]
        ...
        [Tooltip("Should the character move relative to the camera's direction?")] [SerializeField]
        protected bool m_RelativeCameraMovement = true;

        public bool RelativeCameraMovement
        {
            get => m_RelativeCameraMovement;
            set => m_RelativeCameraMovement = value;
        }
        ...
        public override void Initialize(UltimateCharacterLocomotion characterLocomotion)
        {
            ...
            if (m_Head == null)
            {
                ...
                if (m_Animator != null) m_Head = m_Animator.GetBoneTransform(HumanBodyBones.Head);
            }

            if (m_Head == null) m_Head = m_Transform;
        }
            ...
            if (m_LookInMoveDirection)
            {
                if (characterHorizontalMovement != 0 || characterForwardMovement != 0)
                {
                    ...
                    var lookRotation = Quaternion.LookRotation(
                        Quaternion.LookRotation(m_LookSource.Transform.up, Vector3.up) * inputVector.normalized);
        ...
        {
            if (!m_RelativeCameraMovement) return inputVector;
            ...
            if (m_LookSource != null)
            {
                var localEuler = MathUtility
                    .InverseTransformQuaternion(Quaternion.LookRotation(Vector3.forward, m_CharacterLocomotion.Up),
                        m_LookSource.Transform.rotation).eulerAngles;
        ...
        {
            if (m_LookInMoveDirection || base.UseIndependentLook(characterLookDirection)) return true;
