...
{
    /// <summary>
    ///     With the Adventure movement type the character will always move forward in the movement direction. This is relative
    ///     to the camera direction and prevents the
    ...
    public class Adventure : MovementType
    {
        ...
        {
            if (useAbility.FaceTargetItem != null) m_FaceUseTargetCount += start ? 1 : -1;
        }
            ...
            float cameraHorizontalMovement, float cameraVerticalMovement)
        {
            ...
            if (characterHorizontalMovement != 0 || characterForwardMovement != 0)
            {
                var lookRotation = Quaternion.LookRotation(m_LookSource.Transform.rotation *
                                                           new Vector3(characterHorizontalMovement, 0,
                                                               characterForwardMovement).normalized,
        ...
        public override Vector2 GetInputVector(Vector2 inputVector)
        {
            ...
            if (m_AimActive || m_FaceUseTargetCount > 0) return inputVector;
