...
{
    /// <summary>
    ///     The FourLegged MovementType allows the character to move as if they are on four legs. The character cannot strafe
    ///     and will rotate when trying to turn.
    /// </summary>
    public class FourLegged : MovementType
    {
            ...
            float cameraHorizontalMovement, float cameraVerticalMovement)
        {
...
#endif
            var localEuler = MathUtility.InverseTransformQuaternion(m_Transform.rotation,
                Quaternion.LookRotation(m_Transform.forward, m_Transform.up)).eulerAngles;
            ...
            {
                var cameraAngle = MathUtility
                    .InverseTransformQuaternion(m_Transform.rotation, m_LookSource.Transform.rotation).eulerAngles;
