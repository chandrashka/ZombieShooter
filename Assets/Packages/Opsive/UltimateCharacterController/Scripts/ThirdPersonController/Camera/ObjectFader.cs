...
{
    /// <summary>
    ///     Can fade the character's materials if the camera gets too close or any materials which are obstructing the view
    ...
    public class ObjectFader : StateBehavior
    {
        ...
        [Tooltip("Should the character fade out when the camera is near?")] [SerializeField]
        protected bool m_CharacterFade = true;

        [Tooltip(
            "Should the character materials be cached at the start? If false the material values will be saved each time the character starts to fade.")]
        [SerializeField]
        protected bool m_CacheCharacterMaterials = true;

        [Tooltip("The distance between the character and camera that the character materials should start to fade.")]
        [SerializeField]
        protected float m_StartFadeDistance = 1.8f;

        [Tooltip(
            "The distance between the character and camera that the character materials should be completely invisible.")]
        [SerializeField]
        protected float m_EndFadeDistance = 1;

        [Tooltip(
            "Prevents the character change from updating for the specified number of seconds after a state change.")]
        [SerializeField]
        protected float m_CharacterFadeStateChangeCooldown = 0.3f;

        [Tooltip("Should any objects obstructing the camera's view fade out?")] [SerializeField]
        protected bool m_ObstructingObjectsFade;
        ...
        [Tooltip("Specifies the speed at which the obstructing material can fade.")] [SerializeField]
        protected float m_FadeSpeed = 0.02f;

        [Tooltip("The color that the obstructed object will fade to.")] [SerializeField]
        protected Color m_FadeColor = new(1, 1, 1, 0);

        [Tooltip("Should the material mode be set automatically when an object is obstructing the view?")]
        [SerializeField]
        protected bool m_AutoSetMode;

        [Tooltip("Should the obstructing object's collider be disabled when the material is faded?")] [SerializeField]
        protected bool m_DisableCollider = true;
        ...
        [Tooltip(
            "The maximum number of obstricting materials that can be faded at one time. This value should be greater than the collider count.")]
        [SerializeField]
        protected int m_MaxObstructingMaterialCount = 30;

        [Tooltip(
            "The offset to apply to the character when determining if the character should fade/is considered obstructed.")]
        [SerializeField]
        protected Vector3 m_TransformOffset = new(0, 0.9f, 0);
        ...
        private readonly Dictionary<Material, OriginalMaterialValue> m_OriginalMaterialValuesMap = new();
        ...
        public bool CharacterFade
        {
            ...
            set
            {
                m_CharacterFade = value;
                if (!value) DisableCharacterFade();
            }
        }
        ...
        public float StartFadeDistance
        {
            ...
            set => m_StartFadeDistance = value;
        }

        public float EndFadeDistance
        {
            get => m_EndFadeDistance;
            set => m_EndFadeDistance = value;
        }

        public float CharacterFadeStateChangeCooldown
        {
            get => m_CharacterFadeStateChangeCooldown;
            set => m_CharacterFadeStateChangeCooldown = value;
        }

        public bool ObstructingObjectsFade
        {
            get => m_ObstructingObjectsFade;
            set
            {
                m_ObstructingObjectsFade = value;
                if (!value) DisableObstructingFade();
            }
        }
        ...
        {
            get => m_AutoSetMode;
            set => m_AutoSetMode = value;
        }

        public bool DisableCollider
        {
            get => m_DisableCollider;
            set => m_DisableCollider = value;
        }
        ...
        public int IndependentCharacterFadeCount { get; set; }
        ...
        protected virtual void OnAttachCharacter(GameObject character)
        {
                ...
                if (m_Character != null && m_Character != character)
                {
                    ...
                    if (m_CharacterFadeMaterials != null)
                    {
                        if (m_CacheCharacterMaterials)
                            for (var i = 0; i < m_CharacterFadeMaterials.Length; ++i)
                            {
                                if (!m_OriginalMaterialValuesMap.ContainsKey(m_CharacterFadeMaterials[i])) continue;
                    ...
                    EventHandler.UnregisterEvent<bool, bool>(m_Character, "OnCharacterIndependentFade",
                        OnIndependentFade);
                    EventHandler.UnregisterEvent<Item>(m_Character, "OnInventoryAddItem", OnAddItem);
                    EventHandler.UnregisterEvent<GameObject, bool>(m_Character, "OnShootableWeaponShowProjectile",
                        OnShowProjectile);
            ...
            if (m_Character != null)
            {
                ...
                if (m_CharacterFade)
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        else if (m_CharacterFadeMaterials.Length != count)
                        {
                            ...
                            Array.Resize(ref m_CharacterFadeMaterials, count);
                        }
                        ...
                        for (var i = 0; i < renderers.Length; ++i)
                        {
                            ...
                            for (var j = 0; j < materials.Length; ++j)
                            {
                                if (m_RegisteredMaterial.Contains(materials[j])) continue;
                                if (materials[j].HasProperty(m_ColorID))
                                {
                                    ...
                                    if (m_CacheCharacterMaterials)
                                    {
                                        ...
                                        originalMaterialValues.Initialize(materials[j], m_ColorID,
                                            m_MaterialModeSet.Contains(materials[j]));
                    ...
                    EventHandler.RegisterEvent<GameObject, bool>(m_Character, "OnShootableWeaponShowProjectile",
                        OnShowProjectile);
                    EventHandler.RegisterEvent<bool, bool>(m_Character, "OnCharacterIndependentFade",
                        OnIndependentFade);
                ...
                if (m_ObstructingObjectsFade) FadeObstructingObjects(true);
            }
        }
        ...
        {
            if (!m_CharacterFade || Time.time < m_CharacterFadeCooldownElapsedTime) return;

            m_FadeOffset = MathUtility.InverseTransformPoint(m_CharacterTransform.TransformPoint(m_TransformOffset),
                m_CharacterTransform.rotation, m_Transform.position);
            // Other components can control when the materials fade.
            if (IndependentCharacterFadeCount != 0) return;
        ...
        public void MultiCameraRender(bool start)
        {
            ...
            if (start && m_FadeOffset.magnitude <= m_StartFadeDistance)
                EnableCharacterFade(m_FadeOffset.magnitude);
            else if (!start) DisableCharacterFade();
        }
        ...
        private void EnableCharacterFade(float distance)
        {
            if (m_CharacterFadeMaterials != null)
            {
                ...
                for (var i = 0; i < m_CharacterFadeMaterialsCount; ++i)
                {
                    if (!m_CharacterFaded) EnableFadeMaterial(m_CharacterFadeMaterials[i]);
            ...
            if (m_CharacterFadeMaterials != null)
                for (var i = 0; i < m_CharacterFadeMaterialsCount; ++i)
                    if (m_OriginalMaterialValuesMap.TryGetValue(m_CharacterFadeMaterials[i], out var materialValue))
                    {
                        ...
                        if (!m_CacheCharacterMaterials) m_OriginalMaterialValuesMap.Remove(m_CharacterFadeMaterials[i]);
                    }
        ...
        private void RevertMaterial(Material material, OriginalMaterialValue originalMaterialValue)
        {
            ...
            if (!originalMaterialValue.AlphaBlend) material.DisableKeyword(OriginalMaterialValue.AlphaBlendString);
        ...
        {
            if (!m_ObstructingObjectsFade) return;

            // Disable any obstructing colliders so the sphere cast can detect which objects are obstructing.
            for (var i = 0; i < m_ObstructingCollidersCount; ++i) m_ObstructingColliders[i].enabled = true;
                    ...
                    for (var j = 0; j < renderers.Length; ++j)
                    {
                        ...
                        for (var k = 0; k < materials.Length; ++k)
                        {
                            if (!m_CanObstructionFade.TryGetValue(materials[k], out var canFade))
                            {
                                ...
                                canFade = materials[k].HasProperty(m_ColorID) && (m_AutoSetMode ||
                                    materials[k].renderQueue >= (int)RenderQueue.Transparent);
                            ...
                            if (canFade)
                            {
                                ...
                                if (m_DisableCollider && !obstructing) obstructing = CanMaterialFade(material);
                                ...
                                if (!m_OriginalMaterialValuesMap.ContainsKey(material))
                                {
                                    ...
                                    originalMaterialValues.Initialize(material, m_ColorID,
                                        m_MaterialModeSet.Contains(material));
                                    m_OriginalMaterialValuesMap.Add(material, originalMaterialValues);
            ...
            for (var i = m_ObstructingMaterialsCount - 1; i >= 0; --i)
                if (!FadeMaterial(m_ObstructingMaterials[i], m_ObstructionHitSet.Contains(m_ObstructingMaterials[i]),
                        immediateFade))
        ...
        public bool CanMaterialFade(Material material)
        {
            ...
            return m_DisableCollider && material.HasProperty(OriginalMaterialValue.ModeID) &&
                   (m_AutoSetMode || material.GetInt(OriginalMaterialValue.ModeID) == 2);
        }
        ...
        private void RemoveObstructingMaterial(int index)
        {
            ...
            for (var j = index; j < m_ObstructingMaterialsCount - 1; ++j)
        ...
        {
            for (var i = m_ObstructingMaterialsCount - 1; i >= 0; --i) RemoveObstructingMaterial(i);

            for (var i = 0; i < m_ObstructingCollidersCount; ++i)
        ...
        private void OnChangePerspectives(bool firstPersonPerspective)
        {
            ...
            if (firstPersonPerspective) DisableFades();
        ...
        private void OnAddItem(Item item)
        {
            ...
            if (m_CharacterFade)
            {
                var perspectiveItem = item.GetComponent<ThirdPersonPerspectiveItem>();
                if (perspectiveItem == null || perspectiveItem.Object == null) return;
        ...
        {
            IndependentCharacterFadeCount += enable ? 1 : -1;
            if (enable && revertFade) DisableCharacterFade();
        }
        ...
        private void InitializeCharacterFadeRenderers(Renderer[] renderers)
        {
            ...
            if (count > 0)
            {
                ...
                if (m_CharacterFadeMaterials == null)
                    ...
                    Array.Resize(ref m_CharacterFadeMaterials, totalCount);
        ...
        private void OnShowProjectile(GameObject projectile, bool show)
        {
            ...
            if (show)
                ...
                {
                    var materials = renderers[i].materials;
                    for (var j = materials.Length - 1; j > -1; --j)
                    {
                        if (!m_RegisteredMaterial.Contains(materials[j])) continue;
                        ...
                        var index = -1;
                        for (var k = m_CharacterFadeMaterialsCount - 1; k > -1; --k)
                        ...
                        for (var k = index; k < m_CharacterFadeMaterialsCount - 1; ++k)
                        ...
                        if (m_OriginalMaterialValuesMap.TryGetValue(materials[j], out var materialValue))
                        {
                            ...
                            if (!m_CacheCharacterMaterials) m_OriginalMaterialValuesMap.Remove(materials[j]);
                        }
                    }
                }
        }
