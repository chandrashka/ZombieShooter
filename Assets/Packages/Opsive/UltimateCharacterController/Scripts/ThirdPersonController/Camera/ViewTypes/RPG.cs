...
{
    /// <summary>
    ///     The RPG ViewType uses a control scheme similar to the standard in the RPG genre. The ViewType works with the RPG
    ///     MovementType to move and rotate the camera.
    /// </summary>
    [RecommendedMovementType(typeof(Character.MovementTypes.RPG))]
    ...
    public class RPG : ThirdPerson
    {
        ...
        [SerializeField]
        protected bool m_AllowFreeMovement;

        [Tooltip("The name of the camera free movement input mapping.")] [SerializeField]
        protected string m_CameraFreeMovementInputName = "Fire1";
        ...
        [SerializeField]
        protected float m_YawSnapDamping = 0.1f;

        private float m_YawSnapVelocity;

        public float YawSnapDamping
        {
            get => m_YawSnapDamping;
            set => m_YawSnapDamping = value;
        }

        public bool AllowFreeMovement
        {
            get => m_AllowFreeMovement;
            set => m_AllowFreeMovement = value;
        }

        public string CameraFreeMovementInputName
        {
            get => m_CameraFreeMovementInputName;
            set => m_CameraFreeMovementInputName = value;
        }
        ...
        public override void ChangeViewType(bool activate, float pitch, float yaw, Quaternion characterRotation)
        {
            ...
            if (activate)
            {
                ...
                if (m_Handler != null)
                {
                    ...
                    m_StartFreeMovementInputEvent.Initialize(ActiveInputEvent.Type.ButtonDown,
                        m_CameraFreeMovementInputName, "OnRPGViewTypeStartFreeMovement");

                    m_StopFreeMovementInputEvent = GenericObjectPool.Get<ActiveInputEvent>();
                    m_StopFreeMovementInputEvent.Initialize(ActiveInputEvent.Type.ButtonUp,
                        m_CameraFreeMovementInputName, "OnRPGViewTypeStopFreeMovement");
        ...
        public override Quaternion Rotate(float horizontalMovement, float verticalMovement, bool immediatePosition)
        {
            ...
            else
            {
                ...
                if (m_FreeMovement)
                    ...
                    m_YawOffset = Mathf.SmoothDamp(m_YawOffset, 0, ref m_YawSnapVelocity,
                        m_YawSnapDamping * m_CharacterLocomotion.TimeScale * Time.timeScale *
                        TimeUtility.FramerateDeltaTime);
                var deltaRotation =
                    MathUtility.InverseTransformQuaternion(m_CharacterRotation, m_CharacterTransform.rotation);
            ...
            if (!m_FreeMovement && !m_CharacterRotate) verticalMovement = 0;
            ...
            bool includeRecoil, bool includeMovementSpread)
        {
            if (!characterLookDirection)
                return base.LookDirection(lookPosition, characterLookDirection, layerMask, includeRecoil,
        ...
        {
            if (!(ability is MoveTowards)) return;
            ...
            if (m_RotateWithCharacter) m_Yaw = 0;
        }
    }
}