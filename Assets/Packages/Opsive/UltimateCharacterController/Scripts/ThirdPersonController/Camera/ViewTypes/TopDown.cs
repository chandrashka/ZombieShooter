...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Camera.ViewTypes
{
    ...
    /// <summary>
    ///     The Top Down View Type allows the camera to be placed in a top down perspective with the character in view with any
    ///     pitch (as long as top down) or rotation.
    /// </summary>
    ...
    public class TopDown : ViewType
    {
        ...
        [Tooltip("The up axis that the camera should adjust towards.")] [SerializeField]
        protected Vector3 m_UpAxis = Vector3.up;

        [Tooltip("The speed at which the camera rotates to face the character.")] [SerializeField]
        protected float m_RotationSpeed = 1.5f;

        [Tooltip("The minimum pitch angle (in degrees).")] [SerializeField]
        protected float m_MinPitchLimit = 70;

        [Tooltip("The maximum pitch angle (in degrees).")] [SerializeField]
        protected float m_MaxPitchLimit = 89;
        ...
        [Tooltip("The number of degrees to adjust if the anchor is obstructed by an object.")] [SerializeField]
        protected float m_ViewStep = 2;
        ...
        [SerializeField]
        protected bool m_VerticalLookDirection;

        [Tooltip("Can the camera dynamically rotate when a state has been changed?")] [SerializeField]
        protected bool m_AllowDynamicCameraRotation;

        [Tooltip("The desired angle if the camera is dynamically rotating.")] [SerializeField]
        protected float m_DesiredAngle;

        [Tooltip("The speed at which the camera rotates if it is dynamically rotating.")] [SerializeField]
        protected float m_ChangeAngleSpeed = 1;

        [Tooltip("The curve used by the dynamic camera rotation.")] [SerializeField]
        protected AnimationCurve m_RotationTransitionCurve = AnimationCurve.EaseInOut(0, 0, 1, 1);

        [Tooltip("Can the camera dynamically change pitch when a state has been changed?")] [SerializeField]
        protected bool m_AllowDynamicPitchAdjustment;

        [Tooltip("The desired pitch if the camera is dynamically rotating.")] [SerializeField]
        protected float m_DesiredPitch;

        [Tooltip("The speed at which the camera pitches if it is dynamically changing the pitch.")] [SerializeField]
        protected float m_ChangePitchSpeed = 1;

        [Tooltip(
            "Is a separate Animation curve required for pitch adjustment, if not, the rotation transition will be used.")]
        [SerializeField]
        protected bool m_UseIndependentPitchTransition;

        [Tooltip("The curve used by the dynamic camera pitch change.")] [SerializeField]
        protected AnimationCurve m_PitchTransitionCurve = AnimationCurve.EaseInOut(0, 0, 1, 1);

        [Tooltip("Can the camera dynamically change distance when a state has been changed?")] [SerializeField]
        protected bool m_AllowDynamicDistanceAdjustment;

        [Tooltip("The desired distance if the camera is dynamically adjusting distance.")] [SerializeField]
        protected float m_DesiredDistance;

        [Tooltip("The speed at which the camera changes distance if it is dynamically changing the distance.")]
        [SerializeField]
        protected float m_ChangeDistanceSpeed;

        [Tooltip(
            "Is a separate Animation curve required for distance adjustment, if not, the rotation transition will be used.")]
        [SerializeField]
        protected bool m_UseIndependentDistanceTransition;

        [Tooltip("The curve used by the dynamic camera distance change.")] [SerializeField]
        protected AnimationCurve m_DistanceTransitionCurve = AnimationCurve.EaseInOut(0, 0, 1, 1);
        ...
        public float ViewStep
        {
            get => m_ViewStep;
            set => m_ViewStep = value;
        }
        ...
        public bool VerticalLookDirection
        {
            get => m_VerticalLookDirection;
            set => m_VerticalLookDirection = value;
        }

        public bool AllowDynamicCameraRotation
        {
            get => m_AllowDynamicCameraRotation;
            set => m_AllowDynamicCameraRotation = value;
        }
        ...
        {
            get => m_ChangeAngleSpeed;
            set => m_ChangeAngleSpeed = value;
        }

        public AnimationCurve RotationTransitionCurve
        {
            get => m_RotationTransitionCurve;
            set => m_RotationTransitionCurve = value;
        }

        public bool AllowDynamicPitchAdjustment
        {
            get => m_AllowDynamicPitchAdjustment;
            set => m_AllowDynamicPitchAdjustment = value;
        }

        public float DesiredPitch
        {
            get => m_DesiredPitch;
            set => m_DesiredPitch = value;
        }

        public float ChangePitchSpeed
        {
            get => m_ChangePitchSpeed;
            set => m_ChangePitchSpeed = value;
        }

        public bool UseIndependentPitchTransition
        {
            get => m_UseIndependentPitchTransition;
            set => m_UseIndependentPitchTransition = value;
        }

        public AnimationCurve PitchTransitionCurve
        {
            get => m_PitchTransitionCurve;
            set => m_PitchTransitionCurve = value;
        }

        public bool AllowDynamicDistanceAdjustment
        {
            get => m_AllowDynamicDistanceAdjustment;
            set => m_AllowDynamicDistanceAdjustment = value;
        }

        public float DesiredDistance
        {
            get => m_DesiredDistance;
            set => m_DesiredDistance = value;
        }

        public float ChangeDistanceSpeed
        {
            get => m_ChangeDistanceSpeed;
            set => m_ChangeDistanceSpeed = value;
        }

        public bool UseIndependentDistanceTransition
        {
            get => m_UseIndependentDistanceTransition;
            set => m_UseIndependentDistanceTransition = value;
        }

        public AnimationCurve DistanceTransitionCurve
        {
            get => m_DistanceTransitionCurve;
            set => m_DistanceTransitionCurve = value;
        }
        ...
        public override bool RotatePriority => false;
        ...
        {
            if (m_Character != null) m_PlayerInput = null;
        ...
        public override void ChangeViewType(bool activate, float pitch, float yaw, Quaternion characterRotation)
        {
            if (activate)
            {
                if (Time.frameCount != m_DeactivateFrame) m_Ray.direction = GetAnchorPosition() - m_Transform.position;
            }
            else
            {
        ...
        public override Quaternion Rotate(float horizontalMovement, float verticalMovement, bool immediateUpdate)
        {
...
#endif
            if (m_RotateCamera) RotateCamera();
            if (m_AdjustPitch) AdjustCameraPitch();
            if (m_AdjustDistance) AdjustDistance();
            ...
            return (immediateUpdate
                       ? rotation
                       : Quaternion.Slerp(m_Transform.rotation, rotation,
                           m_RotationSpeed * m_CharacterLocomotion.TimeScale * Time.timeScale * Time.deltaTime))
        ...
        public override Vector3 Move(bool immediateUpdate)
        {
            ...
            var up = m_CharacterLocomotion.AlignToGravity ? m_CharacterLocomotion.Up : m_UpAxis;
            m_Ray.direction =
                MathUtility.TransformQuaternion(lookRotation, Quaternion.Euler(90 - m_MinPitchLimit, 0, 0)) * up;
            ...
            while (Physics.SphereCast(m_Ray, m_CollisionRadius, out m_RaycastHit, m_ViewDistance,
                       m_CharacterLayerManager.IgnoreInvisibleCharacterWaterLayers, QueryTriggerInteraction.Ignore))
            {
                ...
                {
                    m_Ray.direction =
                        MathUtility.TransformQuaternion(lookRotation, Quaternion.Euler(90 - m_MaxPitchLimit, 0, 0)) *
                        up;
                    break;
                }
                ...
                step += m_ViewStep;
                m_Ray.direction =
                    MathUtility.TransformQuaternion(lookRotation, Quaternion.Euler(90 - m_MinPitchLimit - step, 0, 0)) *
                    up;
            }
        ...
        /// <summary>
        ///     Returns the direction that the character is looking.
        ///     As topdown aiming relies on raycasting planes the plane position must match the IK position. Meaning the Movement
        ///     Type is requesting the plane
        ///     at the head's position (to match CharacterIK and prevent rotation glitches with the mouse and IK that occur because
        ///     of angle rays to parallel offset planes).
        ///     As a result the VerticalLookDirection is limited to above head height. Also, to cover all angles, the Anchor offset
        ///     should be above head height.
        /// </summary>
            ...
            else
            {
                ...
                if (m_PlayerInput.IsCursorVisible() || !m_PlayerInput.ControllerConnected)
                {
                    ...
                    if (planeRaycast)
                    {
                        ...
                        if (!hitPointValid)
                        {
                            m_HitPlane.SetNormalAndPosition(-m_Transform.forward,
                                lookPosition + m_Transform.forward * m_LookDirectionDistance);
                            if (m_HitPlane.Raycast(ray, out distance))
                            {
                                hitPoint = ray.GetPoint(distance);
                                hitPoint.y = lookPosition.y;
                    ...
                    if (hitPointValid)
                        ...
                        if (characterLookDirection || ((m_CharacterTransform.position - hitPoint).sqrMagnitude >=
                                                       (m_CharacterTransform.position - lookPosition).sqrMagnitude *
                                                       1.5f &&
                ...
                else
                {
                    ...
                    var forward =
                        m_Transform.up; // Use camera up because in a Top Down setting it is always forward facing.
                    forward.y = 0;
                    forward = forward.normalized;
                    var right = m_Transform.right;
                    right.y = 0;
                    var direction = m_PlayerInput.GetAxis(m_PlayerInput.VerticalLookInputName) * forward +
                                    m_PlayerInput.GetAxis(m_PlayerInput.HorizontalLookInputName) * right;
        ...
        public override void StateChange()
        {
            ...
            if (m_AllowDynamicCameraRotation)
            {
                ...
                m_CameraRotateSpeedCorrection =
                    0.2f / ((Mathf.Max(angle, desiredAngle) - Mathf.Min(angle, desiredAngle)) / 360);
            }
            ...
            if (m_AllowDynamicPitchAdjustment)
            {
                ...
                m_CameraPitchSpeedCorrection = 0.2f /
                                               ((Mathf.Max(m_CameraPitch, m_DesiredPitch) -
                                                 Mathf.Min(m_CameraPitch, m_DesiredPitch)) / 360);
            }
        ...
        {
            m_CameraRotationLerp += m_CharacterLocomotion.TimeScale * Time.timeScale * Time.deltaTime *
                                    m_ChangeAngleSpeed * m_CameraRotateSpeedCorrection;
            ...
            var angle = Mathf.LerpAngle(m_CameraAngle, m_DesiredAngle,
                m_RotationTransitionCurve.Evaluate(m_CameraRotationLerp));
            if (!m_RotateCamera) m_CameraAngle = GetForwardAngle();
            m_ForwardAxis = new Vector3(Mathf.Cos(angle * Mathf.Deg2Rad), m_ForwardAxis.y,
        ...
        {
            m_CameraPitchLerp += m_CharacterLocomotion.TimeScale * Time.timeScale * Time.deltaTime *
                                 m_ChangePitchSpeed * m_CameraPitchSpeedCorrection;
            ...
            m_MinPitchLimit =
                Mathf.LerpAngle(m_CameraPitch, m_DesiredPitch, animationCurve.Evaluate(m_CameraPitchLerp));
            if (!m_AdjustPitch) m_CameraPitch = m_MinPitchLimit;
        }
        ...
        {
            m_CameraDistanceLerp += m_CharacterLocomotion.TimeScale * Time.timeScale * Time.deltaTime *
                                    m_ChangeDistanceSpeed;
            ...
            var animationCurve = m_UseIndependentDistanceTransition
                ? m_DistanceTransitionCurve
                : m_RotationTransitionCurve;
            m_ViewDistance = Mathf.Lerp(m_CameraDistance, m_DesiredDistance,
                animationCurve.Evaluate(m_CameraDistanceLerp));
            if (!m_AdjustDistance) m_CameraDistance = m_ViewDistance;
        }
