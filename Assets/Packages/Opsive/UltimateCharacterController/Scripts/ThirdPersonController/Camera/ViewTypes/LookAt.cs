...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Camera.ViewTypes
{
    ...
    public class LookAt : ViewType
    {
        ...
        [Tooltip("The maximum distance away from the target that the camera should move towards.")] [SerializeField]
        protected float m_MaxLookDistance = 5;

        [Tooltip("The minimum distance away from the target that the camera should move towards.")] [SerializeField]
        protected float m_MinLookDistance = 1;

        [Tooltip("The speed at which the camera should move.")] [SerializeField]
        protected float m_MoveSpeed = 10;

        [Tooltip("The offset relative to the target.")] [SerializeField]
        protected Vector3 m_Offset = new(0, 3, -2);
        ...
        [Range(0, 1)]
        [SerializeField]
        protected float m_RotationalLerpSpeed = 0.9f;

        [Tooltip("The spring used for applying a rotation to the camera.")] [SerializeField]
        protected Spring m_RotationSpring = new();

        [Tooltip("The object to look at. If null then the character's transform will be used.")] [SerializeField]
        ...
        public float MinLookDistance
        {
            get => m_MinLookDistance;
            set => m_MinLookDistance = value;
        }

        public float MaxLookDistance
        {
            get => m_MaxLookDistance;
            set => m_MaxLookDistance = value;
        }
        ...
        public float RotationalLerpSpeed
        {
            get => m_RotationalLerpSpeed;
            set => m_RotationalLerpSpeed = value;
        }
        ...
        public override bool FirstPersonPerspective =>
            m_CharacterLocomotion != null ? m_CharacterLocomotion.FirstPersonPerspective : false;

        public override float LookDirectionDistance => m_Offset.magnitude;
        ...
        {
            var rotation = Quaternion.LookRotation((m_Target.position - m_Transform.position).normalized,
                -m_CharacterLocomotion.GravityDirection);
            if (!immediateUpdate) rotation = Quaternion.Slerp(m_Transform.rotation, rotation, m_RotationalLerpSpeed);
        ...
        public override Vector3 Move(bool immediateUpdate)
        {
            ...
            var cameraPosition =
                MathUtility.TransformPoint(targetTransformPosition, m_CharacterTransform.rotation, m_Offset);
            ...
            if (distance > m_MaxLookDistance)
                targetPosition = Vector3.MoveTowards(transformPosition, cameraPosition,
                    immediateUpdate ? distance - m_MaxLookDistance * 0.8f : Time.deltaTime * m_MoveSpeed);
            else if (distance < m_MinLookDistance)
                targetPosition = Vector3.MoveTowards(transformPosition,
                    cameraPosition - (m_Target.position - m_Transform.position) * (m_MaxLookDistance - distance),
            ...
            if (Physics.SphereCast(targetTransformPosition, m_CollisionRadius, direction.normalized, out m_RaycastHit,
                    Mathf.Max(direction.magnitude - m_MinLookDistance, 0.01f),
