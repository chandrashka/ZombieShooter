...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Camera.ViewTypes
{
    ...
    public abstract class ThirdPerson : ViewType
    {
        ...
        protected Vector3 m_LookOffset = new(0.5f, 0, -2.5f);

        [Tooltip("The amount of smoothing to apply to the look offset. Can be zero.")] [SerializeField]
        protected float m_LookOffsetSmoothing = 0.05f;
        ...
        [Tooltip("The offset from the anchor position when determining if there is a collision.")] [SerializeField]
        protected Vector3 m_CollisionAnchorOffset;

        [Tooltip("The amount of smoothing to apply to the position. Can be zero.")] [SerializeField]
        protected float m_PositionSmoothing;

        [Tooltip(
            "The amount of smoothing to apply to the position when an object is obstructing the target position. Can be zero.")]
        [SerializeField]
        protected float m_ObstructionPositionSmoothing = 0.04f;

        [Tooltip(
            "Should the rotation of the character be updated? Set to false to prevent the camera from rotating with the character.")]
        [SerializeField]
        protected bool m_UpdateCharacterRotation = true;

        [Tooltip("The lerping speed when determining the align to gravity character rotation.")] [SerializeField]
        protected float m_AlignToGravityRotationSpeed = 0.8f;

        [Tooltip("The minimum pitch angle (in degrees).")] [SerializeField]
        protected float m_MinPitchLimit = -72;

        [Tooltip("The maximum pitch angle (in degrees).")] [SerializeField]
        protected float m_MaxPitchLimit = 72;

        [Tooltip("The positional spring used for regular movement.")] [SerializeField]
        protected Spring m_PositionSpring = new();

        [Tooltip("The rotational spring used for regular movement.")] [SerializeField]
        protected Spring m_RotationSpring = new();

        [Tooltip("The positional spring which returns to equilibrium after a small amount of time (for recoil).")]
        [SerializeField]
        protected Spring m_SecondaryPositionSpring = new();

        [Tooltip("The rotational spring which returns to equilibrium after a small amount of time (for recoil).")]
        [SerializeField]
        protected Spring m_SecondaryRotationSpring = new();

        [Tooltip("The name of the step zoom input mapping.")] [SerializeField]
        protected string m_StepZoomInputName = "Mouse ScrollWheel";

        [Tooltip("Specifies how quickly the camera zooms when step zooming.")] [SerializeField]
        protected float m_StepZoomSensitivity;

        [Tooltip("The minimum distance that the step zoom can zoom.")] [SerializeField]
        protected float m_MinStepZoom;

        [Tooltip("The maximum distance that the step zoom can zoom.")] [SerializeField]
        protected float m_MaxStepZoom = 1;

        public override float LookDirectionDistance => m_LookDirectionDistance;
        ...
        {
            get => m_LookOffset;
            set => m_LookOffset = value;
        }

        public float LookOffsetSmoothing
        {
            get => m_LookOffsetSmoothing;
            set => m_LookOffsetSmoothing = value;
        }
        ...
        public Vector3 CollisionAnchorOffset
        {
            get => m_CollisionAnchorOffset;
            set => m_CollisionAnchorOffset = value;
        }

        public float PositionSmoothing
        {
            get => m_PositionSmoothing;
            set => m_PositionSmoothing = value;
        }

        public float ObstructionPositionSmoothing
        {
            get => m_ObstructionPositionSmoothing;
            set => m_ObstructionPositionSmoothing = value;
        }

        public bool UpdateCharacterRotation
        {
            get => m_UpdateCharacterRotation;
            set => m_UpdateCharacterRotation = value;
        }

        public float AlignToGravityRotationSpeed
        {
            get => m_AlignToGravityRotationSpeed;
            set => m_AlignToGravityRotationSpeed = value;
        }

        public float MinPitchLimit
        {
            get => m_MinPitchLimit;
            set => m_MinPitchLimit = value;
        }

        public float MaxPitchLimit
        {
            get => m_MaxPitchLimit;
            set => m_MaxPitchLimit = value;
        }

        public Spring PositionSpring
        {
            get => m_PositionSpring;
            set
            {
        ...
        public string StepZoomInputName
        {
            get => m_StepZoomInputName;
            set => m_StepZoomInputName = value;
        }

        public float StepZoomSensitivity
        {
            get => m_StepZoomSensitivity;
            set => m_StepZoomSensitivity = value;
        }

        public float MinStepZoom
        {
            get => m_MinStepZoom;
            set => m_MinStepZoom = value;
        }

        public float MaxStepZoom
        {
            get => m_MaxStepZoom;
            set => m_MaxStepZoom = value;
        }

        private Transform m_CrosshairsTransform;
        private Canvas m_CrosshairsCanvas;
        private CrosshairsMonitor m_CrosshairsMonitor;
        ...
        public override Quaternion CharacterRotation => m_CharacterRotation;
        ...
        {
            if (m_Character != null)
                EventHandler.UnregisterEvent<Transform>(m_Character, "OnCharacterChangeMovingPlatforms",
                    OnCharacterChangeMovingPlatforms);

            base.AttachCharacter(character);

            if (m_Character != null)
                EventHandler.RegisterEvent<Transform>(m_Character, "OnCharacterChangeMovingPlatforms",
                    OnCharacterChangeMovingPlatforms);
        }
            ...
            if (activate)
            {
                ...
                if (m_StepZoomSensitivity > 0)
                {
                    if (m_Handler != null)
                    {
                        ...
                        m_StepZoomInputEvent.Initialize(ActiveInputEvent.Type.Axis, m_StepZoomInputName,
                            "OnThirdPersonViewTypeStepZoom");
        ...
        public override void Reset(Quaternion characterRotation)
        {
            ...
            m_Yaw = 0;
            m_CharacterRotation = characterRotation;
            if (m_CharacterLocomotion.Platform != null) UpdatePlatformRotationOffset(m_CharacterLocomotion.Platform);
        ...
        public override void SetCrosshairs(Transform crosshairs)
        {
            ...
            if (m_CrosshairsTransform != null)
            {
                ...
                m_CrosshairsMonitor = crosshairs.gameObject.GetCachedComponent<CrosshairsMonitor>();
        ...
        {
            if (m_CrosshairsTransform == null) return Quaternion.identity;
            ...
            if (m_CrosshairsPosition != m_CrosshairsTransform.position)
            {
                ...
                m_CrosshairsDeltaRotation = Quaternion.LookRotation(direction, m_Transform.up) *
        ...
        private void UpdatePlatformRotationOffset(Transform platform)
        {
            ...
            if (!m_CharacterLocomotion.AlignToGravity)
            {
                ...
                var localPlatformRotationOffset = MathUtility
                    .InverseTransformQuaternion(m_CharacterRotation, m_CharacterPlatformRotationOffset).eulerAngles;
                localPlatformRotationOffset.x = localPlatformRotationOffset.z = 0;
                m_CharacterPlatformRotationOffset = MathUtility.TransformQuaternion(m_CharacterRotation,
                    Quaternion.Euler(localPlatformRotationOffset));
            }
        }
        ...
        public override Quaternion Rotate(float horizontalMovement, float verticalMovement, bool immediateUpdate)
        {
            ...
            if (m_UpdateCharacterRotation)
            {
                ...
                if (m_CharacterLocomotion.Platform != null)
                {
                    m_PlatformRotation = MathUtility.InverseTransformQuaternion(m_CharacterLocomotion.Platform.rotation,
                                             m_CharacterPlatformRotationOffset) *
                                         Quaternion.Inverse(MathUtility.InverseTransformQuaternion(
                                             m_CharacterLocomotion.Platform.rotation, m_CharacterRotation *
                                             Quaternion.Inverse(m_CharacterLocomotion.Platform.rotation)));
                    if (!m_CharacterLocomotion.AlignToGravity)
                    {
                        // Only the local y rotation should affect the character's rotation.
                        var localPlatformTorque = MathUtility
                            .InverseTransformQuaternion(m_CharacterTransform.rotation, m_PlatformRotation).eulerAngles;
                        localPlatformTorque.x = localPlatformTorque.z = 0;
                        m_PlatformRotation = MathUtility.TransformQuaternion(m_CharacterTransform.rotation,
                            Quaternion.Euler(localPlatformTorque));
                    }
                ...
                if (m_CharacterLocomotion.AlignToGravity && m_AlignToGravityRotationSpeed > 0)
                {
                    var localRotation = MathUtility
                        .InverseTransformQuaternion(m_CharacterTransform.rotation, m_CharacterRotation).eulerAngles;
                    localRotation.x = localRotation.z = 0;
                    m_CharacterRotation = Quaternion.Slerp(m_CharacterRotation,
                        MathUtility.TransformQuaternion(m_CharacterTransform.rotation, Quaternion.Euler(localRotation)),
                        m_AlignToGravityRotationSpeed);
                }
            }
            ...
            if (m_AimAssist != null)
            {
                ...
                if (m_AimAssist.HasTarget())
                {
                    ...
                    var assistRotation = rotation *
                                         MathUtility.InverseTransformQuaternion(rotation,
                                             m_AimAssist.TargetRotation(rotation));
                    // Set the pitch and yaw so when the target is lost the view type won't snap back to the previous rotation value.
                    var localAssistRotation = MathUtility
                        .InverseTransformQuaternion(m_CharacterRotation, assistRotation).eulerAngles;
            ...
            return MathUtility.TransformQuaternion(m_CharacterRotation, Quaternion.Euler(m_Pitch, m_Yaw, 0)) *
                   Quaternion.LookRotation(m_ForwardAxis) * Quaternion.Euler(m_RotationSpring.Value) *
        ...
        public override Vector3 Move(bool immediateUpdate)
        {
            ...
            m_CurrentLookOffset = immediateUpdate
                ? m_LookOffset
                : Vector3.SmoothDamp(m_CurrentLookOffset, m_LookOffset, ref m_SmoothLookOffsetVelocity,
                    m_LookOffsetSmoothing);
            var lookPosition = anchorPosition + m_CurrentLookOffset.x * m_Transform.right +
                               m_CurrentLookOffset.y * m_CharacterTransform.up +
                               (m_CurrentLookOffset.z + m_StepZoom) * m_Transform.forward;
            ...
            if (immediateUpdate)
                ...
                targetPosition = Vector3.SmoothDamp(m_Transform.position, lookPosition, ref m_SmoothPositionVelocity,
                    m_PositionSmoothing);
            ...
            if (Physics.SphereCast(anchorPosition + m_CollisionAnchorOffset - normalizedDirection * m_CollisionRadius,
                    m_CollisionRadius, normalizedDirection, out m_RaycastHit, directionMagnitude,
                    m_CharacterLayerManager.IgnoreInvisibleCharacterWaterLayers, QueryTriggerInteraction.Ignore))
            {
                ...
                if (!immediateUpdate)
                    targetPosition = Vector3.SmoothDamp(m_Transform.position + m_CharacterLocomotion.PlatformMovement,
                ...
                if (localDirection.y > 0)
                {
                    ...
                    var constantHeightPosition = MathUtility.InverseTransformPoint(m_CharacterTransform.position,
                        m_CharacterRotation, targetPosition);
                    constantHeightPosition.y = MathUtility.InverseTransformPoint(m_CharacterTransform.position,
                        m_CharacterRotation, lookPosition).y;
                    constantHeightPosition = MathUtility.TransformPoint(m_CharacterTransform.position,
                        m_CharacterRotation, constantHeightPosition);
                    direction = constantHeightPosition - (anchorPosition + m_CollisionAnchorOffset);
                    if (!Physics.SphereCast(
                            anchorPosition + m_CollisionAnchorOffset - normalizedDirection * m_CollisionRadius,
                            m_CollisionRadius, normalizedDirection,
                            out m_RaycastHit, directionMagnitude - m_CollisionRadius,
        ...
        public override Vector3 LookDirection(bool characterLookDirection)
        {
            ...
            return m_Transform.rotation * crosshairsDeltaRotation * Quaternion.Inverse(platformRotation) *
                   Vector3.forward;
        }
            ...
            bool includeRecoil, bool includeMovementSpread)
        {
            ...
            var rotation =
                (includeRecoil
                    ...
                    : MathUtility.TransformQuaternion(m_CharacterRotation, Quaternion.Euler(m_Pitch, m_Yaw, 0)) *
                      Quaternion.LookRotation(m_ForwardAxis)) *
                crosshairsDeltaRotation * Quaternion.Inverse(platformRotation);
            Vector3 hitPoint;
            if (Physics.Raycast(m_Transform.position, rotation * Vector3.forward, out var hit, m_LookDirectionDistance,
            ...
            if (includeMovementSpread && m_CrosshairsMonitor != null)
            {
                ...
                if (spread > 0)
                    direction = Quaternion.AngleAxis(Random.Range(-spread, spread), m_CharacterTransform.up) *
                                Quaternion.AngleAxis(Random.Range(-spread, spread), m_CharacterTransform.right) *
                                m_Transform.forward;
            }
        ...
        {
            m_StepZoom = Mathf.Clamp(m_StepZoom + m_StepZoomSensitivity * amount * Time.deltaTime, m_MinStepZoom,
                m_MaxStepZoom);
        }
        ...
        public override void StateChange()
        {
            ...
            if (!string.IsNullOrEmpty(m_CameraController.ZoomState) && !m_AppendingZoomState)
            {
                ...
                for (var i = 0; i < m_States.Length; ++i)
                    StateManager.SetState(m_GameObject, m_States[i].Name + m_CameraController.ZoomState,
                        m_States[i].Active && m_CameraController.ZoomInput);
               ...
               )
            {
                ...
                if (m_CameraController.ActiveViewType == this)
                {
                    ...
                    if (m_PositionSmoothing == 0 ||
                        (m_PrevPositionSmoothing == 0 && m_PositionSmoothing != m_PrevPositionSmoothing))
                        KinematicObjectManager.SetCameraPosition(m_CameraController.KinematicObjectIndex,
                            (m_Transform.position + Move(true)) * 0.5f);
                }
            }
