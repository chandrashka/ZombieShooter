...
namespace Opsive.UltimateCharacterController.Objects
{
    ...
    /// <summary>
    ///     The Destructible class is an abstract class which acts as the base class for any object that destroys itself and
    ///     applies a damange.
    ...
    {
        [Tooltip("The layers that the object can stick to.")] [SerializeField]
        protected LayerMask m_StickyLayers = ~((1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.Water) |
                                               (1 << LayerManager.UI) | (1 << LayerManager.VisualEffect) |
                                               (1 << LayerManager.Overlay) | (1 << LayerManager.Character) |
                                               (1 << LayerManager.SubCharacter));

        [Tooltip("Should the projectile be destroyed when it collides with another object?")] [SerializeField]
        protected bool m_DestroyOnCollision = true;

        [Tooltip("Should the projectile be destroyed after the particle has stopped emitting?")] [SerializeField]
        protected bool m_WaitForParticleStop;

        [Tooltip("The amount of time after a collision that the object should be destroyed.")] [SerializeField]
        protected float m_DestructionDelay;

        [Tooltip("The objects which should spawn when the object is destroyed.")] [SerializeField]
        protected ObjectSpawnInfo[] m_SpawnedObjectsOnDestruction;

        [Tooltip("Unity event invoked when the destructable hits another object.")] [SerializeField]
        protected UnityFloatVector3Vector3GameObjectEvent m_OnImpactEvent;

        public GameObject Owner => m_Originator;
        public GameObject OriginatingGameObject => m_Originator;
        ...
        protected override void Awake()
        {
            ...
            if (m_TrailRenderer != null) m_TrailRenderer.enabled = false;
            m_ParticleSystem = GetComponent<ParticleSystem>();
            if (m_ParticleSystem != null) m_ParticleSystem.Stop();
            ...
            if (m_DestroyOnCollision && m_CollisionMode != CollisionMode.Collide &&
                GetType() != typeof(MagicProjectile))
            {
                Debug.LogWarning(
                    $"Warning: The Destructible {name} will be destroyed on collision but does not have a Collision Mode set to Collide.");
        ...
        /// <param name="impactStateDisableTimer">The number of seconds until the impact state is disabled.</param>
        /// <param name="surfaceImpact">A reference to the Surface Impact triggered when the object hits an object.</param>
        public void InitializeDestructibleProperties(DamageProcessor damageProcessor, float damageAmount,
            float impactForce, int impactForceFrames, LayerMask impactLayers, string impactStateName,
            float impactStateDisableTimer, SurfaceImpact surfaceImpact)
        {
            m_Destroyed = false;
            ...
            if (m_SurfaceImpact == null) m_SurfaceImpact = surfaceImpact;
        ...
        protected override void OnCollision(RaycastHit? hit)
        {
                ...
                if (hit != null && hit.HasValue && m_Collider != null)
                {
                    ...
                    if (MathUtility.IsUniform(hit.Value.transform.localScale))
                    {
                        ...
                        if (MathUtility.InLayerMask(hit.Value.transform.gameObject.layer, m_StickyLayers))
                        {
                            ...
                            m_StickyCharacterLocomotion = hit.Value.transform.gameObject
                                .GetCachedComponent<UltimateCharacterLocomotion>();
                            if (m_StickyCharacterLocomotion != null)
            ...
            if (m_GameObject == null) InitializeComponentReferences();
            ...
            if (hit != null && hit.HasValue)
            {
                ...
                EventHandler.ExecuteEvent<float, Vector3, Vector3, GameObject, object, Collider>(hitGameObject,
                    "OnObjectImpact", damageAmount, hitValue.point, m_Velocity.normalized * m_ImpactForce, m_Originator,
                    this, hitValue.collider);
                if (m_OnImpactEvent != null)
                    m_OnImpactEvent.Invoke(damageAmount, hitValue.point, m_Velocity.normalized * m_ImpactForce,
                        m_Originator);
                ...
                if (damageAmount > 0)
                {
                    ...
                    if (damageTarget != null)
                    {
                        ...
                        pooledDamageData.SetDamage(this, damageAmount, hitValue.point, -hitValue.normal, m_ImpactForce,
                            m_ImpactForceFrames, 0, hitValue.collider);
                    ...
                    else if (m_ImpactForce > 0)
                    {
                        ...
                        {
                            collisionRigidbody.AddForceAtPosition(
                                m_ImpactForce * MathUtility.RigidbodyForceMultiplier * -hitValue.normal,
                                hitValue.point);
                        }
                        else
                        {
                            var forceObject = hitGameObject.GetCachedParentComponent<IForceObject>();
                            if (forceObject != null) forceObject.AddForce(m_Transform.forward * m_ImpactForce);
                        }
                    }
                }
        ...
        {
            for (var i = 0; i < m_SpawnedObjectsOnDestruction.Length; ++i)
            {
                if (m_SpawnedObjectsOnDestruction[i] == null) continue;

                var spawnedObject = m_SpawnedObjectsOnDestruction[i]
                    .Instantiate(hitPosition, hitNormal, m_NormalizedGravity);
