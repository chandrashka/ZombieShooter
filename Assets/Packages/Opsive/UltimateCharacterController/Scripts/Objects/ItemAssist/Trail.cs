...
{
    /// <summary>
    ///     Builds a mesh which can show a trail following a melee weapon. This is typically used when the melee weapon is
    ///     slashed.
    /// </summary>
    public class Trail : MonoBehaviour
    {
        ...
        [Tooltip("The vertical length of the trail.")] [SerializeField]
        ...
        [SerializeField]
        protected int m_MaxSliceCount = 50;

        [Tooltip(
            "The smoothing value of the curve. A larger value will have a smoother curve compared to a smaller value.")]
        [SerializeField]
        protected int m_CurveSmoothness = 10;

        [Tooltip("The steepness of the curve. A value closer to 1 will increase the steepness.")]
        [Range(0, 1)]
        [SerializeField]
        protected float m_CurveSteepness = 0.5f;

        [Tooltip("The start color of the trail, near the melee weapon object.")] [SerializeField]
        protected Color m_StartColor = Color.white;

        [Tooltip("The end color of the trail.")] [SerializeField]
        protected Color m_EndColor = Color.white;

        [Tooltip("The amount of time the slice should be visible.")] [SerializeField]
        protected float m_VisibilityTime = 1;
        ...
        private readonly TrailSlice[] m_TrailSlices = new TrailSlice[4];
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogError(
                    "Error: Unable to find the MeshFilter component. Enssure the Trail object has been created through the Object Manager.");
                return;
            }
        ...
        {
            if (m_GenerateSlices) SampleTrail();
        }
        ...
        private void SampleTrail()
        {
            ...
            if (m_TrailSlicesCount == 0 ||
                (m_TrailSlices[m_TrailSlicesIndex].Point - m_Transform.position).sqrMagnitude > m_MinDistanceSquared)
            {
                ...
                if (m_TrailSlicesCount > 3)
                {
                    ...
                    if (m_TrailSlicesIndex < 0) m_TrailSlicesIndex += m_TrailSlicesCount;
                ...
                if (m_TrailSlicesCount > 3)
                {
                    ...
                    if (prevIndex < 0) prevIndex = m_TrailSlicesCount - 1;
                    ...
                    AddTrailSlice(trailSlice.Point + (trailSlice.Point - prevTrailSlice.Point).normalized,
                        (trailSlice.Up + prevTrailSlice.Up) / 2);
                }
            }
        }
        ...
        private void AddTrailSlice(Vector3 point, Vector3 up)
        {
            ...
            if (m_TrailSlicesCount > 0 && m_TrailSlices[m_TrailSlicesIndex].Point == point) return;

            m_TrailSlicesIndex = (m_TrailSlicesIndex + 1) % m_TrailSlices.Length;
            m_TrailSlices[m_TrailSlicesIndex].Initialize(point, up);
            if (m_TrailSlicesIndex + 1 > m_TrailSlicesCount) m_TrailSlicesCount++;
        ...
        private void SmoothTrailSlice()
        {
            ...
            if (m_TrailSlicesCount < 4) return;
            ...
            if (startIndex < 0) startIndex = m_TrailSlices.Length + startIndex;
            ...
            for (var t = t1; t < t2; t += iterAmount)
            {
                ...
                if (m_SmoothedTrailSlicesIndex + 1 > m_SmoothedTrailSlicesCount) m_SmoothedTrailSlicesCount++;
            }
        }

        /// <summary>
        ///     Returns the time of the centripetral catmull-rom curve, defined in
        ...
        /// <summary>
        ///     Returns the vertex of the centripetral catmull-rom curve, defined in
        ///     https://en.wikipedia.org/wiki/Centripetal_Catmull–Rom_spline.
        /// </summary>
        /// <returns>The vertex of the centripetral catmull-rom curve.</returns>
        private Vector3 CentripetralCatmullRomValue(Vector3 v0, Vector3 v1, Vector3 v2, Vector3 v3, float t0, float t1,
        ...
        private void RemoveOldSlices()
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                if (trailSlice.Time + m_VisibilityTime > Time.time) break;

                // The slice has existed for more than the visiblity time - remove it by decreasing the count.
                m_TrailSlicesCount--;
            }

            if (m_SmoothedTrailSlicesCount == 0) return;

            startIndex = m_SmoothedTrailSlicesIndex - m_SmoothedTrailSlicesCount + 1;
            if (startIndex < 0) startIndex = m_SmoothedTrailSlices.Length + startIndex;
            ...
            if (m_TrailSlicesCount == 0 && m_SmoothedTrailSlicesCount == 0) m_GenerateSlices = false;
        }
        ...
        private void BuildMesh()
        {
            ...
            for (var i = 0; i < m_SmoothedTrailSlicesCount; ++i)
            {
                var trailSlice = m_SmoothedTrailSlices[(startIndex + i) % m_SmoothedTrailSlices.Length];
