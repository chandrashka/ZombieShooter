...
{
    /// <summary>
    ///     The Spherical Gravity Zone extends the Gravity Zone component by implementing a gravity force that is affected by
    ///     spherical directions.
    /// </summary>
    public class SphericalGravityZone : GravityZone
    {
        [Tooltip(
            "The amount of influence that the gravity force has. An x value of 0 represents the further point away from the sphere (at the distance of the radius), " +
        ...
        [SerializeField]
        protected float m_InfluenceMultiplier = 1;
        ...
        public override Vector3 DetermineGravityDirection(Vector3 position)
        {
            ...
            var influenceFactor =
                m_Influence.Evaluate(1 - direction.magnitude /
                    (m_SphereCollider.radius * MathUtility.ColliderRadiusMultiplier(m_SphereCollider))) *
                m_InfluenceMultiplier;
