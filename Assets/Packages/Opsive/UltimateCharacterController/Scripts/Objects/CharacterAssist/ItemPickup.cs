...
namespace Opsive.UltimateCharacterController.Objects.CharacterAssist
{
    ...
    public class ItemPickup : ItemPickupBase
    {
        ...
        [FormerlySerializedAs("m_ItemTypeCounts")]
        [SerializeField]
            ...
            int slotID, bool immediatePickup, bool forceEquip)
        {
            ...
            if (m_ItemDefinitionAmounts != null)
                for (var i = 0; i < m_ItemDefinitionAmounts.Length; ++i)
                    if (inventory.Pickup(m_ItemDefinitionAmounts[i].ItemIdentifier, m_ItemDefinitionAmounts[i].Amount,
                            slotID, immediatePickup, forceEquip))
                        pickedUp = true;
            return pickedUp;
        }
    }
