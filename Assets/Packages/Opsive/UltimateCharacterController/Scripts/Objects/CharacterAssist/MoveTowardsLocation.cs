...
namespace Opsive.UltimateCharacterController.Objects.CharacterAssist
{
    ...
    {
        [Tooltip("The offset relative to the transform that the character should move towards.")] [SerializeField]
        protected Vector3 m_Offset = new(0, 0, 1);

        [Tooltip("The yaw offset relative to the transform that the character should rotate towards.")] [SerializeField]
        protected float m_YawOffset = 180;
        ...
        {
            get => m_PrecisionStart;
            set => m_PrecisionStart = value;
        }

        public float MovementMultiplier
        {
            get => m_MovementMultiplier;
            set => m_MovementMultiplier = value;
        }

        public Vector3 TargetPosition => m_Transform.TransformPoint(m_Offset);
        public Quaternion TargetRotation => m_Transform.rotation * Quaternion.Euler(0, m_YawOffset, 0);
        public float StartYawOffset { get; private set; }
        ...
        public Vector3 GetTargetDirection(Vector3 position, Quaternion rotation)
        {
            ...
            if (m_Size.sqrMagnitude == 0) return MathUtility.InverseTransformDirection(direction, rotation);
        ...
        public bool IsRotationValid(Quaternion rotation)
        {
            ...
            return Vector3.Angle(Quaternion.Euler(0, m_YawOffset, 0) * forwardDirection, rotation * Vector3.forward) <=
                   m_Angle / 2 + 0.001f;
        }
    }
}