...
namespace Opsive.UltimateCharacterController.Objects.CharacterAssist
{
    ...
    public class AnimatedInteractable : MonoBehaviour, IInteractableTarget, IInteractableMessage
    {
        ...
        protected bool m_SingleInteract;
        ...
        [Tooltip("The trigger parameter name that should be set when interacted with. Can be empty.")] [SerializeField]
        protected string m_TriggerParameter;
        ...
        public bool ActiveBoolInteractable { get; private set; }
        ...
        protected virtual void Awake()
        {
            ...
            if (!string.IsNullOrEmpty(m_BoolParameter)) m_BoolParameterHash = Animator.StringToHash(m_BoolParameter);
            ...
            if (animatedInteractables.Length > 1)
            {
                ...
                var count = 0;
                for (var i = 0; i < animatedInteractables.Length; ++i)
                {
                    if (animatedInteractables[i] == this) continue;
        ...
        public string AbilityMessage()
        {
            if (m_BoolParameterHash != 0)
            {
                ...
                if (m_AnimatedInteractables != null)
                    for (var i = 0; i < m_AnimatedInteractables.Length; ++i)
        ...
        public void Interact(GameObject character)
        {
            if (m_BoolParameterHash != 0)
            {
                ...
                if (m_ToggleBoolInteractValue)
                {
                    ...
                    ActiveBoolInteractable = !ActiveBoolInteractable;
                }
            }

            if (m_TriggerParameterHash != 0) m_Animator.SetTrigger(m_TriggerParameterHash);
