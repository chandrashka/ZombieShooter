...
namespace Opsive.UltimateCharacterController.Objects.CharacterAssist
{
    ...
    {
        [Tooltip("An array of items and ItemSets to pick up.")] [SerializeField]
        protected PickupSet[] m_ItemPickupSet;

        [Tooltip("Should the object be picked up even if the inventory cannot hold any more of the ItemIdentifier?")]
        [SerializeField]
        protected bool m_AlwaysPickup;

        private bool m_PickedUp;

        public PickupSet[] ItemPickupSet
        {
            get => m_ItemPickupSet;
            set => m_ItemPickupSet = value;
        }
        ...
        protected override void Awake()
        {
            ...
            if (m_ItemPickupSet != null)
                for (var i = 0; i < m_ItemPickupSet.Length; ++i)
        ...
        public void TriggerEnter(GameObject other, int slotID)
        {
            ...
            if (inventory == null || !inventory.enabled) return;
            ...
            if (layerManager == null || !MathUtility.InLayerMask(other.layer, layerManager.CharacterLayer)) return;
        ...
        private void TryItemPickup(InventoryBase inventory, int slotID)
        {
            ...
            else
            {
                ...
                var character = inventory.gameObject.GetCachedComponent<UltimateCharacterLocomotion>();
                if (character != null)
                {
                    var pickup = character.GetAbility<Pickup>();
        ...
        /// <param name="pickupItemIdentifier">
        ///     Should the ItemIdentifier be picked up? This should be false if the ItemIdentifier
        ///     will later be picked up.
        /// </param>
        public void DoItemPickup(GameObject character, InventoryBase inventory, int slotID, bool immediatePickup,
            bool pickupItemIdentifier)
        {
            ...
            if (m_ItemPickupSet != null && m_ItemPickupSet.Length > 0)
            {
                ...
                for (var i = 0; i < m_ItemPickupSet.Length; ++i)
                {
                    ...
                    if (m_ItemPickupSet[i].Item == null)
                    {
                        ...
                        if (itemSetManager == null) continue;
                        ...
                        {
                            for (var j = 0; j < m_ItemPickupSet[i].ItemSet.Slots.Length; ++j)
                            {
                                var itemDefinition = m_ItemPickupSet[i].ItemSet.Slots[j];
                                if (itemDefinition != null) category = itemDefinition.GetItemCategory();
                            }
                        }
                        else
                        {
                            ...
                            if (itemSetManager.CategoryItemSets != null)
                                for (var j = 0; j < itemSetManager.CategoryItemSets.Length; ++j)
                        ...
                        if (category != null)
                            itemSetManager.AddItemSet(m_ItemPickupSet[i].ItemSet, m_ItemPickupSet[i].Default, category,
                                addItemSetParents);

                        continue;
                    }

                    if (slotID != -1 && (slotID >= m_ItemPickupSet[i].ItemSet.Slots.Length ||
                                         m_ItemPickupSet[i].ItemSet.Slots[slotID] == null)) continue;
                    ...
                    if (m_ItemPickupSet[i].ItemSet != null)
                    {
                        ...
                        {
                            m_ItemPickupSet[i].ItemSet.ItemIdentifiers =
                                new IItemIdentifier[m_ItemPickupSet[i].ItemSet.Slots.Length];
                            if (item.SlotID >= m_ItemPickupSet[i].ItemSet.ItemIdentifiers.Length)
                            {
                                Debug.LogError(
                                    $"Error: Unable to assign the ItemIdentifier at slot {item.SlotID}. The Slot Count should be increased on the item pickup.");
                                continue;
                            }
            ...
            if (pickupItemIdentifier)
            {
                ...
                if (DoItemIdentifierPickup(character, inventory, slotID, immediatePickup, true)) m_PickedUp = true;
            ...
            if (m_PickedUp) ObjectPickedUp(character);
        }
        ...
        public bool DoItemIdentifierPickup(GameObject character, InventoryBase inventory, int slotID,
        ...
        /// <param name="forceEquip">Should the item be force equipped?</param>
        /// <returns>True if an ItemIdentifier was picked up.</returns>
        protected abstract bool DoItemIdentifierPickupInternal(GameObject character, InventoryBase inventory,
        ...
        {
            [Tooltip("The item prefab that can be picked up.")] [SerializeField]
            protected GameObject m_Item;

            [Tooltip("The ID of the category that the ItemSet should be added to.")] [SerializeField]
            protected uint m_CategoryID;

            [Tooltip("The ItemSet to load when the item is picked up.")] [SerializeField]
            protected ItemSet m_ItemSet;

            [Tooltip("Is the ItemSet the default ItemSet within the category?")] [SerializeField]
