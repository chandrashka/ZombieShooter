...
namespace Opsive.UltimateCharacterController.Objects
{
    ...
    public class TrajectoryObject : MonoBehaviour, IForceObject
    {
        ...
        protected bool m_InitializeOnEnable;
        ...
        protected float m_GravityMagnitude = 9.8f;
        ...
        protected LayerMask m_ImpactLayers = ~((1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.Water) |
                                               (1 << LayerManager.SubCharacter) | (1 << LayerManager.Overlay) |
                                               (1 << LayerManager.VisualEffect));

        [Tooltip("The identifier that is used when the object collides with another object.")] [SerializeField]
        ...
        [FormerlySerializedAs("m_BounceMode")] // 2.2.
        [SerializeField]
        protected CollisionMode m_CollisionMode = CollisionMode.Reflect;
        ...
        [Range(0, 4)]
        [SerializeField]
        protected float m_ReflectMultiplier = 1;
        ...
        [Tooltip("The audio that should be looped while the object is active.")] [SerializeField]
        protected AudioClipSet m_ActiveAudioClipSet = new();
        ...
        {
            get => m_CollisionMode;
            set => m_CollisionMode = value;
        }

        public float ReflectMultiplier
        {
            get => m_ReflectMultiplier;
            set => m_ReflectMultiplier = value;
        }

        public AudioClipSet ActiveAudioClipSet
        {
            get => m_ActiveAudioClipSet;
            set => m_ActiveAudioClipSet = value;
        }

        public GameObject Originator => m_Originator;
        public Vector3 Velocity => m_Velocity;
        public Vector3 Torque => m_Torque;
        ...
        protected virtual void FixedUpdate()
        {
            ...
            if (m_AutoDisable && m_MovementSettled && m_RotationSettled) enabled = false;
        }
        ...
        public void AddForce(Vector3 force)
        {
            ...
            if (m_MovementSettled) m_MovementSettled = false;
        }

        /// <summary>
        ///     Runs a simulation of the parabolic trajectory with the given start and end position. The trajectory will then be
        ///     displayed with the attached LineRenderer.
        /// </summary>
        ...
        public void SimulateTrajectory(GameObject originator, Vector3 startPosition, Vector3 endPosition)
        {
            ...
            // Show the curve.
            LineRenderer.positionCount = m_Positions.Count;
            LineRenderer.SetPositions(m_Positions.ToArray());
        ...
        /// <summary>
        ///     Runs a simulation of the parabolic trajectory.  The trajectory will then be displayed with the attached
        ///     LineRenderer.
        /// </summary>
        /// <param name="originator">The object that instantiated the trajectory object.</param>
        ...
        public void SimulateTrajectory(GameObject originator, Vector3 position, Quaternion rotation, Vector3 velocity,
            Vector3 torque)
        {
        ...
        public void SimulateTrajectory(GameObject originator, Vector3 velocity, Vector3 torque, List<Vector3> positions,
            int positionsSkip)
        {
            Initialize(velocity, torque, originator, false);
        ...
        /// <param name="positions">The list of positions that the object will move through.</param>
        /// <param name="positionsSkip">Reduce the number of saved positions by skipping a specified number of positions.</param>
        public void SimulateTrajectory(Vector3 position, Quaternion rotation, List<Vector3> positions,
            int positionsSkip)
        {
            for (var i = 0; i < m_MaxPositionCount; i++)
            {
                ...
                for (var j = 0; j < positionsSkip + 1; ++j)
                {
                    if (!Move(ref position, rotation))
                    {
        ...
        /// <param name="originator">The object that instantiated the trajectory object.</param>
        /// <param name="originatorCollisionCheck">Should a collision check against the originator be performed?</param>
        /// <param name="defaultNormalizedGravity">
        ///     The normalized gravity direction if a character isn't specified for the
        ///     originator.
        /// </param>
        public virtual void Initialize(Vector3 velocity, Vector3 torque, GameObject originator,
        ...
        {
            if (m_Originator == originator) return;
        ...
        private bool OverlapCast(Vector3 position, Quaternion rotation)
        {
            ...
            if (m_OriginatorTransform == null) return false;

            var hit = 0;
            if (m_Collider is SphereCollider)
            {
                var sphereCollider = m_Collider as SphereCollider;
                hit = Physics.OverlapSphereNonAlloc(
                    MathUtility.TransformPoint(position, m_Transform.rotation, sphereCollider.center),
                    sphereCollider.radius * MathUtility.ColliderRadiusMultiplier(sphereCollider), m_ColliderHit,
            ...
            else if (m_Collider is CapsuleCollider)
            {
                ...
                hit = Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap,
                    capsuleCollider.radius * MathUtility.ColliderRadiusMultiplier(capsuleCollider), m_ColliderHit,
                    m_ImpactLayers, QueryTriggerInteraction.Ignore);
            }
            else if (m_Collider is BoxCollider)
            {
                ...
                hit = Physics.OverlapBoxNonAlloc(MathUtility.TransformPoint(position, boxRotation, boxCollider.center),
                    Vector3.Scale(boxCollider.size, boxCollider.transform.lossyScale) / 2, m_ColliderHit, boxRotation,
                    m_ImpactLayers, QueryTriggerInteraction.Ignore);
            }
...
// The object should not hit any colliders who are a child of the camera.
        ...
        protected void InitializeComponentReferences()
        {
            ...
            for (var i = 0; i < colliders.Length; ++i)
            {
                ...
                if (!(colliders[i] is SphereCollider) && !(colliders[i] is CapsuleCollider) &&
                    !(colliders[i] is BoxCollider)) continue;
        ...
        private bool Move(ref Vector3 position, Quaternion rotation)
        {
            ...
            if (m_MovementSettled && m_RotationSettled && m_SettleThreshold > 0) return true;
            ...
            if (SingleCast(position, rotation, direction))
            {
                ...
                if (m_CollisionMode != CollisionMode.Ignore)
                {
                    // Reflect and Random Reflect.
                    Vector3 velocity;
                    ...
                    var dynamicFrictionValue = m_Collider != null
                        ? Mathf.Clamp01(1 - MathUtility.FrictionValue(m_Collider.material,
                            m_RaycastHit.collider.material, true))
                        : 0;
                    // Update the velocity to the reflection direction.
                    m_Velocity = dynamicFrictionValue * m_ReflectMultiplier *
                                 Vector3.Reflect(velocity, m_RaycastHit.normal);
                    if (m_Velocity.magnitude < m_StartSidewaysVelocityMagnitude) m_MovementSettled = true;
                    m_Collided = true;
                    return false;
                }
            }
        ...
        protected virtual void OnCollision(RaycastHit? hit)
        {
            if (hit != null && hit.HasValue)
            {
                ...
                if (m_SurfaceImpact != null)
                    SurfaceManager.SpawnEffect(hit.Value, m_SurfaceImpact, m_NormalizedGravity, m_TimeScale,
                        m_GameObject);
            }
        }
        ...
        protected virtual bool SingleCast(Vector3 position, Quaternion rotation, Vector3 direction)
        {
            ...
            {
                var sphereCollider = m_Collider as SphereCollider;
                hit = Physics.SphereCast(position,
                    sphereCollider.radius * MathUtility.ColliderRadiusMultiplier(sphereCollider), direction.normalized,
                    out m_RaycastHit, direction.magnitude + c_ColliderSpacing, m_ImpactLayers,
            ...
            else if (m_Collider is CapsuleCollider)
            {
                ...
                hit = Physics.CapsuleCast(startEndCap, endEndCap, radius, direction.normalized, out m_RaycastHit,
                    direction.magnitude + c_ColliderSpacing, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            }
            else if (m_Collider is BoxCollider)
            {
                var boxCollider = m_Collider as BoxCollider;
                hit = Physics.BoxCast(m_Transform.TransformPoint(boxCollider.center), boxCollider.size / 4,
                    direction.normalized, out m_RaycastHit, m_Transform.rotation,
                    direction.magnitude + c_ColliderSpacing, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            }
            else
            {
                // No collider attached.
                hit = Physics.Raycast(position, direction.normalized, out m_RaycastHit,
                    direction.magnitude + c_ColliderSpacing, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            }
        ...
        private void Rotate(Vector3 position, ref Quaternion rotation)
        {
            ...
            if ((m_CollisionMode == CollisionMode.Collide || m_Collided) &&
            ...
            if (m_Collider is CapsuleCollider)
            {
                Vector3 startEndCap, endEndCap;
                var capsuleCollider = m_Collider as CapsuleCollider;
                MathUtility.CapsuleColliderEndCaps(capsuleCollider, position, targetRotation, out startEndCap,
                    out endEndCap);
                hitCount = Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap,
                    ...
                    m_ColliderHit, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            }
            else if (m_Collider is BoxCollider)
            {
                var boxCollider = m_Collider as BoxCollider;
                ...
                hitCount = Physics.OverlapBoxNonAlloc(
                    MathUtility.TransformPoint(position, boxRotation, boxCollider.center),
                    Vector3.Scale(boxCollider.size, boxCollider.transform.lossyScale) / 2, m_ColliderHit, boxRotation,
                    m_ImpactLayers, QueryTriggerInteraction.Ignore);
            }

            // Apply the rotation if the rotation doesnt intersect any object.
            if (hitCount == 0) rotation = targetRotation;
        }
