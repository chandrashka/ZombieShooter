...
{
    /// <summary>
    ///     The MovingPlatform component will move an object from one point to another. GameObjects with the Moving Platform
    ///     component should be on the MovingPlatform layer.
    /// </summary>
    ...
    public class MovingPlatform : StateBehavior, IKinematicObject, IInteractableTarget
    {
        ...
        [Tooltip("The waypoints to traverse.")] [SerializeField]
        protected Waypoint[] m_Waypoints;

        [Tooltip("Specifies the direction that the platform should traverse.")] [SerializeField]
        protected PathDirection m_Direction;

        [Tooltip("Specifies how the platform traverses through waypoints.")] [SerializeField]
        protected PathMovementType m_MovementType;

        [Tooltip("If using the Target PathMovementType, specifies the waypoint index to move towards.")]
        [SerializeField]
        protected int m_TargetWaypoint;

        [Tooltip("The speed at which the platform should move.")] [SerializeField]
        protected float m_MovementSpeed = 0.1f;

        [Tooltip("Specifies how the platform should interpolate the movement speed.")] [SerializeField]
        protected MovementInterpolationMode m_MovementInterpolation = MovementInterpolationMode.EaseInOut;

        [Tooltip("Specifies how the platform should interpolate the rotation speed.")] [SerializeField]
        protected RotateInterpolationMode m_RotationInterpolation;

        [Tooltip(
            "If using the CustomEaseOut RotationInterpolationMode, specifies the amount to ease into the target rotation.")]
        [SerializeField]
        protected float m_RotationEaseAmount = 0.1f;

        [Tooltip("If using the CustomRotate RotationInterpolationMode, specifies the rotation speed.")] [SerializeField]
        protected Vector3 m_CustomRotationSpeed;

        [Tooltip("The maximum angle that the platform can rotate. Set to -1 to have no max angle.")] [SerializeField]
        protected float m_MaxRotationDeltaAngle = -1;

        [Tooltip("The layer of the character that can activate the moving platform.")] [SerializeField]
        protected LayerMask m_CharacterTriggerLayer = 1 << LayerManager.Character;

        [Tooltip("The state name that should activate when the character enters the platform trigger.")]
        [SerializeField]
        protected string m_CharacterTriggerState;

        [Tooltip("Should the platform be enabled when interacted with?")] [SerializeField]
        protected bool m_EnableOnInteract;

        [Tooltip("Should the directions be changed if the character interacts with the platform while it is moving?")]
        [SerializeField]
        protected bool m_ChangeDirectionsOnInteract;

        protected int m_ActiveCharacterCount;
        private readonly AnimationCurve m_EaseInOutCurve = AnimationCurve.EaseInOut(0, 0, 1, 1);

        protected GameObject m_GameObject;

        private readonly AnimationCurve m_LinearCurve = AnimationCurve.Linear(0, 0, 1, 1);
        ...
        public PathMovementType MovementType
        {
            get => m_MovementType;
        ...
        {
            get => m_TargetWaypoint;
            set => m_TargetWaypoint = value;
        }
        ...
        public MovementInterpolationMode MovementInterpolation
        {
            get => m_MovementInterpolation;
            set => m_MovementInterpolation = value;
        }

        public RotateInterpolationMode RotationInterpolation
        {
            get => m_RotationInterpolation;
            set => m_RotationInterpolation = value;
        }

        public float RotationEaseAmount
        {
            get => m_RotationEaseAmount;
            set => m_RotationEaseAmount = value;
        }

        public Vector3 CustomRotationSpeed
        {
            get => m_CustomRotationSpeed;
            set => m_CustomRotationSpeed = value;
        }

        public LayerMask CharacterTriggerLayer
        {
            get => m_CharacterTriggerLayer;
            set => m_CharacterTriggerLayer = value;
        }

        public string CharacterTriggerState
        {
            get => m_CharacterTriggerState;
            set => m_CharacterTriggerState = value;
        }

        public bool EnableOnInteract
        {
            get => m_EnableOnInteract;
            set => m_EnableOnInteract = value;
        }

        public bool ChangeDirectionsOnInteract
        {
            get => m_ChangeDirectionsOnInteract;
            set => m_ChangeDirectionsOnInteract = value;
        }

        public int NextWaypoint => m_NextWaypoint;
        public int ActiveCharacterCount => m_ActiveCharacterCount;
        ...
        protected override void Awake()
        {
            ...
            for (var i = 0; i < m_Waypoints.Length; ++i)
                ...
                {
                    Debug.LogError(
                        $"Error: Moving Platform {gameObject.name} has a null waypoint. This platform will be disabled.");
                    enabled = false;
                    return;
                }
#endif
            ...
            {
                Debug.LogWarning(
                    $"Warning: {m_GameObject.name} is a moving platform not using the MovingPlatform layer. Please change this layer.",
                    m_GameObject);
            ...
            {
                for (var i = 0; i < m_Waypoints.Length; ++i) m_Waypoints[i].Initialize();
            ...
            if (m_EnableOnInteract) enabled = false;
        }
        ...
        {
            if (!MathUtility.InLayerMask(other.gameObject.layer, m_CharacterTriggerLayer)) return;
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            if (m_ActiveCharacterCount == 0) return;
        ...
        public virtual void Move()
        {
            ...
            if (m_NextWaypointEvent != null || m_Waypoints.Length == 0) return;
        ...
        {
            if (GetRemainingDistance() < 0.01f && m_NextWaypointEvent == null &&
                (m_MovementType != PathMovementType.Target || m_NextWaypoint != m_TargetWaypoint))
        ...
        protected void UpdateWaypoint()
        {
            ...
            if (m_Direction == PathDirection.Backwards) UpdateState();
            m_PreviousWaypoint = m_NextWaypoint;
            m_NextWaypointEvent = null;
            ...
            switch (m_MovementType)
            {
                case PathMovementType.Target:
                    if (m_NextWaypoint != m_TargetWaypoint) GoToNextWaypoint();
                    break;
                ...
                case PathMovementType.PingPong:
                    ...
                    {
                        if (m_NextWaypoint == 0) m_Direction = PathDirection.Forward;
                    }
                    else
                    {
                        if (m_NextWaypoint == m_Waypoints.Length - 1) m_Direction = PathDirection.Backwards;
                    }
            ...
            // The state should always reflect the state of the next waypoint. If moving in reverse then the state has to be updated before the index changes.
            if (m_Direction == PathDirection.Forward) UpdateState();
            m_NextWaypointEvent = null;
        }
        ...
        {
            if (m_Waypoints.Length == 0) return float.MaxValue;
        ...
        private int GetNextWaypoint(bool increase)
        {
            ...
            if (m_NextWaypoint < 0) m_NextWaypoint = 0;
        ...
        private void UpdateRotation()
        {
            switch (m_RotationInterpolation)
            {
                case RotateInterpolationMode.SyncToMovement:
                    if (m_NextWaypointEvent == null && m_NextWaypointDistance > 0)
                        m_MoveRotation = Quaternion.Lerp(m_OriginalRotation, m_TargetRotation,
                            1.0f - GetRemainingDistance() / m_NextWaypointDistance);
                    break;
                case RotateInterpolationMode.EaseOut:
                    m_MoveRotation = Quaternion.Lerp(m_Transform.rotation, m_TargetRotation,
            ...
            if (m_MaxRotationDeltaAngle != -1)
                m_MoveRotation =
                    Quaternion.RotateTowards(m_Transform.rotation, m_MoveRotation, m_MaxRotationDeltaAngle);
        }
        ...
        private void UpdatePosition()
        {
            switch (m_MovementInterpolation)
            {
                case MovementInterpolationMode.EaseInOut:
                    m_MovePosition = Vector3.Lerp(m_Transform.position, m_TargetPosition,
                ...
                case MovementInterpolationMode.Slerp:
                    m_MovePosition = Vector3.Slerp(m_Transform.position, m_TargetPosition,
                        m_LinearCurve.Evaluate(m_MoveTime));
                    break;
        ...
        {
            [Tooltip("The transform the waypoint that the platform should traverse.")] [SerializeField]
            private Transform m_Transform;

            [Tooltip(
                "The amount of time that the platform should stay at the current waypoint before moving to the next waypoint.")]
            [SerializeField]
            private float m_Delay;

            [Tooltip("The state that should be triggered when the platform is moving towards it.")] [SerializeField]
            ...
            {
                StateHash = Serialization.StringHash(m_State);
            }
        }
#if UNITY_EDITOR
        [Tooltip("The color to draw the editor gizmo in (editor only).")] [SerializeField]
        protected Color m_GizmoColor = new(0, 0, 1, 0.3f);

        [Tooltip("Should the delay and distance labels be drawh t0 tye scene view (editor only)?")] [SerializeField]
        protected bool m_DrawDebugLabels;
#endif
#if UNITY_EDITOR
        [Shared.Utility.NonSerialized]
        ...
        {
            get => m_DrawDebugLabels;
            set => m_DrawDebugLabels = value;
        }
#endif
    }
}