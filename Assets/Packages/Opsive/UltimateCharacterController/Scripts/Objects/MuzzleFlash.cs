...
{
    /// <summary>
    ///     Shows an object which slowly fades out with time. Can optionally attach a light to the GameObject and that light
    ///     will be faded as well.
    /// </summary>
    public class MuzzleFlash : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (muzzleRenderer != null) m_Material = muzzleRenderer.sharedMaterial;
            ...
            if (m_Light != null) m_StartLightIntensity = m_Light.intensity;
        ...
        {
            m_Color.a = 0;
            if (m_Material != null) m_Material.SetColor(m_TintColorPropertyID, m_Color);
            if (m_Light != null) m_Light.intensity = 0;
            if (m_Particles != null) m_Particles.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
        }

        /// <summary>
        ///     A weapon has been fired and the muzzle flash needs to show. Set the starting alpha value and light intensity if the
        ///     light exists.
        /// </summary>
        ...
        public void Show(Item item, int itemActionID, bool pooled, UltimateCharacterLocomotion characterLocomotion)
        {
            ...
            if (!m_Pooled) m_GameObject.SetActive(true);
            ...
            if (m_Light != null) m_Light.intensity = m_StartLightIntensity;
            if (m_Particles != null) m_Particles.Play(true);
            // The muzzle flash may be inactive if the object isn't pooled.
            if (!m_Pooled) m_GameObject.SetActive(true);
        }

        /// <summary>
        ///     Decrease the alpha value of the muzzle flash to give it a fading effect. As soon as the alpha value reaches zero
        ///     place the muzzle flash back in
        ...
        private void Update()
        {
            if (m_Color.a > 0)
            {
                ...
                if (m_Light != null) m_Light.intensity = m_StartLightIntensity * (m_Color.a / m_StartAlpha);
            }
            else
            {
                if (m_Pooled)
 ...
 (firstPersonPerspective ? itemAction.FirstPersonPerspectiveProperties : itemAction.ThirdPersonPerspectiveProperties);
            var muzzleFlashLocation =
 (perspectiveProperties as IShootableWeaponPerspectiveProperties).MuzzleFlashLocation;
