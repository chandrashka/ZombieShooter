...
namespace Opsive.UltimateCharacterController.Objects
{
    ...
    {
        [Tooltip("Should the object explode when the object is enabled?")] [SerializeField]
        protected bool m_ExplodeOnEnable;

        [Tooltip("Determines how far out the explosion affects other objects.")] [SerializeField]
        protected float m_Radius = 5;
        ...
        protected int m_ImpactForceFrames = 1;

        [Tooltip("The layers that the explosion can affect.")] [SerializeField]
        ...
        [Tooltip("Does the explosion require line of sight in order to damage the hit object?")] [SerializeField]
        protected bool m_LineOfSight;

        [Tooltip("The duration of the explosion.")] [SerializeField]
        protected float m_Lifespan = 3;

        [Tooltip("The maximum number of objects that the explosions can detect.")] [SerializeField]
        protected int m_MaxCollisionCount = 100;

        [Tooltip("A set of AudioClips that can be played when the explosion occurs.")] [SerializeField]
        protected AudioClipSet m_ExplosionAudioClipSet = new();

        [Tooltip("Unity event invoked when the explosion hits another object.")] [SerializeField]
        ...
        private readonly HashSet<object> m_ObjectExplosions = new();
        private RaycastHit m_RaycastHit;
        private Transform m_Transform;

        public bool ExplodeOnEnable
        {
            get => m_ExplodeOnEnable;
            set => m_ExplodeOnEnable = value;
        }
        ...
        public AudioClipSet ExplosionAudioClipSet
        {
            get => m_ExplosionAudioClipSet;
            set => m_ExplosionAudioClipSet = value;
        }
        ...
        {
            if (m_ExplodeOnEnable) Explode(m_DamageAmount, m_ImpactForce, m_ImpactForceFrames, null);
        }
        ...
        public void Explode(float damageAmount, float impactForce, int impactForceFrames, GameObject originator)
        {
            ...
            var hitCount = Physics.OverlapSphereNonAlloc(m_Transform.position, m_Radius, m_CollidersHit, m_ImpactLayers,
                QueryTriggerInteraction.Ignore);
#if UNITY_EDITOR
            if (hitCount == m_MaxCollisionCount)
                Debug.LogWarning("Warning: The maximum number of colliders have been hit by " + m_GameObject.name +
                                 ". Consider increasing the Max Collision Count value.");
#endif
            for (var i = 0; i < hitCount; ++i)
            {
                ...
                if (m_ObjectExplosions.Contains(m_CollidersHit[i].gameObject)) continue;
                ...
                {
                    if (m_ObjectExplosions.Contains(forceObject)) continue;
                ...
                if (m_LineOfSight)
                {
                    ...
                    if (Physics.Raycast(position - direction.normalized * 0.1f, direction, out m_RaycastHit,
                            direction.magnitude, m_ImpactLayers, QueryTriggerInteraction.Ignore) &&
                        !m_RaycastHit.transform.IsChildOf(m_CollidersHit[i].transform))
                    {
                        ...
                        var parentAnimator =
                            m_CollidersHit[i].transform.gameObject.GetCachedParentComponent<Animator>();
                        if (parentAnimator != null && parentAnimator.isHuman)
                        {
                            ...
                            if (Physics.Raycast(position, direction, out m_RaycastHit, direction.magnitude,
                                    m_ImpactLayers, QueryTriggerInteraction.Ignore) &&
                                !m_RaycastHit.transform.IsChildOf(m_CollidersHit[i].transform) &&
                                !m_CollidersHit[i].transform.IsChildOf(m_RaycastHit.transform) &&
                ...
                if ((shieldCollider = m_CollidersHit[i].transform.gameObject.GetCachedComponent<ShieldCollider>()) !=
                    null) hitDamageAmount = shieldCollider.Shield.Damage(this, hitDamageAmount);
#endif

                // ClosestPoint only works with a subset of collider types.
                Vector3 closestPoint;
                if (m_CollidersHit[i] is BoxCollider || m_CollidersHit[i] is SphereCollider ||
                    m_CollidersHit[i] is CapsuleCollider || (m_CollidersHit[i] is MeshCollider &&
                                                             (m_CollidersHit[i] as MeshCollider).convex))
                ...
                EventHandler.ExecuteEvent<float, Vector3, Vector3, GameObject, object, Collider>(
                    m_CollidersHit[i].transform.gameObject, "OnObjectImpact", hitDamageAmount, closestPoint,
                    hitDirection * m_ImpactForce, originator, this, m_CollidersHit[i]);
                ...
                if (hitDamageAmount > 0)
                {
                    ...
                    if (damageTarget != null)
                    {
                        ...
                        pooledDamageData.SetDamage(hitDamageAmount * damageModifier, m_Transform.position,
                            hitDirection.normalized, impactForce * damageModifier, impactForceFrames, m_Radius,
                            originator, this, null);
                    ...
                    {
                        colliderRigidbody.AddExplosionForce(impactForce * MathUtility.RigidbodyForceMultiplier,
                            m_Transform.position, m_Radius);
                    }
                }
            }
