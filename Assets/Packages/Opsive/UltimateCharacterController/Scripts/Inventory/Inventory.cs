...
namespace Opsive.UltimateCharacterController.Inventory
{
    ...
    public class Inventory : InventoryBase
    {
        ...
        [FormerlySerializedAs("m_ItemTypeCount")]
        [SerializeField]
        protected ItemDefinitionAmount[] m_DefaultLoadout;

        private Item[] m_ActiveItem;
        private readonly Dictionary<IItemIdentifier, int> m_ItemIdentifierAmount = new();

        private Dictionary<IItemIdentifier, Item>[] m_ItemIdentifierMap;

        public ItemDefinitionAmount[] DefaultLoadout
        {
        ...
        protected override void Awake()
        {
            ...
            for (var i = 0; i < m_SlotCount; ++i) m_ItemIdentifierMap[i] = new Dictionary<IItemIdentifier, Item>();
        ...
        public override void LoadDefaultLoadout()
        {
            if (m_DefaultLoadout != null)
                for (var i = 0; i < m_DefaultLoadout.Length; ++i)
        ...
        protected override bool AddItemInternal(Item item)
        {
            ...
            {
                Debug.LogError(
                    $"Error: Unable to add {item.gameObject.name} because the inventory component doesn't exist.");
                return false;
            }
            ...
            {
                Debug.LogError(
                    $"Error: Unable to add {item.gameObject.name} because the slot id is greater than the number of slots that exist in the inventory.");
                return false;
            }

            var itemIdentifier = item.ItemIdentifier;
            if (itemIdentifier == null) itemIdentifier = item.ItemDefinition.CreateItemIdentifier();

            // The item may already exist in the inventory.
            if (m_ItemIdentifierMap[item.SlotID].ContainsKey(itemIdentifier)) return false;
            ...
            if (!m_ItemIdentifierAmount.ContainsKey(itemIdentifier)) m_ItemIdentifierAmount.Add(itemIdentifier, 0);
            return true;
        }
        ...
        protected override bool PickupInternal(IItemIdentifier itemIdentifier, int amount)
        {
            ...
            else
            {
                if (itemIdentifier is ItemType)
                {
                    ...
                    if (existingAmount == itemType.Capacity) return false;
        ...
        {
            if (slotID < -1 || slotID >= m_ItemIdentifierMap.Length) return null;
        ...
        {
            if (itemIdentifier == null || slotID < 0 || slotID >= m_ItemIdentifierMap.Length) return null;

            if (m_ItemIdentifierMap[slotID].TryGetValue(itemIdentifier, out var item)) return item;
            return null;
        }
        ...
        {
            if (itemIdentifier == null || slotID < -1 || slotID >= m_ItemIdentifierMap.Length) return null;

            // The ItemIdentifier has to exist in the inventory.
            if (!m_ItemIdentifierMap[slotID].TryGetValue(itemIdentifier, out var item))
            {
                Debug.LogError(
                    $"Error: Unable to equip item with ItemIdentifier {itemIdentifier}: the itemIdentifier hasn't been added to the inventory.");
                return null;
            }
        ...
        protected override void RemoveItemIdentifierInternal(IItemIdentifier itemIdentifier, int slotID, int amount)
        {
            if (itemIdentifier == null || !(itemIdentifier is ItemType) || slotID < -1 ||
                slotID >= m_ItemIdentifierMap.Length ||
                !m_ItemIdentifierAmount.TryGetValue(itemIdentifier, out var existingAmount))
                return;
            ...
            var itemType = itemIdentifier as ItemType;
            m_ItemIdentifierAmount[itemType] =
                Mathf.Clamp(item.FullInventoryDrop ? 0 : existingAmount - 1, 0, itemType.Capacity);
