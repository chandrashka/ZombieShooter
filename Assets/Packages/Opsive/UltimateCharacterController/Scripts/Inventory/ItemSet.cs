    ...
    public class ItemSet : StateObject
    {
        ...
        public ItemSet(ItemSet itemSet)
        {
            ...
            if (itemSet.ItemIdentifiers != null && itemSet.ItemIdentifiers.Length > 0)
                Array.Copy(itemSet.ItemIdentifiers, m_ItemIdentifiers, itemSet.ItemIdentifiers.Length);
        ...
        public ItemSet(int slotCount, int slotID, ItemDefinitionBase itemDefinition, IItemIdentifier itemIdentifier,
            string state)
        {
        ...
        public void Initialize(GameObject gameObject, ItemSetManagerBase itemSetManager, uint categoryID,
            int categoryIndex, int index)
        {
            ...
            var equipUnequipAbilities = gameObject.GetCachedComponent<UltimateCharacterLocomotion>()
                .GetAbilities<EquipUnequip>();
            ...
            EventHandler.RegisterEvent<int, int>(m_ItemSetManager.gameObject, "OnItemSetManagerUpdateItemSet",
                OnUpdateItemSet);
        }
        ...
        public override void StateChange()
        {
            ...
            if (m_Active)
            {
                if (m_Enabled)
                {
                    var targetItemSetIndex = m_EquipUnequip.IsActive
                        ? m_EquipUnequip.ActiveItemSetIndex
                        : m_ItemSetManager.ActiveItemSetIndex[m_CategoryIndex];
                    if ((targetItemSetIndex == -1 ||
                         targetItemSetIndex == m_ItemSetManager.GetDefaultItemSetIndex(m_CategoryIndex)) &&
                        targetItemSetIndex != m_Index) m_EquipUnequip.StartEquipUnequip(m_Index);
                }
                else
                {
                    if (m_DisabledIndex == -1)
                    {
                        ...
                        if (m_Index == defaultItemSetIndex ||
                            !m_ItemSetManager.IsItemSetValid(m_CategoryIndex, defaultItemSetIndex, false))
        ...
        {
            if (m_ItemSetManager == null) return;
            EventHandler.UnregisterEvent<int, int>(m_ItemSetManager.gameObject, "OnItemSetManagerUpdateItemSet",
                OnUpdateItemSet);
        }
    }
