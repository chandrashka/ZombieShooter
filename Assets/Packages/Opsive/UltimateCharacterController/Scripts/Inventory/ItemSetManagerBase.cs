...
namespace Opsive.UltimateCharacterController.Inventory
{
    ...
    {
        [Tooltip("Sepcifies the order that the items can be equipped.")] [SerializeField]
        protected CategoryItemSet[] m_CategoryItemSets;

        protected int[] m_ActiveItemSetIndex;
        protected Dictionary<IItemCategoryIdentifier, int> m_CategoryIndexMap;
        private readonly HashSet<IItemIdentifier> m_CheckedItemIdentifiers = new();
        ...
        public CategoryItemSet[] CategoryItemSets
        {
            get => m_CategoryItemSets;
            set => m_CategoryItemSets = value;
        }

        public int[] ActiveItemSetIndex => m_ActiveItemSetIndex;
        public int[] NextItemSetIndex => m_NextItemSetIndex;
        ...
        public int CategoryToIndex(IItemCategoryIdentifier category)
        {
            ...
            if (m_CategoryIndexMap.TryGetValue(category, out var index)) return index;
        ...
        public bool IsCategoryMember(ItemDefinitionBase itemDefinition, int categoryIndex)
        {
            ...
            if (itemDefinition.GetItemCategory() == null) return true;
        ...
        {
            if (categoryIndex >= m_CategoryItemSets.Length || categoryIndex < 0) return false;

            if (itemCategory == m_CategoryItemSets[categoryIndex].ItemCategory) return true;

            // Recursively search the parents.
            var categoryParents = itemCategory.GetDirectParents();
            if (categoryParents == null) return false;
            for (var i = 0; i < categoryParents.Count; ++i)
        ...
        private bool IsDefaultItemCategory(ItemDefinitionBase itemDefinition, IItemCategoryIdentifier itemCategory)
        {
            ...
            if (m_CategoryItemSets[index].DefaultItemSetIndex != m_ActiveItemSetIndex[index]) return false;
            ...
            for (var i = 0; i < itemSetList.Slots.Length; ++i)
            ...
            if (!hasItemDefinition) return false;
        ...
        protected virtual void AddItem(Item item, IItemCategoryIdentifier category)
        {
            ...
            {
                Debug.LogError(
                    "Error: No category has been specified. Ensure the ItemIdentifier has been added to a category.");
                return;
            }
        ...
        protected void AddItemSet(Item item, IItemCategoryIdentifier category, Func<ItemSet, bool> ignoreItemSet)
        {
            ...
            if (category == null || !m_CategoryIndexMap.TryGetValue(category, out var categoryIndex)) return;
            ...
            for (var i = 0; i < m_CategoryItemSets.Length; ++i)
            {
                ...
                for (var j = 0; j < itemSetList.Count; ++j)
                {
                    if (ignoreItemSet?.Invoke(itemSetList[j]) ?? false) continue;
                    ...
                    if (itemSetList[j].ItemIdentifiers[item.SlotID] == null &&
                        itemSetList[j].Slots[item.SlotID] == item.ItemDefinition)
                    {
                        // The item definition matches. Newly added items need to have an ItemSet to themselves.
                        var dedicatedItemDefinition = true;
                        for (var k = 0; k < itemSetList[j].Slots.Length; ++k)
                        {
                            if (k == item.SlotID) continue;
                        ...
                        if (dedicatedItemDefinition)
                        {
                            ...
                            addItemSet =
                                false; // Do not break within the loop because multiple definitions may exist for the same identifier.
                        }
                        else
                        {
                            itemSetList[j].ItemIdentifiers[item.SlotID] = item.ItemIdentifier;
                    ...
                    else if (IsChildOf(item.ItemDefinition, itemSetList[j].Slots[item.SlotID]) &&
                             !string.IsNullOrEmpty(itemSetList[j].State) && string.IsNullOrEmpty(stateName))
                    {
                        var dedicatedItemDefinition = true;
                        for (var k = 0; k < itemSetList[j].Slots.Length; ++k)
                        {
                        ...
                        if (dedicatedItemDefinition) stateName = itemSetList[j].State;
                    }
                }
            }
            ...
            if (addItemSet)
            {
                var itemSet = new ItemSet(m_Inventory.SlotCount, item.SlotID, item.ItemDefinition, item.ItemIdentifier,
                    stateName);
                ...
                {
                    var insertIndex = -1;
                    for (var i = 0; i < itemSetList.Count; ++i)
                    {
                        if (ignoreItemSet?.Invoke(itemSetList[i]) ?? false) continue;
                        if (IsChildOf(item.ItemDefinition, itemSetList[i].Slots[item.SlotID]))
                        {
                            // The other slot elements must be empty.
                            var canInsert = true;
                            for (var j = 0; j < itemSetList[i].Slots.Length; ++j)
                            {
                                if (j == item.SlotID) continue;
                    ...
                    if (insertIndex <= m_ActiveItemSetIndex[categoryIndex])
                    {
                        ...
                        EventHandler.ExecuteEvent(m_GameObject, "OnItemSetIndexChange", categoryIndex,
                            m_ActiveItemSetIndex[categoryIndex]);
                    }

                    // The ItemSet must be duplicated for any ItemDefinition that has a similar parent.
                    for (var i = itemSetList.Count - 1; i >= 0; --i)
                    {
                        if (ignoreItemSet?.Invoke(itemSetList[i]) ?? false) continue;
                        if (IsChildOf(item.ItemDefinition, itemSetList[i].Slots[item.SlotID]))
                ...
                else
                {
                    ...
                    itemSet.Initialize(m_GameObject, this, m_CategoryItemSets[categoryIndex].CategoryID, categoryIndex,
                        itemSetList.Count);
                    // The ItemDefinition may already exists within the ItemSetList. Use the same state name.
                    for (var i = 0; i < itemSetList.Count; ++i)
                        if (IsChildOf(item.ItemDefinition, itemSetList[i].Slots[item.SlotID]) &&
                            !string.IsNullOrEmpty(itemSetList[i].State))
                        {
            ...
            else if (!item.UniqueItemSet)
            {
                ...
                for (var i = 0; i < itemSetList.Count; ++i)
                {
                    if (itemSetList[i].Slots[item.SlotID] != item.ItemDefinition ||
                        itemSetList[i].ItemIdentifiers[item.SlotID] == item.ItemIdentifier) continue;
                    if (ignoreItemSet?.Invoke(itemSetList[i]) ?? false) continue;
        ...
        {
            if (possibleChild == null || possibleParent == null) return false;

            var itemDefinition = possibleChild;
            while (itemDefinition != null)
            {
                if (itemDefinition == possibleParent) return true;
        ...
        private void DuplicateItemSet(Item item, int categoryIndex, int itemSetIndex)
        {
            ...
            duplicatedItemSet.Initialize(m_GameObject, this, m_CategoryItemSets[categoryIndex].CategoryID,
                categoryIndex, itemSetIndex + 1);
            itemSetList.Insert(itemSetIndex + 1, duplicatedItemSet);

            // All of the subsequent ItemSets need to update their index.
            for (var i = itemSetIndex + 2; i < itemSetList.Count; ++i) itemSetList[i].Index = i;
        ...
        private void AddItemSet(ItemSet itemSet, bool defaultItemSet, IItemCategoryIdentifier category)
        {
            ...
            // The category may not have been added to the ItemSetManager.
            if (!m_CategoryIndexMap.TryGetValue(category, out var categoryIndex)) return;
            ...
            if (addItemSet) AddItemSet(itemSet, defaultItemSet, categoryIndex);
        }
        ...
        public void AddItemSet(ItemSet itemSet, bool defaultItemSet, IItemCategoryIdentifier category, bool addParents)
        {
            ...
            for (var i = 0; i < itemSet.Slots.Length; ++i)
        ...
        public int GetItemSetIndex(Item item, int categoryIndex, bool checkIfValid)
        {
            ...
            if (m_NextItemSetIndex[categoryIndex] != -1)
            {
                ...
                if (itemSet.ItemIdentifiers[item.SlotID] == item.ItemIdentifier && (!checkIfValid ||
                        IsItemSetValid(categoryIndex, m_NextItemSetIndex[categoryIndex], false)))
            ...
            for (var i = 0; i < itemSetList.Count; ++i)
                ...
                if (itemSetList[i].ItemIdentifiers[item.SlotID] == item.ItemIdentifier &&
                    (!checkIfValid || IsItemSetValid(categoryIndex, i, false)))
                {
                    ...
                    for (var j = 0; j < itemSetList[i].ItemIdentifiers.Length; ++j)
                    {
                        if (j == item.SlotID) continue;
                        if (IsChildOf(item.ItemDefinition, itemSetList[i].Slots[j])) validSlotCount++;
                    }

                    if (itemCount == validSlotCount)
                        return i;
                    if (validItemSet == -1) validItemSet = i;
                }
        ...
        public int GetTargetItemSetIndex(int categoryIndex, int allowedMask)
        {
            ...
            if (IsItemSetValid(categoryIndex, itemSetIndex, false, allowedMask)) return itemSetIndex;
            ...
            if (prevItemSetIndex < 0) prevItemSetIndex = itemSetListCount - 1;
            if (itemSetIndex != -1 && prevItemSetIndex != itemSetIndex &&
                IsItemSetValid(categoryIndex, prevItemSetIndex, false, allowedMask))
                for (var i = 0; i < m_CategoryItemSets[categoryIndex].ItemSetList[prevItemSetIndex].Slots.Length; ++i)
                {
                    var prevSlots = m_CategoryItemSets[categoryIndex].ItemSetList[prevItemSetIndex].Slots;
                    if (m_CategoryItemSets[categoryIndex].ItemSetList[itemSetIndex].Slots[i] == prevSlots[i] &&
                        prevSlots[i] != null)
            ...
            if (IsItemSetValid(categoryIndex, m_CategoryItemSets[categoryIndex].DefaultItemSetIndex, false,
                    allowedMask)) return m_CategoryItemSets[categoryIndex].DefaultItemSetIndex;
            ...
            } while (itemSetIndex == prevItemSetIndex ||
                     itemSetIndex == m_CategoryItemSets[categoryIndex].DefaultItemSetIndex ||
                     !IsItemSetValid(categoryIndex, itemSetIndex, false, allowedMask));

            return itemSetIndex;
        }

        /// <summary>
        ///     Returns true if the specified ItemSet is valid. A valid ItemSet means the character has all of the items specified
        ///     in the inventory.
        /// </summary>
        /// <param name="categoryIndex">The index of the ItemSet category.</param>
        ...
        public bool IsItemSetValid(int categoryIndex, int itemSetIndex, bool checkIfCanSwitchTo,
            int allowedSlotsMask = -1)
        {
            if (itemSetIndex == -1 || itemSetIndex >= m_CategoryItemSets[categoryIndex].ItemSetList.Count) return false;

            var itemSet = m_CategoryItemSets[categoryIndex].ItemSetList[itemSetIndex];

            // The ItemSet isn't valid if it isn't enabled.
            if (!itemSet.Enabled) return false;

            // The ItemSet may not be able to be switched to.
            if (checkIfCanSwitchTo && !itemSet.CanSwitchTo) return false;
            ...
            for (var i = 0; i < itemSet.Slots.Length; ++i)
            {
                if (itemSet.Slots[i] == null) continue;

                // If the ItemIdentifier is null then the item hasn't been added yet.
                if (itemSet.ItemIdentifiers[i] == null) return false;

                // The item may not be in the allowed layer mask.
                if (allowedSlotsMask != -1 && m_CategoryItemSets[categoryIndex].DefaultItemSetIndex != itemSetIndex &&
                    !MathUtility.InLayerMask(i, allowedSlotsMask)) return false;
                ...
                for (var j = 0; j < item.ItemActions.Length; ++j)
                {
                    var usableItem = item.ItemActions[j] as IUsableItem;
                    if (usableItem != null)
                        if (!usableItem.CanEquipEmptyItem && usableItem.GetConsumableItemIdentifier() != null &&
                            m_Inventory.GetItemIdentifierAmount(usableItem.GetConsumableItemIdentifier()) == 0)
                            return false;
                }
        ...
        public void UpdateNextItemSet(int categoryIndex, int itemSetIndex)
        {
            ...
            if (nextItemSetIndex == itemSetIndex) return;
            ...
            EventHandler.ExecuteEvent(m_GameObject, "OnItemSetManagerUpdateNextItemSet", categoryIndex,
        ...
        public void UpdateActiveItemSet(int categoryIndex, int itemSetIndex)
        {
            ...
            // No updates are necessary if the indicies are the same.
            if (activeItemSetIndex == itemSetIndex) return;
            ...
            if (itemSetIndex != -1) m_CategoryItemSets[categoryIndex].ItemSetList[itemSetIndex].Active = true;
            ...
            if (activeItemSetIndex != -1 &&
                !string.IsNullOrEmpty(state = m_CategoryItemSets[categoryIndex].ItemSetList[activeItemSetIndex].State))
        ...
        public void SetDefaultItemSet(int categoryIndex)
        {
            ...
            if (IsItemSetValid(categoryIndex, itemSetIndex, false)) UpdateActiveItemSet(categoryIndex, itemSetIndex);
        }
        ...
        {
            if (slot == -1 || slot >= m_Inventory.SlotCount) return null;
            for (var i = 0; i < m_CategoryItemSets.Length; ++i)
                if (m_ActiveItemSetIndex[i] != -1)
                {
                    var itemSet = m_CategoryItemSets[i].ItemSetList[m_ActiveItemSetIndex[i]];
                    if (itemSet.Enabled && itemSet.ItemIdentifiers[slot] != null) return itemSet.ItemIdentifiers[slot];
                }
        ...
        public IItemIdentifier GetEquipItemIdentifier(int categoryIndex, int slot)
        {
            if (categoryIndex == -1 || categoryIndex >= m_CategoryItemSets.Length || slot == -1 ||
                slot >= m_Inventory.SlotCount) return null;
        ...
        /// <param name="slot">The slot to get the ItemIdentifier of.</param>
        /// <returns>
        ///     The ItemIdentifier which should be equipped for the specified categoryIndex, ItemIdentifier, and slot. Can be
        ///     null.
        /// </returns>
