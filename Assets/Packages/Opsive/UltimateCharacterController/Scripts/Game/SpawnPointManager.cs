...
namespace Opsive.UltimateCharacterController.Game
{
    ...
    public class SpawnPointManager : MonoBehaviour
    {
        ...
        [Tooltip("Checks the first spawn point before checking the other spawn points.")] [SerializeField]
        protected bool m_FirstSpawnPreferred;

        private readonly Dictionary<int, SpawnPoint> m_FirstSpawnPoint = new();

        private readonly Dictionary<int, List<SpawnPoint>> m_SpawnPointGroupings = new();
        ...
        private void AddSpawnPointGrouping(SpawnPoint spawnPoint, int groupingIndex)
        {
            ...
            if (!m_FirstSpawnPoint.ContainsKey(groupingIndex)) m_FirstSpawnPoint.Add(groupingIndex, spawnPoint);
        }
        ...
        public static bool GetPlacement(GameObject spawningObject, int grouping, ref Vector3 position,
        ...
        /// <param name="rotation">The rotation of the spawn point.</param>
        /// <returns>True if the spawn point was successfully retrieved.</returns>
        protected virtual bool GetPlacementInternal(GameObject spawningObject, int grouping, ref Vector3 position,
            ref Quaternion rotation)
        {
            List<SpawnPoint> spawnPoints;
            ...
            {
                if (spawnPoints[attempt].GetPlacement(spawningObject, ref position, ref rotation)) return true;
                attempt++;
            }
