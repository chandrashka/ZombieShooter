...
{
    /// <summary>
    ///     The KinematicObjectManager acts as a central organizer for determining when the characters, cameras, and kinematic
    ///     objects should update. The update order that the objects
    ///     are updated matter and the KinematicObjectManager ensures the objects are updated in the correct order to allow for
    ///     smooth movement.
    /// </summary>
    public class KinematicObjectManager : MonoBehaviour
    {
        ...
        [Tooltip(
            "The number of starting characters. For best performance this value should be the maximum number of characters that can be active within the scene.")]
        [SerializeField]
        protected int m_StartCharacterCount = 1;

        [Tooltip(
            "The number of starting cameras. For best performance this value should be the maximum number of cameras that can be active within the scene.")]
        [SerializeField]
        protected int m_StartCameraCount = 1;

        [UnityEngine.Serialization.FormerlySerializedAs("m_StartDeterministicObjectCount")]
        [Tooltip(
            "The number of starting kinematic objects. For best performance this value should be the maximum number of kinematic objects that can be active within the scene.")]
        [SerializeField]
        protected int m_StartKinematicObjectCount;

        [Tooltip(
            "Should the Auto Sync Transforms be enabled? See this page for more info: https://docs.unity3d.com/ScriptReference/Physics-autoSyncTransforms.html.")]
        [SerializeField]
        protected bool m_AutoSyncTransforms;
        ...
        public int StartCharacterCount => m_StartCharacterCount;
        public int StartCameraCount => m_StartCameraCount;
        public int StartKinematicObjectCount => m_StartKinematicObjectCount;
        ...
        private void Update()
        {
            ...
            if (float.IsNaN(interpAmount)) interpAmount = 0;
            for (var i = 0; i < m_KinematicObjectCount; ++i)
            ...
            if (m_KinematicObjectCount > 0 && !m_AutoSyncTransforms) Physics.SyncTransforms();
            for (var i = 0; i < m_CameraCount; ++i)
            {
                if (m_Cameras[i].CharacterLocomotion.UpdateLocation == UpdateLocation.Update) continue;

                m_Cameras[i].SmoothMove(interpAmount);
            }

            for (var i = 0; i < m_CharacterCount; ++i)
                ...
                else
                {
                    ...
                    // loop has run.
                    if (m_FixedUpdate) m_Characters[i].AssignFixedLocation(true);
        ...
        private void FixedUpdate()
        {
            ...
            for (var i = 0; i < m_KinematicObjectCount; ++i)
            {
                if (m_KinematicObjects[i].IKinematicObject.UpdateLocation == UpdateLocation.Update) continue;
                m_KinematicObjects[i].Move(true);
            }

            for (var i = 0; i < m_CharacterCount; ++i)
            {
                if (m_Characters[i].CharacterLocomotion.UpdateLocation == UpdateLocation.Update) continue;
                if (!m_AutoSyncTransforms) Physics.SyncTransforms();
                m_Characters[i].Move(false);
                if (m_Characters[i].CharacterIK != null && m_Characters[i].CharacterIK.enabled)
        ...
        private void OnEnable()
        {
            ...
            if (s_Instance == null)
            {
                ...
                for (var i = 0; i < characterLocomotions.Length; ++i)
                {
                    if (characterLocomotions[i].KinematicObjectIndex == -1) continue;
                ...
                {
                    if (cameras[i].KinematicObjectIndex == -1) continue;
        ...
        private int RegisterCharacterInternal(UltimateCharacterLocomotion characterLocomotion)
        {
            ...
            {
                Array.Resize(ref m_Characters, m_Characters.Length + 1);
                Debug.LogWarning(
        ...
        private int RegisterCameraInternal(CameraController cameraController)
        {
            ...
            {
                Array.Resize(ref m_Cameras, m_Cameras.Length + 1);
                Debug.LogWarning(
                    "Cameras array resized. For best performance increase the size of the Start Camera Count variable " +
        ...
        private int RegisterKinematicObjectInternal(IKinematicObject kinematicObject)
        {
            ...
            {
                Array.Resize(ref m_KinematicObjects, m_KinematicObjects.Length + 1);
                Debug.LogWarning(
                    "Kinematic objects array resized. For best performance increase the size of the Start Kinematic Object Count variable " +
        ...
        public static void SetCharacterMovementInput(int characterIndex, float horizontalMovement,
        ...
        /// <param name="horizontalMovement">-1 to 1 value specifying the amount of horizontal movement.</param>
        /// <param name="forwardMovement">-1 to 1 value specifying the amount of forward movement.</param>
        private void SetCharacterMovementInputInternal(int characterIndex, float horizontalMovement,
            float forwardMovement)
        {
            if (m_Characters == null || characterIndex >= m_Characters.Length || characterIndex < 0) return;
        ...
        {
            if (m_Cameras == null || cameraIndex >= m_Cameras.Length || cameraIndex < 0) return;
        ...
        {
            if (m_KinematicObjects == null || kinematicObjectIndex >= m_KinematicObjects.Length ||
                kinematicObjectIndex < 0) return;
        ...
        private void BeginCharacterMovementInternal(int characterIndex)
        {
            ...
            if (updateLocation == UpdateLocation.FixedUpdate) m_Characters[characterIndex].RestoreFixedLocation();

            // If the character has a camera attached the camera should first be rotated.
            int cameraIndex;
            if (m_Characters[characterIndex].AttachedCamera != null &&
                (cameraIndex = m_Characters[characterIndex].AttachedCamera.KinematicObjectIndex) >= 0)
        ...
        private void EndCharacterMovementInternal(int characterIndex)
        {
            ...
            var updateLocation = m_Characters[characterIndex].CharacterLocomotion.UpdateLocation;
            if (updateLocation == UpdateLocation.FixedUpdate) m_Characters[characterIndex].AssignFixedLocation();
        ...
        private void UnregisterCharacterInternal(int characterIndex)
        {
            ...
            for (var i = characterIndex + 1; i < m_CharacterCount; ++i)
            {
        ...
        private void UnregisterCameraInternal(int cameraIndex)
        {
            ...
            // Keep the array packed by shifting all of the subsequent elements over by one.
            for (var i = cameraIndex + 1; i < m_CameraCount; ++i)
            {
        ...
        {
            if (kinematicObjectIndex < 0) return;

            GenericObjectPool.Return(m_KinematicObjects[kinematicObjectIndex]);
            // Keep the array packed by shifting all of the subsequent elements over by one.
            for (var i = kinematicObjectIndex + 1; i < m_KinematicObjectCount; ++i)
            {
        ...
        /// <summary>
        ///     A small storage class used for storing the fixed and smooth location. This component will also move the interpolate
        ///     the objects during the Update loop.
        /// </summary>
        private class SmoothFixedLocation
        {
            ...
            /// <summary>
            ///     The object is moved within FixedUpdate while the camera is moved within Update. This would normally cause jitters
            ///     but a separate smooth variable
            ...
            {
                Transform.SetPositionAndRotation(Vector3.Lerp(m_SmoothPosition, m_FixedPosition, interpAmount),
                    Quaternion.Slerp(m_SmoothRotation, m_FixedRotation, interpAmount));
            }

            /// <summary>
            ///     Restores the location back to the fixed location. This will be performed immediately before the object is moved
            ///     within FixedUpdate.
            /// </summary>
            ...
            {
                Transform.position = m_FixedPosition = m_SmoothPosition = position;
            }
            ...
            {
                Transform.rotation = m_FixedRotation = m_SmoothRotation = rotation;
            }
        }
        ...
        private class KinematicCharacter : SmoothFixedLocation
        {
            ...
            public CameraController AttachedCamera { get; private set; }

            public CharacterIKBase CharacterIK { get; private set; }

            public float HorizontalMovement
            {
                set => m_HorizontalMovement = value;
            }

            public float ForwardMovement
            {
                set => m_ForwardMovement = value;
            }

            public float DeltaYawRotation
            {
                set => m_DeltaYawRotation = value;
            }
            ...
            public void Initialize(UltimateCharacterLocomotion characterLocomotion)
            {
                ...
                OnAttachLookSource(CharacterLocomotion.LookSource);
                EventHandler.RegisterEvent<ILookSource>(CharacterLocomotion.gameObject, "OnCharacterAttachLookSource",
                    OnAttachLookSource);
                EventHandler.RegisterEvent<bool>(CharacterLocomotion.gameObject, "OnCharacterChangeUpdateLocation",
                ...
                m_CharacterHandler = CharacterLocomotion.GetComponent<UltimateCharacterLocomotionHandler>();
                CharacterIK = CharacterLocomotion.GetComponent<CharacterIKBase>();
                ...
                m_CompleteInitEvent = SchedulerBase.ScheduleFixed(Time.fixedDeltaTime / 2, () =>
                {
                    if (m_CharacterHandler == null)
                        m_CharacterHandler = CharacterLocomotion.GetComponent<UltimateCharacterLocomotionHandler>();
                    if (CharacterIK == null) CharacterIK = CharacterLocomotion.GetComponent<CharacterIKBase>();
                    m_CompleteInitEvent = null;

                    var smoothedBones = CharacterLocomotion.SmoothedBones;
                    if (smoothedBones != null && smoothedBones.Length > 0)
                    {
                        var validBones = 0;
                        for (var i = 0; i < smoothedBones.Length; ++i)
                        ...
                        if (validBones > 0)
                        {
                            ...
                            for (var i = 0; i < smoothedBones.Length; ++i)
                            {
                                if (smoothedBones[i] == null) continue;
            ...
            {
                if (lookSource != null)
                    AttachedCamera = lookSource.GameObject.GetCachedComponent<CameraController>();
                else
                    AttachedCamera = null;
            }
            ...
            {
                if (CharacterLocomotion.ManualMove) return;

                base.SmoothMove(interpAmount);

                if (m_SmoothedBones != null)
                    for (var i = 0; i < m_SmoothedBones.Length; ++i)
            ...
            {
                if (CharacterLocomotion.ManualMove != manualMove) return;

                if (m_CharacterHandler != null) m_DeltaYawRotation = m_CharacterHandler.GetDeltaYawRotation();
                CharacterLocomotion.Move(m_HorizontalMovement, m_ForwardMovement, m_DeltaYawRotation);
            }
            ...
            public void UnregisterCharacter()
            {
                ...
                if (m_SmoothedBones != null)
                {
                    for (var i = 0; i < m_SmoothedBones.Length; ++i) GenericObjectPool.Return(m_SmoothedBones[i]);
                    m_SmoothedBones = null;
                }

                EventHandler.UnregisterEvent<ILookSource>(CharacterLocomotion.gameObject, "OnCharacterAttachLookSource",
                    OnAttachLookSource);
                EventHandler.UnregisterEvent<bool>(CharacterLocomotion.gameObject, "OnCharacterChangeUpdateLocation",
                    OnChangeUpdateLocation);
            }
        }
        ...
        {
            public IKinematicObject IKinematicObject { get; private set; }
            ...
            {
                IKinematicObject = kinematicObject;
                Initialize(IKinematicObject.transform);
            }
            ...
            {
                if (applyFixedLocation) RestoreFixedLocation();
                IKinematicObject.Move();
                if (applyFixedLocation) AssignFixedLocation();
            }
        }
        ...
        private class KinematicCamera : SmoothFixedLocation
        {
            ...
            public void Initialize(CameraController cameraController)
            {
                ...
                OnAttachCharacter(CameraController.Character);
                EventHandler.RegisterEvent<GameObject>(CameraController.gameObject, "OnCameraAttachCharacter",
                    OnAttachCharacter);
            }
            ...
            {
                if (character != null)
                    CharacterLocomotion = character.GetCachedComponent<UltimateCharacterLocomotion>();
                else
            ...
            public void Rotate()
            {
                ...
                CameraController.Rotate(m_LookVector.x, m_LookVector.y);
            }
            ...
            public void Move()
            {
                CameraController.Move(m_LookVector.x, m_LookVector.y);
            ...
            {
                EventHandler.UnregisterEvent<GameObject>(CameraController.gameObject, "OnCameraAttachCharacter",
                    OnAttachCharacter);
            }
        }
    }
