...
namespace Opsive.UltimateCharacterController.Game
{
    ...
    public class SpawnPoint : MonoBehaviour
    {
        ...
        [SerializeField]
        protected int m_Grouping = -1;

        [Tooltip("Specifies the shape in which the spawn point should randomly be determined.")] [SerializeField]
        protected SpawnShape m_Shape;

        [Tooltip("The size of the spawn shape.")] [SerializeField]
        protected float m_Size;

        [Tooltip("Should the object be spawned randomly within the shape?")] [SerializeField]
        protected bool m_RandomShapeSpawn = true;

        [Tooltip("Specifies the height of the ground check.")] [SerializeField]
        protected float m_GroundSnapHeight;

        [Tooltip("Should the character spawn with a random y direction?")] [SerializeField]
        protected bool m_RandomDirection;

        [Tooltip("Should a check be performed to determine if there are any objects obstructing the spawn point?")]
        [SerializeField]
        protected bool m_CheckForObstruction;
        ...
        [Tooltip("The layers which can obstruct the spawn point.")] [SerializeField]
        protected LayerMask m_ObstructionLayers =
            ~((1 << LayerManager.Default) | (1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.TransparentFX) |
        ...
        [SerializeField]
        protected int m_PlacementAttempts = 10;
#if UNITY_EDITOR
        [Tooltip("The color to draw the editor gizmo in (editor only).")] [SerializeField]
        protected Color m_GizmoColor = new(1, 0, 0, 0.3f);
#endif
        ...
        public int Grouping
        {
            ...
            set
            {
                if (m_Grouping != value)
                {
                    ...
                    if (Application.isPlaying) SpawnPointManager.UpdateSpawnPointGrouping(this, value);
                    m_Grouping = value;
                }
            }
        }
        ...
        {
            get => m_GroundSnapHeight;
            set => m_GroundSnapHeight = value;
        }

        public bool RandomDirection
        {
            get => m_RandomDirection;
            set => m_RandomDirection = value;
        }

        public bool CheckForObstruction
        {
            get => m_CheckForObstruction;
            set => m_CheckForObstruction = value;
        }

        public LayerMask ObstructionLayers
        {
            get => m_ObstructionLayers;
            set => m_ObstructionLayers = value;
        }

        public int PlacementAttempts
        {
            get => m_PlacementAttempts;
            set => m_PlacementAttempts = value;
        }
#if UNITY_EDITOR
        ...
        private void Awake()
        {
            ...
            if (m_CheckForObstruction) m_ObstructionColliders = new Collider[m_MaxCollisionCount];
        }
        ...
        /// <summary>
        ///     Gets the position and rotation of the spawn point. If false is returned then the point wasn't successfully
        ///     retrieved.
        /// </summary>
        ...
        public virtual bool GetPlacement(GameObject spawningObject, ref Vector3 position, ref Quaternion rotation)
        {
            ...
            if (m_CheckForObstruction)
            {
                ...
                while (attempt < m_PlacementAttempts)
                {
                    ...
                    else if (m_Shape == SpawnShape.Sphere)
                    {
                        ...
                        var overlapCount = Physics.OverlapSphereNonAlloc(position, m_Size / 2, m_ObstructionColliders,
                            m_ObstructionLayers, QueryTriggerInteraction.Ignore);
                        if (spawningObject != null)
                            for (var i = overlapCount - 1; i > -1; --i)
                            {
                                if (!m_ObstructionColliders[i].transform.IsChildOf(spawningObject.transform)) break;
                                overlapCount--;
                            }
                    ...
                    else
                    {
                        ...
                        // Ignore any collisions with itself.
                        var overlapCount = Physics.OverlapBoxNonAlloc(position, extents, m_ObstructionColliders,
                            m_Transform.rotation, m_ObstructionLayers, QueryTriggerInteraction.Ignore);
                        if (spawningObject != null)
                            for (var i = overlapCount - 1; i > -1; --i)
                            {
                                if (!m_ObstructionColliders[i].transform.IsChildOf(spawningObject.transform)) break;
            ...
            if (m_GroundSnapHeight > 0)
            {
                ...
                if (Physics.Raycast(position + m_Transform.up * m_GroundSnapHeight, -m_Transform.up, out raycastHit,
                        m_GroundSnapHeight + 0.2f, m_ObstructionLayers, QueryTriggerInteraction.Ignore))
        ...
        private Vector3 RandomPosition(int attempt)
        {
            ...
            if (attempt == 0 || !m_RandomShapeSpawn) return m_Transform.position;
