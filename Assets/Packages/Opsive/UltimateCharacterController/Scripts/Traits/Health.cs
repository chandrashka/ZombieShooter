...
namespace Opsive.UltimateCharacterController.Traits
{
    ...
    {
        [Tooltip("Is the object invincible?")] [SerializeField]
        protected bool m_Invincible;

        [Tooltip("The amount of time that the object is invincible after respawning.")] [SerializeField]
        protected float m_TimeInvincibleAfterSpawn;

        [Tooltip("The name of the health attribute.")] [SerializeField]
        protected string m_HealthAttributeName = "Health";

        [Tooltip("The name of the shield attribute.")] [SerializeField]
        protected string m_ShieldAttributeName;

        [Tooltip("The list of Colliders that should apply a multiplier when damaged.")] [SerializeField]
        ...
        [SerializeField]
        protected int m_MaxHitboxCollisionCount = 10;

        [Tooltip("Any object that should spawn when the object dies.")] [SerializeField]
        protected GameObject[] m_SpawnedObjectsOnDeath;

        [Tooltip("Any object that should be destroyed when the object dies.")] [SerializeField]
        protected GameObject[] m_DestroyedObjectsOnDeath;

        [Tooltip("Should the object be deactivated on death?")] [SerializeField]
        protected bool m_DeactivateOnDeath;

        [Tooltip("If DeactivateOnDeath is enabled, specify a delay for the object to be deactivated.")] [SerializeField]
        protected float m_DeactivateOnDeathDelay;

        [Tooltip("The layer that the GameObject should switch to upon death.")] [SerializeField]
        protected LayerMask m_DeathLayer;

        [Tooltip("A set of AudioClips that can be played when the object takes damage.")]
        [HideInInspector]
        [SerializeField]
        protected AudioClipSet m_TakeDamageAudioClipSet = new();

        [Tooltip("A set of AudioClips that can be played when the object is healed.")]
        [HideInInspector]
        [SerializeField]
        protected AudioClipSet m_HealAudioClipSet = new();

        [Tooltip("A set of AudioClips that can be played when the object dies.")] [HideInInspector] [SerializeField]
        protected AudioClipSet m_DeathAudioClipSet = new();

        [Tooltip("The ID of the Damage Pop up Manager.")] [SerializeField]
        protected int m_DamagePopupManagerID = -1;

        [Tooltip("Unity event invoked when taking damage.")] [SerializeField]
        protected UnityFloatVector3Vector3GameObjectEvent m_OnDamageEvent;

        [Tooltip("Unity event invoked when healing.")] [SerializeField]
        protected UnityFloatEvent m_OnHealEvent;

        [Tooltip("Unity event invoked when the object dies.")] [SerializeField]
        protected UnityVector3Vector3GameObjectEvent m_OnDeathEvent;
        ...
        {
            get => m_DamagePopupManager;
            set => m_DamagePopupManager = value;
        }
        ...
        public float HealthValue => m_HealthAttribute != null ? m_HealthAttribute.Value : 0;
        public float ShieldValue => m_ShieldAttribute != null ? m_ShieldAttribute.Value : 0;
        public float Value => HealthValue + ShieldValue;
        ...
        {
            if (m_DamagePopupManager == null) SetDamagePopupManagerUsingID();
        }
        ...
        /// <param name="radius">The radius of the explosive damage. If 0 then a non-exposive force will be used.</param>
        /// <param name="attacker">The GameObject that did the damage.</param>
        public void Damage(float amount, Vector3 position, Vector3 direction, float forceMagnitude, int frames,
            float radius, GameObject attacker)
        {
        ...
        public void Damage(float amount, Vector3 position, Vector3 direction, float forceMagnitude, int frames,
            float radius, GameObject attacker, Collider hitCollider)
        {
            ...
            float radius, GameObject attacker, object attackerObject, Collider hitCollider)
        {
            ...
            pooledDamageData.SetDamage(amount, position, direction, forceMagnitude, frames, radius, attacker,
                attackerObject, hitCollider);
        ...
        public virtual void OnDamage(DamageData damageData)
        {
            ...
            if (damageData.Radius == 0 && damageData.Direction != Vector3.zero && damageData.HitCollider != null)
                if (m_ColliderHitboxMap != null && m_ColliderHitboxMap.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        var hitCount = Physics.RaycastNonAlloc(damageData.Position, damageData.Direction, m_RaycastHits,
                            distance,
                            ~((1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.Overlay) |
                              (1 << LayerManager.VisualEffect)), QueryTriggerInteraction.Ignore);
                        for (var i = 0; i < hitCount; ++i)
                        {
                            var closestRaycastHit =
                                QuickSelect.SmallestK(m_RaycastHits, hitCount, i, m_RaycastHitComparer);
                            if (closestRaycastHit.collider == damageData.HitCollider) continue;
            ...
            if (m_HealthAttribute != null && m_HealthAttribute.Value > m_HealthAttribute.MinValue)
                m_HealthAttribute.Value -=
                    Mathf.Min(damageData.Amount, m_HealthAttribute.Value - m_HealthAttribute.MinValue);
                ...
                else
                {
                    ...
                    if (m_Rigidbody != null && !m_Rigidbody.isKinematic)
                    {
                        if (damageData.Radius == 0)
                            m_Rigidbody.AddForceAtPosition(force * MathUtility.RigidbodyForceMultiplier,
                                damageData.Position);
                        else
                            m_Rigidbody.AddExplosionForce(force.magnitude * MathUtility.RigidbodyForceMultiplier,
                                damageData.Position, damageData.Radius);
                    }
                }
            }
            ...
            EventHandler.ExecuteEvent(m_GameObject, "OnHealthDamage", damageData.Amount, damageData.Position, force,
                attacker, damageData.HitCollider);
            EventHandler.ExecuteEvent(m_GameObject, "OnHealthDamageWithData", damageData);
            if (m_OnDamageEvent != null)
                m_OnDamageEvent.Invoke(damageData.Amount, damageData.Position, force, attacker);
            if (m_DamagePopupManager != null) m_DamagePopupManager.OpenDamagePopup(damageData);
        ...
        public virtual void Die(Vector3 position, Vector3 force, GameObject attacker)
        {
            ...
            if (m_SpawnedObjectsOnDeath != null)
                for (var i = 0; i < m_SpawnedObjectsOnDeath.Length; ++i)
                {
                    var spawnedObject = ObjectPoolBase.Instantiate(m_SpawnedObjectsOnDeath[i], m_Transform.position,
                    ...
                    for (var j = 0; j < rigidbodies.Length; ++j) rigidbodies[j].AddForceAtPosition(force, position);
                }
            ...
            if (m_DeactivateOnDeath) SchedulerBase.Schedule(m_DeactivateOnDeathDelay, Deactivate);

            // The attributes shouldn't regenerate.
            if (m_ShieldAttribute != null) m_ShieldAttribute.CancelAutoUpdate();
            if (m_HealthAttribute != null) m_HealthAttribute.CancelAutoUpdate();

            // Notify those interested.
            EventHandler.ExecuteEvent(m_GameObject, "OnDeath", position, force, attacker);
            if (m_OnDeathEvent != null) m_OnDeathEvent.Invoke(position, force, attacker);
        }
        ...
        {
            var amount = 0f;
            if (m_HealthAttribute != null) amount += m_HealthAttribute.Value;
            if (m_ShieldAttribute != null) amount += m_ShieldAttribute.Value;
        ...
        public virtual bool Heal(float amount)
        {
            ...
            EventHandler.ExecuteEvent(m_GameObject, "OnHealthHeal", healAmount);
            if (m_OnHealEvent != null) m_OnHealEvent.Invoke(healAmount);
            if (m_DamagePopupManager != null) m_DamagePopupManager.OpenHealPopup(m_Transform.position, amount);
        ...
        {
            if (m_HealthAttribute != null) m_HealthAttribute.ResetValue();
            if (m_ShieldAttribute != null) m_ShieldAttribute.ResetValue();
            // Change the layer back to the alive layer.
            if (m_DeathLayer.value != 0) m_GameObject.layer = m_AliveLayer;
