...
namespace Opsive.UltimateCharacterController.Traits.Damage
{
    ...
    {
        [Tooltip(
            "The unique ID of the Damage Popup Manager. The Health component can specify the ID of the popup used.")]
        [SerializeField]
        protected uint m_ID;

        [Tooltip("The prefab for showing the damage popup.")] [SerializeField]
        protected GameObject m_DamagePrefab;

        [Tooltip("The prefab for showing the health popup.")] [SerializeField]
        protected GameObject m_HealPrefab;
        ...
        {
            var popupGameObject = ObjectPoolBase.Instantiate(m_DamagePrefab, transform);
            var popup = popupGameObject.GetCachedComponent<IDamagePopup>();
            if (popup != null) popup.Open(damageData);
        }
        ...
        {
            var popupGameObject = ObjectPoolBase.Instantiate(m_HealPrefab, transform);
            var popup = popupGameObject.GetCachedComponent<IDamagePopup>();
            if (popup != null) popup.Open(position, amount);
        }
    }
}