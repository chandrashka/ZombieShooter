...
namespace Opsive.UltimateCharacterController.Traits
{
    ...
    /// <summary>
    ///     Represents any object that can be interacted with by the character. Acts as a link between the character and
    ///     IInteractableTarget.
    /// </summary>
    public class Interactable : MonoBehaviour
    {
        [Tooltip(
            "The ID of the Interactable, used by the Interact ability for filtering. A value of -1 indicates no ID.")]
        [SerializeField]
        protected int m_ID = -1;

        [Tooltip(
            "The object(s) that the interaction is performend on. This component must implement the IInteractableTarget.")]
        [SerializeField]
        protected MonoBehaviour[] m_Targets;
        ...
        public AbilityIKTarget[] IKTargets { get; private set; }
        ...
        private void Awake()
        {
            ...
            for (var i = 0; i < m_Targets.Length; ++i)
                if (m_Targets[i] == null || !(m_Targets[i] is IInteractableTarget))
                    Debug.LogError("Error: element " + i +
                                   " of the Targets array is null or does not subscribe to the IInteractableTarget iterface.");
                else
            ...
            IKTargets = GetComponentsInChildren<AbilityIKTarget>();
        }
        ...
        {
            for (var i = 0; i < m_InteractableTargets.Length; ++i)
        ...
        public virtual void Interact(GameObject character)
        {
            ...
            for (var i = 0; i < m_InteractableTargets.Length; ++i) m_InteractableTargets[i].Interact(character);
        }
