...
namespace Opsive.UltimateCharacterController.Editor.Managers
{
    ...
    public static class ManagerUtility
    {
        ...
        private static readonly string[] s_AnimatorControllerGUIDs =
        {
            "00734c75e5484e24697dddaf47e8c152", "1c65957c39679034fb94019d52d6a984", "79f4dab00da40824fbd3697b6c773522",
        ...
        private static readonly string[] s_StateConfigurationGUIDs =
        {
            "9d35e75efc940dd4184470a31d744f39", "c7627c1aa2c6b264d87709008477a69e", "da4073e1f8f631445b1aea02f03f4760",
        ...
        public static string StateConfigurationGUID => s_StateConfigurationGUIDs[0];
        public static string LastItemCollectionGUIDString => c_LastItemCollectionGUIDString;
        public static string LastStateConfigurationGUIDString => c_LastStateConfigurationGUIDString;
        ...
        public static void DrawControlBox(string title, Action additionalControls, string message, bool enableButton,
            string button, Action action, string successLog)
        {
            ...
            {
                action();
                if (!string.IsNullOrEmpty(successLog)) Debug.Log(successLog);
            }
        ...
        public static RuntimeAnimatorController FindAnimatorController(ScriptableObject editorWindow)
        {
            ...
            for (var i = 0; i < s_AnimatorControllerGUIDs.Length; ++i)
            {
                ...
                if (!string.IsNullOrEmpty(animatorControllerPath))
                {
                    var animatorController =
                        AssetDatabase.LoadAssetAtPath(animatorControllerPath, typeof(RuntimeAnimatorController)) as
            ...
            animatorControllerPath =
                Path.GetDirectoryName(AssetDatabase.GetAssetPath(MonoScript.FromScriptableObject(editorWindow)))
                    .Replace("\\", "/").Replace("Editor/Managers", "Demo/Animator/Characters/Demo.controller");
            if (File.Exists(Application.dataPath + "/" + animatorControllerPath.Substring(7)))
                return AssetDatabase.LoadAssetAtPath(animatorControllerPath, typeof(RuntimeAnimatorController)) as
                    RuntimeAnimatorController;
        ...
        /// </summary>
        public static ItemCollection FindItemCollection(ScriptableObject editorWindow)
        {
            // If an ItemCollection asset exists within the scene then use that.
            var itemSetManager = Object.FindObjectOfType<ItemSetManager>();
            ...
            if (!string.IsNullOrEmpty(lastItemCollectionGUID))
            {
                ...
                {
                    var itemCollection =
                        AssetDatabase.LoadAssetAtPath(lastItemCollectionPath, typeof(ItemCollection)) as ItemCollection;
            ...
            {
                var itemCollection =
                    AssetDatabase.LoadAssetAtPath(itemCollectionPath, typeof(ItemCollection)) as ItemCollection;
                if (itemCollection != null) return itemCollection;
            }
            ...
            itemCollectionPath =
                Path.GetDirectoryName(AssetDatabase.GetAssetPath(MonoScript.FromScriptableObject(editorWindow)))
                    .Replace("\\", "/").Replace("Editor/Managers", "Demo/Inventory/DemoItemCollection.asset");
            if (File.Exists(Application.dataPath + "/" + itemCollectionPath.Substring(7)))
                return AssetDatabase.LoadAssetAtPath(itemCollectionPath, typeof(ItemCollection)) as ItemCollection;
            ...
            if (itemCollections != null && itemCollections.Length > 0) return itemCollections[0];
        ...
        public static Material FindInvisibleShadowCaster(ScriptableObject editorWindow)
        {
            ...
            {
                var invisibleShadowCaster =
                    AssetDatabase.LoadAssetAtPath(shadowCasterPath, typeof(Material)) as Material;
                if (invisibleShadowCaster != null) return invisibleShadowCaster;
            }
                ...
                shadowCasterPath =
                    Path.GetDirectoryName(AssetDatabase.GetAssetPath(MonoScript.FromScriptableObject(editorWindow)))
                        .Replace("\\", "/").Replace("Editor/Managers",
                            "FirstPersonController/Materials/InvisibleShadowCaster.mat");
                if (File.Exists(Application.dataPath + "/" + shadowCasterPath.Substring(7)))
        ...
        /// </summary>
        public static StateConfiguration FindStateConfiguration(ScriptableObject editorWindow)
        {
            ...
            if (!string.IsNullOrEmpty(lastStateConfigurationGUID))
            {
                ...
                if (!string.IsNullOrEmpty(lastStateConfigurationPath))
                {
                    var stateConfiguration =
                        AssetDatabase.LoadAssetAtPath(lastStateConfigurationPath, typeof(StateConfiguration)) as
                            StateConfiguration;
                    if (stateConfiguration != null) return stateConfiguration;
                }
            }
            ...
            for (var i = 0; i < s_StateConfigurationGUIDs.Length; ++i)
            {
                ...
                {
                    var stateConfiguration =
                        AssetDatabase.LoadAssetAtPath(stateConfigurationPath, typeof(StateConfiguration)) as
                            StateConfiguration;
                    if (stateConfiguration != null) return stateConfiguration;
                }
            }
            ...
            stateConfigurationPath =
                Path.GetDirectoryName(AssetDatabase.GetAssetPath(MonoScript.FromScriptableObject(editorWindow)))
                    .Replace("\\", "/").Replace("Editor/Managers", "Demo/Presets/DemoStateConfiguration.asset");
            if (File.Exists(Application.dataPath + "/" + stateConfigurationPath.Substring(7)))
                return AssetDatabase.LoadAssetAtPath(stateConfigurationPath, typeof(StateConfiguration)) as
                    StateConfiguration;

            // Last chance: use resources to try to find the StateConfiguration.
            var stateConfigurations = Resources.FindObjectsOfTypeAll<StateConfiguration>();
            if (stateConfigurations != null && stateConfigurations.Length > 0) return stateConfigurations[0];
