...
namespace Opsive.UltimateCharacterController.Editor.Managers
{
    ...
    public class ObjectManager : Manager
    {
        ...
        {
            ManagerUtility.DrawControlBox("Object Builder", DrawObjectTypes,
                "Builds a new object with the specified type.",
        ...
        private void DrawObjectTypes()
        {
            ...
            if (RequiresGameObject())
            {
                ...
                if (m_Object != null && m_ObjectType == ObjectType.ItemPickup)
                    if (m_Object.GetComponent<Item>() != null)
                    {
                        EditorGUILayout.HelpBox(
                            "The Item Pickup should not reference an already created item. This GameObject should reference the item model.",
                            MessageType.Error);
                        m_CanBuild = false;
                    }
            }
            ...
            if (m_ObjectType == ObjectType.MagicProjectile || m_ObjectType == ObjectType.Particle)
                m_MagicParticleCollisions = EditorGUILayout.Toggle(
                    new GUIContent("Magic Particle Collisions",
                        "Should the particles respond to magic collision events?"),
        ...
        private void BuildObject()
        {
            ...
            if (path.Length == 0 || Application.dataPath.Length > path.Length) return;

            var createdObject = m_Object;
            if (createdObject == null)
                createdObject = new GameObject();
            else if (EditorUtility.IsPersistent(createdObject)) createdObject = Object.Instantiate(createdObject);
            ...
            switch (m_ObjectType)
            {
                ...
                case ObjectType.DroppedItem:
                    ...
                    trajectoryObject.ImpactLayers =
                        ~((1 << LayerManager.IgnoreRaycast) | (1 << LayerManager.Water) |
                ...
                case ObjectType.MagicProjectile:
                    ...
                    if (m_MagicParticleCollisions) AddComponentIfNotAdded<MagicParticle>(createdObject);
                    break;
            ...
            Selection.activeGameObject =
                AssetDatabase.LoadAssetAtPath("Assets" + relativePath, typeof(GameObject)) as GameObject;
        ...
        {
            T component;
            if ((component = obj.GetComponent<T>()) == null) component = obj.AddComponent<T>();
            return component;
        }
