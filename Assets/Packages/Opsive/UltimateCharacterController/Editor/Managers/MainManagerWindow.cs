...
namespace Opsive.UltimateCharacterController.Editor.Managers
{
    ...
    public class MainManagerWindow : EditorWindow
    {
        ...
        private UnityWebRequest m_UpdateCheckRequest;
        ...
        public float MenuWidth => c_MenuWidth;

        public string LatestVersion
        {
            get => EditorPrefs.GetString("Opsive.UltimateCharacterController.Editor.LatestVersion", AssetInfo.Version);
        ...
        private DateTime LastUpdateCheck
        {
            get
            {
                try
                {
                    ...
                    if (m_LastUpdateCheck != DateTime.MinValue) return m_LastUpdateCheck;

                    m_LastUpdateCheck =
                        DateTime.Parse(
                            EditorPrefs.GetString("Opsive.UltimateCharacterController.Editor.LastUpdateCheck",
                                "1/1/1971 00:00:01"), CultureInfo.InvariantCulture);
                }
                catch (Exception /*e*/)
                {
            ...
            set
            {
                ...
                EditorPrefs.SetString("Opsive.UltimateCharacterController.Editor.LastUpdateCheck",
                    m_LastUpdateCheck.ToString(CultureInfo.InvariantCulture));
            }
        }
        ...
        private GUIStyle MenuBackground
        {
            get
            {
                if (m_MenuBackground == null)
                {
                    ...
                    background.SetPixel(0, 0,
                        EditorGUIUtility.isProSkin
                            ? new Color(0.258f, 0.258f, 0.258f)
                            : new Color(0.819f, 0.819f, 0.819f));
                    background.Apply();
        ...
        private GUIStyle SelectedMenuButton
        {
            get
            {
                if (m_SelectedMenuButton == null) m_SelectedMenuButton = new GUIStyle(MenuButton);
                if (m_SelectedMenuButton.active.background == null)
                {
                    var background = new Texture2D(1, 1, TextureFormat.RGBA32, false);
                    background.SetPixel(0, 0,
                        EditorGUIUtility.isProSkin
                            ? new Color(0.243f, 0.373f, 0.588f)
                            : new Color(0.247f, 0.494f, 0.871f));
                    background.Apply();
        ...
        private void OnGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck()) SerializeManagers();
        }
        ...
        {
            var window = GetWindow<MainManagerWindow>(false, "Character Manager");
        ...
        private static void EditorStartup()
        {
            ...
            if (importStatus == null)
            {
                ...
                if (string.IsNullOrEmpty(domainResetterPath)) return;

                importStatus = CreateInstance<ImportStatus>();
                AssetDatabase.CreateAsset(importStatus,
                    Path.GetDirectoryName(domainResetterPath) + "/ImportStatus.asset");
            ...
            if (!importStatus.CharacterProjectSettingsShown)
            {
                ...
                if (!EditorPrefs.HasKey("Opsive.UltimateCharacterController.Editor.UpdateProject") ||
                    EditorPrefs.GetBool("Opsive.UltimateCharacterController.Editor.UpdateProject", true))
                {
                    var window = ShowWindow();
        ...
        private bool UpdateCheck()
        {
            ...
            if (m_UpdateCheckRequest == null && DateTime.Compare(LastUpdateCheck.AddDays(1), DateTime.UtcNow) < 0)
            {
                var url = string.Format(
                    "https://opsive.com/asset/UpdateCheck.php?asset=UltimateCharacterController&type={0}&version={1}&unityversion={2}&devplatform={3}&targetplatform={4}",
                    AssetInfo.Name.Replace(" ", ""), AssetInfo.Version, Application.unityVersion, Application.platform,
                    EditorUserBuildSettings.activeBuildTarget);
                m_UpdateCheckRequest = UnityWebRequest.Get(url);
        ...
        private void OnMenuGUI()
        {
            ...
            for (var i = 0; i < m_Managers.Length; ++i)
                if (GUILayout.Button(m_ManagerNames[i], i == m_MenuSelection ? SelectedMenuButton : MenuButton,
                        GUILayout.Height(32)))
                    m_MenuSelection = i;
        ...
        private void BuildManagerItems()
        {
            ...
            for (var i = 0; i < assemblies.Length; ++i)
            {
                ...
                for (var j = 0; j < assemblyTypes.Length; ++j)
                {
                    ...
                    if (!typeof(Manager).IsAssignableFrom(assemblyTypes[j])) continue;
            ...
            if (m_Managers != null && m_Managers.Length == managers.Count) return;
            ...
            for (var i = 0; i < managerTypes.Length; ++i)
            {
                ...
                {
                    var item =
                        managerTypes[i].GetCustomAttributes(typeof(OrderedEditorItem), true)[0] as OrderedEditorItem;
        ...
        private void DeserializeManagers()
        {
            if (m_ManagerData != null)
            {
                ...
                for (var i = 0; i < m_ManagerData.Length; ++i)
                {
    ...
    {
        public OrderedEditorItem(string name, int index)
        {
            Name = name;
            Index = index;
        }
