...
namespace Opsive.UltimateCharacterController.Editor.Managers
{
    ...
    public class AddOnsManager : Manager
    {
        ...
        private UnityWebRequest m_AddOnsReqest;
        ...
        private readonly string[] m_ToolbarStrings = { "Installed Add-Ons", "Available Add-Ons" };
        ...
        private void BuildInstalledAddOns()
        {
            ...
            for (var i = 0; i < assemblies.Length; ++i)
            {
                ...
                for (var j = 0; j < assemblyTypes.Length; ++j)
                {
                    ...
                    if (!typeof(AddOnInspector).IsAssignableFrom(assemblyTypes[j])) continue;
            ...
            if (m_AddOnInspectors != null && m_AddOnInspectors.Length == addOnInspectors.Count) return;
        ...
        {
            var toolbarSelection = GUILayout.Toolbar(m_DrawInstalledAddOns ? 0 : 1, m_ToolbarStrings,
        ...
        private void DrawInstalledAddOns()
        {
            ...
            for (var i = 0; i < m_AddOnInspectors.Length; ++i)
            {
                ...
                if (i != m_AddOnInspectors.Length - 1) GUILayout.Space(20);
            }
        ...
        private void DrawAvailableAddOns()
        {
            ...
            {
                m_AddOnsReqest =
                    UnityWebRequest.Get("https://opsive.com/asset/UltimateCharacterController/AddOnsList.txt");
                     ...
                     Event.current.type == EventType.Layout)
            {
                ...
                var count = 0;
                for (var i = 0; i < splitAddOns.Length; ++i)
                {
                    if (string.IsNullOrEmpty(splitAddOns[i])) continue;

                    // The data must contain info on the add-on name, id, icon, add-on url, description, and type.
                    var addOnData = splitAddOns[i].Split(',');
                    if (addOnData.Length < 6) continue;

                    m_AvailableAddOns[count] = new AvailableAddOn(int.Parse(addOnData[0].Trim()), addOnData[1].Trim(),
                        addOnData[2].Trim(), addOnData[3].Trim(), addOnData[4].Trim(), addOnData[5].Trim(),
                        m_MainManagerWindow);
                    count++;
                }

                if (count != m_AvailableAddOns.Length) Array.Resize(ref m_AvailableAddOns, count);
                m_AddOnsReqest = null;
            }
            ...
            if (m_AvailableAddOns != null && m_AvailableAddOns.Length > 0)
            {
                ...
                // Draw each add-on.
                for (var i = 0; i < m_AvailableAddOns.Length; ++i)
                {
                    m_AvailableAddOns[i].DrawAddOn();
                    if (i != m_AvailableAddOns.Length - 1) GUILayout.Space(20);
                }
        ...
        {
            private readonly string m_AddOnURL;
            private readonly string m_Description;
            private Texture2D m_Icon;

            private UnityWebRequest m_IconRequest;
            private readonly int m_ID;
            private readonly bool m_Installed;
                ...
                MainManagerWindow mainManagerWindow)
            {
                ...
                m_Installed = !string.IsNullOrEmpty(type) && TypeUtility.GetType(type) != null;
            ...
            public void DrawAddOn()
            {
                ...
                if (m_Icon != null) GUILayout.Label(m_Icon);

                EditorGUILayout.BeginVertical();
                var name = m_Name;
                if (m_Installed) name += " (INSTALLED)";
