...
namespace Opsive.UltimateCharacterController.Editor.Managers
{
    ...
    {
        private readonly string[] m_ToolbarStrings = { "New Item", "Existing Item" };

        private enum ThirdPersonHumanoidParentHand
        {
            Left,
            Right
        }
        ...
        [SerializeField] private readonly bool m_AddFirstPersonPerspective = true;
        [SerializeField] private GameObject m_FirstPersonObject;
        [SerializeField] private readonly RuntimeAnimatorController m_FirstPersonObjectAnimatorController = null;
        [SerializeField] private GameObject m_FirstPersonVisibleItem;
        [SerializeField] private readonly RuntimeAnimatorController m_FirstPersonVisibleItemAnimatorController = null;
        ...
        [SerializeField]
        private ThirdPersonHumanoidParentHand m_ThirdHumanoidParentHand = ThirdPersonHumanoidParentHand.Right;
        ...
        [SerializeField]
        private ThirdPersonHumanoidParentHand m_ExistingThirdHumanoidParentHand = ThirdPersonHumanoidParentHand.Right;
        ...
        private readonly ItemSlot m_FirstPersonItemSlot = null;
        private ItemSlot m_ThirdPersonItemSlot;
        private readonly ItemSlot m_ExistingFirstPersonItemSlot = null;
        private ItemSlot m_ExistingThirdPersonItemSlot;
        ...
        public override void OnGUI()
        {
            var toolbarSelection =
                GUILayout.Toolbar(m_DrawNewItems ? 0 : 1, m_ToolbarStrings, EditorStyles.toolbarButton);
            ...
            if (m_DrawNewItems)
            {
                ...
                if (GUILayout.Button("this page", InspectorStyles.LinkStyle, GUILayout.Width(55)))
                    Application.OpenURL(
                        "https://opsive.com/support/documentation/ultimate-character-controller/getting-started/setup/item-creation/");
                GUILayout.Space(-1);
        ...
        {
            var canBuild = true;
            m_Name = EditorGUILayout.TextField(
                new GUIContent("Name",
            ...
            m_ItemDefinition = EditorGUILayout.ObjectField(
                new GUIContent("Item Definition",
                    "The Item Definition that the Item should use. The Item Definition works with the Inventory to determine the properties for that item."),
            ...
            else
            {
                ...
                if (m_ItemDefinition != null && m_Character != null &&
                    (itemSetManager = m_Character.GetComponent<ItemSetManager>()) != null &&
                    itemSetManager.ItemCollection != null)
                    if (AssetDatabase.GetAssetPath(m_ItemDefinition) !=
            ...
            var character = EditorGUILayout.ObjectField(
 ...
 m_Character.GetComponentInChildren<FirstPersonController.Character.FirstPersonObjects>();
                    if (firstPersonObjects != null) {
                        var firstPersonBaseObject =
 firstPersonObjects.GetComponentInChildren<FirstPersonController.Character.Identifiers.FirstPersonBaseObject>();
            ...
            if (m_Character == null)
            {
                m_SlotID = EditorGUILayout.IntField(new GUIContent("Slot ID",
                    "The ID of the slot that the Item should occupy. " +
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (m_Character.GetComponentInChildren<ItemPlacement>() == null)
                    {
                        if (canBuild)
                        {
                            EditorGUILayout.HelpBox("The character must be an already created character.",
                                MessageType.Error);
                            canBuild = false;
                        }
                    }
                    ...
                    {
                        if ((m_ItemDefinition != null) & (m_Character.GetComponent<Inventory.Inventory>() != null))
                        {
                            // The item can automatically be added to the default loadout if the inventory component exists.
                            EditorGUI.indentLevel++;
                            m_AddToDefaultLoadout = EditorGUILayout.Toggle(
                                new GUIContent("Add to Default Loadout",
                                    "If a character is specified the Item Definition can automatically be added to the Inventory's Default Loadout."),
            ...
            m_AddFirstPersonPerspective =
 EditorGUILayout.Toggle(new GUIContent("Add First Person Item", "Should the first person item perspective be added?"), m_AddFirstPersonPerspective);
            GUI.enabled = m_AddFirstPersonPerspective;
            var firstPersonSuccess =
 DrawFirstPersonObject(m_Character, ref m_FirstPersonObject, ref m_FirstPersonObjectAnimatorController, ref m_FirstPersonVisibleItem, 
            ...
            if (m_Character == null || (m_Character != null && m_Character.GetComponent<Animator>() != null))
            {
                ...
                m_AddThirdPersonPerspective = EditorGUILayout.Toggle(
                    new GUIContent("Add Third Person Item", "Should the third person item perspective be added?"),
                    m_AddThirdPersonPerspective);
                GUI.enabled = m_AddThirdPersonPerspective;
                var thirdPersonSuccess = DrawThirdPersonObject(m_Character, ref m_ThirdPersonObject,
                    ref m_ThirdHumanoidParentHand, ref m_ThirdPersonParent, ref m_ThirdPersonItemSlot,
                    ref m_ThirdPersonObjectAnimatorController,
                    m_FirstPersonItemSlot != null ? m_FirstPersonItemSlot.ID : 0, characterUpdate,
                    canBuild && m_AddThirdPersonPerspective);
                GUI.enabled = true;
                if (canBuild && m_AddThirdPersonPerspective)
                {
                    ...
                    {
                        canBuild = false;
                        EditorGUILayout.HelpBox("Magic items require an item GameObject (can be empty).",
                            MessageType.Error);
                    }
                }
            }
            ...
            {
                EditorGUI.indentLevel++;
                m_ActionItemDefinition = EditorGUILayout.ObjectField("Consumable Item Definition",
                    m_ActionItemDefinition, typeof(ItemDefinitionBase), false) as ItemDefinitionBase;
                EditorGUI.indentLevel--;
            }

            // Setup profiles.
            GUILayout.Space(5);
            var updatedStateConfiguration = EditorGUILayout.ObjectField(new GUIContent("State Configuration",
                    "Allows for the item to be preconfigured with already defined values. " +
            ...
            if (m_AddStateConfiguration != null)
            {
                EditorGUI.indentLevel++;
                var profiles =
                    m_AddStateConfiguration.GetProfilesForGameObject(null, StateConfiguration.Profile.ProfileType.Item);
                if (profiles.Count > 0)
                {
            ...
            {
                var item = ItemBuilder.BuildItem(m_Name, m_ItemDefinition, m_AnimatorItemID, m_Character, m_SlotID,
                    m_AddToDefaultLoadout, m_AddFirstPersonPerspective, m_FirstPersonObject,
                    m_FirstPersonObjectAnimatorController,
                    m_FirstPersonVisibleItem, m_FirstPersonItemSlot, m_FirstPersonVisibleItemAnimatorController,
                    m_AddThirdPersonPerspective, m_ThirdPersonObject, m_ThirdPersonItemSlot,
                    m_ThirdPersonObjectAnimatorController,
                ...
                else if (m_AddFirstPersonPerspective && m_AddThirdPersonPerspective)
                {
                    ...
                    var preset =
                        Shared.Editor.Utility.EditorUtility.LoadAsset<PersistablePreset>(
                            "50a5f74ba80091b47954d1f678ac7823");
                    if (preset != null)
                    {
                        var createdItem = item.GetComponent<Item>();
                ...
                if (m_FirstPersonObjectAnimatorController != null)
                    AnimatorBuilder.AddParameters((AnimatorController)m_FirstPersonObjectAnimatorController);
                if (m_FirstPersonVisibleItemAnimatorController != null)
                    AnimatorBuilder.AddParameters((AnimatorController)m_FirstPersonVisibleItemAnimatorController);
                if (m_ThirdPersonObjectAnimatorController != null)
                    AnimatorBuilder.AddParameters((AnimatorController)m_ThirdPersonObjectAnimatorController);
 ...
 firstPersonObjects.GetComponentInChildren<FirstPersonController.Character.Identifiers.FirstPersonBaseObject>();
                                            ...
                                            int defaultItemSlotIndex, bool characterUpdate, bool showError)
        {
            var success = true;
            firstPersonObject =
 EditorGUILayout.ObjectField(new GUIContent("First Person Base", "A reference to the base object that should be used by the first person item. This will usually be the character’s separated arms. " +
            ...
            if (character != null && firstPersonObject == null) {
                ...
                if (EditorUtility.IsPersistent(firstPersonObject)) {
                    ...
                    if (firstPersonObject.GetComponent<Character.UltimateCharacterLocomotion>() != null) {
                        ...
                        Animator animator;
                        if ((animator =
 firstPersonObject.GetComponent<Animator>()) == null || animator.runtimeAnimatorController == null) {
                            EditorGUI.indentLevel++;
                            firstPersonObjectAnimatorController =
 EditorGUILayout.ObjectField(new GUIContent("Animator Controller",
            ...
            var visibleItem =
 EditorGUILayout.ObjectField(new GUIContent("First Person Visible Item", "Specifies the Item object that is actually visible and rendered to the screen, such as the assault rifle or sword. " +
            ...
            if ((character != null && (firstPersonObject == null && firstPersonVisibleItem != null)) ||
                ...
                firstPersonParent =
 EditorGUILayout.ObjectField(new GUIContent("Item Parent", "Specifies the object that the First Person Visible Item should be parented to. This GameObject must have the ItemSlot component."),
                    ...
                    if ((firstPersonObject == null && firstPersonParent.GetComponentInParent<UltimateCharacterController.FirstPersonController.Character.FirstPersonObjects>() == null) || 
                        ...
                        if (itemSlots.Length == 1) {
                            ...
                            if (GUILayout.Button("Add ItemSlot", GUILayout.Width(150))) {
                                firstPersonParent =
 AddItemSlot(character != null ? character : firstPersonObject, firstPersonParent.transform, true);
                            }
                        }
                    }
                }
 ...
 EditorGUILayout.ObjectField(new GUIContent("Animator Controller", "Specifies the Animator Controller that should be used by the First Person Visible Item."), 
        ...
        /// <param name="showError">Should the error be shown (if any)?</param>
        /// <returns>Was the objects drawn successfully?</returns>
        private bool DrawThirdPersonObject(GameObject character, ref GameObject thirdPersonObject,
            ref ThirdPersonHumanoidParentHand parentHand, ref GameObject thirdPersonParent,
            ...
            int defaultItemSlotIndex, bool characterUpdate, bool showError)
        {
            ...
            thirdPersonObject = EditorGUILayout.ObjectField(new GUIContent("Third Person Visible Item",
                    "Specifies the third person item object. " +
            ...
            if (thirdPersonObject != null && thirdPersonObject.GetComponent<Item>())
            {
                ...
                if (showError)
                    EditorGUILayout.HelpBox(
                        "The visible item should not be an already created item. The visible item should be a model representing the item.",
                        MessageType.Error);
            }
            ...
            if (thirdPersonObject != null && character != null)
            {
                ...
                if (prevThirdPersonObject != thirdPersonObject || characterUpdate)
                {
                    ...
                    for (var i = 0; i < itemSlots.Length; ++i)
                    {
                        ...
                        if (itemSlots[i].ID == defaultItemSlotIndex)
                        {
                            ...
                            {
                                if (thirdPersonParent.transform.IsChildOf(
                ...
                if (animator != null && animator.GetBoneTransform(HumanBodyBones.Head) != null)
                {
                    ...
                    {
                        parentHand = hand;
                        var handTransform = animator.GetBoneTransform(hand == ThirdPersonHumanoidParentHand.Right
                ...
                {
                    thirdPersonParent = EditorGUILayout.ObjectField(
                        new GUIContent("Item Parent",
                            "Specifies the object that the Third Person Visible Item should be parented to. This GameObject must have the ItemSlot component."),
                ...
                if (invalidItemSlot)
                {
                    ...
                    if (showError)
                        EditorGUILayout.HelpBox(
                            "The third person Item Parent field does not specify a valid ItemSlot GameObject.",
                            MessageType.Error);
                }
            ...
            {
                EditorGUI.indentLevel++;
                thirdPersonObjectAnimatorController = EditorGUILayout.ObjectField(
                        new GUIContent("Animator Controller",
                            "Specifies the Animator Controller that should be used by the Third Person Visible Item."),
                        thirdPersonObjectAnimatorController, typeof(RuntimeAnimatorController), false) as
...
#if FIRST_PERSON_CONTROLLER
            var firstPersonObjects =
 baseParent.GetComponentInChildren<UltimateCharacterController.FirstPersonController.Character.FirstPersonObjects>();
#endif
...
#endif
                if (allItemSlots[i].ID > maxID) maxID = allItemSlots[i].ID;
            }

            // Setup the new ItemSlot.
            var itemSlotGameObject = new GameObject("Items", typeof(ItemSlot));
        ...
        private void DrawExistingItem()
        {
            ...
            {
#if FIRST_PERSON_CONTROLLER
                var firstPersonVisibleItemObject =
 m_Item.GetComponent<UltimateCharacterController.FirstPersonController.Items.FirstPersonPerspectiveItem>();
                if (firstPersonVisibleItemObject != null) {
                    ItemBuilder.RemoveFirstPersonObject(firstPersonVisibleItemObject);
                }
#endif
                var thirdPersonVisibleItemObject = m_Item.GetComponent<ThirdPersonPerspectiveItem>();
                ...
                var inventory = m_Item.GetComponentInParent<Inventory.Inventory>();
                if (inventory != null)
                {
                    var defaultLoadout = new List<ItemDefinitionAmount>(inventory.DefaultLoadout);
                    for (var i = defaultLoadout.Count - 1; i > -1; --i)
                ...
                {
                    for (var i = 0; i < itemSetManager.CategoryItemSets.Length; ++i)
                    {
                        var category = itemSetManager.CategoryItemSets[i];
                        for (var j = category.ItemSetList.Count - 1; j > -1; --j)
            ...
            if (m_Item != null)
            {
                ...
                if (actions.Length > 0)
                {
                    ...
                    for (var i = 0; i < actions.Length; ++i)
                    {
                        ...
                        if (actions.Length > 1) actionStrings[i] += " (ID " + actions[i].ID + ")";
                    }

                    EditorGUILayout.BeginHorizontal();
                    m_RemoveActionTypeIndex =
                        EditorGUILayout.Popup("Remove Action", m_RemoveActionTypeIndex, actionStrings);
                    if (GUILayout.Button("Remove", GUILayout.Width(80)))
                    {
            ...
            {
                EditorGUI.indentLevel++;
                m_ExistingAddActionItemDefinition = EditorGUILayout.ObjectField("Consumable Item Definition",
                    m_ExistingAddActionItemDefinition, typeof(ItemDefinitionBase), false) as ItemDefinitionBase;
                EditorGUI.indentLevel--;
            }
            ...
            if (m_Item != null) {
                ...
                if (firstPersonVisibleItem != null) {
                    ...
                    if (m_ExistingFirstPersonVisibleItem != null) {
                        var firstPersonVisibleItemAnimator =
 firstPersonVisibleItem.VisibleItem.GetComponent<Animator>();
                        if (firstPersonVisibleItemAnimator != null) {
                            m_ExistingFirstPersonVisibleItemAnimatorController =
 firstPersonVisibleItemAnimator.runtimeAnimatorController;
                        } else {
            ...
            EditorGUI.indentLevel++;
            ThirdPersonPerspectiveItem thirdPersonVisibleItem = null;
            if (m_Item != null)
            {
                thirdPersonVisibleItem = m_Item.GetComponent<ThirdPersonPerspectiveItem>();
                ...
                if (thirdPersonVisibleItem != null)
                {
                    ...
                    if (m_ExistingThirdPersonObject != null)
                    {
                        ...
                        if (thirdPersonAnimator != null)
                            m_ExistingThirdPersonObjectAnimatorController =
                ...
                var character = m_Item.GetComponentInParent<UltimateCharacterLocomotion>();
                if (character == null || (character != null && character.GetComponent<Animator>() != null))
                    DrawThirdPersonObject(character != null ? character.gameObject : null,
                        ref m_ExistingThirdPersonObject, ref m_ExistingThirdHumanoidParentHand,
                        ref m_ExistingThirdPersonParent,
            ...
            {
                var character = m_Item.GetComponentInParent<UltimateCharacterLocomotion>();
                ItemBuilder.AddThirdPersonObject(character.gameObject, m_Item.name, m_Item.gameObject,
                    ref m_ExistingThirdPersonObject, m_ExistingThirdPersonItemSlot,
                    m_ExistingThirdPersonObjectAnimatorController, m_InvisibleShadowCaster, false);
                // Ensure the animators have the required parameters.
                if (m_ExistingThirdPersonObjectAnimatorController != null)
                    AnimatorBuilder.AddParameters((AnimatorController)m_ExistingThirdPersonObjectAnimatorController);
            }

            GUI.enabled = m_Item != null && thirdPersonVisibleItem != null;
            if (GUILayout.Button("Remove")) ItemBuilder.RemoveThirdPersonObject(thirdPersonVisibleItem);
            EditorGUILayout.EndHorizontal();
            EditorGUI.indentLevel--;
            ...
            EditorGUI.indentLevel++;
            var updatedStateConfiguration = EditorGUILayout.ObjectField("State Configuration",
                m_ExistingStateConfiguration, typeof(StateConfiguration), false) as StateConfiguration;
            ...
            {
                var profiles = m_ExistingStateConfiguration.GetProfilesForGameObject(
                    m_Item == null ? null : m_Item.gameObject, StateConfiguration.Profile.ProfileType.Item);
                EditorGUILayout.BeginHorizontal();
                var canSetup = true;
                if (profiles.Count == 0)
                {
                ...
                {
                    m_ExistingStateConfiguration.AddStatesToGameObject(profiles[m_ExistingProfileIndex],
                        m_Item.gameObject);
