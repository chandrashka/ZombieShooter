...
namespace Opsive.UltimateCharacterController.Editor.Managers
{
    ...
    public abstract class IntegrationInspector
    {
        ...
        public MainManagerWindow MainManagerWindow
        {
            set => m_MainManagerWindow = value;
        }
    ...
    public class IntegrationsManager : Manager
    {
        ...
        private UnityWebRequest m_IntegrationsReqest;

        private Vector2 m_ScrollPosition;

        private readonly string[] m_ToolbarStrings = { "Integration Inspectors", "Available Integrations" };
        ...
        private void BuildInstalledIntegrations()
        {
            ...
            for (var i = 0; i < assemblies.Length; ++i)
            {
                ...
                for (var j = 0; j < assemblyTypes.Length; ++j)
                {
                    ...
                    if (!typeof(IntegrationInspector).IsAssignableFrom(assemblyTypes[j])) continue;
                    ...
                    {
                        var item =
                            assemblyTypes[j].GetCustomAttributes(typeof(OrderedEditorItem), true)[0] as
                                OrderedEditorItem;
                        index = item.Index;
                    }
            ...
            if (m_IntegrationInspectors != null &&
                m_IntegrationInspectors.Length == integrationInspectors.Count) return;
            ...
            for (var i = 0; i < inspectorTypes.Length; ++i)
            {
                ...
                var name = EditorUtility.SplitCamelCase(inspectorTypes[i].Name);
                if (integrationInspectors[i].GetCustomAttributes(typeof(OrderedEditorItem), true).Length > 0)
                {
                    var item =
                        inspectorTypes[i].GetCustomAttributes(typeof(OrderedEditorItem), true)[0] as OrderedEditorItem;
                    name = item.Name;
                }
        ...
        {
            var toolbarSelection = GUILayout.Toolbar(m_DrawIntegrationInspectors ? 0 : 1, m_ToolbarStrings,
        ...
        private void DrawIntegrationInspectors()
        {
            ...
            for (var i = 0; i < m_IntegrationInspectors.Length; ++i)
            {
                ...
                if (i != m_IntegrationInspectors.Length - 1) GUILayout.Space(20);
            }
        ...
        private void DrawAvailableIntegrations()
        {
            ...
            {
                m_IntegrationsReqest =
                    UnityWebRequest.Get("https://opsive.com/asset/UltimateCharacterController/IntegrationsList.txt");
                m_IntegrationsReqest.SendWebRequest();
            }
            else if (m_Integrations == null && m_IntegrationsReqest.isDone &&
                     string.IsNullOrEmpty(m_IntegrationsReqest.error))
            {
                ...
                var count = 0;
                for (var i = 0; i < splitIntegrations.Length; ++i)
                {
                    if (string.IsNullOrEmpty(splitIntegrations[i])) continue;
                    ...
                    if (integrationData.Length < 4) continue;

                    m_Integrations[count] = new AssetIntegration(int.Parse(integrationData[0].Trim()),
                        integrationData[1].Trim(), integrationData[2].Trim(), integrationData[3].Trim(),
                        m_MainManagerWindow);
                    count++;
                }

                if (count != m_Integrations.Length) Array.Resize(ref m_Integrations, count);
            ...
            if (m_Integrations != null && m_Integrations.Length > 0)
            {
                ...
                var cellsPerRow = (int)(Screen.width - m_MainManagerWindow.MenuWidth - 2) /
                                  (c_IntegrationCellWidth + c_IntegrationCellSpacing);
                m_ScrollPosition = GUI.BeginScrollView(
                    new Rect(0, lastRect.y, Screen.width - m_MainManagerWindow.MenuWidth - 2, Screen.height - 96),
                    m_ScrollPosition,
                    new Rect(0, 0, Screen.width - m_MainManagerWindow.MenuWidth - 25,
                        (m_Integrations.Length / cellsPerRow + (m_Integrations.Length % 2 == 0 ? 0 : 1)) *
                        (c_IntegrationCellHeight + c_IntegrationCellSpacing)));
                var position = new Vector2(0, 20);
                // Draw each integration.
                for (var i = 0; i < m_Integrations.Length; ++i)
                {
                    position.x = i % cellsPerRow * c_IntegrationCellWidth;
                    m_Integrations[i].DrawIntegration(position +
                                                      new Vector2(0,
                                                          c_IntegrationCellHeight + c_IntegrationCellSpacing) *
                                                      (i / cellsPerRow));
                }
            ...
            else
            {
                if (m_IntegrationsReqest == null)
                {
                    if (Event.current.type == EventType.Repaint)
                    {
                        m_IntegrationsReqest =
                            UnityWebRequest.Get(
                                "https://opsive.com/asset/UltimateCharacterController/IntegrationsList.txt");
                        m_IntegrationsReqest.SendWebRequest();
                    }
                }
                else
...
#pragma warning restore 0162
        ...
        private class AssetIntegration
        {
            ...
            private UnityWebRequest m_IconRequest;

            private readonly int m_ID;
            private readonly string m_IntegrationURL;
            private readonly MainManagerWindow m_MainManagerWindow;
            private readonly string m_Name;
            private readonly DownloadHandlerTexture m_TextureDownloadHandler;
            ...
            public AssetIntegration(int id, string name, string iconURL, string integrationURL,
                MainManagerWindow mainManagerWindow)
            {
                m_ID = id;
                m_Name = name;
                ...
                m_TextureDownloadHandler = new DownloadHandlerTexture();
                m_IconRequest = UnityWebRequest.Get(iconURL);
            ...
            public void DrawIntegration(Vector2 position)
            {
                if (m_IconRequest != null)
                {
                    ...
                    {
                        if (string.IsNullOrEmpty(m_IconRequest.error)) m_Icon = m_TextureDownloadHandler.texture;
                        m_IconRequest = null;
                    }
                    else
                    {
                ...
                if (m_Icon != null) GUI.DrawTexture(new Rect(position.x, position.y, c_IconSize, c_IconSize), m_Icon);
                ...
                if (!string.IsNullOrEmpty(m_IntegrationURL) &&
                    GUI.Button(new Rect(rect.x, rect.y + 23, 80, 18), "Integration"))
                    Application.OpenURL(m_IntegrationURL);

                if (m_ID > 0 &&
                    GUI.Button(new Rect(rect.x, rect.y + (string.IsNullOrEmpty(m_IntegrationURL) ? 23 : 47), 80, 18),
                        "Asset Store"))
                    Application.OpenURL(
                        "https://opsive.com/asset/UltimateCharacterController/AssetRedirect.php?asset=" + m_ID);
            }
        }
    }
}