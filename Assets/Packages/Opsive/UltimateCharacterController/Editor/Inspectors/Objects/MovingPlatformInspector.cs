    ...
    public class MovingPlatformInspector : StateBehaviorInspector
    {
        ...
        protected override Action GetDrawCallback()
        {
            ...
            baseCallback += () =>
            {
                ...
                if (Foldout("Path"))
                {
                    ...
                    {
                        if (m_Platform.Waypoints == null) m_Platform.Waypoints = new MovingPlatform.Waypoint[0];
                        m_WaypointReorderableList = new ReorderableList(m_Platform.Waypoints,
                            typeof(MovingPlatform.Waypoint), true, true, true, true);
                ...
                if (Foldout("Movement"))
                {
                    ...
                    if (rotationInterpolation.enumValueIndex ==
                        (int)MovingPlatform.RotateInterpolationMode.CustomEaseOut)
                    {
                        EditorGUI.indentLevel++;
                        EditorGUILayout.PropertyField(PropertyFromName("m_RotationEaseAmount"));
                        EditorGUI.indentLevel--;
                    }
                    else if (rotationInterpolation.enumValueIndex ==
                             (int)MovingPlatform.RotateInterpolationMode.CustomRotate)
                    {
                        EditorGUI.indentLevel++;
        ...
        private void OnAnimatorWaypointStateListHeaderDraw(Rect rect)
        {
            EditorGUI.LabelField(
                new Rect(rect.x + 12, rect.y, rect.width - c_DelayWidth - c_StateWidth,
                    EditorGUIUtility.singleLineHeight), "Transform");
            EditorGUI.LabelField(
                new Rect(rect.x + (rect.width - c_DelayWidth - c_StateWidth), rect.y, c_DelayWidth,
                    EditorGUIUtility.singleLineHeight), "Delay");
            EditorGUI.LabelField(
                new Rect(rect.x + (rect.width - c_StateWidth) + 4, rect.y, c_StateWidth - 4,
                    EditorGUIUtility.singleLineHeight), "State");
        }
        ...
        {
            m_Platform.Waypoints[index].Transform = (Transform)EditorGUI.ObjectField(
                new Rect(rect.x, rect.y + 1, rect.width - c_DelayWidth - c_StateWidth - 2,
                    EditorGUIUtility.singleLineHeight),
                m_Platform.Waypoints[index].Transform, typeof(Transform), true);
            m_Platform.Waypoints[index].Delay = EditorGUI.FloatField(new Rect(
                rect.x + (rect.width - c_DelayWidth - c_StateWidth), rect.y + 1, c_DelayWidth,
                EditorGUIUtility.singleLineHeight), m_Platform.Waypoints[index].Delay);
            m_Platform.Waypoints[index].State = EditorGUI.TextField(new Rect(rect.x + (rect.width - c_StateWidth) + 4,
                rect.y + 1, c_StateWidth - 4,
        ...
        private static void DrawMovingPlatformGizmo(MovingPlatform movingPlatform, GizmoType gizmoType)
        {
            if (movingPlatform.Waypoints == null) return;

            Mesh mesh = null;
            var meshFilter = movingPlatform.GetComponent<MeshFilter>();
            if (meshFilter != null) mesh = meshFilter.sharedMesh;
            for (var i = 0; i < movingPlatform.Waypoints.Length; ++i)
            {
                if (movingPlatform.Waypoints[i].Transform == null) continue;

                Gizmos.color = movingPlatform.GizmoColor;
                // Draw the mesh if it exists.
                if (mesh != null)
                    Gizmos.DrawMesh(mesh, movingPlatform.Waypoints[i].Transform.position,
                        movingPlatform.Waypoints[i].Transform.rotation, movingPlatform.transform.localScale);
                ...
                if (movingPlatform.DrawDebugLabels)
                {
                    if (s_DebugLabelStyle == null)
                    {
                        ...
                        s_DebugLabelStyle.normal.textColor =
                            Utility.InspectorUtility.GetContrastColor(movingPlatform.GizmoColor);
                    }

                    // Draw the delay in the center of the platform.
                    Handles.Label(movingPlatform.Waypoints[i].Transform.position,
                        movingPlatform.Waypoints[i].Delay.ToString(CultureInfo.InvariantCulture), s_DebugLabelStyle);
                }

                // Draw a line connecting the platforms.
                if (i > 0 && movingPlatform.Waypoints[i - 1].Transform != null &&
                    movingPlatform.MovementType != MovingPlatform.PathMovementType.Target)
                {
                    Gizmos.color = Utility.InspectorUtility.GetContrastColor(movingPlatform.GizmoColor);
                    Gizmos.DrawLine(movingPlatform.Waypoints[i - 1].Transform.position,
                        movingPlatform.Waypoints[i].Transform.position);
                    ...
                    if (movingPlatform.DrawDebugLabels)
                    {
                        ...
                        var distance =
                            decimal.Round(
                                (decimal)Vector3.Distance(movingPlatform.Waypoints[i - 1].Transform.position,
                                    movingPlatform.Waypoints[i].Transform.position), 3);
                        Handles.Label(
                            (movingPlatform.Waypoints[i - 1].Transform.position +
                             movingPlatform.Waypoints[i].Transform.position) / 2,
                            distance.ToString(CultureInfo.InvariantCulture), s_DebugLabelStyle);
                    }
                }
            }

            // Complete the path drawing.
            if (movingPlatform.MovementType == MovingPlatform.PathMovementType.Loop &&
                movingPlatform.Waypoints.Length > 0 && movingPlatform.Waypoints[0].Transform != null &&
                movingPlatform.Waypoints[movingPlatform.Waypoints.Length - 1].Transform != null)
            {
                Gizmos.color = Utility.InspectorUtility.GetContrastColor(movingPlatform.GizmoColor);
                Gizmos.DrawLine(movingPlatform.Waypoints[0].Transform.position,
                    movingPlatform.Waypoints[movingPlatform.Waypoints.Length - 1].Transform.position);
                ...
                if (movingPlatform.DrawDebugLabels)
                {
                    ...
                    var distance =
                        decimal.Round(
                            (decimal)Vector3.Distance(movingPlatform.Waypoints[0].Transform.position,
                                movingPlatform.Waypoints[movingPlatform.Waypoints.Length - 1].Transform.position), 3);
                    Handles.Label(
                        (movingPlatform.Waypoints[0].Transform.position + movingPlatform
                            .Waypoints[movingPlatform.Waypoints.Length - 1].Transform.position) / 2,
                        distance.ToString(CultureInfo.InvariantCulture), s_DebugLabelStyle);
                }
            }
            else if (movingPlatform.MovementType == MovingPlatform.PathMovementType.Target &&
                     movingPlatform.TargetWaypoint < movingPlatform.Waypoints.Length &&
                     movingPlatform.Waypoints[movingPlatform.TargetWaypoint].Transform != null)
            {
                Gizmos.color = Utility.InspectorUtility.GetContrastColor(movingPlatform.GizmoColor);
                Gizmos.DrawLine(movingPlatform.transform.position,
                    movingPlatform.Waypoints[movingPlatform.TargetWaypoint].Transform.position);
            }
            ...
            if (Application.isPlaying && movingPlatform.enabled && movingPlatform.Waypoints.Length > 0)
            {
                ...
                Gizmos.DrawLine(movingPlatform.transform.position,
                    movingPlatform.Waypoints[movingPlatform.NextWaypoint].Transform.position);
            }
        }
    }
}