...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Objects.CharacterAssist
{
    ...
    public abstract class ItemPickupBaseInspector : ObjectPickupInspector
    {
        ...
        private const string c_EditorPrefsSelectedPickupSetIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.ItemPickup.SelectedPickupSetIndex";

        private const string c_EditorPrefsSelectedPickupSetStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.ItemPickup.SelectedPickupSetStateIndex";
        ...
        private string SelectedPickupSetIndexKey =>
            c_EditorPrefsSelectedPickupSetIndexKey + "." + target.GetType() + "." + target.name;
        ...
        protected override void DrawObjectPickupFields()
        {
            ...
            if (Foldout("Item Pickup Set"))
            {
                ...
                if (m_SlotCount != slotCount)
                {
                    ...
                    for (var i = 0; i < pickupSet.Length; ++i) UpdateSlotCount(pickupSet, i);
                }
                ...
                if (itemListProperty.arraySize == 0)
                    EditorGUILayout.HelpBox(
                        "No Pickup Set specified. The character must have a Pickup Set specified in order to equip the item.",
                        MessageType.Info);

                if (m_ReorderablePickupSet == null)
                {
                    m_ReorderablePickupSet =
                        new ReorderableList(serializedObject, itemListProperty, true, false, true, true);
                    m_ReorderablePickupSet.drawHeaderCallback = rect =>
                    {
                    ...
                    m_ReorderablePickupSet.onSelectCallback = list =>
                    {
                ...
                if (m_ItemPickup.ItemPickupSet.Length > 0)
                    m_ReorderablePickupSet.elementHeight =
                        (c_SlotRowHeight + c_SlotRowPadding) * (m_SlotCount + 2) + 21;
                else
                ...
                if (m_ReorderablePickupSet.index != -1 &&
                    m_ReorderablePickupSet.index < m_ItemPickup.ItemPickupSet.Length)
                {
                    GUI.enabled = !Application.isPlaying;
                    DrawSelectedPickupSet(m_ItemPickup.ItemPickupSet[m_ReorderablePickupSet.index],
                        m_ReorderablePickupSet.index);
                    GUI.enabled = true;
                }
        ...
        private void OnPickupSetElementDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            for (var i = 0; i < m_SlotCount + 2; ++i)
            {
                ...
                if (i == 0)
                {
                    ...
                    pickupSet[index].Item =
                        EditorGUI.ObjectField(objRect, pickupSet[index].Item, typeof(GameObject), false) as GameObject;
                }
                else if (i == 1)
                {
                ...
                else
                {
                    ...
                    pickupSet[index].ItemSet.Slots[i - 2] = EditorGUI.ObjectField(objRect,
                        pickupSet[index].ItemSet.Slots[i - 2], typeof(ItemDefinitionBase), false) as ItemDefinitionBase;
                    ...
                    if (pickupSet[index].Item != null)
                    {
                        ...
                        if (item != null && item.SlotID < pickupSet[index].ItemSet.Slots.Length &&
            ...
            if (EditorGUI.EndChangeCheck()) Shared.Editor.Utility.EditorUtility.SetDirty(target);
            GUI.enabled = true;
        }
        ...
        private void UpdateSlotCount(ItemPickupBase.PickupSet[] pickupSet, int index)
        {
            if (pickupSet[index].ItemSet.Slots == null || pickupSet[index].ItemSet.Slots.Length != m_SlotCount)
            {
                ...
                Array.Resize(ref slots, m_SlotCount);
        ...
        private void OnPickupSetListAdd(ReorderableList reorderableList)
        {
            ...
            if (index == 0) m_ItemPickup.ItemPickupSet[index].ItemSet.Enabled = true;
        ...
        private void OnPickupSetListRemove(ReorderableList reorderableList)
        {
            ...
            if (reorderableList.index == -1 && m_ItemPickup.ItemPickupSet.Length > 0) reorderableList.index = 0;
            EditorPrefs.SetInt(SelectedPickupSetIndexKey, reorderableList.index);
        ...
        private void DrawSelectedPickupSet(ItemPickupBase.PickupSet pickupSet, int index)
        {
            ...
            pickupSet.ItemSet.State = EditorGUILayout.TextField(
                new GUIContent("State",
                    "Optionally specify a state that the character should switch to when the Item Set is active."),
                pickupSet.ItemSet.State);
            // Draws all of the slots ItemIdentifiers.
            for (var i = 0; i < m_SlotCount; ++i)
                pickupSet.ItemSet.Slots[i] = (ItemDefinitionBase)EditorGUILayout.ObjectField("Slot " + i,
                    pickupSet.ItemSet.Slots[i], typeof(ItemDefinitionBase), false);
            pickupSet.Default =
                EditorGUILayout.Toggle(new GUIContent("Default", "True if the ItemSet is the default Item Set."),
                    pickupSet.Default);
            pickupSet.ItemSet.Enabled =
                EditorGUILayout.Toggle(new GUIContent("Enabled", "True if the ItemSet can be equipped."),
                    pickupSet.ItemSet.Enabled);
            pickupSet.ItemSet.CanSwitchTo = EditorGUILayout.Toggle(
                new GUIContent("Can Switch To",
                    "True if the ItemSet can be switched to by the EquipNext/EquipPrevious abilities."),
                pickupSet.ItemSet.CanSwitchTo);
            pickupSet.ItemSet.DisabledIndex = EditorGUILayout.IntField(
                new GUIContent("Disabled Index",
                    "The ItemSet that should be activated if the current ItemSet is disabled."),
                pickupSet.ItemSet.DisabledIndex);
            ...
            if (InspectorUtility.Foldout(pickupSet, new GUIContent("States"), false))
            {
                ...
                try
                {
                    ...
                    m_ReorderablePickupSetStateList = StateInspector.DrawStates(m_ReorderablePickupSetStateList,
                        serializedObject,
        ...
        {
            return c_EditorPrefsSelectedPickupSetStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   " " + itemSetIndex;
        }
        ...
        {
            var pickupSet = m_ItemPickup.ItemPickupSet[EditorPrefs.GetInt(SelectedPickupSetIndexKey, -1)];
            var states = StateInspector.AddExistingPreset(pickupSet.ItemSet.GetType(), pickupSet.ItemSet.States,
                m_ReorderablePickupSetStateList,
            ...
            if (pickupSet.ItemSet.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderablePickupSetStateList.serializedProperty);
                Shared.Editor.Utility.EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var pickupSet = m_ItemPickup.ItemPickupSet[EditorPrefs.GetInt(SelectedPickupSetIndexKey, -1)];
            var states = StateInspector.CreatePreset(pickupSet.ItemSet, pickupSet.ItemSet.States,
                m_ReorderablePickupSetStateList,
                GetSelectedPickupSetStateIndexKey(EditorPrefs.GetInt(SelectedPickupSetIndexKey, -1)));
            if (pickupSet.ItemSet.States.Length != states.Length)
            {
        ...
        private void OnPickupSetStateListReorder(ReorderableList list)
        {
            ...
            var copiedStates = new State[pickupSet.ItemSet.States.Length];
            Array.Copy(pickupSet.ItemSet.States, copiedStates, pickupSet.ItemSet.States.Length);
            for (var i = 0; i < pickupSet.ItemSet.States.Length; ++i)
            {
        ...
        {
            var pickupSet = m_ItemPickup.ItemPickupSet[EditorPrefs.GetInt(SelectedPickupSetIndexKey, -1)];
            var states = StateInspector.OnStateListRemove(pickupSet.ItemSet.States,
                GetSelectedPickupSetStateIndexKey(EditorPrefs.GetInt(SelectedPickupSetIndexKey, -1)), list);
            if (pickupSet.ItemSet.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderablePickupSetStateList.serializedProperty);
