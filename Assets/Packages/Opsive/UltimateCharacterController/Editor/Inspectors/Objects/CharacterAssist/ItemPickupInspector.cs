...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Objects.CharacterAssist
{
    ...
    public class ItemPickupInspector : ItemPickupBaseInspector
    {
        ...
        {
            var categoryNames = new string[(m_ItemCollection != null && m_ItemCollection.Categories != null
                ? m_ItemCollection.Categories.Length
                : 0) + 1];
            categoryNames[0] = "(Not Specified)";
            var selected = 0;
            if (categoryNames.Length > 1 && GUI.enabled)
                for (var i = 0; i < m_ItemCollection.Categories.Length; ++i)
                {
                    categoryNames[i + 1] = m_ItemCollection.Categories[i].name;
                    if (pickupSet.CategoryID == m_ItemCollection.Categories[i].ID) selected = i + 1;
                }
        ...
        {
            ItemDefinitionAmountInspector.OnItemDefinitionAmountElementDraw(PropertyFromName("m_ItemDefinitionAmounts"),
