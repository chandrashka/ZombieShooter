...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Objects.CharacterAssist
{
    ...
    public class MoveTowardsLocationGizmo
    {
        ...
        private static void DrawMoveTowardsLocationGizmo(MoveTowardsLocation startLocation, GizmoType gizmoType)
        {
            ...
            Handles.matrix = Gizmos.matrix = Matrix4x4.TRS(transform.TransformPoint(startLocation.Offset),
                transform.rotation * Quaternion.Euler(0, startLocation.YawOffset, 0), Vector3.one);
            ...
            Handles.DrawSolidArc(Vector3.zero, Vector3.up,
                Quaternion.AngleAxis(startLocation.Angle / 2 - startLocation.Angle, Vector3.up) * Vector3.forward,
            ...
            Handles.DrawLine(2 * radius * Vector3.forward,
                1.5f * radius * Vector3.forward + 0.5f * radius * Vector3.left);
            Handles.DrawLine(2 * radius * Vector3.forward,
                1.5f * radius * Vector3.forward + 0.5f * radius * Vector3.right);
            if (startLocation.Angle >= 180)
            {
                Handles.DrawLine(2 * radius * Vector3.left, 2 * radius * Vector3.right);
                Handles.DrawLine(2 * radius * Vector3.left,
                    1.5f * radius * Vector3.left + 0.5f * radius * Vector3.forward);
                Handles.DrawLine(2 * radius * Vector3.left,
                    1.5f * radius * Vector3.left + 0.5f * radius * Vector3.back);
                Handles.DrawLine(2 * radius * Vector3.right,
                    1.5f * radius * Vector3.right + 0.5f * radius * Vector3.back);
                Handles.DrawLine(2 * radius * Vector3.right,
                    1.5f * radius * Vector3.right + 0.5f * radius * Vector3.back);
                if (startLocation.Angle == 360)
                {
                    Handles.DrawLine(Vector3.zero, Vector3.back * 2f * radius);
                    Handles.DrawLine(2 * radius * Vector3.back,
                        1.5f * radius * Vector3.back + Vector3.left * 0.5f * radius);
                    Handles.DrawLine(2 * radius * Vector3.back,
                        1.5f * radius * Vector3.back + Vector3.right * 0.5f * radius);
                }
            }
        }
    }
