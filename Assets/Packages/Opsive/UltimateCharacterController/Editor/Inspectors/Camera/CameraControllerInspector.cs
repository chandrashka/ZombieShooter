...
using Transition = Opsive.UltimateCharacterController.Camera.ViewTypes.Transition;
...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Camera
{
    ...
    {
        private const string c_EditorPrefsSelectedViewTypeIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Camera.SelectedViewTypeIndex";

        private const string c_EditorPrefsSelectedViewTypeStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Camera.SelectedViewTypeStateIndex";
        ...
        private string SelectedViewTypeIndexKey =>
            c_EditorPrefsSelectedViewTypeIndexKey + "." + target.GetType() + "." + target.name;
            ...
            baseCallback += () =>
            {
                if (Foldout("Character"))
                {
                    ...
                    if (initOnAwake.boolValue || Application.isPlaying)
                    {
                        ...
                        if (!Application.isPlaying && characterProperty.objectReferenceValue != null)
                            if (!string.IsNullOrEmpty(
                                    AssetDatabase.GetAssetPath(characterProperty.objectReferenceValue)))
                                EditorGUILayout.HelpBox(
                                    "The Camera Controller Character property cannot point to a prefab.",
                                    MessageType.Error);
                    }
                    ...
                    else
                    {
                        ...
                        anchorProperty.objectReferenceValue = EditorGUILayout.ObjectField("Anchor",
                            anchorProperty.objectReferenceValue, typeof(Transform), true,
                            GUILayout.MinWidth(80)) as Transform;
                        if (anchorProperty.objectReferenceValue == null)
                            EditorGUILayout.HelpBox(
                                "The anchor specifies the Transform that the camera should follow. If null it will use the Character's Transform.",
                                MessageType.Info);
                    }
                ...
                if (Foldout("View Types"))
                {
                    ...
                    if (!string.IsNullOrEmpty(m_CameraController.FirstPersonViewTypeFullName) &&
                        !string.IsNullOrEmpty(m_CameraController.ThirdPersonViewTypeFullName))
                    {
                        var selectedIndex = 0;
                        for (var i = 0; i < m_FirstPersonViewTypes.Length; ++i)
                        ...
                        var index = EditorGUILayout.Popup("First Person View Type", selectedIndex,
                            m_FirstPersonViewTypeNames);
                        if (index != selectedIndex)
                        {
                            m_CameraController.FirstPersonViewTypeFullName =
                                PropertyFromName("m_FirstPersonViewTypeFullName").stringValue =
                                    m_FirstPersonViewTypes[index].FullName;
                        ...
                        for (var i = 0; i < m_ThirdPersonViewTypes.Length; ++i)
                        ...
                        index = EditorGUILayout.Popup("Third Person View Type", selectedIndex,
                            m_ThirdPersonViewTypeNames);
                        if (index != selectedIndex)
                        {
                            m_CameraController.ThirdPersonViewTypeFullName =
                                PropertyFromName("m_ThirdPersonViewTypeFullName").stringValue =
                                    m_ThirdPersonViewTypes[index].FullName;
                    ...
                    ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableViewTypeList, this,
                        m_CameraController.ViewTypes, "m_ViewTypeData",
                        OnViewTypeListDrawHeader, OnViewTypeListDraw, OnViewTypeListReorder, OnViewTypeListAdd,
                        OnViewTypeListRemove, OnViewTypeListSelect, DrawSelectedViewType, SelectedViewTypeIndexKey,
                        true, true);
                    EditorGUI.indentLevel--;
                ...
                if (Foldout("Zoom"))
                {
                    ...
                    appendItemIdentifierNameProperty.boolValue = EditorGUILayout.ToggleLeft(
                        new GUIContent("Append Item", "Should the ItemIdentifier name be appened to the state name?"),
        ...
        private void OnViewTypeListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var label = Utility.InspectorUtility.DisplayTypeName(viewType.GetType(), true);
            ...
            if (!(m_CameraController.ViewTypes[index] is Transition))
            {
                EditorGUI.BeginChangeCheck();
                activeRect = rect;
                activeRect.x += activeRect.width - 32;
                activeRect.width = 20;
                EditorGUI.Toggle(activeRect, m_CameraController.ViewTypeFullName == viewType.GetType().FullName,
                    EditorStyles.radioButton);
                if (EditorGUI.EndChangeCheck())
                {
                    m_CameraController.ViewTypeFullName = PropertyFromName("m_ViewTypeFullName").stringValue =
                        viewType.GetType().FullName;
                    if (m_CameraController.ViewTypes[index].FirstPersonPerspective)
                        m_CameraController.FirstPersonViewTypeFullName =
                            PropertyFromName("m_FirstPersonViewTypeFullName").stringValue =
                                m_CameraController.ViewTypeFullName;
                    else
                        m_CameraController.ThirdPersonViewTypeFullName =
                            PropertyFromName("m_ThirdPersonViewTypeFullName").stringValue =
                                m_CameraController.ViewTypeFullName;
        ...
        {
            ReorderableListSerializationHelper.AddObjectType(typeof(ViewType), true, m_CameraController.ViewTypes,
                AddViewType);
        }
        ...
        private void AddViewType(object obj)
        {
            ...
            var inspectorDrawer =
                InspectorDrawerUtility.InspectorDrawerForType(viewType.GetType()) as ViewTypeInspectorDrawer;
            if (inspectorDrawer != null) inspectorDrawer.ViewTypeAdded(viewType, target);
        }
        ...
        private void OnViewTypeListRemove(ReorderableList list)
        {
            ...
            var inspectorDrawer =
                InspectorDrawerUtility.InspectorDrawerForType(viewType.GetType()) as ViewTypeInspectorDrawer;
            if (inspectorDrawer != null) inspectorDrawer.ViewTypeRemoved(viewType, target);
            ...
            if (list.index == -1 && viewTypes.Count > 0) list.index = 0;
            if (removedSelected) m_CameraController.ViewTypeFullName = viewTypes[list.index].GetType().FullName;
            EditorPrefs.SetInt(SelectedViewTypeIndexKey, list.index);
        ...
        {
            var viewType = m_CameraController.ViewTypes[index];
            Utility.InspectorUtility.DrawObject(viewType, true, true, target, true, SerializeViewTypes);
            ...
            if (InspectorUtility.Foldout(viewType, new GUIContent("States"), false))
            {
                ...
                try
                {
                    ...
                    m_ReorderableViewTypeStateList = StateInspector.DrawStates(m_ReorderableViewTypeStateList,
                        serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                        GetSelectedViewTypeStateIndexKey(selectedViewType), OnViewTypeStateListDraw,
                        OnViewTypeStateListAdd, OnViewTypeStateListReorder,
        ...
        {
            return c_EditorPrefsSelectedViewTypeStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   viewType.GetType();
        }
        ...
        {
            if (m_ReorderableViewTypeStateList == null) return;

            if (m_CameraController.ViewTypes == null ||
                m_CameraController.ViewTypes.Length <= EditorPrefs.GetInt(SelectedViewTypeIndexKey))
            {
                m_ReorderableViewTypeList.index = -1;
        ...
        {
            var viewType = m_CameraController.ViewTypes[EditorPrefs.GetInt(SelectedViewTypeIndexKey)];
            var states = StateInspector.AddExistingPreset(viewType.GetType(), viewType.States,
                m_ReorderableViewTypeStateList, GetSelectedViewTypeStateIndexKey(viewType));
            if (viewType.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableViewTypeStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var viewType = m_CameraController.ViewTypes[EditorPrefs.GetInt(SelectedViewTypeIndexKey)];
            var states = StateInspector.CreatePreset(viewType, viewType.States, m_ReorderableViewTypeStateList,
                GetSelectedViewTypeStateIndexKey(viewType));
            if (viewType.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableViewTypeStateList.serializedProperty);
        ...
        private void OnViewTypeStateListReorder(ReorderableList list)
        {
            ...
            var copiedStates = new State[viewType.States.Length];
            Array.Copy(viewType.States, copiedStates, viewType.States.Length);
            for (var i = 0; i < viewType.States.Length; ++i)
            {
        ...
        private void OnViewTypeStateListRemove(ReorderableList list)
        {
            var viewType = m_CameraController.ViewTypes[EditorPrefs.GetInt(SelectedViewTypeIndexKey)];
            var states =
                StateInspector.OnStateListRemove(viewType.States, GetSelectedViewTypeStateIndexKey(viewType), list);
            if (viewType.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableViewTypeStateList.serializedProperty);
        ...
        private void UpdateDefaultViewTypes()
        {
            ...
            if (TypeUtility.GetType(m_CameraController.FirstPersonViewTypeFullName) == null)
            {
                m_CameraController.FirstPersonViewTypeFullName = string.Empty;
                EditorUtility.SetDirty(target);
            }

            if (TypeUtility.GetType(m_CameraController.ThirdPersonViewTypeFullName) == null)
            {
                m_CameraController.ThirdPersonViewTypeFullName = string.Empty;
                EditorUtility.SetDirty(target);
            }
            ...
            if (viewTypes != null)
                ...
                {
                    if (viewTypes[i] == null) continue;
                    // Transition view types are not limited to one perspective.
                    if (viewTypes[i] is Transition) continue;
                    if (viewTypes[i].FirstPersonPerspective)
                    {
                        ...
                        firstPersonViewTypeNames.Add(
                            Utility.InspectorUtility.DisplayTypeName(viewTypes[i].GetType(), false));
                    }
                    else
                    {
                        // Third Person.
