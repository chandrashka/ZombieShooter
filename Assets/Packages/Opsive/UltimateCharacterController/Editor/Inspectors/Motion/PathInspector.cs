...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Motion
{
    ...
    public class PathInspector : UnityEditor.Editor
    {
        private const string c_EditorPrefsSelectedIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Path.SelectedIndex";

        private const string c_EditorPrefsSelectedMiddleIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Path.SelectedMiddleIndex";

        private Path m_Path;
        private int m_SelectedIndex = -1;
        private int m_SelectedMiddleIndex = -1;

        private string EditorPrefsSelectedIndexKey => c_EditorPrefsSelectedIndexKey + "." + target.name;
        private string EditorPrefsSelectedMiddleIndexKey => c_EditorPrefsSelectedMiddleIndexKey + "." + target.name;
        ...
        {
            if (m_Path.ControlPoints == null || m_Path.ControlPoints.Length == 0) return;
            ...
            for (var i = 1; i < m_Path.ControlPoints.Length; i += 3)
            {
                var p1 = DrawControlPoint(i);
                var p2 = DrawControlPoint(i + 1);
                var p3 = DrawControlPoint(i + 2);
                Handles.color = Color.gray;
                if (i - 1 == m_SelectedMiddleIndex) Handles.DrawLine(p0, p1);
                if (i + 2 == m_SelectedMiddleIndex) Handles.DrawLine(p2, p3);
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                m_Path.ControlPoints[m_SelectedIndex] =
                    EditorGUILayout.Vector3Field("Position", m_Path.ControlPoints[m_SelectedIndex]);
            ...
            if (GUILayout.Button("Add Curve Segment"))
            {
                ...
                else
                {
                    ...
                    Array.Resize(ref controlPoints, controlPoints.Length + count);
                }
                ...
                if (controlPoints.Length > 4) AdjustControlPoint(controlPoints.Length - 3);
            ...
            if (EditorGUI.EndChangeCheck()) EditorUtility.SetDirty(m_Path);
        }
        ...
        private void AdjustControlPoint(int index)
        {
            ...
            if (index <= 1 || index >= m_Path.ControlPoints.Length - 2 || index % 3 == 0) return;
            ...
            if (index == m_SelectedIndex)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (index % 3 == 0)
                    {
                        ...
                        if (index > 0) m_Path.ControlPoints[index - 1] += delta;
                        if (index < m_Path.ControlPoints.Length - 1) m_Path.ControlPoints[index + 1] += delta;
                    }
