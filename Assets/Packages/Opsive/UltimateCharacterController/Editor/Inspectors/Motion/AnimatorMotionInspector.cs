...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Motion
{
    ...
    public class AnimatorMotionInspector : InspectorBase
    {
        ...
        {
            var path = EditorUtility.SaveFilePanel("Save Animator Motion", InspectorUtility.GetSaveFilePath(),
                "AnimatorMotion.asset", "asset");
            if (path.Length != 0 && Application.dataPath.Length < path.Length)
            {
                var animatorMotion = CreateInstance<AnimatorMotion>();
        ...
        public override void OnInspectorGUI()
        {
            ...
            var duration =
                EditorGUILayout.FloatField(new GUIContent("Duration", "The duration of all of the animation curves."),
                    m_Duration);
