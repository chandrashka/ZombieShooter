...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Utility
{
    ...
    {
        private static readonly Dictionary<string, FieldInfo> s_FieldNameMap = new();

        public static Dictionary<Type, string[]> s_TypeNameMap = new();
        public static Dictionary<Type, int[]> s_TypeIndexMap = new();
        public static Dictionary<Type, Type[]> s_TypeTypeMap = new();
        ...
        {
            var name = EditorUtility.SplitCamelCase(type.Name);
            // Show a friendly version of the full path.
            if (friendlyNamespacePrefix) name = UnityEngineUtility.GetDisplayName(type.FullName, name);
            return name;
        }
        ...
        public static void DrawAnimationEventTrigger(object obj, string name,
            SerializedProperty animationEventTriggerProperty)
        {
            if (Shared.Editor.Inspectors.Utility.InspectorUtility.Foldout(obj,
                    new GUIContent(name, GetFieldTooltip(obj, animationEventTriggerProperty.name))))
            {
                EditorGUI.indentLevel++;
                var waitForAnimationEventProperty =
                    animationEventTriggerProperty.FindPropertyRelative("m_WaitForAnimationEvent");
                ...
                if (!waitForAnimationEventProperty.boolValue)
                {
                    ...
                    EditorGUILayout.HelpBox(
                        $"The event will be triggered after {durationProperty.floatValue} seconds. Ensure no animation events exist which could trigger the event sooner.",
                        MessageType.Info);
                }
                else
                {
                    var slotEventProperty = animationEventTriggerProperty.FindPropertyRelative("m_WaitForSlotEvent");
                    if (slotEventProperty != null) EditorGUILayout.PropertyField(slotEventProperty);
                }
        ...
        public static string DrawTypePopup(Type type, string value, string label, bool allowNone)
        {
            ...
            {
                if (allowNone && index > 0) index--; // Remove the (none).
        ...
        /// </summary>
        public static void SynchronizePropertyCount(State[] states, SerializedProperty serializedProperty)
        {
            var serializedCount = serializedProperty.arraySize;
            if (serializedCount > states.Length)
                for (var i = serializedCount - 1; i >= states.Length; --i)
                    serializedProperty.DeleteArrayElementAtIndex(i);
            else if (serializedCount < states.Length)
                for (var i = serializedCount; i < states.Length; ++i)
                {
        ...
        ///     Draws the object.
        /// </summary>
        public static void DrawObject(object obj, bool drawHeader, bool friendlyNamespacePrefix, Object target,
            bool drawNoFieldsNotice, Action changeCallback)
        {
            if (obj == null) return;

            if (drawHeader)
                EditorGUILayout.LabelField(DisplayTypeName(obj.GetType(), friendlyNamespacePrefix),
        ...
        public static void DrawField(object obj, string name)
        {
            ...
            if (field != null)
                try
                {
                    ...
                    var value = ObjectInspector.DrawObject(
                        new GUIContent(EditorUtility.SplitCamelCase(name), GetFieldTooltip(field)), field.FieldType,
                        obj, prevValue, name, 0, null, null, field, true);
                    if (prevValue != value && GUI.changed) field.SetValue(obj, value);
                }
                catch (Exception /*e*/)
                {
                }
        ...
        public static void DrawFieldSlider(object obj, string name, float minValue, float maxValue)
        {
            ...
            var value = EditorGUILayout.Slider(
                new GUIContent(EditorUtility.SplitCamelCase(name), GetFieldTooltip(obj, name)), sliderValue, minValue,
                maxValue);
            if (sliderValue != value) SetFieldValue(obj, name, value);
        }
        ...
        public static void DrawSpring(object owner, string foldoutName, string fieldName)
        {
            ...
            if (Shared.Editor.Inspectors.Utility.InspectorUtility.Foldout(owner,
                    new GUIContent(foldoutName, GetFieldTooltip(GetField(owner, fieldName))), false, fieldName))
            {
                EditorGUI.indentLevel++;
        ...
        public static string DrawAttribute(AttributeManager attributeManager, string attributeName, string fieldName)
        {
            if (attributeManager != null)
            {
                ...
                for (var i = 0; i < attributeManager.Attributes.Length; ++i)
                {
                    attributeNames[i + 1] = attributeManager.Attributes[i].Name;
                    if (attributeName == attributeNames[i + 1]) attributeIndex = i + 1;
                }
                ...
                if (attributeIndex != selectedAttributeIndex)
                    attributeName = selectedAttributeIndex == 0
                        ? string.Empty
                        : attributeManager.Attributes[selectedAttributeIndex - 1].Name;
            }
            else
            {
        ...
        public static void DrawAttributeModifier(AttributeManager attributeManager, AttributeModifier attributeModifier,
            string fieldName)
        {
            ...
            if (!string.IsNullOrEmpty(attributeModifier.AttributeName))
            {
                ...
                {
                    attributeModifier.AutoUpdateStartDelay =
                        EditorGUILayout.FloatField("Start Delay", attributeModifier.AutoUpdateStartDelay);
                    attributeModifier.AutoUpdateInterval =
                        EditorGUILayout.FloatField("Interval", attributeModifier.AutoUpdateInterval);
                    attributeModifier.AutoUpdateDuration =
                        EditorGUILayout.FloatField("Duration", attributeModifier.AutoUpdateDuration);
                }
        ...
        public static string GetFieldTooltip(FieldInfo field)
        {
            ...
            if (attributes != null && attributes.Length > 0) return attributes[0].tooltip;
            return string.Empty;
        }
        ...
        {
            var field = GetField(obj, name);
            if (field != null) return GetFieldTooltip(field);
            return string.Empty;
        }
        ...
        /// <summary>
        ///     Unity will immediately select all the text in a textfield and you can't clear the selection. This workaround will
        ///     allow the text to be
        ...
        public static T DrawEditorWithoutSelectAll<T>(Func<T> guiCall)
        {
