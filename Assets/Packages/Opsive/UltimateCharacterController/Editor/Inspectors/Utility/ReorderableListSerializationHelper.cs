...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Utility
{
    ...
    {
        private static readonly Dictionary<Type, List<Type>> s_ObjectTypes = new();
        private static readonly Dictionary<Type, bool> s_CanAddMultipleTypes = new();
        ...
        /// </summary>
        public static void DrawReorderableList(ref ReorderableList reorderableList, InspectorBase inspector,
            Array drawnObject, string serializedData,
            ReorderableList.HeaderCallbackDelegate drawHeaderCallback,
            ...
            Action<int> drawSelectedElementCallback, string key, bool requireOne, bool indentList)
        {
            ...
            if (reorderableList == null)
            {
                ...
                reorderableList = new ReorderableList(inspector.serializedObject, data, reorderCallback != null, true,
                ...
                if (drawHeaderCallback != null) reorderableList.drawHeaderCallback = drawHeaderCallback;
                reorderableList.drawElementCallback = drawElementCallback;
                if (reorderCallback != null) reorderableList.onReorderCallback = reorderCallback;
                ...
                if (EditorPrefs.GetInt(key, -1) != -1) reorderableList.index = EditorPrefs.GetInt(key, -1);
            }
        ...
        /// </summary>
        public static void AddObjectType(Type type, bool friendlyNamespacePrefix, Array existingTypes,
            GenericMenu.MenuFunction2 addCallback)
        {
            if (!s_ObjectTypes.TryGetValue(type, out var typeList))
            {
                ...
                for (var i = 0; i < assemblies.Length; ++i)
                {
                    ...
                    for (var j = 0; j < assemblyTypes.Length; ++j)
                    {
                        ...
                        if (!type.IsAssignableFrom(assemblyTypes[j])) continue;

                        // Ignore abstract classes.
                        if (assemblyTypes[j].IsAbstract) continue;

                        // Ability types should not show ItemAbilities.
                        if (type == typeof(Ability) && typeof(ItemAbility).IsAssignableFrom(assemblyTypes[j])) continue;
            ...
            for (var i = 0; i < typeList.Count; ++i)
            {
                ...
                var group = TypeUtility.GetAttribute(typeList[i], typeof(Group)) as Group;
                addMenu.AddItem(
                    new GUIContent((group != null ? group.Name + "/" : string.Empty) +
                                   InspectorUtility.DisplayTypeName(typeList[i], friendlyNamespacePrefix)), false,
                    addCallback, typeList[i]);
            }
        ...
        {
            if (s_CanAddMultipleTypes.TryGetValue(type, out var multipleTypes)) return multipleTypes;
