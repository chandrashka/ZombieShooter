...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.StateSystem
{
    ...
    {
        private const string c_EditorPrefsSelectedProfileKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedProfile";
        ...
        private string SelectedProfileKey =>
            c_EditorPrefsSelectedProfileKey + "." + target.GetType() + "." + target.name;
        ...
        {
            var path = EditorUtility.SaveFilePanel("Save State Configuration", InspectorUtility.GetSaveFilePath(),
                "StateConfiguration.asset", "asset");
            if (path.Length != 0 && Application.dataPath.Length < path.Length)
            {
                var stateConfiguration = CreateInstance<StateConfiguration>();
        ...
        public override void OnInspectorGUI()
        {
            ...
            m_CopyFromObject =
                EditorGUILayout.ObjectField("Copy From", m_CopyFromObject, typeof(GameObject), true) as GameObject;
            ...
            if (m_StateConfiguration.Profiles != null)
                for (var i = 0; i < m_StateConfiguration.Profiles.Length; ++i)
                    stateNames.Add(m_StateConfiguration.Profiles[i].Name);
            var prevSelectedProfile = EditorPrefs.GetInt(SelectedProfileKey, 0);
            var selectedProfileIndex =
                EditorGUILayout.Popup("Selected Profile", prevSelectedProfile, stateNames.ToArray());
            ...
            if (GUILayout.Button(InspectorStyles.RemoveIcon, InspectorStyles.NoPaddingButtonStyle, GUILayout.Width(18)))
            {
                ...
                for (var j = 0; j < removeProfile.StateElements.Length; ++j)
            ...
            if (GUILayout.Button(InspectorStyles.DuplicateIcon, InspectorStyles.NoPaddingButtonStyle,
                    GUILayout.Width(20)))
            {
                ...
                profile.StateElements =
                    new StateConfiguration.Profile.StateElement[duplicateProfile.StateElements.Length];
                for (var i = 0; i < profile.StateElements.Length; ++i)
                {
                    var preset = Instantiate(duplicateProfile.StateElements[i].Preset);
                    preset.name = duplicateProfile.StateElements[i].Preset.name;
                    AssetDatabase.AddObjectToAsset(preset, m_StateConfiguration);
                    profile.StateElements[i] = new StateConfiguration.Profile.StateElement(
                        duplicateProfile.StateElements[i].Name, preset,
            ...
            var selectedProfile =
                m_StateConfiguration.Profiles != null && selectedProfileIndex < m_StateConfiguration.Profiles.Length
                    ? m_StateConfiguration.Profiles[selectedProfileIndex]
                    : null;
            GUI.enabled = selectedProfile != null;
            GUILayout.EndHorizontal();
            var selectedProfileName =
                EditorGUILayout.TextField("Name", selectedProfile != null ? selectedProfile.Name : string.Empty);
            if (selectedProfile != null)
                if (IsUniqueProfileName(selectedProfileName))
                    selectedProfile.Name = selectedProfileName;
            var profileType = (StateConfiguration.Profile.ProfileType)EditorGUILayout.EnumPopup("Profile Type",
                selectedProfile != null ? selectedProfile.Type : StateConfiguration.Profile.ProfileType.Character);
            if (selectedProfile != null) selectedProfile.Type = profileType;
            if (m_StateReorderableList == null)
            {
                var states = selectedProfile != null ? selectedProfile.StateElements : null;
                if (states == null) states = new StateConfiguration.Profile.StateElement[0];
                m_StateReorderableList = new ReorderableList(states, typeof(StateConfiguration.Profile.StateElement),
                    true, true,
                    m_StateConfiguration.Profiles != null && m_StateConfiguration.Profiles.Length > 0,
                    states.Length > 0);
        ...
        private void CopyFromGameObject(GameObject gameObject)
        {
            ...
            var items = gameObject.GetComponentsInChildren<Item>();
            for (var i = 0; i < items.Length; ++i)
            {
                // Ignore itself - the states have already been added.
                if (items[i].gameObject == gameObject) continue;

                stateBehaviors = items[i].GetComponents<StateBehavior>();
                stateList.Clear();
                for (var j = 0; j < stateBehaviors.Length; ++j) AddStatesFromObject(stateBehaviors[j], stateList);
        ...
        private void AddStatesFromObject(object obj, List<State> stateList)
        {
            ...
            if (m_CopyType == CopyType.All || m_CopyType == CopyType.Default)
            {
                ...
                AssetDatabase.AddObjectToAsset(preset, m_StateConfiguration);
                stateList.Add(new State("Default" + preset.name, preset, null));
            }
        }
        ...
        private void AddStatesToProfile(GameObject gameObject, List<State> stateList)
        {
            ...
            if (m_StateConfiguration.Profiles != null)
                for (var i = 0; i < m_StateConfiguration.Profiles.Length; ++i)
                    if (m_StateConfiguration.Profiles[i].Name == gameObject.name)
                    {
                        ...
                        for (var j = 0; j < profile.StateElements.Length; ++j)
            ...
            for (var i = 0; i < stateList.Count; ++i)
            {
                ...
                stateElements[i] = new StateConfiguration.Profile.StateElement(stateList[i].Name,
                    stateList[i].Preset as PersistablePreset, stateList[i].BlockList, withinConfiguration);
            }
        ...
        private void OnStateListHeaderDraw(Rect rect)
        {
            ...
            var nameWidth = Mathf.Max(0, rect.width - presetWidth - blockedByWidth - c_WidthBuffer * 4) +
                            EditorGUI.indentLevel * InspectorUtility.IndentWidth * 3;
            ...
            EditorGUI.LabelField(new Rect(startRectX, rect.y + 1, presetWidth, EditorGUIUtility.singleLineHeight),
                "Preset");
            startRectX += presetWidth - c_WidthBuffer * 3 - EditorGUI.indentLevel * InspectorUtility.IndentWidth;
            EditorGUI.LabelField(
                new Rect(startRectX, rect.y + 1, blockedByWidth + EditorGUI.indentLevel * InspectorUtility.IndentWidth,
        ...
        public void OnStateListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (index >= profile.StateElements.Length) return;
            ...
            var desiredName =
                EditorGUI.TextField(new Rect(startRectX, rect.y + 1, nameWidth, EditorGUIUtility.singleLineHeight),
                    stateElement.Name);
            ...
            var preset = EditorGUI.ObjectField(new Rect(startRectX, rect.y + 1, presetWidth,
                    EditorGUIUtility.singleLineHeight), string.Empty, stateElement.Preset, typeof(PersistablePreset),
            ...
            if (stateElement.Preset != null)
            {
                ...
                for (var i = 0; i < profile.StateElements.Length; ++i)
                {
                    ...
                    if (currentState.Preset == null || objType != currentState.Preset.Data.ObjectType) continue;

                    string currentName;
                    // The current state cannot block itself.
                    if ((currentName = currentState.Name) == stateName) continue;
        ...
        /// </summary>
        private bool IsUniqueStateName(StateConfiguration.Profile.StateElement[] states,
            StateConfiguration.Profile.StateElement state, string stateName)
        {
            // A preset is required in order to match the object type.
            if (state.Preset == null) return true;
            ...
            for (var i = 0; i < states.Length; ++i)
            {
                ...
                if (states[i].Preset == null || objType != states[i].Preset.Data.ObjectType) continue;
                if (states[i] == state) continue;
                if (states[i].Name == stateName) return false;
            }
        ...
        public void OnStateListRemove(ReorderableList list)
        {
            ...
            if (stateList[list.index].Default) DestroyImmediate(stateList[list.index].Preset, true);
        ...
        {
            All, // Copies both the default and non-default states.
