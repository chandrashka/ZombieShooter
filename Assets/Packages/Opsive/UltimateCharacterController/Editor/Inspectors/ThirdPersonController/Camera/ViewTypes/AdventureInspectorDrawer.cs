    ...
    public class AdventureInspectorDrawer : ThirdPersonInspectorDrawer
    {
        ...
        protected override void OnDrawLimits(object target)
        {
            ...
            InspectorUtility.MinMaxSlider(ref minValue, ref maxValue, -180, 180,
                new GUIContent("Yaw Limit", "The min and max limit of the yaw angle (in degrees)."));
            if (minValue != minYawLimit) InspectorUtility.SetFieldValue(target, "m_MinYawLimit", minValue);
            if (minValue != maxYawLimit) InspectorUtility.SetFieldValue(target, "m_MaxYawLimit", maxValue);
