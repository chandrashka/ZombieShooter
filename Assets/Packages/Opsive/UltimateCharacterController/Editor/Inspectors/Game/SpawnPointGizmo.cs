...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Game
{
    ...
    public class SpawnPointGizmo
    {
        ...
        private static void DrawSpawnPointGizmo(SpawnPoint spawnPoint, GizmoType gizmoType)
        {
            ...
            {
                if (s_ObstructionColliders == null) s_ObstructionColliders = new Collider[1];
                ...
                var overlapCount = Physics.OverlapBoxNonAlloc(boxPosition, extents, s_ObstructionColliders,
                    spawnTransform.rotation, spawnPoint.ObstructionLayers, QueryTriggerInteraction.Ignore);
                if (overlapCount != 0)
                    Handles.Label(spawnPoint.transform.position,
                        "Obstruction: " + s_ObstructionColliders[0].transform.gameObject.name);
            }
            ...
            if (MathUtility.InLayerMask((int)GizmoType.Selected, (int)gizmoType))
            {
                ...
                if (spawnPoint.RandomDirection)
                {
                    ...
                    Gizmos.DrawLine(2 * rad * Vector3.forward,
                        1.5f * rad * Vector3.forward + 0.5f * rad * Vector3.left);
                    Gizmos.DrawLine(Vector3.forward * 2 * rad,
                        1.5f * rad * Vector3.forward + 0.5f * rad * Vector3.right);
