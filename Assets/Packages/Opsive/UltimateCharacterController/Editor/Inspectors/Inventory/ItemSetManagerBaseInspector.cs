...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Inventory
{
    ...
    public abstract class ItemSetManagerBaseInspector : InspectorBase
    {
        ...
        private const string c_EditorPrefsSelectedItemSetIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.ItemSets.SelectedItemSetIndex";

        private const string c_EditorPrefsSelectedItemSetStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.ItemSets.SelectedItemSetStateIndex";
        ...
        private readonly Dictionary<ReorderableList, int> m_ReorderableListCategoryMap = new();

        private string SelectedItemSetIndexKey => c_EditorPrefsSelectedItemSetIndexKey + "." + target.GetType() + "." +
                                                  target.name + "." + m_ItemSetListIndex;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (InitializeCategories())
            {
                ...
                for (var i = 0; i < m_ItemSetReorderableList.Length; ++i)
                {
                    ...
                    {
                        m_ItemSetReorderableList[i] = new ReorderableList(
                            m_ItemSetManager.CategoryItemSets[i].ItemSetList, typeof(ItemSet), true, true, true, true);
                        m_ItemSetReorderableList[i].drawHeaderCallback = rect =>
                        {
                            GUI.Label(rect,
                                m_ItemSetManager.CategoryItemSets[m_ItemSetListIndex].CategoryName +
                                " (select row to edit)");
                        };
                        m_ItemSetReorderableList[i].drawElementCallback = OnItemSetElementDraw;
                        m_ItemSetReorderableList[i].onSelectCallback = list =>
                        {
                    ...
                    if (m_ItemSetManager.CategoryItemSets[i].ItemSetList.Count > 0)
                        m_ItemSetReorderableList[i].elementHeight =
                            (c_SlotRowHeight + c_SlotRowPadding) * m_InventoryBase.SlotCount + 25;
                    else
                    ...
                    if (m_ItemSetReorderableList[i].index != -1 && m_ItemSetReorderableList[i].index <
                        m_ItemSetManager.CategoryItemSets[i].ItemSetList.Count)
                    {
                        GUI.enabled = !Application.isPlaying;
                        DrawSelectedItemSet(
                            m_ItemSetManager.CategoryItemSets[i].ItemSetList[m_ItemSetReorderableList[i].index],
                            m_ItemSetReorderableList[i].index);
                        GUI.enabled = true;
                    }
                }
            }
        ...
        protected void CheckCategories(IItemCategoryIdentifier[] categories)
        {
            ...
            if (categoryItemSets == null) return;

            for (var i = categoryItemSets.Length - 1; i >= 0; --i)
            {
            ...
            if (diff > 0)
            {
                Array.Resize(ref categoryItemSets, categoryItemSets.Length + diff);
                for (var i = categoryItemSets.Length - diff; i < categoryItemSets.Length; ++i)
                    // A category has been added. Populate the new category.
                for (var j = 0; j < categories.Length; ++j)
                {
                    if (!IsItemSetCategory(categories[j])) continue;
                    var hasCategory = false;
                    for (var k = 0; k < categoryItemSets.Length; ++k)
                    ...
                    categoryItemSets[i] =
                        new CategoryItemSet(categories[j].ID, categories[j].ToString(), categories[j]);
                    break;
                }
        ...
        private void OnItemSetElementDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (itemSet[index].Slots == null || itemSet[index].Slots.Length != m_InventoryBase.SlotCount)
            {
                ...
                Array.Resize(ref slots, m_InventoryBase.SlotCount);
            ...
            var itemSetActive = Application.isPlaying && !UnityEditor.EditorUtility.IsPersistent(m_ItemSetManager) &&
                                m_ItemSetManager.gameObject.activeSelf &&
                                m_ItemSetManager.ActiveItemSetIndex != null &&
                                m_ItemSetListIndex < m_ItemSetManager.ActiveItemSetIndex.Length &&
                                m_ItemSetManager.ActiveItemSetIndex[m_ItemSetListIndex] == index;
            GUI.Label(elementTitleRect, "Item Set " + index + (itemSetActive ? " (Active)" : ""),
                InspectorStyles.CenterBoldLabel);
            ...
            for (var i = 0; i < m_InventoryBase.SlotCount; ++i)
            {
                ...
                itemSet[index].Slots[i] =
                    EditorGUI.ObjectField(objRect, itemSet[index].Slots[i], typeof(ItemDefinitionBase), false) as
                        ItemDefinitionBase;
            }
        }
        ...
        private void OnItemSetListRemove(ReorderableList reorderableList)
        {
            ...
            if (reorderableList.index == -1 &&
                m_ItemSetManager.CategoryItemSets[m_ItemSetListIndex].ItemSetList.Count > 0) reorderableList.index = 0;
            EditorPrefs.SetInt(SelectedItemSetIndexKey, reorderableList.index);
            // The item set's state list should start out fresh so a reference doesn't have to be cached for each item set.
            m_ReorderableItemSetStateList[m_ItemSetListIndex] = null;
        }
        ...
        private void DrawSelectedItemSet(ItemSet itemSet, int index)
        {
            ...
            itemSet.State = EditorGUILayout.TextField(
                new GUIContent("State",
                    "Optionally specify a state that the character should switch to when the Item Set is active."),
                itemSet.State);

            // Draws all of the slots ItemDefinitions.
            for (var i = 0; i < m_InventoryBase.SlotCount; ++i)
            {
                var itemDefinition = (ItemDefinitionBase)EditorGUILayout.ObjectField("Slot " + i, itemSet.Slots[i],
                    typeof(ItemDefinitionBase), false);
                ...
                if (itemDefinition != null && m_ItemSetManager.IsCategoryMember(itemDefinition, m_ItemSetListIndex))
                {
                    ...
                    if (Application.isPlaying)
                    {
                        ...
                        EditorGUILayout.LabelField("Item Identifier",
                            itemSet.ItemIdentifiers[i] == null ? "(none)" : itemSet.ItemIdentifiers[i].ToString());
                        EditorGUI.indentLevel--;
                    }
                }
                else
                {
                    ...
                    if (itemDefinition != null)
                        Debug.LogError(
                            $"Error: Unable to add ItemDefinition {itemDefinition.name}. The ItemDefinition category doesn't match the parent category.");
                }
            }

            var isDefaultIndex = m_ItemSetManager.CategoryItemSets[m_ItemSetListIndex].DefaultItemSetIndex == index;
            var isDefaultIndexToggle =
                EditorGUILayout.Toggle(new GUIContent("Default", "True if the Item Set is the default Item Set."),
                    isDefaultIndex);
            if (isDefaultIndex != isDefaultIndexToggle)
                m_ItemSetManager.CategoryItemSets[m_ItemSetListIndex].DefaultItemSetIndex =
                    isDefaultIndexToggle ? index : -1;

            itemSet.Enabled = EditorGUILayout.Toggle(new GUIContent("Enabled", "True if the Item Set can be equipped."),
                itemSet.Enabled);
            itemSet.CanSwitchTo =
                EditorGUILayout.Toggle(
                    new GUIContent("Can Switch To",
                        "True if the ItemSet can be switched to by the EquipNext/EquipPrevious abilities."),
                    itemSet.CanSwitchTo);
            itemSet.DisabledIndex =
                EditorGUILayout.IntField(
                    new GUIContent("Disabled Index",
                        "The ItemSet that should be activated if the current ItemSet is disabled."),
                    itemSet.DisabledIndex);
            ...
            if (Shared.Editor.Inspectors.Utility.InspectorUtility.Foldout(itemSet, new GUIContent("States"), false))
            {
                ...
                for (var i = 0; i < stateIndexHelper.StateIndexData.Length; ++i) stateIndexHelper.StateIndexData[i] = i;
                var stateIndexSerializedObject = new SerializedObject(stateIndexHelper);
                m_ReorderableItemSetStateList[m_ItemSetListIndex] = StateInspector.DrawStates(
                    m_ReorderableItemSetStateList[m_ItemSetListIndex], serializedObject,
                ...
                if (!m_ReorderableListCategoryMap.ContainsKey(m_ReorderableItemSetStateList[m_ItemSetListIndex]))
                    m_ReorderableListCategoryMap.Add(m_ReorderableItemSetStateList[m_ItemSetListIndex],
                        m_ItemSetListIndex);
        ...
        {
            return c_EditorPrefsSelectedItemSetStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   m_ItemSetListIndex + " " + itemSetIndex;
        }
        ...
        private void OnItemSetStateListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (m_ItemSetListIndex >= m_ItemSetManager.CategoryItemSets.Length ||
                ...
                EditorPrefs.GetInt(SelectedItemSetIndexKey, -1) >=
                m_ItemSetManager.CategoryItemSets[m_ItemSetListIndex].ItemSetList.Count)
            {
                m_ReorderableItemSetStateList[m_ItemSetListIndex].index = -1;
                return;
            }

            var itemSet = m_ItemSetManager.CategoryItemSets[m_ItemSetListIndex]
                .ItemSetList[EditorPrefs.GetInt(SelectedItemSetIndexKey, -1)];
        ...
        private void AddExistingItemSetPreset()
        {
            ...
            var itemSet = m_ItemSetManager.CategoryItemSets[m_ItemSetListIndex]
                .ItemSetList[EditorPrefs.GetInt(SelectedItemSetIndexKey, -1)];
            var states = StateInspector.AddExistingPreset(itemSet.GetType(), itemSet.States,
                m_ReorderableItemSetStateList[m_ItemSetListIndex],
            ...
            if (itemSet.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableItemSetStateList[m_ItemSetListIndex].serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        private void CreateItemSetPreset()
        {
            ...
            var itemSet = m_ItemSetManager.CategoryItemSets[m_ItemSetListIndex]
                .ItemSetList[EditorPrefs.GetInt(SelectedItemSetIndexKey, -1)];
            var states = StateInspector.CreatePreset(itemSet, itemSet.States,
                m_ReorderableItemSetStateList[m_ItemSetListIndex],
                GetSelectedItemSetStateIndexKey(EditorPrefs.GetInt(SelectedItemSetIndexKey, -1)));
            if (itemSet.States.Length != states.Length)
            {
        ...
        private void OnItemSetStateListReorder(ReorderableList list)
        {
            ...
            var copiedStates = new State[itemSet.States.Length];
            Array.Copy(itemSet.States, copiedStates, itemSet.States.Length);
            for (var i = 0; i < itemSet.States.Length; ++i)
            {
        ...
        private void OnItemSetStateListRemove(ReorderableList list)
        {
            ...
            var itemSet = m_ItemSetManager.CategoryItemSets[m_ItemSetListIndex]
                .ItemSetList[EditorPrefs.GetInt(SelectedItemSetIndexKey, -1)];
            var states = StateInspector.OnStateListRemove(itemSet.States,
                GetSelectedItemSetStateIndexKey(EditorPrefs.GetInt(SelectedItemSetIndexKey, -1)), list);
            if (itemSet.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableItemSetStateList[m_ItemSetListIndex].serializedProperty);
