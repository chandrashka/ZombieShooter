...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Character.Abilities
{
    ...
    public class RestrictPositionInspectorDrawer : AbilityInspectorDrawer
    {
        ...
        {
            var restriction = (RestrictPosition.RestrictionType)EditorGUILayout.EnumPopup(
                new GUIContent("Restriction", InspectorUtility.GetFieldTooltip(target, "m_Restriction")),
            ...
            if (restriction != RestrictPosition.RestrictionType.RestrictZ)
            {
                ...
                minValue = EditorGUILayout.FloatField(
                    new GUIContent("Min X Position", InspectorUtility.GetFieldTooltip(target, "m_MinXPosition")),
                    minValue);
                if (minValue > maxValue) maxValue = minValue;
                maxValue = EditorGUILayout.FloatField(
                    new GUIContent("Max X Position", InspectorUtility.GetFieldTooltip(target, "m_MaxXPosition")),
                    maxValue);
            ...
            if (restriction != RestrictPosition.RestrictionType.RestrictX)
            {
                ...
                minValue = EditorGUILayout.FloatField(
                    new GUIContent("Min Z Position", InspectorUtility.GetFieldTooltip(target, "m_MinZPosition")),
                    minValue);
                if (minValue > maxValue) maxValue = minValue;
                maxValue = EditorGUILayout.FloatField(
                    new GUIContent("Max Z Position", InspectorUtility.GetFieldTooltip(target, "m_MaxZPosition")),
                    maxValue);
                if (maxValue < minValue) minValue = maxValue;
