    ...
    public class ComboTimeoutInspectorDrawer : InspectorDrawer
    {
        ...
        public override void OnInspectorGUI(object target, Object parent)
        {
            ...
            for (var i = 0; i < elements.Length; ++i)
            {
                ...
                GUILayout.Space(-30);
                elements[i].AxisInput = EditorGUILayout.Popup(elements[i].AxisInput ? 1 : 0,
                    new[] { "Button Down", "Axis" }, GUILayout.MaxWidth(180)) == 1;
                GUILayout.Space(-30);
                // The first element does not use the timeout value.
                GUI.enabled = i > 0;
                elements[i].Timeout = EditorGUILayout.FloatField(elements[i].Timeout,
                    GUILayout.Width(140 - (i == elements.Length - 1 ? 44 : 0)));
                GUI.enabled = true;
                ...
                if (i == elements.Length - 1)
                {
                    if (i > 0 && GUILayout.Button(InspectorStyles.RemoveIcon, InspectorStyles.NoPaddingButtonStyle,
                            GUILayout.Width(18))) Array.Resize(ref elements, elements.Length - 1);
