...
using InspectorUtility = Opsive.Shared.Editor.Inspectors.Utility.InspectorUtility;
...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Character
{
    ...
    {
        private const string c_EditorPrefsSelectedMovementTypeIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedMovementTypeIndex";

        private const string c_EditorPrefsSelectedMovementTypeStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.MovementTypes.SelectedMovementTypeStateIndex";

        private const string c_EditorPrefsSelectedAbilityIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedAbilityIndex";

        private const string c_EditorPrefsSelectedAbilityStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Ability.SelectedAbilityStateIndex";

        private const string c_EditorPrefsSelectedItemAbilityIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedItemAbilityIndex";

        private const string c_EditorPrefsSelectedItemAbilityStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Ability.SelectedItemAbilityStateIndex";

        private const string c_EditorPrefsSelectedEffectIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedEffectIndex";

        private const string c_EditorPrefsSelectedEffectStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Effect.SelectedAbilityStateIndex";

        private const string c_EditorPrefsLastLastAnimatorCodePathKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Ability.LastAnimatorCodePath";
        ...
        private string SelectedMovementTypeIndexKey =>
            c_EditorPrefsSelectedMovementTypeIndexKey + "." + target.GetType() + "." + target.name;

        private string SelectedAbilityIndexKey =>
            c_EditorPrefsSelectedAbilityIndexKey + "." + target.GetType() + "." + target.name;

        private string SelectedItemAbilityIndexKey =>
            c_EditorPrefsSelectedItemAbilityIndexKey + "." + target.GetType() + "." + target.name;

        private string SelectedEffectIndexKey =>
            c_EditorPrefsSelectedEffectIndexKey + "." + target.GetType() + "." + target.name;
        ...
        protected override void OnEnable()
        {
            ...
            if (Application.isPlaying)
                Debug.LogWarning(
                    "Warning: Selecting the character in the inspector will decrease your framerate. For best performance deselect the character GameObject " +
            ...
            baseCallback += () =>
            {
                if (Application.isPlaying)
                    EditorGUILayout.HelpBox(
                        "Having this inspector open may decrease your framerate. For best performance the character GameObject " +
                ...
                {
                    var selectedIndex = 0;
                    for (var i = 0; i < m_FirstPersonMovementTypes.Length; ++i)
                        if (m_FirstPersonMovementTypes[i].FullName ==
                            m_CharacterLocomotion.FirstPersonMovementTypeFullName)
                        {
                            selectedIndex = i;
                            break;
                        }

                    var index = EditorGUILayout.Popup("First Person Movement Type", selectedIndex,
                        m_FirstPersonMovementTypeNames);
                    if (index != selectedIndex)
                    {
                        m_CharacterLocomotion.FirstPersonMovementTypeFullName =
                            m_FirstPersonMovementTypes[index].FullName;
                        ...
                        if (Application.isPlaying && m_CharacterLocomotion.ActiveMovementType.GetType().FullName
                                .Contains("FirstPerson"))
                            m_CharacterLocomotion.MovementTypeFullName =
                                m_CharacterLocomotion.FirstPersonMovementTypeFullName;
                    }
                }
                ...
                {
                    var selectedIndex = 0;
                    for (var i = 0; i < m_ThirdPersonMovementTypes.Length; ++i)
                        if (m_ThirdPersonMovementTypes[i].FullName ==
                            m_CharacterLocomotion.ThirdPersonMovementTypeFullName)
                        {
                            selectedIndex = i;
                            break;
                        }

                    var index = EditorGUILayout.Popup("Third Person Movement Type", selectedIndex,
                        m_ThirdPersonMovementTypeNames);
                    if (index != selectedIndex)
                    {
                        m_CharacterLocomotion.ThirdPersonMovementTypeFullName =
                            m_ThirdPersonMovementTypes[index].FullName;
                        ...
                        if (!Application.isPlaying && (m_CharacterLocomotion.ActiveMovementType == null ||
                                                       m_CharacterLocomotion.ActiveMovementType.GetType().FullName
                                                           .Contains("ThirdPerson")))
                            m_CharacterLocomotion.MovementTypeFullName =
                                m_CharacterLocomotion.ThirdPersonMovementTypeFullName;
                    }
                }
                ...
                ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableMovementTypeList, this,
                    m_CharacterLocomotion.MovementTypes, "m_MovementTypeData",
                    OnMovementTypeListDrawHeader, OnMovementTypeListDraw, OnMovementTypeListReorder,
                    OnMovementTypeListAdd,
                ...
                if (Foldout("Motor"))
                {
                    ...
                    if (updateLocation.enumValueIndex == (int)KinematicObjectManager.UpdateLocation.Update)
                        EditorGUILayout.HelpBox(
                            "It is recommended that the framerate is limited using Application.targetFrameRate.",
                            MessageType.Info);
                    ...
                    if (Application.isPlaying)
                    {
                        ...
                        var timeScaleValue = EditorGUILayout.Slider(
                            new GUIContent(EditorUtility.SplitCamelCase(timeScaleProperty.name),
                                timeScaleProperty.tooltip), timeScaleProperty.floatValue, 0, 4);
                        if (timeScaleValue != prevTimeScaleValue) m_CharacterLocomotion.TimeScale = timeScaleValue;
                    }
                    else
                    {
                ...
                if (Foldout("Physics"))
                {
                    ...
                    if (stepHeight.floatValue > maxColliderRadius)
                        EditorGUILayout.HelpBox(
                            "Warning: The Max Step Height is greater than the max collider radius (" +
                            maxColliderRadius + "). The step height should " +
                            "be decreased or the collider radius should be increased in order for the step height to work correctly.",
                            MessageType.Warning);
                    ...
                    if (useGravity.boolValue)
                    {
                        ...
                        if (gravityDirection != PropertyFromName("m_GravityDirection").vector3Value)
                            m_CharacterLocomotion.GravityDirection =
                                PropertyFromName("m_GravityDirection").vector3Value;
                ...
                if (Foldout("Abilities"))
                {
                    EditorGUI.indentLevel++;
                    EditorGUILayout.BeginVertical("Box");
                    ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableAbilityList, this,
                        m_CharacterLocomotion.Abilities, "m_AbilityData",
                        OnAbilityListDrawHeader, OnAbilityListDraw, OnAbilityListReorder, OnAbilityListAdd,
                        OnAbilityListRemove, OnAbilityListSelect,
                ...
                if (Foldout("Item Abilities"))
                {
                    EditorGUI.indentLevel++;
                    EditorGUILayout.BeginVertical("Box");
                    ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableItemAbilityList, this,
                        m_CharacterLocomotion.ItemAbilities, "m_ItemAbilityData",
                        OnItemAbilityListDrawHeader, OnItemAbilityListDraw, OnItemAbilityListReorder,
                        OnItemAbilityListAdd, OnItemAbilityListRemove,
                ...
                if (Foldout("Effects"))
                {
                    EditorGUI.indentLevel++;
                    EditorGUILayout.BeginVertical("Box");
                    ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableEffectList, this,
                        m_CharacterLocomotion.Effects, "m_EffectData", OnEffectListDrawHeader,
                        OnEffectListDraw, OnEffectListReorder, OnEffectListAdd, OnEffectListRemove, OnEffectListSelect,
                        DrawSelectedEffect,
        ...
        private void OnMovementTypeListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var label = Utility.InspectorUtility.DisplayTypeName(movementType.GetType(), true);
            ...
            activeRect.x += activeRect.width - 32;
            activeRect.width = 20;
            EditorGUI.Toggle(activeRect, m_CharacterLocomotion.MovementTypeFullName == movementType.GetType().FullName,
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (!Application.isPlaying)
                {
                    var camera = CameraUtility.FindCamera(m_CharacterLocomotion.gameObject);
                    if (camera != null)
                    {
                        var prevFirstPerson =
                            m_CharacterLocomotion.MovementTypeFullName.Contains("FirstPersonController");
                        ...
                        if (prevFirstPerson != firstPerson)
                        {
                            ...
                            var viewType = firstPerson
                                ? cameraController.FirstPersonViewTypeFullName
                                : cameraController.ThirdPersonViewTypeFullName;
                            if (!string.IsNullOrEmpty(viewType))
                            {
                                cameraController.SetViewType(TypeUtility.GetType(viewType), true);
                                var cameraSerializedObject = new SerializedObject(cameraController);
                                PropertyFromName(cameraSerializedObject, "m_ViewTypeFullName").stringValue =
                                    TypeUtility.GetType(viewType).FullName;
                                if (firstPerson)
                                    PropertyFromName(cameraSerializedObject, "m_FirstPersonViewTypeFullName")
                                        .stringValue = TypeUtility.GetType(viewType).FullName;
                                else
                                    PropertyFromName(cameraSerializedObject, "m_ThirdPersonViewTypeFullName")
                                        .stringValue = TypeUtility.GetType(viewType).FullName;
                ...
                m_CharacterLocomotion.MovementTypeFullName = PropertyFromName("m_MovementTypeFullName").stringValue =
                    movementType.GetType().FullName;
                if (movementType.GetType().FullName.Contains("FirstPerson"))
                    m_CharacterLocomotion.FirstPersonMovementTypeFullName =
                        PropertyFromName("m_FirstPersonMovementTypeFullName").stringValue =
                            m_CharacterLocomotion.MovementTypeFullName;
                else
                    m_CharacterLocomotion.ThirdPersonMovementTypeFullName =
                        PropertyFromName("m_ThirdPersonMovementTypeFullName").stringValue =
                            m_CharacterLocomotion.MovementTypeFullName;
        ...
        private void OnAbilityListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var label = Utility.InspectorUtility.DisplayTypeName(ability.GetType(), true);
            ...
            if (!string.IsNullOrEmpty(description)) label = string.Format("{0} ({1})", label, description);
            if (ability.IsActive) label += " (Active)";
        ...
        private void OnItemAbilityListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var label = Utility.InspectorUtility.DisplayTypeName(itemAbility.GetType(), true);
            ...
            if (!string.IsNullOrEmpty(description)) label = string.Format("{0} ({1})", label, description);
            if (itemAbility.IsActive) label += " (Active)";
        ...
        private void OnAbilityListReorder(ReorderableList list)
        {
            ...
            for (var i = 0; i < m_CharacterLocomotion.Abilities.Length; ++i)
        ...
        private void OnItemAbilityListReorder(ReorderableList list)
        {
            ...
            // Update the ability index.
            for (var i = 0; i < m_CharacterLocomotion.ItemAbilities.Length; ++i)
        ...
        {
            ReorderableListSerializationHelper.AddObjectType(typeof(MovementType), true,
                m_CharacterLocomotion.MovementTypes, AddMovementType);
        }
        ...
        {
            ReorderableListSerializationHelper.AddObjectType(typeof(Ability), true, m_CharacterLocomotion.Abilities,
                AddAbility);
        }
        ...
        {
            ReorderableListSerializationHelper.AddObjectType(typeof(ItemAbility), true,
                m_CharacterLocomotion.ItemAbilities, AddItemAbility);
        }
        ...
        {
            ReorderableListSerializationHelper.AddObjectType(typeof(Effect), true, m_CharacterLocomotion.Effects,
                AddEffect);
        }
        ...
        private void AddAbility(object obj)
        {
            ...
            var inspectorDrawer =
                InspectorDrawerUtility.InspectorDrawerForType(ability.GetType()) as AbilityInspectorDrawer;
            if (inspectorDrawer != null) inspectorDrawer.AbilityAdded(ability, target);
        }
        ...
        private void OnMovementTypeListRemove(ReorderableList list)
        {
            ...
            var removedSelected = movementTypes[list.index].GetType().FullName ==
                                  m_CharacterLocomotion.MovementTypeFullName;
            ...
            if (list.index == -1 && movementTypes.Count > 0) list.index = 0;
            if (removedSelected)
        ...
        private void OnAbilityListRemove(ReorderableList list)
        {
            ...
            if (ability != null)
            {
                var inspectorDrawer =
                    InspectorDrawerUtility.InspectorDrawerForType(ability.GetType()) as AbilityInspectorDrawer;
                if (inspectorDrawer != null) inspectorDrawer.AbilityRemoved(ability, target);
            }
            ...
            list.index = list.index - 1;
            if (list.index == -1 && abilities.Count > 0) list.index = 0;
        ...
        private void OnItemAbilityListRemove(ReorderableList list)
        {
            ...
            // Update the index to point to no longer point to the now deleted ability.
            list.index = list.index - 1;
            if (list.index == -1 && itemAbilities.Count > 0) list.index = 0;
        ...
        private void OnEffectListRemove(ReorderableList list)
        {
            ...
            list.index = list.index - 1;
            if (list.index == -1 && effects.Count > 0) list.index = 0;
        ...
        private void DrawSelectedMovementType(int index)
        {
            EditorGUI.indentLevel++;
            var movementType = m_CharacterLocomotion.MovementTypes[index];
            Utility.InspectorUtility.DrawObject(movementType, true, true, target, true, SerializeMovementTypes);
            ...
            if (InspectorUtility.Foldout(movementType, new GUIContent("States"), false))
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < stateIndexHelper.StateIndexData.Length; ++i)
                        stateIndexHelper.StateIndexData[i] = i;
                    var stateIndexSerializedObject = new SerializedObject(stateIndexHelper);
                    m_ReorderableMovementTypeStateList = StateInspector.DrawStates(m_ReorderableMovementTypeStateList,
                        serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                        GetSelectedMovementTypeStateIndexKey(selectedMovementType), OnMovementTypeStateListDraw,
                        OnMovementTypeStateListAdd,
        ...
        {
            return c_EditorPrefsSelectedMovementTypeStateIndexKey + "." + target.GetType() + "." + target.name + "." +
        ...
        {
            if (m_CharacterLocomotion.MovementTypes == null || EditorPrefs.GetInt(SelectedMovementTypeIndexKey) >=
                m_CharacterLocomotion.MovementTypes.Length)
            {
            ...
            {
                m_ReorderableMovementTypeStateList.index = -1;
                EditorPrefs.SetInt(GetSelectedMovementTypeStateIndexKey(movementType),
                    m_ReorderableMovementTypeStateList.index);
                return;
            }
        ...
        {
            var movementType = m_CharacterLocomotion.MovementTypes[EditorPrefs.GetInt(SelectedMovementTypeIndexKey)];
            var states = StateInspector.AddExistingPreset(movementType.GetType(), movementType.States,
                m_ReorderableMovementTypeStateList, GetSelectedMovementTypeStateIndexKey(movementType));
            if (movementType.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableMovementTypeStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var movementType = m_CharacterLocomotion.MovementTypes[EditorPrefs.GetInt(SelectedMovementTypeIndexKey)];
            var states = StateInspector.CreatePreset(movementType, movementType.States,
                m_ReorderableMovementTypeStateList, GetSelectedMovementTypeStateIndexKey(movementType));
            if (movementType.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
        ...
        private void OnMovementTypeStateListReorder(ReorderableList list)
        {
            ...
            var copiedStates = new State[movementType.States.Length];
            Array.Copy(movementType.States, copiedStates, movementType.States.Length);
            for (var i = 0; i < movementType.States.Length; ++i)
            {
        ...
        {
            var movementType = m_CharacterLocomotion.MovementTypes[EditorPrefs.GetInt(SelectedMovementTypeIndexKey)];
            var states = StateInspector.OnStateListRemove(movementType.States,
                GetSelectedMovementTypeStateIndexKey(movementType), list);
            if (movementType.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableMovementTypeStateList.serializedProperty);
        ...
        {
            var ability = m_CharacterLocomotion.Abilities[index];
            Utility.InspectorUtility.DrawObject(ability, true, false, target, true, SerializeAbilities);
            ...
            if (InspectorUtility.Foldout(ability, new GUIContent("States"), false))
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < stateIndexHelper.StateIndexData.Length; ++i)
                        stateIndexHelper.StateIndexData[i] = i;
                    var stateIndexSerializedObject = new SerializedObject(stateIndexHelper);
                    m_ReorderableAbilityStateList = StateInspector.DrawStates(m_ReorderableAbilityStateList,
                        serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                        GetSelectedAbilityStateIndexKey(selectedAbility), OnAbilityStateListDraw, OnAbilityStateListAdd,
                        OnAbilityStateListReorder,
        ...
        private void DrawEditorControls(Ability ability)
        {
            ...
            if (inspectorDrawer != null)
            {
                ...
                if ((editorCallback != null || inspectorDrawer.CanBuildAnimator ||
                     ability.AbilityIndexParameter != -1) &&
                    InspectorUtility.Foldout(ability, new GUIContent("Editor"), false))
                {
                    EditorGUI.indentLevel++;
                    if (editorCallback != null) editorCallback();
                    var animator = (target as UltimateCharacterLocomotion).GetComponent<Animator>();
                    AnimatorController animatorController = null;
                    if (animator != null) animatorController = animator.runtimeAnimatorController as AnimatorController;

                    AnimatorController firstPersonAnimatorController = null;
#if FIRST_PERSON_CONTROLLER
                    var firstPersonBaseObjects =
 (target as UltimateCharacterLocomotion).GetComponentsInChildren<UltimateCharacterController.FirstPersonController.Character.Identifiers.FirstPersonBaseObject>();
 ...
 (UnityEditor.Animations.AnimatorController)animator.runtimeAnimatorController;
                    ...
                    {
                        var baseDirectory = EditorPrefs.GetString(c_EditorPrefsLastLastAnimatorCodePathKey,
                            Path.GetDirectoryName(AssetDatabase.GetAssetPath(MonoScript.FromScriptableObject(this))));
                        var path = inspectorDrawer.GenerateAnimatorCode(ability, animatorController,
                            firstPersonAnimatorController, baseDirectory);
                        if (!string.IsNullOrEmpty(path))
                            EditorPrefs.SetString(c_EditorPrefsLastLastAnimatorCodePathKey,
                                Path.GetFullPath(path.Replace(Application.dataPath, "Assets")));
                    }
        ...
        {
            return c_EditorPrefsSelectedAbilityStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   ability.GetType() + "." + ability.Index;
        }
        ...
        {
            if (m_ReorderableAbilityStateList == null || m_CharacterLocomotion.Abilities == null ||
                m_CharacterLocomotion.Abilities.Length <= EditorPrefs.GetInt(SelectedAbilityIndexKey))
            {
                if (m_ReorderableAbilityStateList != null) EditorPrefs.SetInt(SelectedAbilityIndexKey, -1);
                return;
            }
        ...
        {
            var ability = m_CharacterLocomotion.Abilities[EditorPrefs.GetInt(SelectedAbilityIndexKey)];
            var states = StateInspector.AddExistingPreset(ability.GetType(), ability.States,
                m_ReorderableAbilityStateList, GetSelectedAbilityStateIndexKey(ability));
            if (ability.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableAbilityStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var ability = m_CharacterLocomotion.Abilities[EditorPrefs.GetInt(SelectedAbilityIndexKey)];
            var states = StateInspector.CreatePreset(ability, ability.States, m_ReorderableAbilityStateList,
                GetSelectedAbilityStateIndexKey(ability));
            if (ability.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableAbilityStateList.serializedProperty);
        ...
        private void OnAbilityStateListReorder(ReorderableList list)
        {
            ...
            // Use the dummy array in order to determine what element the selected index was swapped with.
            var copiedStates = new State[ability.States.Length];
            Array.Copy(ability.States, copiedStates, ability.States.Length);
            for (var i = 0; i < ability.States.Length; ++i)
            {
                var element = list.serializedProperty.GetArrayElementAtIndex(i);
                if (element.intValue != i)
                {
        ...
        private void OnAbilityStateListRemove(ReorderableList list)
        {
            var ability = m_CharacterLocomotion.Abilities[EditorPrefs.GetInt(SelectedAbilityIndexKey)];
            var states =
                StateInspector.OnStateListRemove(ability.States, GetSelectedAbilityStateIndexKey(ability), list);
            if (ability.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableAbilityStateList.serializedProperty);
        ...
        {
            var itemAbility = m_CharacterLocomotion.ItemAbilities[index];
            Utility.InspectorUtility.DrawObject(itemAbility, true, false, target, true, SerializeItemAbilities);
            ...
            if (InspectorUtility.Foldout(itemAbility, new GUIContent("States"), false))
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < stateIndexHelper.StateIndexData.Length; ++i)
                        stateIndexHelper.StateIndexData[i] = i;
                    var stateIndexSerializedObject = new SerializedObject(stateIndexHelper);
                    m_ReorderableItemAbilityStateList = StateInspector.DrawStates(m_ReorderableItemAbilityStateList,
                        serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                        GetSelectedItemAbilityStateIndexKey(selectedItemAbility), OnItemAbilityStateListDraw,
                        OnItemAbilityStateListAdd,
        ...
        {
            return c_EditorPrefsSelectedItemAbilityStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   itemAbility.GetType() + "." + itemAbility.Index;
        }
        ...
        private void OnItemAbilityStateListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            // The index may be out of range if the component was copied.
            if (index >= m_CharacterLocomotion.ItemAbilities[EditorPrefs.GetInt(SelectedItemAbilityIndexKey)].States
                    .Length)
            {
                if (m_ReorderableItemAbilityStateList != null)
                {
                    m_ReorderableItemAbilityStateList.index = -1;
                    EditorPrefs.SetInt(GetSelectedAbilityStateIndexKey(itemAbility),
                        m_ReorderableItemAbilityStateList.index);
                }
        ...
        {
            var itemAbility = m_CharacterLocomotion.ItemAbilities[EditorPrefs.GetInt(SelectedItemAbilityIndexKey)];
            var states = StateInspector.AddExistingPreset(itemAbility.GetType(), itemAbility.States,
                m_ReorderableItemAbilityStateList, GetSelectedItemAbilityStateIndexKey(itemAbility));
            if (itemAbility.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableItemAbilityStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var itemAbility = m_CharacterLocomotion.ItemAbilities[EditorPrefs.GetInt(SelectedItemAbilityIndexKey)];
            var states = StateInspector.CreatePreset(itemAbility, itemAbility.States, m_ReorderableItemAbilityStateList,
                GetSelectedItemAbilityStateIndexKey(itemAbility));
            if (itemAbility.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableItemAbilityStateList.serializedProperty);
        ...
        private void OnItemAbilityStateListReorder(ReorderableList list)
        {
            ...
            // Use the dummy array in order to determine what element the selected index was swapped with.
            var copiedStates = new State[itemAbility.States.Length];
            Array.Copy(itemAbility.States, copiedStates, itemAbility.States.Length);
            for (var i = 0; i < itemAbility.States.Length; ++i)
            {
                var element = list.serializedProperty.GetArrayElementAtIndex(i);
                if (element.intValue != i)
                {
        ...
        {
            var itemAbility = m_CharacterLocomotion.ItemAbilities[EditorPrefs.GetInt(SelectedItemAbilityIndexKey)];
            var states = StateInspector.OnStateListRemove(itemAbility.States,
                GetSelectedItemAbilityStateIndexKey(itemAbility), list);
            if (itemAbility.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableItemAbilityStateList.serializedProperty);
        ...
        {
            var effect = m_CharacterLocomotion.Effects[index];
            Utility.InspectorUtility.DrawObject(effect, true, false, target, true, SerializeEffects);
            ...
            if (InspectorUtility.Foldout(effect, new GUIContent("States"), false))
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < stateIndexHelper.StateIndexData.Length; ++i)
                        stateIndexHelper.StateIndexData[i] = i;
                    var stateIndexSerializedObject = new SerializedObject(stateIndexHelper);
                    m_ReorderableEffectStateList = StateInspector.DrawStates(m_ReorderableEffectStateList,
                        serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                        GetSelectedEffectStateIndexKey(selectedEffect), OnEffectStateListDraw, OnEffectStateListAdd,
                        OnEffectStateListReorder,
        ...
        {
            return c_EditorPrefsSelectedEffectStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   effect.GetType();
        }
        ...
        {
            var effect = m_CharacterLocomotion.Effects[EditorPrefs.GetInt(SelectedEffectIndexKey)];
            var states = StateInspector.AddExistingPreset(effect.GetType(), effect.States, m_ReorderableEffectStateList,
                GetSelectedEffectStateIndexKey(effect));
            if (effect.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableEffectStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var effect = m_CharacterLocomotion.Effects[EditorPrefs.GetInt(SelectedEffectIndexKey)];
            var states = StateInspector.CreatePreset(effect, effect.States, m_ReorderableEffectStateList,
                GetSelectedEffectStateIndexKey(effect));
            if (effect.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
        ...
        private void OnEffectStateListReorder(ReorderableList list)
        {
            ...
            // Use the dummy array in order to determine what element the selected index was swapped with.
            var copiedStates = new State[effect.States.Length];
            Array.Copy(effect.States, copiedStates, effect.States.Length);
            for (var i = 0; i < effect.States.Length; ++i)
            {
                var element = list.serializedProperty.GetArrayElementAtIndex(i);
                if (element.intValue != i)
                {
        ...
        private void UpdateDefaultMovementTypes()
        {
            ...
            if (!string.IsNullOrEmpty(m_CharacterLocomotion.FirstPersonMovementTypeFullName) &&
                TypeUtility.GetType(m_CharacterLocomotion.FirstPersonMovementTypeFullName) == null)
            {
            ...
            if (!string.IsNullOrEmpty(m_CharacterLocomotion.ThirdPersonMovementTypeFullName) &&
                TypeUtility.GetType(m_CharacterLocomotion.ThirdPersonMovementTypeFullName) == null)
            {
            ...
            if (movementTypes != null)
                ...
                {
                    if (movementTypes[i] == null) continue;
                    if (movementTypes[i].GetType().FullName.Contains("FirstPerson"))
                    {
                        ...
                        firstPersonMovementTypeNames.Add(
                            Utility.InspectorUtility.DisplayTypeName(movementTypes[i].GetType(), false));
                    }
                    else
                    {
                        // Third Person.
                        ...
                        thirdPersonMovementTypeNames.Add(
                            Utility.InspectorUtility.DisplayTypeName(movementTypes[i].GetType(), false));
                    }
        ...
        private float GetMaxColliderRadius()
        {
            ...
            {
                if (colliders[i].isTrigger || colliders[i].gameObject.layer != layer) continue;
                var radius = 0f;
                if (colliders[i] is CapsuleCollider)
                    radius = (colliders[i] as CapsuleCollider).radius;
                else if (colliders[i] is SphereCollider) radius = (colliders[i] as SphereCollider).radius;
                if (radius > maxColliderRadius) maxColliderRadius = radius;
            }
