...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Traits
{
    ...
    {
        private const string c_EditorPrefsSelectedAttributeIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Traits.SelectedAttributeTypeIndex";

        private const string c_EditorPrefsSelectedAttributeStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Traits.SelectedAttributeStateIndex";
        ...
        private string SelectedAttributeIndexKey =>
            c_EditorPrefsSelectedAttributeIndexKey + "." + target.GetType() + "." + target.name;
        ...
        public override void OnInspectorGUI()
        {
            ...
            ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableAttributeList, this,
                m_AttributeManager.Attributes, "m_Attributes",
                OnAttributeListDrawHeader, OnAttributeListDraw, null, OnAttributeListAdd, OnAttributeListRemove,
                OnAttributeListSelect,
        ...
        private void OnAttributeListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var label = m_AttributeManager.Attributes[index].Name + " (Value: " +
                        m_AttributeManager.Attributes[index].Value + ")";
        ...
        private void OnAttributeListAdd(ReorderableList list)
        {
            ...
            {
                var postfixIndex = 1;
                while (!IsUniqueName(attributes, attributeName + " " + postfixIndex)) postfixIndex++;
            ...
            EditorPrefs.SetInt(SelectedAttributeIndexKey, m_ReorderableAttributeList.index);
            EditorUtility.SetDirty(m_AttributeManager);
        }
        ...
        private void OnAttributeListRemove(ReorderableList list)
        {
            ...
            if (list.index == -1 && attributes.Count > 0) list.index = 0;
        ...
        private void DrawSelectedAttribute(int index)
        {
            ...
            if (attributeProperty == null) return;

            // The name must be unique.
            var attributeName = attributeProperty.FindPropertyRelative("m_Name");
            var desiredName = EditorGUILayout.TextField(new GUIContent("Name", "The name of the attribute."),
            ...
            if (minValue.floatValue > maxValue.floatValue) maxValue.floatValue = minValue.floatValue;
            EditorGUILayout.PropertyField(maxValue);
            if (maxValue.floatValue < minValue.floatValue) minValue.floatValue = maxValue.floatValue;
            ...
            else if (minValue.floatValue > value.floatValue) value.floatValue = minValue.floatValue;
            if (value.floatValue > maxValue.floatValue)
                maxValue.floatValue = value.floatValue;
            else if (value.floatValue < minValue.floatValue) minValue.floatValue = value.floatValue;
            ...
            if (InspectorUtility.Foldout(attribute, new GUIContent("States"), false))
            {
                ...
                m_ReorderableAttributeStateList = StateInspector.DrawStates(m_ReorderableAttributeStateList,
                    serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                    GetSelectedAttributeStateIndexKey(selectedAttribute), OnAttributeStateListDraw,
                    OnAttributeStateListAdd, OnAttributeStateListReorder,
        ...
        {
            return c_EditorPrefsSelectedAttributeStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   attribute.Name;
        }
        ...
        {
            var attribute = m_AttributeManager.Attributes[EditorPrefs.GetInt(SelectedAttributeIndexKey)];
            var states = StateInspector.AddExistingPreset(attribute.GetType(), attribute.States,
                m_ReorderableAttributeStateList, GetSelectedAttributeStateIndexKey(attribute));
            if (attribute.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableAttributeStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var attribute = m_AttributeManager.Attributes[EditorPrefs.GetInt(SelectedAttributeIndexKey)];
            var states = StateInspector.CreatePreset(attribute, attribute.States, m_ReorderableAttributeStateList,
                GetSelectedAttributeStateIndexKey(attribute));
            if (attribute.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableAttributeStateList.serializedProperty);
        ...
        private void OnAttributeStateListReorder(ReorderableList list)
        {
            ...
            var copiedStates = new State[attribute.States.Length];
            Array.Copy(attribute.States, copiedStates, attribute.States.Length);
            for (var i = 0; i < attribute.States.Length; ++i)
            {
        ...
        private void OnAttributeStateListRemove(ReorderableList list)
        {
            var attribute = m_AttributeManager.Attributes[EditorPrefs.GetInt(SelectedAttributeIndexKey)];
            var states =
                StateInspector.OnStateListRemove(attribute.States, GetSelectedAttributeStateIndexKey(attribute), list);
            if (attribute.States.Length != states.Length)
            {
                Utility.InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableAttributeStateList.serializedProperty);
