...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Traits
{
	...
	public class HitboxInspector
    {
	    ...
	    ///     Draws the hitbox.
	    /// </summary>
	    public static void DrawHitbox(ref ReorderableList reorderableList, SerializedObject serializedObject,
            SerializedProperty hitboxProperty, ReorderableList.ElementCallbackDelegate elementCallback)
        {
        ...
        {
            EditorGUI.LabelField(new Rect(rect.x, rect.y, rect.width - 130, EditorGUIUtility.singleLineHeight),
                "Collider");
            EditorGUI.LabelField(new Rect(rect.x + (rect.width - 130), rect.y, 130, EditorGUIUtility.singleLineHeight),
                " Damage Multiplier");
        }
	    ...
	    private static void OnHitboxListAdd(ReorderableList reorderableList)
        {
            ...
            var hitbox =
                reorderableList.serializedProperty.GetArrayElementAtIndex(reorderableList.serializedProperty.arraySize -
                                                                          1);
            ...
            EditorUtility.RecordUndoDirtyObject(serializedObject.targetObject, "Change Value");
	    ...
	    /// </summary>
	    public static void HitboxElementDraw(ReorderableList reorderableList, Rect rect, int index, bool isActive,
            bool isFocused)
        {
            ...
            EditorGUI.ObjectField(new Rect(rect.x, rect.y + 1, rect.width - 130, EditorGUIUtility.singleLineHeight),
                colliderProperty, new GUIContent());
            multiplierProperty.floatValue = EditorGUI.FloatField(
                new Rect(rect.x + (rect.width - 126), rect.y + 1, 126, EditorGUIUtility.singleLineHeight),
                multiplierProperty.floatValue);
	    ...
	    public static void DrawHitboxGizmo(Hitbox[] hitboxes, GizmoType gizmoType)
        {
            ...
            for (var i = 0; i < hitboxes.Length; ++i)
            {
                ...
                var color = multiplier > 1
                    ? Color.Lerp(Color.yellow, Color.red, Mathf.Clamp01(multiplier - 1))
                    : Color.Lerp(Color.green, Color.yellow, Mathf.Clamp01(multiplier));
                color.a = 0.5f;
                Gizmos.color = color;
                ...
                if (collider is BoxCollider)
                {
                    ...
                    Gizmos.color = Utility.InspectorUtility.GetContrastColor(color);
	    ...
	    /// </summary>
	    public static void DrawCapsuleColliderGizmo(CapsuleCollider collider, Color color = default)
        {
            ...
            switch (collider.direction) // X = 0, Y = 1, Z = 2
            {
                case 0:
                {
                    capsuleRotation = Quaternion.LookRotation(colliderRotation * Vector3.forward,
                        colliderRotation * Vector3.right);
                    break;
                }
                case 1:
                {
                    capsuleRotation = collider.transform.rotation;
                    break;
                }
                case 2:
                {
                    capsuleRotation = Quaternion.LookRotation(colliderRotation * Vector3.up,
                        colliderRotation * Vector3.forward);
                    break;
                }
            }
