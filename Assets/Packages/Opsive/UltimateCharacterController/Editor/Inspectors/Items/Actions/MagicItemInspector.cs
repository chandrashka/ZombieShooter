...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Items.Actions
{
    ...
    {
        private const string c_EditorPrefsSelectedBeginActionIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedBeginActionIndex";

        private const string c_EditorPrefsSelectedBeginActionStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Magic.SelectedBeginActionStateIndex";

        private const string c_EditorPrefsSelectedCastActionIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedCastActionIndex";

        private const string c_EditorPrefsSelectedCastActionStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Magic.SelectedCastActionStateIndex";

        private const string c_EditorPrefsSelectedImpactActionIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedImpactActionIndex";

        private const string c_EditorPrefsSelectedImpactActionStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Magic.SelectedImpactActionStateIndex";

        private const string c_EditorPrefsSelectedEndActionIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedEndActionIndex";

        private const string c_EditorPrefsSelectedEndActionStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.Magic.SelectedEndActionStateIndex";
        ...
        private string SelectedBeginActionIndexKey =>
            c_EditorPrefsSelectedBeginActionIndexKey + "." + target.GetType() + "." + target.name;

        private string SelectedCastActionIndexKey =>
            c_EditorPrefsSelectedCastActionIndexKey + "." + target.GetType() + "." + target.name;

        private string SelectedImpactActionIndexKey =>
            c_EditorPrefsSelectedImpactActionIndexKey + "." + target.GetType() + "." + target.name;

        private string SelectedEndActionIndexKey =>
            c_EditorPrefsSelectedEndActionIndexKey + "." + target.GetType() + "." + target.name;
        ...
        protected override Action GetDrawCallback()
        {
            ...
            baseCallback += () =>
            {
                if (Foldout("Magic"))
                {
                    ...
                    var castInterruptSource =
                        (int)InspectorUtility.GetFieldValue<MagicItem.CastInterruptSource>(target, "m_InterruptSource");
                    var castInterruptSourceString = Enum.GetNames(typeof(MagicItem.CastInterruptSource));
                    var value = EditorGUILayout.MaskField(
                        new GUIContent("Interrupt Source",
                            InspectorUtility.GetFieldTooltip(target, "m_InterruptSource")), castInterruptSource,
                        castInterruptSourceString);
                    ...
                    {
                        EditorGUI.indentLevel++;
                        ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableBeginActionList, this,
                            m_MagicItem.BeginActions, "m_BeginActionData",
                            OnBeginActionListDrawHeader, OnBeginActionListDraw, OnBeginActionListReorder,
                            OnBeginActionListAdd, OnBeginActionListRemove, OnBeginActionListSelect,
                    ...
                    {
                        EditorGUI.indentLevel++;
                        ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableCastActionList, this,
                            m_MagicItem.CastActions, "m_CastActionData",
                            OnCastActionListDrawHeader, OnCastActionListDraw, OnCastActionListReorder,
                            OnCastActionListAdd, OnCastActionListRemove, OnCastActionListSelect,
                    ...
                    {
                        EditorGUI.indentLevel++;
                        ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableImpactActionList, this,
                            m_MagicItem.ImpactActions, "m_ImpactActionData",
                            OnImpactActionListDrawHeader, OnImpactActionListDraw, OnImpactActionListReorder,
                            OnImpactActionListAdd, OnImpactActionListRemove, OnImpactActionListSelect,
                    ...
                    {
                        EditorGUI.indentLevel++;
                        ReorderableListSerializationHelper.DrawReorderableList(ref m_ReorderableEndActionList, this,
                            m_MagicItem.EndActions, "m_EndActionData",
                            OnEndActionListDrawHeader, OnEndActionListDraw, OnEndActionListReorder, OnEndActionListAdd,
                            OnEndActionListRemove, OnEndActionListSelect,
                    ...
                    if (Foldout("Events"))
                    {
                        ...
                        Shared.Editor.Inspectors.Utility.InspectorUtility.UnityEventPropertyField(
                            PropertyFromName("m_OnStartStopBeginEndActionsEvent"));
                        Shared.Editor.Inspectors.Utility.InspectorUtility.UnityEventPropertyField(
                            PropertyFromName("m_OnCastEvent"));
                        Shared.Editor.Inspectors.Utility.InspectorUtility.UnityEventPropertyField(
                            PropertyFromName("m_OnImpactEvent"));
                        EditorGUI.indentLevel--;
                    }
        ...
        private void OnCastActionListDrawHeader(Rect rect)
        {
            ...
            activeRect.width = 50;
            EditorGUI.LabelField(activeRect,
                new GUIContent("Delay", "The delay to start the cast after the item has been used."));
        }
        ...
        {
            ReorderableListSerializationHelper.AddObjectType(typeof(BeginEndAction), true, m_MagicItem.BeginActions,
                AddBeginAction);
        }
        ...
        {
            ReorderableListSerializationHelper.AddObjectType(typeof(CastAction), true, m_MagicItem.CastActions,
                AddCastAction);
        }
        ...
        {
            ReorderableListSerializationHelper.AddObjectType(typeof(ImpactAction), true, m_MagicItem.ImpactActions,
                AddImpactAction);
        }
        ...
        {
            ReorderableListSerializationHelper.AddObjectType(typeof(BeginEndAction), true, m_MagicItem.EndActions,
                AddEndAction);
        }
        ...
        private void OnBeginActionListRemove(ReorderableList list)
        {
            ...
            if (list.index == -1 && beginActions.Count > 0) list.index = 0;
        ...
        private void OnCastActionListRemove(ReorderableList list)
        {
            ...
            // Update the index to point to no longer point to the now deleted action.
            list.index = list.index - 1;
            if (list.index == -1 && castActions.Count > 0) list.index = 0;
        ...
        private void OnImpactActionListRemove(ReorderableList list)
        {
            ...
            // Update the index to point to no longer point to the now deleted action.
            list.index = list.index - 1;
            if (list.index == -1 && impactActions.Count > 0) list.index = 0;
        ...
        private void OnEndActionListRemove(ReorderableList list)
        {
            ...
            // Update the index to point to no longer point to the now deleted action.
            list.index = list.index - 1;
            if (list.index == -1 && endActions.Count > 0) list.index = 0;
        ...
        private void DrawSelectedBeginAction(int index)
        {
            ...
            var selectedBeginAction = beginAction;
            if (Shared.Editor.Inspectors.Utility.InspectorUtility.Foldout(selectedBeginAction, new GUIContent("States"),
                    false))
            {
                ...
                try
                {
                    ...
                    m_ReorderableBeginActionStateList = StateInspector.DrawStates(m_ReorderableBeginActionStateList,
                        serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                        GetSelectedBeginActionStateIndexKey(selectedBeginAction), OnBeginActionStateListDraw,
                        OnBeginActionStateListAdd, OnBeginActionStateListReorder,
        ...
        private void DrawSelectedCastAction(int index)
        {
            ...
            var selectedCastAction = castAction;
            if (Shared.Editor.Inspectors.Utility.InspectorUtility.Foldout(selectedCastAction, new GUIContent("States"),
                    false))
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < stateIndexHelper.StateIndexData.Length; ++i)
                        stateIndexHelper.StateIndexData[i] = i;
                    var stateIndexSerializedObject = new SerializedObject(stateIndexHelper);
                    m_ReorderableCastActionStateList = StateInspector.DrawStates(m_ReorderableCastActionStateList,
                        serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                        GetSelectedCastActionStateIndexKey(selectedCastAction), OnCastActionStateListDraw,
                        OnCastActionStateListAdd, OnCastActionStateListReorder,
        ...
        private void DrawSelectedImpactAction(int index)
        {
            ...
            var selectedImpactAction = impactAction;
            if (Shared.Editor.Inspectors.Utility.InspectorUtility.Foldout(selectedImpactAction,
                    new GUIContent("States"), false))
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < stateIndexHelper.StateIndexData.Length; ++i)
                        stateIndexHelper.StateIndexData[i] = i;
                    var stateIndexSerializedObject = new SerializedObject(stateIndexHelper);
                    m_ReorderableImpactActionStateList = StateInspector.DrawStates(m_ReorderableImpactActionStateList,
                        serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                        GetSelectedImpactActionStateIndexKey(selectedImpactAction), OnImpactActionStateListDraw,
                        OnImpactActionStateListAdd, OnImpactActionStateListReorder,
        ...
        private void DrawSelectedEndAction(int index)
        {
            ...
            var selectedEndAction = endAction;
            if (Shared.Editor.Inspectors.Utility.InspectorUtility.Foldout(selectedEndAction, new GUIContent("States"),
                    false))
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < stateIndexHelper.StateIndexData.Length; ++i)
                        stateIndexHelper.StateIndexData[i] = i;
                    var stateIndexSerializedObject = new SerializedObject(stateIndexHelper);
                    m_ReorderableEndActionStateList = StateInspector.DrawStates(m_ReorderableEndActionStateList,
                        serializedObject, stateIndexSerializedObject.FindProperty("m_StateIndexData"),
                        GetSelectedEndActionStateIndexKey(selectedEndAction), OnEndActionStateListDraw,
                        OnEndActionStateListAdd, OnEndActionStateListReorder,
        ...
        {
            var beginActions = m_MagicItem.BeginActions == null
                ? new List<BeginEndAction>()
                : new List<BeginEndAction>(m_MagicItem.BeginActions);
            m_MagicItem.BeginActionData = Serialization.Serialize<BeginEndAction>(beginActions);
            m_MagicItem.BeginActions = beginActions.ToArray();
        ...
        {
            var castActions = m_MagicItem.CastActions == null
                ? new List<CastAction>()
                : new List<CastAction>(m_MagicItem.CastActions);
            m_MagicItem.CastActionData = Serialization.Serialize<CastAction>(castActions);
            m_MagicItem.CastActions = castActions.ToArray();
            EditorUtility.SetDirty(target);
        }
        ...
        {
            var impactActions = m_MagicItem.ImpactActions == null
                ? new List<ImpactAction>()
                : new List<ImpactAction>(m_MagicItem.ImpactActions);
            m_MagicItem.ImpactActionData = Serialization.Serialize<ImpactAction>(impactActions);
            m_MagicItem.ImpactActions = impactActions.ToArray();
            EditorUtility.SetDirty(target);
        }
        ...
        {
            var endActions = m_MagicItem.EndActions == null
                ? new List<BeginEndAction>()
                : new List<BeginEndAction>(m_MagicItem.EndActions);
            m_MagicItem.EndActionData = Serialization.Serialize<BeginEndAction>(endActions);
            m_MagicItem.EndActions = endActions.ToArray();
            EditorUtility.SetDirty(target);
        }
        ...
        {
            return c_EditorPrefsSelectedBeginActionStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   beginAction.GetType();
        }
        ...
        {
            return c_EditorPrefsSelectedCastActionStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   castAction.GetType();
        }
        ...
        {
            return c_EditorPrefsSelectedImpactActionStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   impactAction.GetType();
        }
        ...
        {
            return c_EditorPrefsSelectedEndActionStateIndexKey + "." + target.GetType() + "." + target.name + "." +
                   endAction.GetType();
        }
        ...
        {
            if (m_ReorderableBeginActionStateList == null) return;
            ...
            if (index >= m_MagicItem.BeginActions[EditorPrefs.GetInt(SelectedBeginActionIndexKey)].States.Length)
            {
                ...
                EditorPrefs.SetInt(GetSelectedBeginActionStateIndexKey(beginAction),
                    m_ReorderableBeginActionStateList.index);
                return;
            }
        ...
        {
            if (m_ReorderableCastActionStateList == null) return;
            ...
            if (index >= m_MagicItem.CastActions[EditorPrefs.GetInt(SelectedCastActionIndexKey)].States.Length)
            {
                ...
                EditorPrefs.SetInt(GetSelectedCastActionStateIndexKey(castAction),
                    m_ReorderableCastActionStateList.index);
                return;
            }
        ...
        {
            if (m_ReorderableImpactActionStateList == null) return;
            ...
            if (index >= m_MagicItem.ImpactActions[EditorPrefs.GetInt(SelectedImpactActionIndexKey)].States.Length)
            {
                ...
                EditorPrefs.SetInt(GetSelectedImpactActionStateIndexKey(impactAction),
                    m_ReorderableImpactActionStateList.index);
                return;
            }
        ...
        {
            if (m_ReorderableEndActionStateList == null) return;
        ...
        {
            var beginAction = m_MagicItem.BeginActions[EditorPrefs.GetInt(SelectedBeginActionIndexKey)];
            var states = StateInspector.AddExistingPreset(beginAction.GetType(), beginAction.States,
                m_ReorderableBeginActionStateList, GetSelectedBeginActionStateIndexKey(beginAction));
        ...
        {
            var castAction = m_MagicItem.CastActions[EditorPrefs.GetInt(SelectedCastActionIndexKey)];
            var states = StateInspector.AddExistingPreset(castAction.GetType(), castAction.States,
                m_ReorderableCastActionStateList, GetSelectedCastActionStateIndexKey(castAction));
        ...
        {
            var impactAction = m_MagicItem.ImpactActions[EditorPrefs.GetInt(SelectedImpactActionIndexKey)];
            var states = StateInspector.AddExistingPreset(impactAction.GetType(), impactAction.States,
                m_ReorderableImpactActionStateList, GetSelectedImpactActionStateIndexKey(impactAction));
            if (impactAction.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableImpactActionStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var endAction = m_MagicItem.EndActions[EditorPrefs.GetInt(SelectedEndActionIndexKey)];
            var states = StateInspector.AddExistingPreset(endAction.GetType(), endAction.States,
                m_ReorderableEndActionStateList, GetSelectedEndActionStateIndexKey(endAction));
        ...
        {
            var beginAction = m_MagicItem.BeginActions[EditorPrefs.GetInt(SelectedBeginActionIndexKey)];
            var states = StateInspector.CreatePreset(beginAction, beginAction.States, m_ReorderableBeginActionStateList,
                GetSelectedBeginActionStateIndexKey(beginAction));
            if (beginAction.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states, m_ReorderableBeginActionStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var castAction = m_MagicItem.CastActions[EditorPrefs.GetInt(SelectedCastActionIndexKey)];
            var states = StateInspector.CreatePreset(castAction, castAction.States, m_ReorderableCastActionStateList,
                GetSelectedCastActionStateIndexKey(castAction));
            if (castAction.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states, m_ReorderableCastActionStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var impactAction = m_MagicItem.ImpactActions[EditorPrefs.GetInt(SelectedImpactActionIndexKey)];
            var states = StateInspector.CreatePreset(impactAction, impactAction.States,
                m_ReorderableImpactActionStateList, GetSelectedImpactActionStateIndexKey(impactAction));
            if (impactAction.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states,
        ...
        {
            var endAction = m_MagicItem.EndActions[EditorPrefs.GetInt(SelectedEndActionIndexKey)];
            var states = StateInspector.CreatePreset(endAction, endAction.States, m_ReorderableEndActionStateList,
                GetSelectedEndActionStateIndexKey(endAction));
            if (endAction.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states, m_ReorderableEndActionStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        private void OnBeginActionStateListReorder(ReorderableList list)
        {
            ...
            var copiedStates = new State[beginAction.States.Length];
            Array.Copy(beginAction.States, copiedStates, beginAction.States.Length);
            for (var i = 0; i < beginAction.States.Length; ++i)
            {
        ...
        private void OnCastActionStateListReorder(ReorderableList list)
        {
            ...
            // Use the dummy array in order to determine what element the selected index was swapped with.
            var copiedStates = new State[castAction.States.Length];
            Array.Copy(castAction.States, copiedStates, castAction.States.Length);
            for (var i = 0; i < castAction.States.Length; ++i)
            {
                var element = list.serializedProperty.GetArrayElementAtIndex(i);
                if (element.intValue != i)
                {
        ...
        private void OnImpactActionStateListReorder(ReorderableList list)
        {
            ...
            // Use the dummy array in order to determine what element the selected index was swapped with.
            var copiedStates = new State[impactAction.States.Length];
            Array.Copy(impactAction.States, copiedStates, impactAction.States.Length);
            for (var i = 0; i < impactAction.States.Length; ++i)
            {
                var element = list.serializedProperty.GetArrayElementAtIndex(i);
                if (element.intValue != i)
                {
        ...
        private void OnEndActionStateListReorder(ReorderableList list)
        {
            ...
            // Use the dummy array in order to determine what element the selected index was swapped with.
            var copiedStates = new State[endAction.States.Length];
            Array.Copy(endAction.States, copiedStates, endAction.States.Length);
            for (var i = 0; i < endAction.States.Length; ++i)
            {
                var element = list.serializedProperty.GetArrayElementAtIndex(i);
                if (element.intValue != i)
                {
        ...
        {
            var beginAction = m_MagicItem.BeginActions[EditorPrefs.GetInt(SelectedBeginActionIndexKey)];
            var states = StateInspector.OnStateListRemove(beginAction.States,
                GetSelectedBeginActionStateIndexKey(beginAction), list);
            if (beginAction.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states, m_ReorderableBeginActionStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var castAction = m_MagicItem.CastActions[EditorPrefs.GetInt(SelectedCastActionIndexKey)];
            var states = StateInspector.OnStateListRemove(castAction.States,
                GetSelectedCastActionStateIndexKey(castAction), list);
            if (castAction.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states, m_ReorderableCastActionStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        {
            var impactAction = m_MagicItem.ImpactActions[EditorPrefs.GetInt(SelectedImpactActionIndexKey)];
            var states = StateInspector.OnStateListRemove(impactAction.States,
                GetSelectedImpactActionStateIndexKey(impactAction), list);
            if (impactAction.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableImpactActionStateList.serializedProperty);
        ...
        private void OnEndActionStateListRemove(ReorderableList list)
        {
            var endAction = m_MagicItem.EndActions[EditorPrefs.GetInt(SelectedEndActionIndexKey)];
            var states =
                StateInspector.OnStateListRemove(endAction.States, GetSelectedEndActionStateIndexKey(endAction), list);
            if (endAction.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states, m_ReorderableEndActionStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
