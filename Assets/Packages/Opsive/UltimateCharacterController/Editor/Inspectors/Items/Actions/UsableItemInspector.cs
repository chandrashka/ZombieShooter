...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Items.Actions
{
    ...
    {
        private const string c_EditorPrefsSelectedUseAnimatorAudioStateSetIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedUseAnimatorAudioStateSetIndex";

        private const string c_EditorPrefsSelectedUseAnimatorAudioStateSetStateIndexKey =
            "Opsive.UltimateCharacterController.Editor.Inspectors.SelectedUseAnimatorAudioStateSetStateIndex";
        ...
        private string SelectedUseAnimatorAudioStateSetIndexKey =>
            c_EditorPrefsSelectedUseAnimatorAudioStateSetIndexKey + "." + target.GetType() + "." + target.name +
            m_UsableItem.ID;
        ...
        protected override void OnEnable()
        {
            ...
            m_UsableItem.UseAnimatorAudioStateSet.DeserializeAnimatorAudioStateSelector(m_Item,
                m_Item.GetComponentInParent<UltimateCharacterLocomotion>());
            m_AttributeManager = m_UsableItem.GetComponent<AttributeManager>();
            var character = m_UsableItem.GetComponentInParent<UltimateCharacterLocomotion>();
            if (character != null) m_CharacterAttributeManager = character.GetComponent<AttributeManager>();
        ...
        protected override Action GetDrawCallback()
        {
            ...
            baseCallback += () =>
            {
                if (Foldout("Use"))
                {
                    ...
                    InspectorUtility.DrawAnimationEventTrigger(target, "Use Complete Event",
                        PropertyFromName("m_UseCompleteEvent"));
                    ...
                    {
                        EditorGUI.indentLevel++;
                        var attributeName = InspectorUtility.DrawAttribute(m_AttributeManager,
                            m_UsableItem.UseAttributeName, "Use Attribute");
                        ...
                        {
                            EditorGUI.indentLevel++;
                            EditorGUILayout.PropertyField(PropertyFromName("m_UseAttributeAmount"),
                                new GUIContent("Use Amount"));
                            EditorGUILayout.PropertyField(PropertyFromName("m_DropWhenUseDepleted"));
                            EditorGUI.indentLevel--;
                        }

                        attributeName = InspectorUtility.DrawAttribute(m_CharacterAttributeManager,
                            m_UsableItem.CharacterUseAttributeName, "Character Use Attribute");
                        ...
                        {
                            EditorGUI.indentLevel++;
                            EditorGUILayout.PropertyField(PropertyFromName("m_CharacterUseAttributeAmount"),
                                new GUIContent("Use Amount"));
                            EditorGUI.indentLevel--;
                        }
                    ...
                    if (Foldout("Animator Audio", "Use"))
                    {
                        ...
                        AnimatorAudioStateSetInspector.DrawAnimatorAudioStateSet(m_UsableItem,
                            m_UsableItem.UseAnimatorAudioStateSet, "m_UseAnimatorAudioStateSet", true,
                            ref m_ReorderableUseAnimatorAudioStateSetAudioList, OnAnimatorAudioStateListDraw,
                            OnAnimatorAudioStateListSelect,
                            OnAnimatorAudioStateListAdd, OnAnimatorAudioStateListRemove,
                            SelectedUseAnimatorAudioStateSetIndexKey,
                            ref m_ReorderableUseAnimatorAudioStateSetList, OnUseAudioListElementDraw, OnUseAudioListAdd,
                            OnUseAudioListRemove,
                            ref m_ReorderableUseAnimatorAudioStateSetStateList, OnAnimatorAudioStateSetStateListDraw,
                            OnAnimatorAudioStateSetStateListAdd,
                            ...
                            GetSelectedAnimatorAudioStateSetStateIndexKey(
                                EditorPrefs.GetInt(SelectedUseAnimatorAudioStateSetIndexKey)));
                        EditorGUI.indentLevel--;
                    }
                }
            };
        ...
        private void OnAnimatorAudioStateListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            AnimatorAudioStateSetInspector.OnAnimatorAudioStateElementDraw(
                m_ReorderableUseAnimatorAudioStateSetAudioList, m_UsableItem.UseAnimatorAudioStateSet, rect, index,
        ...
        {
            AnimatorAudioStateSetInspector.OnAnimatorAudioStateListAdd(list, m_UsableItem.UseAnimatorAudioStateSet,
                SelectedUseAnimatorAudioStateSetIndexKey);
        }
        ...
        {
            AnimatorAudioStateSetInspector.OnAnimatorAudioStateListRemove(list, m_UsableItem.UseAnimatorAudioStateSet,
                SelectedUseAnimatorAudioStateSetIndexKey);
        }
        ...
        {
            AnimatorAudioStateSetInspector.OnAudioClipDraw(m_ReorderableUseAnimatorAudioStateSetList, rect, index,
                m_UsableItem.UseAnimatorAudioStateSet.States, SelectedUseAnimatorAudioStateSetIndexKey, target);
        }
        ...
        {
            AnimatorAudioStateSetInspector.OnAudioClipListAdd(list, m_UsableItem.UseAnimatorAudioStateSet.States,
                SelectedUseAnimatorAudioStateSetIndexKey, target);
        }
        ...
        {
            AnimatorAudioStateSetInspector.OnAudioClipListRemove(list, m_UsableItem.UseAnimatorAudioStateSet.States,
                SelectedUseAnimatorAudioStateSetIndexKey, target);
        }
        ...
        {
            return c_EditorPrefsSelectedUseAnimatorAudioStateSetStateIndexKey + "." + target.GetType() + "." +
                   target.name + "." + index;
        }
        ...
        private void OnAnimatorAudioStateSetStateListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var animatorAudioState =
                m_UsableItem.UseAnimatorAudioStateSet.States[
                    EditorPrefs.GetInt(SelectedUseAnimatorAudioStateSetIndexKey)];
            ...
            if (index >= animatorAudioState.States.Length)
            {
                ...
                EditorPrefs.SetInt(
                    GetSelectedAnimatorAudioStateSetStateIndexKey(
                        EditorPrefs.GetInt(SelectedUseAnimatorAudioStateSetIndexKey)),
                    m_ReorderableUseAnimatorAudioStateSetStateList.index);
                return;
            }
        ...
        private void AddExistingAnimatorAudioStateSetStatePreset()
        {
            ...
            var states = StateInspector.AddExistingPreset(animatorAudioState.GetType(), animatorAudioState.States,
                m_ReorderableUseAnimatorAudioStateSetStateList,
                GetSelectedAnimatorAudioStateSetStateIndexKey(
                    EditorPrefs.GetInt(SelectedUseAnimatorAudioStateSetIndexKey)));
            if (animatorAudioState.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableUseAnimatorAudioStateSetStateList.serializedProperty);
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        private void OnAnimatorAudioStateSetStateListRemove(ReorderableList list)
        {
            ...
            var states = StateInspector.OnStateListRemove(animatorAudioState.States,
                GetSelectedAnimatorAudioStateSetStateIndexKey(
                    EditorPrefs.GetInt(SelectedUseAnimatorAudioStateSetIndexKey)), list);
            if (animatorAudioState.States.Length != states.Length)
            {
                InspectorUtility.SynchronizePropertyCount(states,
                    m_ReorderableUseAnimatorAudioStateSetStateList.serializedProperty);
