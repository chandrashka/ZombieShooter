...
namespace Opsive.UltimateCharacterController.Editor.Utility
{
    ...
    {
        private const string c_StartGeneratedCodeComment =
            "// ------------------------------------------- Start Generated Code -------------------------------------------";

        /// <summary>
        ///     Generates the code necessary to recreate the states/transitions that are affected by the specified parameter name
        ///     and value.
        /// </summary>
        ...
        public static string GenerateAnimatorCode(AnimatorController animatorController,
            AnimatorController firstPersonAnimatorController, string parameterName, float parameterValue,
            object parentObject, string baseDirectory)
        {
            var generatedCode = new StringBuilder();
            var generatedStateMachineCode = GenerateAnimatorCode(animatorController, "animatorController",
                parameterName, parameterValue, generatedCode);
            generatedStateMachineCode = GenerateAnimatorCode(firstPersonAnimatorController,
                                            "firstPersonAnimatorController", parameterName, parameterValue,
                                            generatedCode) ||
                                        generatedStateMachineCode;
            ...
            if (generatedStateMachineCode)
            {
                ...
                var path = EditorUtility.SaveFilePanel("Save Item", baseDirectory + "/Abilities",
                    parentName + "InspectorDrawer.cs", "cs");
                if (path.Length != 0 && Application.dataPath.Length < path.Length)
                {
                    ...
                    if (File.Exists(path))
                    {
                        using (var sr = new StreamReader(path))
                        {
                            ...
                            var startIndex = -1;
                            if ((startIndex = contents.IndexOf(c_StartGeneratedCodeComment, StringComparison.Ordinal)) >
                                -1)
                            {
        ...
        /// </summary>
        /// <param name="animatorController">The animator controller to generate the states/transitions of.</param>
        /// <param name="animatorVariableName">
        ///     The name of the animator controller variable name. This name is used within the
        ///     generated code.
        /// </param>
        /// <param name="parameterName">The name of the animator parameter which should have its states/transitions generated.</param>
        /// <param name="parameterValue">The value of the animator parameter which should have its states/transitions generated.</param>
        ...
        private static bool GenerateAnimatorCode(AnimatorController animatorController, string animatorVariableName,
            string parameterName, float parameterValue, StringBuilder generatedCode)
        {
            ...
            for (var i = 0; i < animatorController.layers.Length; ++i)
            {
                ...
                for (var j = 0; j < transitions.Count; ++j)
                for (var k = 0; k < stateMachine.stateMachines.Length; ++k)
                {
                    if (stateMachines.Contains(stateMachine.stateMachines[k])) continue;
                    if (HasState(stateMachine.stateMachines[k].stateMachine,
                            transitions[j].Transition.destinationState))
                ...
                if (stateMachines.Count > 0)
                {
                    ...
                    if (generatedCode.Length > 0) generatedCode.AppendLine();

                    for (var j = 0; j < stateMachines.Count; ++j)
                    {
                        ...
                        generatedCode.AppendLine("\t\t\t}");
                        generatedCode.AppendLine();
                        generatedCode.AppendLine("\t\t\tvar " + baseStateMachineName + " = " + animatorVariableName +
                                                 ".layers[" + i + "].stateMachine;");
                        generatedCode.AppendLine();
                        generatedCode.AppendLine("\t\t\t// The state machine should start fresh.");
                        generatedCode.AppendLine("\t\t\tfor (int i = 0; i < " + animatorVariableName +
                                                 ".layers.Length; ++i) {");
                        generatedCode.AppendLine("\t\t\t\tfor (int j = 0; j < " + baseStateMachineName +
                                                 ".stateMachines.Length; ++j) {");
                        generatedCode.AppendLine("\t\t\t\t\tif (" + baseStateMachineName +
                                                 ".stateMachines[j].stateMachine.name == \"" +
                                                 baseStateMachine.stateMachine.name + "\") {");
                        generatedCode.AppendLine("\t\t\t\t\t\t" + baseStateMachineName + ".RemoveStateMachine(" +
                                                 baseStateMachineName + ".stateMachines[j].stateMachine);");
                    ...
                    for (var j = 0; j < stateMachines.Count; ++j)
                    {
                        ...
                        for (var k = 0; k < transitions.Count; ++k)
                        {
                            ...
                            GenerateTransition(baseStateMachineName, string.Empty, transitions[k].Transition, false,
                                transitions[j].AnyStateTransition, false, generatedCode);
                            if (k != transitions.Count - 1) generatedCode.AppendLine();
                        }
                    }
                }
            }
        ...
        public static void FindTransitions(AnimatorTransitionBase[] transitions, string parameterName,
            float parameterValue, List<AnimatorTransition> validTransitions, bool anyStateTransition)
        {
            ...
            for (var i = 0; i < transitions.Length; ++i)
            {
                ...
                for (var j = 0; j < conditions.Length; ++j)
                    ...
                    if (conditions[j].mode == AnimatorConditionMode.Equals &&
                        conditions[j].parameter == parameterName && conditions[j].threshold == parameterValue)
                    {
                        validTransition = true;
                        break;
                    }

                if (validTransition) validTransitions.Add(new AnimatorTransition(transitions[i], anyStateTransition));
            }
        }
        ...
        /// <param name="generatedCode">The final generated code.</param>
        private static void GenerateMotions(ChildAnimatorStateMachine childStateMachine,
            HashSet<UnityEngine.Motion> motionSet, StringBuilder generatedCode)
        {
            ...
            if (stateMachine.states.Length > 0)
                for (var i = 0; i < stateMachine.states.Length; ++i)
                {
                    ...
                    {
                        if (motion != null) GenerateMotion(motion, motionSet, generatedCode);
                    }
                }
        ...
        /// <param name="motionSet">The list of motions which have been already generated.</param>
        /// <param name="generatedCode">The final generated code.</param>
        private static void GenerateBlendTreeMotions(BlendTree blendTree, HashSet<UnityEngine.Motion> motionSet,
            StringBuilder generatedCode)
        {
            for (var j = 0; j < blendTree.children.Length; ++j)
            {
        ...
        /// <param name="motionSet">The list of motions which have been already generated.</param>
        /// <param name="generatedCode">The final generated code.</param>
        private static void GenerateMotion(UnityEngine.Motion motion, HashSet<UnityEngine.Motion> motionSet,
            StringBuilder generatedCode)
        {
            if (motionSet.Contains(motion)) return;
            ...
            generatedCode.AppendLine("\t\t\tvar " + uniqueMotionNamePath + " = AssetDatabase.GUIDToAssetPath(\"" +
                                     AssetDatabase.AssetPathToGUID(
                                         AssetDatabase.GetAssetPath(motion as AnimationClip)) + "\"); ");
            generatedCode.AppendLine("\t\t\tvar " + uniqueMotionName + " = AnimatorBuilder.GetAnimationClip(" +
                                     uniqueMotionNamePath + ", \"" + motion.name + "\");");
        }
        ...
        /// <param name="generatedCode">The final generated code.</param>
        private static void GenerateStateMachine(string parentStateMachineName,
            ChildAnimatorStateMachine childStateMachine, StringBuilder generatedCode)
        {
            var stateMachine = childStateMachine.stateMachine;
            generatedCode.AppendLine("\t\t\t// State Machine.");
            var uniqueStateMachineName = UniqueName(stateMachine);
            generatedCode.AppendLine("\t\t\tvar " + uniqueStateMachineName + " = " + parentStateMachineName +
                                     ".AddStateMachine(\"" +
            ...
            if (stateMachine.states.Length > 0)
            {
                ...
                for (var i = 0; i < stateMachine.states.Length; ++i)
                {
                    ...
                    generatedCode.AppendLine("\t\t\tvar " + uniqueStateName + " = " + uniqueStateMachineName +
                                             ".AddState(\"" + state.name + "\", " +
                    ...
                    if (!string.IsNullOrEmpty(state.cycleOffsetParameter))
                        generatedCode.AppendLine("\t\t\t" + uniqueStateName + ".cycleOffsetParameter = \"" +
                                                 state.cycleOffsetParameter + "\";");
                    generatedCode.AppendLine("\t\t\t" + uniqueStateName + ".cycleOffsetParameterActive = " +
                                             BoolString(state.cycleOffsetParameterActive) + ";");
                    ...
                    if (!string.IsNullOrEmpty(state.mirrorParameter))
                        generatedCode.AppendLine("\t\t\t" + uniqueStateName + ".mirrorParameter = \"" +
                                                 state.mirrorParameter + "\";");
                    generatedCode.AppendLine("\t\t\t" + uniqueStateName + ".mirrorParameterActive = " +
                                             BoolString(state.mirrorParameterActive) + ";");
                    generatedCode.AppendLine("\t\t\t" + uniqueStateName + ".speed = " + state.speed + "f;");
                    if (!string.IsNullOrEmpty(state.speedParameter))
                        generatedCode.AppendLine("\t\t\t" + uniqueStateName + ".speedParameter = \"" +
                                                 state.speedParameter + "\";");
                    generatedCode.AppendLine("\t\t\t" + uniqueStateName + ".speedParameterActive = " +
                                             BoolString(state.speedParameterActive) + ";");
                    if (!string.IsNullOrEmpty(state.tag))
                        generatedCode.AppendLine("\t\t\t" + uniqueStateName + ".tag = \"" + state.tag + "\";");
                    generatedCode.AppendLine("\t\t\t" + uniqueStateName + ".writeDefaultValues = " +
                                             BoolString(state.writeDefaultValues) + ";");
                    generatedCode.AppendLine();
                }
            }
            ...
            if (stateMachine.anyStateTransitions.Length > 0)
            {
                ...
                {
                    GenerateTransition(uniqueStateMachineName, string.Empty, stateMachine.anyStateTransitions[i], false,
                        true, false, generatedCode);
                    generatedCode.AppendLine();
                }
            }
            ...
            if (stateMachine.entryTransitions.Length > 0)
            {
                ...
                {
                    GenerateTransition(uniqueStateMachineName, string.Empty, stateMachine.entryTransitions[i], false,
                        false, false, generatedCode);
                    generatedCode.AppendLine();
                }
            }
            ...
            generatedCode.AppendLine("\t\t\t" + uniqueStateMachineName + ".anyStatePosition = " +
                                     Vector3String(stateMachine.anyStatePosition) + ";");
            generatedCode.AppendLine("\t\t\t" + uniqueStateMachineName + ".defaultState = " +
                                     UniqueName(stateMachine.defaultState) + ";");
            generatedCode.AppendLine("\t\t\t" + uniqueStateMachineName + ".entryPosition = " +
                                     Vector3String(stateMachine.entryPosition) + ";");
            generatedCode.AppendLine("\t\t\t" + uniqueStateMachineName + ".exitPosition = " +
                                     Vector3String(stateMachine.exitPosition) + ";");
            generatedCode.AppendLine("\t\t\t" + uniqueStateMachineName + ".parentStateMachinePosition = " +
                                     Vector3String(stateMachine.parentStateMachinePosition) + ";");
            generatedCode.AppendLine();
        }

        private static void GenerateStateMachineStateTransitions(ChildAnimatorStateMachine childStateMachine,
            StringBuilder generatedCode)
        {
            var stateMachine = childStateMachine.stateMachine;
            ...
            if (stateMachine.states.Length > 0)
            {
                ...
                for (var i = 0; i < stateMachine.states.Length; ++i)
                {
                    ...
                    {
                        GenerateTransition(uniqueStateName, string.Empty, state.transitions[j], true, false, false,
                            generatedCode);
                        generatedCode.AppendLine();
                    }
                }
            ...
            for (var i = 0; i < stateMachine.stateMachines.Length; ++i)
            {
                ...
                if (transitions != null && transitions.Length > 0)
                {
                    ...
                    {
                        GenerateTransition(uniqueStateName, uniqueParentStateName, transitions[j], false, false, true,
                            generatedCode);
                        generatedCode.AppendLine();
                    }
                }
        ...
        private static void GenerateBlendTree(string blendTreeName, BlendTree blendTree, StringBuilder generatedCode,
            int indentLevel)
        {
            var indentation = "";
            for (var i = 0; i < indentLevel; ++i) indentation += "\t";
            generatedCode.AppendLine(indentation + "var " + blendTreeName + " = new BlendTree();");
            generatedCode.AppendLine(indentation + "AssetDatabase.AddObjectToAsset(" + blendTreeName +
                                     ", animatorController);");
            ...
            generatedCode.AppendLine(indentation + blendTreeName + ".useAutomaticThresholds = " +
                                     BoolString(blendTree.useAutomaticThresholds) + ";");
            for (var j = 0; j < blendTree.children.Length; ++j)
            {
                ...
                generatedCode.AppendLine(indentation + childName + ".cycleOffset = " +
                                         blendTree.children[j].cycleOffset + "f;");
                generatedCode.AppendLine(indentation + childName + ".directBlendParameter = \"" +
                                         blendTree.children[j].directBlendParameter + "\";");
                generatedCode.AppendLine(indentation + childName + ".mirror = " +
                                         BoolString(blendTree.children[j].mirror) + ";");
                generatedCode.AppendLine(indentation + childName + ".position = " +
                                         Vector2String(blendTree.children[j].position) + ";");
                generatedCode.AppendLine(indentation + childName + ".threshold = " + blendTree.children[j].threshold +
                                         "f;");
                generatedCode.AppendLine(indentation + childName + ".timeScale = " + blendTree.children[j].timeScale +
                                         "f;");
            }
            ...
            for (var j = 0; j < blendTree.children.Length; ++j)
            {
                var childName = blendTreeName + "Child" + j;
                generatedCode.AppendLine(
                    indentation + "\t" + childName + (j < blendTree.children.Length - 1 ? "," : ""));
            }
        ...
        /// <param name="parentStateMachineName">
        ///     The name of the parent to the stateMachineName. This is only used by state machine
        ///     transitions.
        /// </param>
        ...
        /// <param name="generatedCode">The final generated code.</param>
        private static void GenerateTransition(string stateMachineName, string parentStateMachineName,
            AnimatorTransitionBase transition, bool isStateTransition, bool isAnyStateTransition,
            bool isStateMachineTransition, StringBuilder generatedCode)
        {
            if (transition.destinationState == null && !transition.isExit) return;

            var transitionName = UniqueName(transition);
            if (isStateTransition || isAnyStateTransition)
            {
                if (isAnyStateTransition)
                    generatedCode.AppendLine("\t\t\tvar " + transitionName + " = " + stateMachineName +
                                             ".AddAnyStateTransition(" + UniqueName(transition.destinationState) +
                                             ");");
                    ...
                    generatedCode.AppendLine("\t\t\tvar " + transitionName + " = " + stateMachineName +
                                             ".AddTransition(" + UniqueName(transition.destinationState) + ");");
                var stateTransition = transition as AnimatorStateTransition;
                generatedCode.AppendLine("\t\t\t" + transitionName + ".canTransitionToSelf = " +
                                         BoolString(stateTransition.canTransitionToSelf) + ";");
                ...
                generatedCode.AppendLine("\t\t\t" + transitionName + ".hasExitTime = " +
                                         BoolString(stateTransition.hasExitTime) + ";");
                generatedCode.AppendLine("\t\t\t" + transitionName + ".hasFixedDuration = " +
                                         BoolString(stateTransition.hasFixedDuration) + ";");
                generatedCode.AppendLine("\t\t\t" + transitionName +
                                         ".interruptionSource = TransitionInterruptionSource." +
                                         stateTransition.interruptionSource + ";");
                generatedCode.AppendLine("\t\t\t" + transitionName + ".offset = " + stateTransition.offset + "f;");
                generatedCode.AppendLine("\t\t\t" + transitionName + ".orderedInterruption = " +
                                         BoolString(stateTransition.orderedInterruption) + ";");
            }
            else if (isStateMachineTransition)
            {
                generatedCode.AppendLine("\t\t\tvar " + transitionName + " = " + parentStateMachineName +
                                         ".AddStateMachineTransition(" + stateMachineName + ");");
                if (transition.destinationStateMachine != null)
                    generatedCode.AppendLine("\t\t\t" + transitionName + ".destinationStateMachine = " +
                                             UniqueName(transition.destinationStateMachine) + ";");
                if (transition.destinationState != null)
                    generatedCode.AppendLine("\t\t\t" + transitionName + ".destinationState = " +
                                             UniqueName(transition.destinationState) + ";");
            }
            else
            {
                generatedCode.AppendLine("\t\t\tvar " + transitionName + " = " + stateMachineName +
                                         ".AddEntryTransition(" + UniqueName(transition.destinationState) + ");");
            }
            ...
            for (var i = 0; i < transition.conditions.Length; ++i)
                generatedCode.AppendLine("\t\t\t" + transitionName + ".AddCondition(AnimatorConditionMode." +
                                         transition.conditions[i].mode + ", " +
                                         transition.conditions[i].threshold + "f, \"" +
                                         transition.conditions[i].parameter + "\");");
        }
        ...
        {
            public AnimatorTransitionBase Transition { get; }

            public bool AnyStateTransition { get; }

            public AnimatorTransition(AnimatorTransitionBase transition, bool anyStateTransition)
            {
                Transition = transition;
                AnyStateTransition = anyStateTransition;
            }
        }
    }
}