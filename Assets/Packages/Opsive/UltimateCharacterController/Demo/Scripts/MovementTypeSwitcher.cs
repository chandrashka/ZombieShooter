...
namespace Opsive.UltimateCharacterController.Demo
{
    ...
    {
        [Tooltip("The keycode that should trigger the switch.")] [SerializeField]
        protected KeyCode m_SwitchKeycode = KeyCode.Return;

        [Tooltip("Can the top down and 2.5D movement type be switched to with a third person perspective?")]
        [SerializeField]
        protected bool m_IncludeTopDownPseudo3D;
        ...
        private readonly string[] m_FirstPersonMovementStates = { "FirstPersonCombat", "FreeLook" };

        private readonly string[] m_ThirdPersonMovementStates =
            { "Adventure", "ThirdPersonCombat", "RPG", "TopDown", "2.5D" };
            ...
            if (Input.GetKeyDown(m_SwitchKeycode))
            {
                ...
                else
                {
                    ...
                    if (!m_IncludeTopDownPseudo3D)
                        ...
                        if (m_CharacterLocomotion.ActiveMovementType is TopDown ||
                            m_CharacterLocomotion.ActiveMovementType is Pseudo3D)
                            return;
#endif
                    UpdateMovementType(false,
                        (m_ActiveThirdPersonIndex + 1) %
                        (m_ThirdPersonMovementStates.Length - (m_IncludeTopDownPseudo3D ? 0 : 2)));
                }
            }
        }
