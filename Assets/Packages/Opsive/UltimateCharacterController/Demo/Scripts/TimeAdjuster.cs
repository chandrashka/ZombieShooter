...
namespace Opsive.UltimateCharacterController.Demo
{
    ...
    {
        [Tooltip("The TimeScale to adjust to.")] [SerializeField]
        protected float m_TimeScale = 0.5f;

        [Tooltip("The rate that the time scale changes from one value to another.")] [SerializeField]
        protected float m_TimeScaleChangeSpeed = 0.1f;

        [Tooltip("The length of time that the timescale is adjusted for.")] [SerializeField]
        protected float m_Duration = 5;

        [Tooltip("The LayerMask that triggers when the time is adjusted.")] [SerializeField]
        protected LayerMask m_LayerMask = 1 << LayerManager.Character;

        [Tooltip("The audio clip that should be played when the object enters the trigger.")] [SerializeField]
        protected AudioClip m_AudioClip;

        [Tooltip("The amount to rotate the object.")] [SerializeField]
        protected Vector3 m_RotationSpeed;

        [Tooltip("The amount that the object bounces.")] [SerializeField]
        protected float m_BounceAmount;

        [Tooltip("The speed at which the object bounces.")] [SerializeField]
        protected float m_BounceSpeed;
        ...
        private void Awake()
        {
            ...
            for (var i = 0; i < m_Transform.childCount; ++i) m_Children[i] = m_Transform.GetChild(i).gameObject;
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (characterLocomotion == null || characterLocomotion.TimeScale != 1) return;
            ...
            for (var i = 0; i < m_Children.Length; ++i) m_Children[i].SetActive(false);
        ...
        private void UpdateTimeScale(bool activate)
        {
            ...
            if (m_Time > 1)
            {
                ...
                {
                    for (var i = 0; i < m_Children.Length; ++i) m_Children[i].SetActive(true);
                }
            }
            else
            {
