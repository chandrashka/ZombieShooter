...
namespace Opsive.UltimateCharacterController.Demo
{
    ...
    {
        [Tooltip("A reference to Blitz.")] [SerializeField]
        protected GameObject m_Blitz;

        [Tooltip("The doors that should be locked when the ride ability is active.")] [SerializeField]
        protected Door[] m_Doors;

        [Tooltip("The objects that should be deactivated when the ride ability is active.")] [SerializeField]
        ...
        private void Awake()
        {
            ...
            EventHandler.RegisterEvent<Transform>(m_Blitz, "OnCharacterChangeMovingPlatforms",
        ...
        private void OnDestroy()
        {
            ...
            EventHandler.UnregisterEvent<Transform>(m_Blitz, "OnCharacterChangeMovingPlatforms",
                OnCharacterChangeMovingPlatforms);
        }
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            var characterLocomotion = other.gameObject.GetComponentInParent<UltimateCharacterLocomotion>();
            if (characterLocomotion == null) return;

            if (characterLocomotion.gameObject != m_Blitz) return;
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            for (var i = 0; i < m_Objects.Length; ++i) m_Objects[i].SetActive(!m_RideActive && !m_BlitzInTrigger);
        }
        ...
        private void OnAbilityActive(Ability ability, bool active)
        {
            ...
            for (var i = 0; i < m_Doors.Length; ++i) m_Doors[i].Locked = active;
            for (var i = 0; i < m_Objects.Length; ++i) m_Objects[i].SetActive(!active && !m_BlitzInTrigger);
            m_RideActive = active;
        }
        ...
        {
            for (var i = 0; i < m_Objects.Length; ++i) m_Objects[i].SetActive(movingPlatform != null);
        }
    }
}