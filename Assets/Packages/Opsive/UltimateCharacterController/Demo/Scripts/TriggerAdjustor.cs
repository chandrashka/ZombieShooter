...
{
    /// <summary>
    ///     Adjusts the size of the trigger when the character enters. This is useful for movement type triggers so the
    ///     character doesn't keep switching modes as the controls
    ///     change while on the edge of the trigger.
    /// </summary>
    public class TriggerAdjustor : MonoBehaviour
    {
        [Tooltip("Specifies the amount to adjust the BoxCollider center by.")] [SerializeField]
        protected Vector3 m_BoxColliderCenterAdjustment;
        ...
        private void Awake()
        {
            ...
            if (m_Collider is MeshCollider) m_OriginalMesh = (m_Collider as MeshCollider).sharedMesh;
        }
        ...
        {
            if (!m_AllowTriggerExit) return;

            if (m_ActiveObject == other.gameObject)
            {
                if (m_Collider is BoxCollider)
                    AdjustBoxCollider(m_Collider as BoxCollider, -m_BoxColliderCenterAdjustment,
                        -m_BoxColliderExpansion);
                else if (m_Collider is MeshCollider) (m_Collider as MeshCollider).sharedMesh = m_OriginalMesh;
                m_ActiveObject = null;
            }
        }
