...
namespace Opsive.UltimateCharacterController.Demo.Objects
{
    ...
    public class Door : MonoBehaviour
    {
        ...
        [Tooltip("The LayerMask of the character.")] [SerializeField]
        ...
        [Tooltip(
            "Should the door be permanently locked? A permanently locked door cannot be opened by the DemoManager.")]
        [FormerlySerializedAs("m_PermantlyLocked")]
        [SerializeField]
        protected bool m_PermanentlyLocked;

        [Tooltip("Should the door be opened at the start?")] [SerializeField]
        protected bool m_OpenAtStart;

        [Tooltip("Should the door close when the character leaves the trigger?")] [SerializeField]
        protected bool m_CloseOnTriggerExit = true;

        [Tooltip("The material that can change colors when the door is locked or unlcoked.")] [SerializeField]
        protected Material m_StatusMaterial;

        [Tooltip("The color of the closed door.")] [SerializeField]
        protected Color m_LockedColor = Color.red;

        [Tooltip("The color of the opened door.")] [SerializeField]
        protected Color m_UnlockedColor = Color.green;

        [Tooltip("The AudioClip that should play when the door is opened.")] [SerializeField]
        protected AudioClip m_OpenAudioClip;

        [Tooltip("The AudioClip that should play when the door is closed.")] [SerializeField]
        protected AudioClip m_CloseAudioClip;
        ...
        {
            get => m_PermanentlyLocked;
            set => m_PermanentlyLocked = value;
        }

        public bool CloseOnTriggerExit
        {
            get => m_CloseOnTriggerExit;
            set => m_CloseOnTriggerExit = value;
        }
        ...
        private void Awake()
        {
            ...
            if (demoManager != null) demoManager.RegisterDoor(this);
            ...
            for (var i = 0; i < renderers.Length; ++i)
            {
                ...
                for (var j = 0; j < materials.Length; ++j)
                {
                    ...
                    if (materials[j] != m_StatusMaterial) continue;

                    if (m_StatusMaterials == null)
                        m_StatusMaterials = new Material[1];
                    else
                        Array.Resize(ref m_StatusMaterials, m_StatusMaterials.Length + 1);
            ...
            if (m_PermanentlyLocked) m_Locked = true;
            if (m_OpenAtStart) OpenClose(true, true, false);
            UpdateDoorStatus();
        }
        ...
        private void OnTriggerEnter(Collider other)
        {
            if (!MathUtility.InLayerMask(other.gameObject.layer, m_LayerMask) ||
                other.gameObject.GetCachedParentComponent<UltimateCharacterLocomotion>() == null) return;
        ...
        {
            if (m_StatusMaterials != null)
                for (var i = 0; i < m_StatusMaterials.Length; ++i)
                {
        ...
        public void OpenClose(bool open, bool fromManager, bool playAudio)
        {
            ...
            if (m_PermanentlyLocked && open) return;

            // Don't close the door if the manager opened the door.
            if (!open && !fromManager && m_ManagerOpen) return;
            ...
            if (fromManager) m_ManagerOpen = open;
        }
    }
}