...
{
    /// <summary>
    ///     Moves the platform between two points. This gives an example of using the Move With Object ability and moving on an
    ///     object that is
    ...
    public class SimplePlatform : MonoBehaviour
    {
        ...
        [SerializeField]
        protected Vector3 m_RestingPosition;

        [Tooltip("The position that the platform should move towards when the character is on top of it.")]
        [SerializeField]
        protected Vector3 m_ActivePosition;

        [Tooltip("The speed that the platform should move.")] [SerializeField]
        protected float m_MoveSpeed = 0.05f;
        ...
        private void FixedUpdate()
        {
            ...
            m_Transform.position = Vector3.MoveTowards(m_Transform.position,
                m_MoveWithObjectAbility != null ? m_ActivePosition : m_RestingPosition, m_MoveSpeed);
        }
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if ((characterLocomotion = other.GetComponentInParent<UltimateCharacterLocomotion>()) == null) return;

            m_MoveWithObjectAbility = characterLocomotion.GetAbility<MoveWithObject>();
            if (m_MoveWithObjectAbility == null) return;
        ...
        private void OnTriggerExit(Collider other)
        {
            if (!MathUtility.InLayerMask(other.gameObject.layer, 1 << LayerManager.Character) ||
                m_MoveWithObjectAbility == null) return;
