...
namespace Opsive.UltimateCharacterController.Demo.Objects
{
    ...
    {
        [Tooltip("The location that the character will teleport to.")] [SerializeField]
        protected Transform m_Destination;

        [Tooltip("Should the character's animator be snapped when teleporting to the destination?")] [SerializeField]
        protected bool m_SnapAnimator;

        [Tooltip("The LayerMask of the character.")] [SerializeField]
        protected LayerMask m_LayerMask = 1 << LayerManager.Character;

        [Tooltip("The AudioClip that should play when the character is teleported.")] [SerializeField]
        protected AudioClip m_TeleportAudioClip;

        [Tooltip("The name of the state that should activate when the character teleports.")] [SerializeField]
        ...
        public bool IgnoreCharacterEnter
        {
            set => m_IgnoreCharacterEnter = value;
        }
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if ((characterLocomotion = other.GetComponentInParent<UltimateCharacterLocomotion>()) != null)
            {
                ...
                if (characterLocomotion.IsAbilityTypeActive<Drive>() ||
                    characterLocomotion.IsAbilityTypeActive<Ride>())
                    return;

                var destinationTeleporter = m_Destination.GetComponentInParent<Teleporter>();
                if (destinationTeleporter != null) destinationTeleporter.IgnoreCharacterEnter = true;
                characterLocomotion.SetPositionAndRotation(m_Destination.position, m_Destination.rotation,
                    m_SnapAnimator, false);
