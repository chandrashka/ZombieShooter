...
namespace Opsive.Shared.Input
{
...
#endif
    {
        ...
        [Tooltip("The name of the horizontal camera input mapping.")] [SerializeField]
        protected string m_HorizontalLookInputName = "Mouse X";

        [Tooltip("The name of the vertical camera input mapping.")] [SerializeField]
        protected string m_VerticalLookInputName = "Mouse Y";

        [Tooltip("Specifies how the look vector is assigned.")] [SerializeField]
        protected LookVectorMode m_LookVectorMode = LookVectorMode.Smoothed;

        [Tooltip(
            "If using look smoothing, specifies how sensitive the mouse is. The higher the value the more sensitive.")]
        [SerializeField]
        protected Vector2 m_LookSensitivity = new(2f, 2f);

        [Tooltip("If using look smoothing, specifies a multiplier to apply to the LookSensitivity value.")]
        [SerializeField]
        protected float m_LookSensitivityMultiplier = 1;

        [Tooltip("If using look smoothing, the amount of history to store of previous look values.")] [SerializeField]
        protected int m_SmoothLookSteps = 20;

        [Tooltip(
            "If using look smoothing, specifies how much weight each element should have on the total smoothed value (range 0-1).")]
        [SerializeField]
        protected float m_SmoothLookWeight = 0.5f;

        [Tooltip("If using look smoothing, specifies an exponent to give a smoother feel with smaller inputs.")]
        [SerializeField]
        protected float m_SmoothExponent = 1.05f;

        [Tooltip(
            "If using look smoothing, specifies a maximum acceleration value of the smoothed look value (0 to disable).")]
        [SerializeField]
        protected float m_LookAccelerationThreshold = 0.4f;
        ...
        public string HorizontalLookInputName
        {
            get => m_HorizontalLookInputName;
            set => m_HorizontalLookInputName = value;
        }

        public string VerticalLookInputName
        {
            get => m_VerticalLookInputName;
            set => m_VerticalLookInputName = value;
        }

        public LookVectorMode LookMode
        {
            get => m_LookVectorMode;
            set
            {
        ...
        {
            get => m_LookSensitivityMultiplier;
            set => m_LookSensitivityMultiplier = value;
        }

        public int SmoothLookSteps
        {
            get => m_SmoothLookSteps;
            set => m_SmoothLookSteps = value;
        }

        public float SmoothLookWeight
        {
            get => m_SmoothLookWeight;
            set => m_SmoothLookWeight = value;
        }

        public float SmoothExponent
        {
            get => m_SmoothExponent;
            set => m_SmoothExponent = value;
        }

        public float LookAccelerationThreshold
        {
            get => m_LookAccelerationThreshold;
            set => m_LookAccelerationThreshold = value;
        }
...
#endif
        public UnityBoolEvent EnableGameplayInputEvent
        {
            get => m_EnableGamplayInputEvent;
            set => m_EnableGamplayInputEvent = value;
        }
        ...
        {
            set => m_RawLookVector = value;
        }

        public Vector2 CurrentLookVector
        {
            set => m_CurrentLookVector = value;
        }

        public bool ControllerConnected { get; private set; }

        protected virtual bool CanCheckForController => true;
        ...
        {
#endif
            if (m_LookVectorMode == LookVectorMode.Smoothed) m_SmoothLookBuffer = new Vector2[m_SmoothLookSteps];
        ...
        /// <param name="buttonName">The name of the button.</param>
        /// <returns>True of the button is being pressed.</returns>
        protected virtual bool GetButtonInternal(string buttonName)
        {
            return false;
        }
        ...
        /// <param name="buttonName">The name of the button.</param>
        /// <returns>True if the button is pressed this frame.</returns>
        protected virtual bool GetButtonDownInternal(string buttonName)
        {
            return false;
        }
        ...
        {
            return GetButtonUpInternal(buttonName);
        }
        ...
        /// <param name="buttonName">The name of the button.</param>
        /// <returns>True if the button is up.</returns>
        protected virtual bool GetButtonUpInternal(string buttonName)
        {
            return false;
        }
        ...
        /// <param name="buttonName">The button name to check for a double press.</param>
        /// <returns>True if a double press occurred (double click or double tap).</returns>
        protected virtual bool GetDoublePressInternal(string buttonName)
        {
            return false;
        }
        ...
        public bool GetTap(string buttonName)
        {
            ...
            {
                if (m_ButtonDownTime == null) m_ButtonDownTime = new Dictionary<string, float>();
                if (!m_ButtonDownTime.ContainsKey(buttonName)) m_ButtonDownTime.Add(buttonName, Time.unscaledTime);
            }
        ...
        ///     Indicates if the long press should occur after the button has been released (true) or
        ///     after the duration (false).
        /// </param>
        ...
        public bool GetLongPress(string buttonName, float duration, bool waitForRelease)
        {
            ...
            if (GetButtonInternal(buttonName))
            {
                if (m_ButtonDownTime.TryGetValue(buttonName, out var downTime))
                {
                    ...
                    if (upTime > downTime) m_ButtonDownTime[buttonName] = downTime = Time.unscaledTime;
        ...
        {
            return GetAxisInternal(buttonName);
        }
        ...
        /// <param name="buttonName">The name of the axis.</param>
        /// <returns>The value of the axis.</returns>
        protected virtual float GetAxisInternal(string buttonName)
        {
            return 0;
        }
        ...
        /// <param name="buttonName">The name of the axis.</param>
        /// <returns>The value of the raw axis.</returns>
        protected virtual float GetAxisRawInternal(string buttonName)
        {
            return 0;
        }
        ...
        {
            if (!CanCheckForController) return;

            var controllerConencted = UnityEngine.Input.GetJoystickNames().Length > 0;
            if (ControllerConnected != controllerConencted)
            {
                ControllerConnected = controllerConencted;
#if FIRST_PERSON_CONTROLLER || THIRD_PERSON_CONTROLLER
                if (!string.IsNullOrEmpty(m_ConnectedControllerState))
                    StateManager.SetState(gameObject, m_ConnectedControllerState, ControllerConnected);
#endif
                EventHandler.ExecuteEvent(gameObject, "OnInputControllerConnected", ControllerConnected);
            }
            ...
            if (m_ControllerConnectedCheckRate > 0 &&
                (m_ControllerCheckEvent == null || !m_ControllerCheckEvent.Active))
        ...
        private void FixedUpdate()
        {
            ...
            if (m_LookVectorMode == LookVectorMode.Smoothed)
            {
                ...
                for (var i = 0; i < m_SmoothLookBufferCount; ++i)
                {
                    ...
                    if (deltaTime > 0) weight *= m_SmoothLookWeight / deltaTime;
                }
                ...
                if (m_LookAccelerationThreshold > 0 && deltaTime != 0)
                {
                    ...
                    if (lookAcceleration > m_LookAccelerationThreshold) lookAcceleration = m_LookAccelerationThreshold;
                }

                // Determine the final value.
                m_CurrentLookVector.x *= (m_LookSensitivity.x * m_LookSensitivityMultiplier + lookAcceleration) *
                                         TimeUtility.FramerateDeltaTime;
                m_CurrentLookVector.y *= (m_LookSensitivity.y * m_LookSensitivityMultiplier + lookAcceleration) *
                                         TimeUtility.FramerateDeltaTime;

                m_CurrentLookVector.x = Mathf.Sign(m_CurrentLookVector.x) *
                                        Mathf.Pow(Mathf.Abs(m_CurrentLookVector.x), m_SmoothExponent);
                m_CurrentLookVector.y = Mathf.Sign(m_CurrentLookVector.y) *
                                        Mathf.Pow(Mathf.Abs(m_CurrentLookVector.y), m_SmoothExponent);
            }
        ...
        {
            if (smoothed) return m_CurrentLookVector;
