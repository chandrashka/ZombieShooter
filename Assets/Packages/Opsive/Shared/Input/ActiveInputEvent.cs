...
namespace Opsive.Shared.Input
{
    ...
    public class ActiveInputEvent
    {
        ...
        ///     Specifies how long the button should be pressed until the event is executed. Only used
        ///     with a Type of LongPress.
        /// </param>
        /// <param name="waitForLongPressRelease">Should the long press wait to be activated until the button has been released?</param>
        /// <param name="eventName">The event to execute.</param>
        public void Initialize(Type inputType, string inputName, float longPressDuration, bool waitForLongPressRelease,
            string eventName)
        {
        ...
        public bool HasButtonEvent(PlayerInput playerInput)
        {
            if ((m_InputType == Type.ButtonDown && playerInput.GetButtonDown(m_InputName)) ||
                ...
                (m_InputType == Type.LongPress &&
                 playerInput.GetLongPress(m_InputName, m_LongPressDuration, m_WaitForLongPressRelease)))
                return true;
            return false;
        }
        ...
        {
            if (m_InputType == Type.Axis) return true;
            return false;
        }
