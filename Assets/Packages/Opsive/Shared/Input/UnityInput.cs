...
{
    /// <summary>
    ///     Acts as a common base class for input using the Unity Input Manager. Works with keyboard/mouse, controller, and
    ///     mobile input.
    /// </summary>
    public class UnityInput : PlayerInput
    {
        ...
        [Range(0, 1)]
        [SerializeField]
        protected float m_JoystickUpThreshold = 1;
        ...
        private void LateUpdate()
        {
            ...
            if (IsControllerConnected())
            {
                ...
                if (m_JoystickDownSet != null)
                {
                    ...
                    foreach (var item in m_ToRemoveJoystickDownSet) m_JoystickDownSet.Remove(item);
                ...
                if (m_ToAddJoystickDownSet != null && m_ToAddJoystickDownSet.Count > 0)
                {
                    ...
                    foreach (var item in m_ToAddJoystickDownSet) m_JoystickDownSet.Add(item);
            ...
            if (m_EnableCursorWithEscape && UnityEngine.Input.GetKeyDown(KeyCode.Escape))
            {
                Cursor.lockState = CursorLockMode.None;
                Cursor.visible = true;
                if (m_PreventLookVectorChanges) OnApplicationFocus(false);
            }
            else if (Cursor.visible && m_DisableCursor && !IsPointerOverUI() &&
                     (UnityEngine.Input.GetKeyDown(KeyCode.Mouse0) || UnityEngine.Input.GetKeyDown(KeyCode.Mouse1)))
            {
                Cursor.lockState = CursorLockMode.Locked;
                Cursor.visible = false;
                if (m_PreventLookVectorChanges) OnApplicationFocus(true);
            }
#if UNITY_EDITOR
            // The cursor should be visible when the game is paused.
            if (!Cursor.visible && EditorApplication.isPaused && !m_DisableCursor)
            {
                Cursor.lockState = CursorLockMode.None;
                Cursor.visible = true;
            }
        ...
        {
            if (m_Input.GetButton(buttonName, InputBase.ButtonAction.GetButton)) return true;
            if (IsControllerConnected())
                if (Mathf.Abs(m_Input.GetAxisRaw(buttonName)) == 1)
                {
                    if (m_JoystickDownSet == null || !m_JoystickDownSet.Contains(buttonName))
                    {
                        if (m_ToAddJoystickDownSet == null) m_ToAddJoystickDownSet = new HashSet<string>();
        ...
        protected override bool GetButtonDownInternal(string buttonName)
        {
            if (IsControllerConnected() && Mathf.Abs(m_Input.GetAxisRaw(buttonName)) == 1)
            {
                ...
                if (m_JoystickDownSet != null && m_JoystickDownSet.Contains(buttonName)) return false;
                if (m_ToAddJoystickDownSet == null) m_ToAddJoystickDownSet = new HashSet<string>();
                m_ToAddJoystickDownSet.Add(buttonName);
                return true;
            }
        ...
        protected override bool GetButtonUpInternal(string buttonName)
        {
            ...
            {
                if (Mathf.Abs(m_Input.GetAxisRaw(buttonName)) == 1 &&
                    (m_JoystickDownSet == null || !m_JoystickDownSet.Contains(buttonName)))
                {
                    if (m_ToAddJoystickDownSet == null) m_ToAddJoystickDownSet = new HashSet<string>();
                    m_ToAddJoystickDownSet.Add(buttonName);
                    return false;
                ...
                if (m_JoystickDownSet != null && m_JoystickDownSet.Contains(buttonName) &&
                    Mathf.Abs(m_Input.GetAxisRaw(buttonName)) < m_JoystickUpThreshold) return true;
                return false;
            }
