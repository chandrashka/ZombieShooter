...
namespace Opsive.Shared.Editor.Inspectors.StateSystem
{
    ...
    public static class StateInspector
    {
        ...
        /// </summary>
        public static ReorderableList DrawStates(ReorderableList reorderableList, SerializedObject serializedObject,
            SerializedProperty states, string selectedIndexKey,
            ReorderableList.ElementCallbackDelegate drawCallback, ReorderableList.AddCallbackDelegate addCallback,
            ReorderableList.ReorderCallbackDelegate reorderCallback,
            ReorderableList.RemoveCallbackDelegate removeCallback)
        {
            // Initialize the reorder list on first run.
            if (reorderableList == null)
            {
                reorderableList = new ReorderableList(serializedObject, states, !Application.isPlaying, true,
                    !Application.isPlaying, !Application.isPlaying && states.arraySize > 1);
                reorderableList.drawHeaderCallback = rect =>
                {
                    ...
                    var blockedByWidth = Mathf.Max(c_MinBlockedByWidth, Mathf.Min(c_MaxBlockedByWidth, fieldWidth)) +
                                         EditorGUI.indentLevel * InspectorUtility.IndentWidth;
                    ...
                    var presetWidth = Mathf.Min(c_MaxPresetWidth, fieldWidth) +
                                      EditorGUI.indentLevel * InspectorUtility.IndentWidth * 2;
                    var nameWidth =
                        Mathf.Max(0,
                            rect.width - presetWidth - blockedByWidth - persistWidth - activateWidth -
                            c_WidthBuffer * 6) + EditorGUI.indentLevel * InspectorUtility.IndentWidth * 3;
                    var startRectX = rect.x;

                    EditorGUI.LabelField(new Rect(startRectX, rect.y + 1, nameWidth, EditorGUIUtility.singleLineHeight),
                        "Name");
                    startRectX += nameWidth + c_WidthBuffer - EditorGUI.indentLevel * InspectorUtility.IndentWidth;
                    EditorGUI.LabelField(
                        new Rect(startRectX, rect.y + 1, presetWidth, EditorGUIUtility.singleLineHeight), "Preset");
                    startRectX += presetWidth - c_WidthBuffer * 3 -
                                  EditorGUI.indentLevel * InspectorUtility.IndentWidth;
                    EditorGUI.LabelField(
                        new Rect(startRectX, rect.y + 1,
                            blockedByWidth + EditorGUI.indentLevel * InspectorUtility.IndentWidth,
                            EditorGUIUtility.singleLineHeight), "Blocked By");
                    startRectX += blockedByWidth + c_WidthBuffer * 5 -
                                  EditorGUI.indentLevel * InspectorUtility.IndentWidth;
                    EditorGUI.LabelField(
                        new Rect(startRectX, rect.y + 1,
                            persistWidth + EditorGUI.indentLevel * InspectorUtility.IndentWidth,
                            EditorGUIUtility.singleLineHeight), "Persist");
                    startRectX += persistWidth;
                    EditorGUI.LabelField(
                        new Rect(startRectX, rect.y + 1,
                            activateWidth + EditorGUI.indentLevel * InspectorUtility.IndentWidth,
                            EditorGUIUtility.singleLineHeight), "Activate");
                };
                ...
                reorderableList.onSelectCallback = list => { EditorPrefs.SetInt(selectedIndexKey, list.index); };
        ...
        /// </summary>
        public static void OnStateListDraw(object obj, State[] states, Rect rect, int index)
        {
        ...
        ///     Draws all of the added states.
        /// </summary>
        public static void OnStateListDraw(object obj, State[] states, SerializedProperty statesProperty, Rect rect,
            int index)
        {
            ...
            if (!Application.isPlaying && index == states.Length - 1)
            {
                ...
                {
                    states[index] = state = new State("Default", true);
                    GUI.changed = true;
                }
            ...
            var nameWidth =
                Mathf.Max(0,
                    rect.width - presetWidth - blockedByWidth - persistWidth - activateWidth - c_WidthBuffer * 6) +
                EditorGUI.indentLevel * 45;
            var startRectX = rect.x;

            // The state name has to be unique. 
            var active = state.Active && !state.IsBlocked();
            var desiredName = EditorGUI.TextField(
                new Rect(startRectX, rect.y + 1, nameWidth, EditorGUIUtility.singleLineHeight), state.Name +
            ...
            if (!Application.isPlaying && desiredName != state.Name && IsUniqueName(states, desiredName))
            {
                ...
                for (var i = 0; i < states.Length; ++i)
                {
                    ...
                    if (states[i].BlockList != null)
                        for (var j = 0; j < states[i].BlockList.Length; ++j)
                            if (states[i].BlockList[j] == state.Name)
                            {
                                if (stateProperty != null)
                                    statesProperty.GetArrayElementAtIndex(i).FindPropertyRelative("m_BlockList")
                                        .GetArrayElementAtIndex(j).stringValue = desiredName;
                                else
            ...
            var desiredPreset = EditorGUI.ObjectField(new Rect(startRectX, rect.y + 1, presetWidth,
                    EditorGUIUtility.singleLineHeight), string.Empty, state.Preset, typeof(PersistablePreset),
                false) as PersistablePreset;
            if (desiredPreset != null)
            {
                ...
                {
                    Debug.LogError(
                        $"Error: Unable to add preset. {desiredPreset.name} ({desiredPreset.Data.ObjectType}) doesn't use the same object type ({obj.GetType().FullName}).");
                }
            }
            ...
            for (var i = 0; i < states.Length; ++i)
            {
                ...
                if (currentStateBlockList != null)
                    for (var j = 0; j < currentStateBlockList.Length; ++j)
            ...
            if (allStates.Count == 0) allStates.Add("Nothing");

            // Draw the actual popup.
            var blockMask =
                EditorGUI.MaskField(new Rect(startRectX, rect.y + 1, blockedByWidth, EditorGUIUtility.singleLineHeight),
                    string.Empty, selected, allStates.ToArray());
            if (blockMask != selected)
            {
                var stateNames = new List<string>();
                var blockListProperty =
                    stateProperty != null ? stateProperty.FindPropertyRelative("m_BlockList") : null;
                if (blockListProperty != null) blockListProperty.ClearArray();
                for (var i = 0; i < allStates.Count; ++i)
                    ...
                    if (((1 << i) & blockMask) != 0 || blockMask == -1)
                    {
                        if (blockListProperty != null)
                        {
                            ...
                            blockListProperty.GetArrayElementAtIndex(blockListProperty.arraySize - 1).stringValue =
                                allStates[i];
                        }
                        else
                        {
                ...
                if (blockListProperty == null) state.BlockList = stateNames.ToArray();
            }
            ...
            if (GUI.Button(new Rect(startRectX + persistWidth / 2, rect.y + 1, 18, EditorGUIUtility.singleLineHeight),
                    InspectorStyles.PersistIcon, InspectorStyles.NoPaddingButtonStyle))
            {
                ...
                for (var i = 0; i < desiredPreset.Data.ValueHashes.Length; ++i)
                ...
                var bitwiseHash = new Version(desiredPreset.Data.Version).CompareTo(new Version("3.1")) >= 0;
                // Remove and add the properties that are being serialized.
                for (var i = 0; i < properties.Length; ++i)
                {
                    var hash = Serialization.StringHash(properties[i].PropertyType.FullName) +
                    ...
                    if (valuePositionMap.ContainsKey(hash))
                    {
                        ...
                        if (!typeof(Object).IsAssignableFrom(property.PropertyType))
                        {
                            ...
                            Serialization.GetUnityObjectIndexes(ref unityObjectIndexes, property.PropertyType,
                                property.Name, 0, valuePositionMap, desiredPreset.Data.ValueHashes,
                                desiredPreset.Data.ValuePositions,
                                desiredPreset.Data.Values, false, MemberVisibility.Public, bitwiseHash);

                            Serialization.RemoveProperty(i, unityObjectIndexes, desiredPreset.Data,
                                MemberVisibility.Public, bitwiseHash);

                            // Get the current value of the active object.
                            var getMethod = property.GetGetMethod();
                            if (getMethod != null) value = getMethod.Invoke(obj, null);
                            // Add the property back with the updated value.
                            Serialization.AddProperty(property, value, unityObjectIndexes, desiredPreset.Data,
            ...
            GUI.enabled = Application.isPlaying && index < states.Length - 1;
            if (GUI.Button(new Rect(startRectX + activateWidth / 2, rect.y + 1, 18, EditorGUIUtility.singleLineHeight),
                    InspectorStyles.ActivateIcon, InspectorStyles.NoPaddingButtonStyle))
        ...
        /// </summary>
        private static bool IsUniqueName(State[] states, string name)
        {
        ...
        /// </summary>
        public static void OnStateListAdd(GenericMenu.MenuFunction addExistingCallback,
            GenericMenu.MenuFunction createNewCallback)
        {
        ...
        /// </summary>
        public static State[] AddExistingPreset(Type objType, State[] states, ReorderableList reorderableList,
            string selectedIndexKey)
        {
            ...
            {
                EditorPrefs.SetString(c_EditorPrefsLastPresetPathKey, Path.GetDirectoryName(path));
                ...
                {
                    Debug.LogError(
                        $"Error: Unable to add preset. {Path.GetFileName(path)} isn't located within the same project directory.");
                    return states;
                }
                ...
                {
                    Debug.LogError(
                        $"Error: Unable to add preset. {preset.name} ({preset.Data.ObjectType}) doesn't use the same object type ({objType.FullName}).");
                }
            }
        ...
        /// </summary>
        public static State[] CreatePreset(object target, State[] states, ReorderableList reorderableList,
            string selectedIndexKey)
        {
                ...
                if (path.Length != 0 && Application.dataPath.Length < path.Length)
                {
                    ...
                    if (existingPreset != null)
                    {
                        EditorUtility.DisplayDialog("Unable to Save Preset",
                            "The preset must reference a unique file name.", "Okay");
                        return states;
                    }
        ...
        /// </summary>
        private static State[] InsertStateElement(State[] states, ReorderableList reorderableList,
            string selectedIndexKey, string name, PersistablePreset preset)
        {
            // The name has to be unique to prevent it from interferring with other state names.
            if (!IsUniqueName(states, name))
            {
                var postfixIndex = 1;
                while (!IsUniqueName(states, name + " " + postfixIndex)) postfixIndex++;
                name += " " + postfixIndex;
            }

            // Create the element.
            var state = new State(name, false);
            state.Preset = preset;
        ...
        /// </summary>
        public static State[] OnStateListReorder(State[] states)
        {
        ...
        /// </summary>
        public static void SwapElements(State[] states, int origIndex, int newIndex)
        {
        ...
        /// </summary>
        public static State[] OnStateListRemove(State[] states, string selectedIndexKey,
        ...
        ///     The ReordableList remove button has been pressed. Remove the selected state.
        /// </summary>
        public static State[] OnStateListRemove(State[] states, SerializedProperty statesProperty,
            string selectedIndexKey, ReorderableList reorderableList)
        {
            ...
            for (var i = 0; i < states.Length; ++i)
            {
                ...
                if (state.BlockList != null && state.BlockList.Length > 0)
                {
                    ...
                    var blockListProperty = statesProperty != null
                        ? statesProperty.GetArrayElementAtIndex(i).FindPropertyRelative("m_BlockList")
                        : null;
                    for (var j = blockList.Count - 1; j > -1; --j)
                        if (blockList[j] == states[reorderableList.index].Name)
                        {
                            ...
                            if (blockListProperty != null) blockListProperty.DeleteArrayElementAtIndex(j);
                        }
            ...
            if (statesProperty != null) statesProperty.DeleteArrayElementAtIndex(reorderableList.index);
            reorderableList.index = reorderableList.index - 1;
            if (reorderableList.index == -1 && stateList.Count > 0) reorderableList.index = 0;
            reorderableList.displayRemove = stateList.Count > 1;
            return stateList.ToArray();
        }
        ...
        public static void UpdateDefaultStateValues(State[] states)
        {
