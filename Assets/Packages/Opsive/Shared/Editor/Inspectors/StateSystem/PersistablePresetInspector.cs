...
namespace Opsive.Shared.Editor.Inspectors.StateSystem
{
    ...
    {
        private readonly List<PropertyInfo> m_AvailableProperies = new();
        ...
        private void OnEnable()
        {
            ...
            {
                var mainAsset = AssetDatabase.LoadAssetAtPath(AssetDatabase.GetAssetPath(target),
                    TypeUtility.GetType("Opsive.UltimateCharacterController.StateSystem.StateConfiguration"));
                if (mainAsset != null) m_Visiblity = MemberVisibility.AllPublic;
            }
        ...
        private void InitializeAvailablePropertyArray()
        {
            ...
            if (objType != null)
            {
                ...
                for (var i = 0; i < preset.Data.ValueHashes.Length; ++i)
                ...
                for (var i = 0; i < properties.Length; ++i)
                {
                    ...
                    if (!valuePositionMap.ContainsKey(hash))
                    {
                        ...
                        if (Serialization.GetValidGetMethod(properties[i], m_Visiblity) == null) continue;

                        // The property is valid. Add it to the list.
                        availablePropertyNames.Add(EditorUtility.SplitCamelCase(properties[i].Name));
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (objType != null)
            {
                ...
                ObjectInspector.DrawProperties(objType, null, 0, valuePositionMap, preset.Data, m_Visiblity,
                    () => { GUILayout.BeginHorizontal(); }, (index, unityObjectIndexes) =>
                    {
                        EditorUtility.RecordUndoDirtyObject(preset, "Change Value");
                        var removed = false;
            ...
            GUILayout.FlexibleSpace();
            GUI.enabled =
                m_AvailableProperies.Count > 0 &&
                !Application.isPlaying; // Only allow the popup if properties can be selected.
        ...
        {
            var bitwiseHash = new Version((target as PersistablePreset).Data.Version).CompareTo(new Version("3.1")) >=
                              0;
