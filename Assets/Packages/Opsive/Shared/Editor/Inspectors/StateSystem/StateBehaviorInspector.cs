...
namespace Opsive.Shared.Editor.Inspectors.StateSystem
{
    ...
    public class StateBehaviorInspector : InspectorBase
    {
        ...
        private readonly List<List<FieldInfo>> m_FoldoutFields = new();
        private readonly List<string> m_FoldoutNames = new();

        private ReorderableList m_ReorderableStateList;
        private string SelectedIndexKey => c_EditorPrefsSelectedIndexKey + "." + target.GetType() + "." + target.name;
        ...
        protected virtual void OnEnable()
        {
            ...
            if (GetType() != typeof(StateBehaviorInspector)) return;
            ...
            var allFields =
                new List<FieldInfo>(Serialization.GetSerializedFields(target.GetType(), MemberVisibility.Public));
            var foldoutMap = new Dictionary<string, int>();
            var foldoutIndex = 0;
            for (var i = 0; i < allFields.Count; ++i)
            {
                // Do not show HideInInspector fields.
                if (TypeUtility.GetAttribute(allFields[i], typeof(HideInInspector)) != null) continue;

                // Create a new foldout if there is an InspectorFoldout Attribute.
                var foldoutAttribute =
                    allFields[i].GetCustomAttributes(typeof(InspectorFoldout), false) as InspectorFoldout[];
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (callback != null)
                ...
                for (var i = 0; i < m_FoldoutFields.Count; ++i)
                    if (m_FoldoutFields[i].Count > 0)
                    {
                        if (!string.IsNullOrEmpty(m_FoldoutNames[i]))
                        {
                            ...
                            if (Foldout(m_FoldoutNames[i]))
                            {
                                ...
                                for (var j = 0; j < m_FoldoutFields[i].Count; ++j)
            ...
            if (Foldout("States"))
                m_ReorderableStateList = StateInspector.DrawStates(m_ReorderableStateList, serializedObject,
                    PropertyFromName("m_States"),
        ...
        private void OnStateListDraw(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            StateInspector.OnStateListDraw(target, (target as StateBehavior).States, PropertyFromName("m_States"), rect,
                index);
        ...
        private void AddExistingPreset()
        {
            ...
            stateComponent.States = StateInspector.AddExistingPreset(stateComponent.GetType(), stateComponent.States,
                m_ReorderableStateList, SelectedIndexKey);
        ...
        private void CreatePreset()
        {
            ...
            var stateComponent = target as StateBehavior;
            stateComponent.States = StateInspector.CreatePreset(target, stateComponent.States, m_ReorderableStateList,
                SelectedIndexKey);
            if (EditorGUI.EndChangeCheck())
            {
                EditorUtility.RecordUndoDirtyObject(target, "Change Value");
        ...
        private void OnStateListRemove(ReorderableList list)
        {
            ...
            var stateComponent = target as StateBehavior;
            stateComponent.States = StateInspector.OnStateListRemove(stateComponent.States,
                PropertyFromName("m_States"), SelectedIndexKey, list);
