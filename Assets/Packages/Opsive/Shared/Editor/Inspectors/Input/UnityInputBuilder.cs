...
namespace Opsive.Shared.Editor.Inspectors.Input
{
    ...
    public class UnityInputBuilder
    {
        ...
        public static Dictionary<string, int> FoundAxes { get; set; }
        ...
        public static void AddInputAxis(SerializedProperty axesProperty, string name, string negativeButton,
            string positiveButton,
            string altNegativeButton, string altPositiveButton, float gravity, float dead, float sensitivity, bool snap,
            bool invert, AxisType axisType, AxisNumber axisNumber)
        {
        ...
        /// <summary>
        ///     Searches for a property with the given name and axis type within the axes property array. If no property is found
        ...
        public static SerializedProperty FindAxisProperty(SerializedProperty axisProperty, string name,
            bool autoCreate = true)
        {
            ...
            if (FoundAxes == null) FoundAxes = new Dictionary<string, int>();
            FoundAxes.TryGetValue(name, out var existingCount);
            var localCount = 0;
            for (var i = 0; i < axisProperty.arraySize; ++i)
            {
            ...
            if (existingCount == 0)
                FoundAxes.Add(name, 1);
            else
                FoundAxes[name] = existingCount + 1;
