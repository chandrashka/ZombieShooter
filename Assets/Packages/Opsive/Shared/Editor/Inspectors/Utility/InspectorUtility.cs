...
namespace Opsive.Shared.Editor.Inspectors.Utility
{
    ...
    public static class InspectorUtility
    {
        ...
        private static readonly Dictionary<object, bool> s_FoldoutValueMap = new();
        private static readonly Dictionary<TypeWithCondition, List<Type>> s_ObjectTypes = new();
        public static int IndentWidth => c_IndentWidth;
        ...
        public static bool Foldout(object obj, GUIContent guiContent, bool defaultExpanded, string identifyingString)
        {
            ...
            var key = c_EditorPrefsFoldoutKey + "." + obj.GetType() +
                      (obj is MonoBehaviour ? "." + (obj as MonoBehaviour).name : string.Empty) + "." +
                      identifyingString + "." + guiContent.text;
            bool prevFoldout;
        ...
        {
            T component;
            if ((component = gameObject.GetComponent<T>()) == null) return gameObject.AddComponent<T>();
            return component;
        }
        ...
        public static void AddObjectType(Type type, Func<Type, bool> condition, GenericMenu.MenuFunction2 addCallback)
        {
            ...
            for (var i = 0; i < typeList.Count; ++i)
                addMenu.AddItem(new GUIContent(EditorUtility.SplitCamelCase(typeList[i].Name)), false, addCallback,
                    typeList[i]);
