...
namespace Opsive.Shared.Editor.Inspectors.Utility
{
    ...
    public static class ObjectInspector
    {
        ...
        ///     The prefix of the hash from the parent class. This value will prevent collisions with
        ...
        private static object DrawFields(object obj, bool drawNoFieldsNotice, int hashPrefix)
        {
            ...
            for (var i = 0; i < fields.Length; ++i)
            {
                ...
                if (TypeUtility.GetAttribute(fields[i], typeof(HideInInspector)) != null) continue;
                ...
                if (tooltip != null)
                    guiContent = new GUIContent(EditorUtility.SplitCamelCase(fields[i].Name), tooltip.tooltip);
                else
                    guiContent = new GUIContent(EditorUtility.SplitCamelCase(fields[i].Name));
                var value = fields[i].GetValue(obj);
                value = DrawObject(guiContent, fields[i].FieldType, obj, value, fields[i].Name, hash, null, null,
                    fields[i], true);
                // Update the object if the value was changed.
                if (EditorGUI.EndChangeCheck()) fields[i].SetValue(obj, value);
                fieldsDrawn = true;
            }

            // Draw a notice if no fields were drawn.
            if (drawNoFieldsNotice && !fieldsDrawn) EditorGUILayout.LabelField("(No Visible Fields)");
        ...
        public static object DrawProperties(Type type, object obj, int hashPrefix,
            Dictionary<int, int> valuePositionMap, Serialization serialization, MemberVisibility visibility,
            Action startDrawElementCallback, Func<int, List<int>, bool> endDrawElementCallback)
        {
            ...
            for (var i = 0; i < properties.Length; ++i)
            {
                int hash;
                if (bitwiseHash)
                    hash = (hashPrefix * Serialization.HashMultiplier) ^
                           (Serialization.StringHash(properties[i].PropertyType.FullName) +
                            Serialization.StringHash(properties[i].Name));
                else
                    hash = hashPrefix + Serialization.StringHash(properties[i].PropertyType.FullName) +
                           Serialization.StringHash(properties[i].Name);
                // The value may not be serialized.
                if (!valuePositionMap.ContainsKey(hash)) continue;

                // Issue the start callback before the value is drawn.
                if (startDrawElementCallback != null) startDrawElementCallback();
                var value = Serializer.BytesToValue(properties[i].PropertyType, properties[i].Name, valuePositionMap,
                    hashPrefix, serialization.Values,
                ...
                Serialization.GetUnityObjectIndexes(ref unityObjectIndexes, properties[i].PropertyType,
                    properties[i].Name, hashPrefix, valuePositionMap, serialization.ValueHashes,
                    serialization.ValuePositions,
                    serialization.Values, false, visibility, bitwiseHash);

                // Draw the property.
                value = DrawObject(new GUIContent(EditorUtility.SplitCamelCase(properties[i].Name)),
                    properties[i].PropertyType, obj, value, properties[i].Name, hashPrefix, valuePositionMap,
                    serialization, properties[i], false);
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (hashPrefix != 0)
                    {
                        ...
                        if (setMethod != null) setMethod.Invoke(obj, new[] { value });
                        return obj;
                    }
        ...
        public static object DrawObject(GUIContent guiContent, Type type, object parent, object value, string name,
            int hashPrefix, Dictionary<int, int> valuePositionMap, Serialization serialization, object fieldProperty,
            bool drawFields)
        {
            if (typeof(IList).IsAssignableFrom(type))
                return DrawArrayObject(guiContent, type, parent, value, name, hashPrefix, valuePositionMap,
                    serialization, fieldProperty, drawFields);
            return DrawSingleObject(guiContent, type, value, name, hashPrefix, valuePositionMap, serialization,
                fieldProperty, drawFields);
        }
        ...
        /// <param name="drawFields">Should the fields be drawn? If false the properties will be drawn.</param>
        /// <returns>The drawn object.</returns>
        private static object DrawArrayObject(GUIContent guiContent, Type type, object parent, object value,
            string name, int hashPrefix, Dictionary<int, int> valuePositionMap, Serialization serialization,
            object fieldProperty, bool drawFields)
        {
            ...
            else
            {
                ...
                while (!baseFieldType.IsGenericType) baseFieldType = baseFieldType.BaseType;
            ...
            if (InspectorUtility.Foldout(list, guiContent))
            {
                ...
                if (s_EditingArray && s_FocusHash == hash && (s_KeybordControl != GUIUtility.keyboardControl ||
                                                              ...
                                                              Event.current.keyCode == KeyCode.KeypadEnter))
                {
                    ...
                    for (var i = 0; i < s_ArraySize; ++i)
                    {
                        if (i < list.Count) origElement = i;
                        if (origElement == -1) break;

                        var listValue = list[origElement];
                        if (i >= list.Count && !typeof(Object).IsAssignableFrom(elementType) &&
                            !typeof(string).IsAssignableFrom(elementType))
                    ...
                    else
                    {
                        if (type.IsGenericType)
                            ...
                            list = Activator.CreateInstance(type, true) as IList;
                        for (var i = 0; i < newArray.Length; ++i) list.Add(newArray.GetValue(i));
                    }
                ...
                for (var i = 0; i < list.Count; ++i)
                {
                    ...
                    if (list[i] == null && !typeof(Object).IsAssignableFrom(elementType) &&
                        elementType != typeof(string)) list[i] = Activator.CreateInstance(elementType);
                    guiContent.text = "Element " + i;
                    list[i] = DrawObject(guiContent, elementType, parent, list[i], name, hash / (i + 2),
                        valuePositionMap, serialization, fieldProperty, drawFields);
                    GUILayout.Space(6);
        ...
        /// <param name="drawFields">Should the fields be drawn? If false the properties will be drawn.</param>
        /// <returns>The drawn object.</returns>
        private static object DrawSingleObject(GUIContent guiContent, Type type, object value, string name,
            int hashPrefix, Dictionary<int, int> valuePositionMap, Serialization serialization, object fieldProperty,
            bool drawFields)
        {
            if (type == typeof(int)) return EditorGUILayout.IntField(guiContent, (int)value);
            if (type == typeof(float))
            {
                // The range attribute may be used instead.
                if (drawFields)
                {
                    var field = (FieldInfo)fieldProperty;
                    ...
                    if (rangeAttribute != null && rangeAttribute.Length > 0)
                        return EditorGUILayout.Slider(guiContent, (float)value, rangeAttribute[0].min,
                            rangeAttribute[0].max);
                }

                return EditorGUILayout.FloatField(guiContent, (float)value);
            }

            if (type == typeof(double)) return EditorGUILayout.FloatField(guiContent, Convert.ToSingle((double)value));
            if (type == typeof(long)) return (long)EditorGUILayout.IntField(guiContent, Convert.ToInt32((long)value));
            if (type == typeof(bool)) return EditorGUILayout.Toggle(guiContent, (bool)value);
            if (type == typeof(string)) return EditorGUILayout.TextField(guiContent, (string)value);
            if (type == typeof(byte))
                return Convert.ToByte(EditorGUILayout.IntField(guiContent, Convert.ToInt32(value)));
            if (type == typeof(Vector2)) return EditorGUILayout.Vector2Field(guiContent, (Vector2)value);
            if (type == typeof(Vector3)) return EditorGUILayout.Vector3Field(guiContent, (Vector3)value);
            if (type == typeof(Vector4)) return EditorGUILayout.Vector4Field(guiContent, (Vector4)value);
            ...
            if (type == typeof(Color)) return EditorGUILayout.ColorField(guiContent, (Color)value);
            if (type == typeof(Rect)) return EditorGUILayout.RectField(guiContent, (Rect)value);
            if (type == typeof(Matrix4x4))
            {
                ...
                if (InspectorUtility.Foldout(value, guiContent))
                {
                    ...
                    for (var i = 0; i < 4; ++i)
                    for (var j = 0; j < 4; ++j)
                        matrix[i, j] = EditorGUILayout.FloatField("E" + i + j, matrix[i, j]);
                    EditorGUI.indentLevel--;
                }
            ...
            if (type == typeof(LayerMask)) return DrawLayerMask(guiContent, (LayerMask)value);
            if (type.IsEnum)
            {
            ...
            if (typeof(Object).IsAssignableFrom(type))
                return EditorGUILayout.ObjectField(guiContent, (Object)value, type, true);
            if (type.IsClass || (type.IsValueType && !type.IsPrimitive))
            {
                // Classes and structs.
                if (typeof(Delegate).IsAssignableFrom(type)) // Delegates are not supported.
                    return null;
                if (s_DrawnObjects == null) s_DrawnObjects = new HashSet<int>();
                ...
                try
                {
                    // Unity may throw an exception so catch it to clean up.
                    ...
                    if (InspectorUtility.Foldout(value, guiContent))
                    {
                        ...
                        if (inspectorDrawer != null)
                            ...
                            value = DrawFields(value, true,
                                hashPrefix + Serialization.StringHash(type.FullName) + Serialization.StringHash(name));
                        else
                            value = DrawProperties(type, value,
                                hashPrefix + Serialization.StringHash(type.FullName) + Serialization.StringHash(name),
                                valuePositionMap, serialization, MemberVisibility.Public, null, null);
                        EditorGUI.indentLevel--;
                    }
        ...
        {
            if (s_LayerNames == null) InitLayers();
        ...
        public static TooltipAttribute GetTooltipAttribute(FieldInfo field)
        {
            ...
            if (tooltipAttributes.Length > 0) return tooltipAttributes[0];
            return null;
        }
    }
}