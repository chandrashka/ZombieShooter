...
namespace Opsive.Shared.Editor.Inspectors.Utility
{
    ...
    public class InspectorDrawerUtility
    {
        ...
        private static List<Assembly> s_LoadedAssemblies;
        private static readonly Dictionary<string, List<Type>> s_TypeLookup = new();
        ...
        public static InspectorDrawer InspectorDrawerForType(Type type)
        {
            ...
            if (s_InspectorDrawerTypeMap == null) BuildInspectorDrawerMap();
        ...
        private static void BuildInspectorDrawerMap()
        {
            ...
            for (var i = 0; i < assemblies.Length; ++i)
            {
                ...
                for (var j = 0; j < assemblyTypes.Length; ++j)
                {
                    ...
                    if (!typeof(InspectorDrawer).IsAssignableFrom(assemblyTypes[j])) continue;
                    ...
                    if ((attribute =
                            assemblyTypes[j].GetCustomAttributes(typeof(InspectorDrawerAttribute), false) as
                                InspectorDrawerAttribute[]).Length > 0 &&
        ...
        public static List<Type> GetAllTypesWithinNamespace(string name)
        {
            ...
            if (s_TypeLookup.TryGetValue(name, out types)) return types;
            ...
            if (s_LoadedAssemblies == null)
            {
                ...
                var assemblies = AppDomain.CurrentDomain.GetAssemblies();
                for (var i = 0; i < assemblies.Length; ++i) s_LoadedAssemblies.Add(assemblies[i]);
            }

            for (var i = 0; i < s_LoadedAssemblies.Count; ++i)
            {
                var assemblyTypes = s_LoadedAssemblies[i].GetTypes();
                for (var j = 0; j < assemblyTypes.Length; ++j)
                {
                    if (!assemblyTypes[j].IsClass || assemblyTypes[j].IsAbstract ||
                        assemblyTypes[j].Namespace != name) continue;
