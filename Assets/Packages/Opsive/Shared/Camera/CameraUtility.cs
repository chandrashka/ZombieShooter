...
namespace Opsive.Shared.Camera
{
    ...
    {
        private static readonly Dictionary<GameObject, UnityEngine.Camera> s_GameObjectCameraMap = new();
        ...
        public static UnityEngine.Camera FindCamera(GameObject character)
        {
            ...
            if (camera == null)
            {
                ...
                if (camera != null)
                {
                    ...
                    var cameraController = camera.GetComponent<ICamera>();
                    if (cameraController.Character != null && cameraController.Character != character) camera = null;
                }
            }

            if (camera != null && character != null) s_GameObjectCameraMap.Add(character, camera);
            return camera;
        }
        ...
        private static UnityEngine.Camera SearchForCamera(GameObject character)
        {
            ...
            if ((mainCamera = UnityEngine.Camera.main) != null &&
                (cameraController = mainCamera.GetComponent<ICamera>()) != null &&
                (character == null || cameraController.Character == character))
                return mainCamera;
            var cameras = Object.FindObjectsOfType<UnityEngine.Camera>();
            for (var i = 0; i < cameras.Length; ++i)
            {
                var iCamera = cameras[i].GetComponent<ICamera>();
                if (iCamera == null) continue;
                if (character == null || iCamera.Character == character) return cameras[i];
            }
