...
namespace Opsive.Shared.Audio
{
    ...
    public struct FloatOverride
    {
        ...
        [Tooltip("The value override option.")] [SerializeField]
        private Override m_ValueOverride;

        [Tooltip("The first constant.")] [SerializeField]
        private float m_Constant1;

        [Tooltip("The second constant.")] [SerializeField]
        private float m_Constant2;
    ...
    public struct BoolOverride
    {
        ...
        [Tooltip("The value of the override.")] [SerializeField]
    ...
    {
        [Tooltip("Allows the audio output to be overridden.")] [SerializeField]
        private AudioMixerGroup m_OutputOverride;

        [Tooltip("Allows the audio loop to be overridden.")] [SerializeField]
        private BoolOverride m_LoopOverride;

        [Tooltip("Allows the volume to be overridden.")] [SerializeField]
        private FloatOverride m_VolumeOverride;

        [Tooltip("Allows the pitch to be overridden.")] [SerializeField]
        private FloatOverride m_PitchOverride;

        [Tooltip("Allows the stereo pan to be overridden.")] [SerializeField]
        private FloatOverride m_StereoPanOverride;

        [Tooltip("Allows the spatial blend to be overridden.")] [SerializeField]
        private FloatOverride m_SpatialBlendOverride;

        [Tooltip("Allows the reverb zone to be overridden.")] [SerializeField]
        private FloatOverride m_ReverbZoneOverride;

        [Tooltip("Allows the audio delay to be overridden.")] [SerializeField]
        private FloatOverride m_DelayOverride;

        /// <summary>
        ///     Five parameter constructor.
        /// </summary>
        public AudioModifier(FloatOverride volumeOverride, FloatOverride pitchOverride, FloatOverride delayOverride,
            BoolOverride loopOverride, AudioMixerGroup outputOverride)
        {
        ...
        public FloatOverride VolumeOverride
        {
            get => m_VolumeOverride;
            set => m_VolumeOverride = value;
        }

        public FloatOverride PitchOverride
        {
            get => m_PitchOverride;
            set => m_PitchOverride = value;
        }

        public FloatOverride DelayOverride
        {
            get => m_DelayOverride;
            set => m_DelayOverride = value;
        }

        public AudioMixerGroup OutputOverride
        {
            get => m_OutputOverride;
            set => m_OutputOverride = value;
        }

        public BoolOverride LoopOverride
        {
            get => m_LoopOverride;
            set => m_LoopOverride = value;
        }

        public FloatOverride StereoPanOverride
        {
            get => m_StereoPanOverride;
            set => m_StereoPanOverride = value;
        }

        public FloatOverride SpatialBlendOverride
        {
            get => m_SpatialBlendOverride;
            set => m_SpatialBlendOverride = value;
        }

        public FloatOverride ReverbZoneOverride
        {
            get => m_ReverbZoneOverride;
            set => m_ReverbZoneOverride = value;
        }
        ...
        {
            if (m_OutputOverride != null) audioSource.outputAudioMixerGroup = m_OutputOverride;
    ...
    public class AudioConfig : ScriptableObject
    {
        ...
        [Tooltip("An array of AudioClips that can be played.")] [SerializeField]
        protected AudioClip[] m_AudioClips;

        [Tooltip("Specifies the element that should be selected within the AudioClips array.")] [SerializeField]
        protected ClipSelection m_ClipSelection;

        [Tooltip("Prefab which contains the AudioSource that should play the audio.")] [SerializeField]
        protected GameObject m_AudioSourcePrefab;

        [Tooltip("Can the AudioSource be shared between multiple AudioClips?")] [SerializeField]
        protected bool m_ShareAudioSource = true;

        [Tooltip("Should the AudioSource be replaced by a new AudioSource?")] [SerializeField]
        protected bool m_ReplacePreviousAudioSource;

        [Tooltip("Should the properties be copied if an AudioSource exists on the originating GameObject?")]
        [SerializeField]
        protected bool m_CopyExistingAudioSourceProperties = true;

        [Tooltip("Optionally specify the AudioModifier for the AudioConfig.")] [SerializeField]
        protected AudioModifier m_AudioModifier;

        [NonSerialized] protected int m_AudioClipIndex;
        ...
        public bool ShareAudioSource
        {
            get => m_ShareAudioSource;
            set => m_ShareAudioSource = value;
        }

        public bool ReplacePreviousAudioSource
        {
            get => m_ReplacePreviousAudioSource;
            set => m_ReplacePreviousAudioSource = value;
        }

        public AudioModifier AudioModifier
        {
            get => m_AudioModifier;
            set => m_AudioModifier = value;
        }
        ...
        public AudioSource OriginalAudioSource
        {
            get
            {
                if (m_AudioSourcePrefab == null) return null;
                if (m_AudioSource == null) m_AudioSource = m_AudioSourcePrefab.GetCachedComponent<AudioSource>();
        ...
        {
            get => m_AudioClipIndex;
            set => m_AudioClipIndex = value;
        }
        ...
        {
            if (m_AudioClips == null || m_AudioClips.Length == 0) return null;

            if (clipIndex >= 0 && clipIndex < m_AudioClips.Length) return m_AudioClips[clipIndex];
            ...
            switch (m_ClipSelection)
            {
                ...
                case ClipSelection.Sequence:
                    if (m_AudioClipIndex < 0 || m_AudioClipIndex >= m_AudioClips.Length) m_AudioClipIndex = 0;
        ...
        public AudioClip GetAudioClip(int clipIndex)
        {
            ...
            if (clipIndex < 0 || clipIndex >= m_AudioClips.Length) return null;
        ...
        public virtual PlayResult Play(AudioSource audioSource, AudioClipInfo audioClipInfo)
        {
            ...
            if (!m_CopyExistingAudioSourceProperties) CopyAudioProperties(OriginalAudioSource, audioSource);
        ...
        protected virtual AudioSource GetNextAudioSource(AudioSourceGroup audioSourceGroup)
        {
            ...
            if (m_ReplacePreviousAudioSource) audioSource = GetActiveAudioSource(audioSourceGroup);
            if (audioSource == null) audioSource = GetAvailableAudioSource(audioSourceGroup);
        ...
        public AudioSource GetAvailableAudioSource(AudioSourceGroup audioSourceGroup)
        {
            if (m_ShareAudioSource)
            {
                ...
                {
                    var sharedAudioSource = audioSourceGroup.SharedAudioSources[i];
                    if (sharedAudioSource.isPlaying == false) return sharedAudioSource;
                }
            }
            else if (audioSourceGroup.AudioSourceListByAudioConfig.TryGetValue(this, out var linkedAudioSources))
            {
            ...
            if (m_CopyExistingAudioSourceProperties)
            {
                ...
                if (existingAudioSource != null) AudioManager.CopyAudioProperties(existingAudioSource, audioSource);
            }
        ...
        protected virtual AudioSource AddAudioSource(AudioSourceGroup audioSourceGroup)
        {
            ...
            if (audioPrefab == null)
            {
                ...
                {
                    Debug.LogWarning(
                        "The AudioManager Module is missing a Default Audio Source Prefab in the Default Audio Config.");
            ...
            if (m_ShareAudioSource) audioSourceGroup.SharedAudioSources.Add(newAudioSource);
