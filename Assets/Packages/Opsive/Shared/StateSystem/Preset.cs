...
{
    /// <summary>
    ///     Represents a set of values for any number of component properties. In order for the value to be applied to a
    ///     property a getter and setter must exist,
    ///     along with a derived class from BaseDelegate which creates the delegate which interfaces with the property getter
    ///     and setter. Properties can be
    ...
    public class Preset : ScriptableObject
    {
        ...
        public bool IsInitialized => m_Delegates != null;
        public BaseDelegate[] Delegates => m_Delegates;
        ...
        /// <summary>
        ///     Initializes the preset. The preset must be initialized before the preset values are applied so the delegates can be
        ///     created.
        /// </summary>
        ...
        public virtual void Initialize(object obj, MemberVisibility visibility)
        {
            ...
            for (var i = 0; i < properties.Length; ++i)
            {
                ...
                if (Serialization.GetValidGetMethod(properties[i], visibility) == null) continue;
            ...
            if (m_Delegates.Length != valueCount) Array.Resize(ref m_Delegates, valueCount);
        }
        ...
        {
            for (var i = 0; i < m_Delegates.Length; ++i) m_Delegates[i].UpdateValue();
        }
        ...
        {
            for (var i = 0; i < m_Delegates.Length; ++i) m_Delegates[i].ApplyValue();
        }
        ...
        /// <summary>
        ///     Abstract class which allows for a delegate to be created which can be called on when the preset value should be
        ///     applied.
        /// </summary>
        [Preserve]
        public abstract class BaseDelegate
        {
            ...
            /// <param name="visibility">Specifies the visibility of the field/properties that should be retrieved.</param>
            public abstract void Initialize(object obj, PropertyInfo property, Dictionary<int, int> valuePositionMap,
                Serialization data, MemberVisibility visibility);
        ...
        /// <summary>
        ///     Generic class which implements a type specific delegate and value that can be called on when the preset value
        ///     should be applied.
        ...
        public class GenericDelegate<T> : BaseDelegate
        {
            ...
            public override MethodInfo SetMethod => m_SetMethod;
            ...
            /// <param name="data">The serialization data which contains the values for the property (as well as all other properties).</param>
            /// <param name="visibility">Specifies the visibility of the field/properties that should be retrieved.</param>
            public override void Initialize(object obj, PropertyInfo property, Dictionary<int, int> valuePositionMap,
                Serialization data, MemberVisibility visibility)
            {
                ...
                if (m_SetMethod != null)
                {
                    ...
                    var value = Serializer.BytesToValue(typeof(T), property.Name, valuePositionMap, 0, data.Values,
                        data.ValuePositions, data.UnityObjects, false, visibility, bitwiseHash);
                    if (value != null && !value.Equals(null)) m_Value = (T)value;
                    var type = typeof(T);
                ...
                if (getMethod != null)
                {
                    ...
                    if (m_IsIList)
                    {
                        if (typeof(T).IsArray)
                        {
                            ...
                            m_Value = (T)(object)Array.CreateInstance(type.GetElementType(),
            ...
            private void UpdateIList(T source, T destination)
            {
                ...
                else
                {
                    ...
                    if (destinationList.Count > sourceList.Count)
                    {
                        ...
                        for (var i = 0; i < removeCount; ++i) destinationList.RemoveAt(destinationList.Count - 1);
                    }
