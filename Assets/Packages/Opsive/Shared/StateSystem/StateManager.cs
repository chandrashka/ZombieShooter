...
namespace Opsive.Shared.StateSystem
{
    ...
    public class StateManager : MonoBehaviour
    {
        ...
        private readonly Dictionary<GameObject, HashSet<string>> m_ActiveCharacterStates = new();
        private Dictionary<GameObject, Dictionary<string, ScheduledEventBase>> m_DisableStateTimerMap;
        private readonly Dictionary<GameObject, Dictionary<string, List<State>>> m_GameObjectNameStateList = new();
        private readonly Dictionary<GameObject, List<GameObject>> m_LinkedGameObjectList = new();

        private readonly Dictionary<object, Dictionary<string, State>> m_ObjectNameStateMap = new();
        private readonly Dictionary<State, State[]> m_StateArrayMap = new();

        public bool SendStateChangeEvent
        {
            get => m_SendStateChangeEvent;
            set => m_SendStateChangeEvent = value;
        }
        ...
        private void InitializeInternal(GameObject stateGameObject, IStateOwner owner, State[] states)
        {
            ...
            if (states[states.Length - 1] == null) states[states.Length - 1] = new State("Default", true);
            ...
            var characterLocomotion = stateGameObject.GetCachedParentComponent<ICharacter>();
            if (characterLocomotion != null) characterGameObject = characterLocomotion.GameObject;
            for (var i = 0; i < states.Length; ++i)
            {
                if (states[i].Preset == null)
                    Debug.LogError(
                        string.Format(
                            "Error: The state {0} on {1} does not have a preset. Ensure each non-default state contains a preset.",
                            states[i].Name, owner), owner as Object);
            ...
            for (var i = 0; i < states.Length; ++i) states[i].Initialize(owner, nameStateMap);
        ...
        /// <summary>
        ///     Links the original GameObject to the linked GameObject. When GameObjects are linked the state will be updated for
        ///     each GameObject even when only the
        ...
        /// <summary>
        ///     Internal method which links the original GameObject to the linked GameObject. When GameObjects are linked the state
        ...
        private void SetStateInternal(object owner, State[] states, string stateName, bool active)
        {
            ...
            if (state.Active != active) ActivateStateInternal(state, active, states);
        }
        ...
        /// <summary>
        ///     Internal method which activates or deactivates all of the states on the specified GameObject with the specified
        ///     name.
        /// </summary>
        /// <param name="stateGameObject">The GameObject to enable or disable all of the states on.</param>
        /// <param name="stateName">The name of the state to change the active status of.</param>
        ...
        private void SetStateInternal(GameObject stateGameObject, string stateName, bool active)
        {
            ...
            var character = stateGameObject.GetCachedComponent<ICharacter>();
            if (character != null)
            ...
            if (m_SendStateChangeEvent) EventHandler.ExecuteEvent("OnStateChange", stateGameObject, stateName, active);
        ...
        /// <summary>
        ///     Internal method which activates or deactivates all of the states on the GameObjects linked from the GameObject with
        ///     the specified name.
        /// </summary>
        /// <param name="stateGameObject">The GameObject to enable or disable all of the states on.</param>
        /// <param name="stateName">The name of the state to change the active status of.</param>
        ...
        private void SetLinkStateInternal(GameObject stateGameObject, string stateName, bool active)
        {
            if (m_LinkedGameObjectList.TryGetValue(stateGameObject, out var linkedGameObjects))
                for (var i = 0; i < linkedGameObjects.Count; ++i)
        ...
        /// <summary>
        ///     Internal method which activates or deactivates the specified state. In most cases SetState should be used instead
        ///     of ActivateState.
        /// </summary>
        /// <param name="state">The state to activate or deactivate.</param>
        /// <param name="active">Should the state be activated?</param>
        ...
        /// <summary>
        ///     Loops through the states and applies the value. The states are looped in the order specified within the inspector
        ///     from top to bottom.
        /// </summary>
        ...
        private void CombineStates(State state, bool active, State[] states)
        {
            if (active)
            {
                ...
                for (var i = states.Length - 2; i > -1; --i)
            ...
            for (var i = states.Length - 2; i > -1; --i)
            {
                ...
                if (states[i].IsBlocked()) continue;
        ...
        public static void AddState(GameObject ownerGameObject, IStateOwner owner, State[] existingStates,
            State newState, int index)
        {
        ...
        /// <param name="index">The index that the state should be added to.</param>
        private void AddStateInternal(GameObject ownerGameObject, IStateOwner owner, State[] states, State state,
            int index)
        {
            // There are many reasons why a new state cannot be added.
            if (!Application.isPlaying || m_ObjectNameStateMap == null) return;
            ...
            {
                Debug.LogError(
                    $"Error: Unable to add the state {state.Name} to index {index}. The index is out of range.");
                return;
            }
            ...
            for (var i = states.Length - 1; i > index; --i) states[i] = states[i - 1];
            states[index] = state;
            if (owner is StateBehavior)
                (owner as StateBehavior).States = states;
            else if (owner is StateObject) (owner as StateObject).States = states;
