...
{
    /// <summary>
    ///     A State contains a preset which can change property values at runtime. The state has a name associated with it
    ///     which allows for easy reference to it.
    /// </summary>
    [Serializable]
    ...
    public class State
    {
        ...
        [Tooltip("The preset used by the state.")] [SerializeField]
        protected Preset m_Preset;

        [Tooltip(
            "A list of other states that the current state can prevent from being enabled if the current state is enabled.")]
        [SerializeField]
        protected string[] m_BlockList;

        [Tooltip("Is the state the default state?")] [SerializeField]
        protected bool m_Default;

        [Tooltip("Is the state active?")] [System.NonSerialized]
        ...
        public IStateOwner Owner { get; private set; }
        ...
        public void Initialize(IStateOwner owner, Dictionary<string, State> nameStateMap)
        {
            ...
            if (m_Preset != null)
            {
                ...
                if (m_Preset.IsInitialized) m_Preset = Object.Instantiate(m_Preset);
        ...
        private void UpdateBlockList(Dictionary<string, State> nameStateMap)
        {
            if (m_BlockList != null && m_BlockList.Length > 0)
            {
                ...
                State state;
                var count = 0;
                for (var i = 0; i < m_BlockList.Length; ++i)
                {
                    if (m_BlockList[i] == null) continue;
                    if (nameStateMap.TryGetValue(m_BlockList[count], out state))
                        m_BlockStates[count] = state;
                    else
                        Debug.LogWarning("Error: Unable to find block state with name \"" + m_BlockList[count] +
                                         "\" within state " + m_Name + " on object " + Owner);
                    count++;
                }

                if (m_BlockList.Length != count)
                {
                    Array.Resize(ref m_BlockList, count);
                    Array.Resize(ref m_BlockStates, count);
                }
            }
        }
        ...
        public bool IsBlocked()
        {
            if (m_BlockStates != null)
                for (var i = 0; i < m_BlockStates.Length; ++i)
        ...
        {
            if (m_Preset != null) m_Preset.ApplyValues();
        }
        ...
        {
            if (m_Preset != null) m_Preset.ApplyValues(delegates);
        }
    }
}