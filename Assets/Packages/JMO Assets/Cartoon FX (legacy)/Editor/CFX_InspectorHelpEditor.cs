...
{
    private CFX_InspectorHelp inspHelp => target as CFX_InspectorHelp;
    ...
    public override void OnInspectorGUI()
    {
        ...
        else
        {
            ...
            inspHelp.MsgType =
                (int)(MessageType)EditorGUILayout.EnumPopup("Message Type", (MessageType)inspHelp.MsgType);

            EditorGUILayout.HelpBox(
                "Use the contextual menu (right click or cog icon) on this Component to enable editing back",
                MessageType.Warning);
            if (GUILayout.Button("Lock Message", GUILayout.Height(30))) inspHelp.Locked = true;

            if (GUI.changed) EditorUtility.SetDirty(inspHelp);
    ...
    private void ShowHelpGUI()
    {
        ...
        if (!string.IsNullOrEmpty(inspHelp.Title)) EditorGUILayout.LabelField(inspHelp.Title, EditorStyles.boldLabel);
