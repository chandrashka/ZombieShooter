...
namespace CartoonFX
{
    ...
    public partial class CFXR_Effect : MonoBehaviour
    {
        ...
        public void Animate(float time)
        {
            ...
            {
#if UNITY_EDITOR
                if (!cameraShake.isShaking) cameraShake.fetchCameras();
#endif
        ...
        public class AnimatedLight
        {
            ...
            public class AnimatedLightDrawer : PropertyDrawer
            {
                ...
                public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
                {
                    ...
                    if (Event.current.type == EventType.Repaint) EditorStyles.helpBox.Draw(rect, GUIContent.none, 0);
                    ...
                    if (animateIntensity.boolValue)
                    {
                        startIndent(ref rect);
                        {
                            ...
                            var val = EditorGUI.Popup(rect, IntensityModeLabel, perlinIntensity.boolValue ? 1 : 0,
                                ModePopupLabels);
                            rect.y += propSpace;
                            if (val == 1 && !perlinIntensity.boolValue)
                                perlinIntensity.boolValue = true;
                            else if (val == 0 && perlinIntensity.boolValue) perlinIntensity.boolValue = false;
                    ...
                    if (animateRange.boolValue)
                    {
                        startIndent(ref rect);
                        {
                            ...
                            var val = EditorGUI.Popup(rect, RangeModeLabel, perlinRange.boolValue ? 1 : 0,
                                ModePopupLabels);
                            rect.y += propSpace;
                            if (val == 1 && !perlinRange.boolValue)
                                perlinRange.boolValue = true;
                            else if (val == 0 && perlinRange.boolValue) perlinRange.boolValue = false;
                    ...
                    if (animateColor.boolValue)
                    {
                        startIndent(ref rect);
                        {
                            ...
                            var val = EditorGUI.Popup(rect, ColorModeLabel, perlinColor.boolValue ? 1 : 0,
                                ModePopupLabels);
                            rect.y += propSpace;
                            if (val == 1 && !perlinColor.boolValue)
                                perlinColor.boolValue = true;
                            else if (val == 0 && perlinColor.boolValue) perlinColor.boolValue = false;
        ...
        private void Update()
        {
            ...
            if (fadeOutReference != null && !fadeOutReference.isEmitting &&
                (fadeOutReference.isPlaying || isFadingOut)) FadeOut(time);
#endif
#if !DISABLE_CLEAR_BEHAVIOR
            if (clearBehavior != ClearBehavior.None)
            {
                if (rootParticleSystem == null) rootParticleSystem = GetComponent<ParticleSystem>();
        ...
        public void FadeOut(float time)
        {
            ...
            foreach (var animLight in animatedLights) animLight.animateFadeOut(time - fadingOutStartTime);
        }
#endif
        ...
        private ParticleSystem parentParticle
        {
            get
            {
                if (_parentParticle == null) _parentParticle = GetComponent<ParticleSystem>();
        ...
        public void RegisterEditorUpdate()
        {
            ...
            if ((type == PrefabAssetType.Regular || type == PrefabAssetType.Variant) &&
                status == PrefabInstanceStatus.NotAPrefab) return;
        ...
        private void onEditorUpdate()
        {
            ...
            if (particleTime != parentParticle.time)
            {
                ...
                if (cameraShake != null && cameraShake.enabled && parentParticle.time < particleTime &&
                    parentParticle.time < 0.05f) cameraShake.StartShake();
#endif
#if !DISABLE_LIGHTS || !DISABLE_CAMERA_SHAKES
                Animate(particleTimeUnwrapped);

                if (!parentParticle.isEmitting) FadeOut(particleTimeUnwrapped);
#endif
            }

            if (particleWasStopped != parentParticle.isStopped)
            {
                if (parentParticle.isStopped) ResetState();
                particleTimeUnwrapped = 0;
            }
    ...
    public class CFXR_Effect_Editor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            foreach (var t in targets)
            {
                ...
                if (cfxr_effect != null)
                {
                    if (isPrefabSource(cfxr_effect.gameObject)) return;
