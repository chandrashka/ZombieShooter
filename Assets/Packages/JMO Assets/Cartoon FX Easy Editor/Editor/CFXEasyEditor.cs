...
public class CFXEasyEditor : EditorWindow
{
    ...
    */
    ...
    private List<string> initialModuleExtraProperties;
    ...
    private GUIContent[] modulesLabels;
    ...
    private void OnGUI()
    {
        ...
        if (colorFoldout)
        {
            ...
            using (new EditorGUI.DisabledScope(!TintStartColor && !TintColorModule && !TintColorSpeedModule &&
                                               !TintTrailsModule && !TintCustomData1 && !TintCustomData2))
            {
                GUILayout.BeginHorizontal();
                if (GUILayout.Button(
                ...
                GUILayout.Label(
                    new GUIContent("Saturation Shift",
                        "Change the saturation of the colors of the Particle System(s)\n(preserving their hue and value)"),
                    GUILayout.Width(120));
                ...
                GUILayout.Label(
                    new GUIContent("Value Shift",
                        "Change the value of the colors of the Particle System(s)\n(preserving their hue and saturation)"),
                    GUILayout.Width(120));
                ...
                GUILayout.BeginHorizontal();
                if (GUILayout.Button(new GUIContent("Gamma -> Linear",
                        "Converts the colors from Gamma to Linear color space"))) gammaToLinear();
                if (GUILayout.Button(new GUIContent("Linear -> Gamma",
                        "Converts the colors from Linear to Gamma color space"))) linearToGamma();
                GUILayout.EndHorizontal();
            }
            ...
            */
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Space(4);
            TintTrailsModule = GUILayout.Toggle(TintTrailsModule,
                new GUIContent("Trails", "If checked, the \"Trails\" value(s) will be affected."),
                EditorStyles.toolbarButton);
            TintCustomData1 = GUILayout.Toggle(TintCustomData1,
                new GUIContent("Custom Data 1",
                    "If checked, the \"Custom Data 1\" value(s) will be affected if they have been set to a color value."),
                EditorStyles.toolbarButton);
            TintCustomData2 = GUILayout.Toggle(TintCustomData2,
                new GUIContent("Custom Data 2",
                    "If checked, the \"Custom Data 2\" value(s) will be affected if they have been set to a color value."),
                EditorStyles.toolbarButton);
            GUILayout.Space(4);
            GUILayout.EndHorizontal();

            TintLights = GUILayout.Toggle(TintLights,
                new GUIContent(" Tint Lights found in the effect's Hierarchy",
                    "Will search for and tint any Lights found in the effect hierarchy."));
        ...
        if (copyFoldout)
        {
            ...
            {
                EditorGUI.BeginChangeCheck();
                copyModulesSourceParticleSystem =
                    (ParticleSystem)EditorGUILayout.ObjectField(copyModulesSourceParticleSystem, typeof(ParticleSystem),
                        true);
                if (GUILayout.Button("Get Selected", EditorStyles.miniButton))
                    if (Selection.activeGameObject != null)
                    {
                        ...
                        if (ps != null) copyModulesSourceParticleSystem = ps;
                    }
            ...
            */
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("Copy properties to\nselected Object(s)",
                        GUILayout.Height(Mathf.Ceil(EditorGUIUtility.singleLineHeight * 2.1f))))
                {
                    var foundPs = false;
                    foreach (var go in Selection.gameObjects)
                    {
                    ...
                    if (!foundPs)
                    {
                        ...
                        Debug.LogWarning(
                            "Cartoon FX Easy Editor: No Particle System found in the selected GameObject(s)!");
                    }
                }
            }
            GUILayout.EndHorizontal();
        ...
        */
        ...
        {
            var h = GUILayoutUtility.GetLastRect().yMax;
            if (lastHeight != h)
            {
                lastHeight = h;
                minSize = new Vector2(300, h);
                maxSize = new Vector2(300, h);
            }
        }
    }
    ...
    private void DrawSelectableModules(ref bool[] selected, ref bool[] enabled, ParticleSystem target)
    {
        ...
        {
            if (enabled == null || enabled.Length != modulesToCopy.Length) enabled = new bool[modulesToCopy.Length];
            ...
            var renderer = selected.Length - 1;
            ...
            if (GUILayout.Button(" All ", EditorStyles.miniButton))
                ...
                {
                    if (!pref_HideDisabledModulesCopy || enabled[i]) selected[i] = true;
            ...
            {
                for (var i = 0; i < modulesToCopy.Length; i++) selected[i] = false;
            ...
            const int row = 4;
            const int padding = 4;
            for (var i = 0; i < modulesToCopy.Length; i += row)
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    for (var j = 0; j < row; j++)
                        if (i + j < modulesLabels.Length)
                        {
                            ...
                            */
                            {
                                var enbl = i + j >= enabled.Length ? true : enabled[i + j];
                                if (pref_HideDisabledModulesCopy && !enbl) continue;
                                if (selected[i + j]) GUI.color *= Color.cyan;
                                selected[i + j] = GUILayout.Toggle(selected[i + j], modulesLabels[i + j],
                                    EditorStyles.toolbarButton);
                            }
                            GUI.color = col;
                        }
    ...
    private void RefreshCurrentlyEnabledModules(ParticleSystem source, ref bool[] enabledModules)
    {
        ...
        {
            for (var i = 0; i < enabledModules.Length; i++) enabledModules[i] = true;
        }
    }
    ...
    private void tintColor()
    {
        ...
        GenericProcessColors(color => { return HSLColor.FromRGBA(color).ColorWithHue(hue, false); });
    }

    private void hueShift(float shift)
    {
        GenericProcessColors(color => { return HSLColor.FromRGBA(color).ColorWithHue(shift, true); });
    }

    private void satShift(float shift)
    {
        GenericProcessColors(color => { return HSLColor.FromRGBA(color).ColorWithSaturationOffset(shift / 100f); });
    }

    private void valShift(float shift)
    {
        GenericProcessColors(color => { return HSLColor.FromRGBA(color).ColorWithLightnessOffset(shift / 100f); });
    }

    private void GenericTintColorProperty(SerializedProperty colorProperty, float hue, bool shift)
    {
        GenericEditColorProperty(colorProperty, color => { return HSLColor.FromRGBA(color).ColorWithHue(hue, shift); });
    }

    private void GenericEditGradient(SerializedProperty gradientProperty, Func<Color, Color> callback)
    {
        gradientProperty.FindPropertyRelative("key0").colorValue =
            callback(gradientProperty.FindPropertyRelative("key0").colorValue);
        gradientProperty.FindPropertyRelative("key1").colorValue =
            callback(gradientProperty.FindPropertyRelative("key1").colorValue);
        gradientProperty.FindPropertyRelative("key2").colorValue =
            callback(gradientProperty.FindPropertyRelative("key2").colorValue);
        gradientProperty.FindPropertyRelative("key3").colorValue =
            callback(gradientProperty.FindPropertyRelative("key3").colorValue);
        gradientProperty.FindPropertyRelative("key4").colorValue =
            callback(gradientProperty.FindPropertyRelative("key4").colorValue);
        gradientProperty.FindPropertyRelative("key5").colorValue =
            callback(gradientProperty.FindPropertyRelative("key5").colorValue);
        gradientProperty.FindPropertyRelative("key6").colorValue =
            callback(gradientProperty.FindPropertyRelative("key6").colorValue);
        gradientProperty.FindPropertyRelative("key7").colorValue =
            callback(gradientProperty.FindPropertyRelative("key7").colorValue);
    }

    private void gammaToLinear()
    {
        GenericProcessColors(color => { return color.gamma; });
    }

    private void linearToGamma()
    {
        GenericProcessColors(color => { return color.linear; });
    }

    private void GenericProcessColors(Func<Color, Color> callback)
    {
        foreach (var go in Selection.gameObjects)
        {
            ParticleSystem[] systems;
            ...
            foreach (var ps in systems)
            {
                ...
                if (TintStartColor)
                    GenericEditColorProperty(psSerial.FindProperty("InitialModule.startColor"), callback);
                if (TintColorModule) GenericEditColorProperty(psSerial.FindProperty("ColorModule.gradient"), callback);
                if (TintColorSpeedModule)
                    GenericEditColorProperty(psSerial.FindProperty("ColorBySpeedModule.gradient"), callback);
                if (TintCustomData1)
                    GenericEditColorProperty(psSerial.FindProperty("CustomDataModule.color0"), callback);
                if (TintCustomData2)
                    GenericEditColorProperty(psSerial.FindProperty("CustomDataModule.color1"), callback);
            ...
            if (TintLights)
            {
                ...
                if (cfxrEffect != null)
                    if (Array.Exists(cfxrEffect.animatedLights, item => item.animateColor))
                    {
                        var psEffect = new SerializedObject(cfxrEffect);
                        var lightsProp = psEffect.FindProperty("animatedLights");
                        var iteration = lightsProp.arraySize;
    ...
    private void GenericEditColorProperty(SerializedProperty colorProperty, Func<Color, Color> callback)
    {
        ...
        switch (state)
        {
            //Constant Color
            case 0:
                colorProperty.FindPropertyRelative("maxColor").colorValue =
                    callback(colorProperty.FindPropertyRelative("maxColor").colorValue);
                break;
            ...
            case 2:
                colorProperty.FindPropertyRelative("minColor").colorValue =
                    callback(colorProperty.FindPropertyRelative("minColor").colorValue);
                colorProperty.FindPropertyRelative("maxColor").colorValue =
                    callback(colorProperty.FindPropertyRelative("maxColor").colorValue);
                break;
    ...
    private void CopyModules(ParticleSystem source, ParticleSystem dest)
    {
        ...
        for (var i = 0; i < modulesToCopy.Length; i++)
        {
            ...
            if (i == 0)
                foreach (var path in initialModuleExtraProperties)
                {
                    ...
                    if (extraSp != null) psDest.CopyFromSerializedProperty(extraSp);
                }
        ...
        if (selectedModules[selectedModules.Length - 1])
        {
            ...
            ComponentUtility.CopyComponent(rendSource);
            ComponentUtility.PasteComponentValues(rendDest);
        }
    }
    ...
    */
    ...
    {
        //Particle System
        if (ps.gameObject != parent) ps.transform.localPosition *= ScalingValue;
    ...
    private class ParticleSystemModule
    {
        ...
        public bool CheckIfEnabledInSource(SerializedObject serializedSource)
        {
            ...
            if (sp != null)
            {
                ...
                if (enabledSp != null) enabled = enabledSp.boolValue;
            }
